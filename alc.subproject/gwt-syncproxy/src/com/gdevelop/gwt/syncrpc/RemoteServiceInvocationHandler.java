...
public class RemoteServiceInvocationHandler implements InvocationHandler {
        ...
        try {
            ...
            if (method.getDeclaringClass().getCanonicalName()
                ...
                if (asyncCallbackWrapperSupplier != null) {
                    callback = asyncCallbackWrapperSupplier.get()
                            .wrap(callback);
                }
            ...
            throw new InvocationException(
                    "Exception while invoking the remote service "
    ...
    public RemoteServiceSyncProxy createSyncProxy() {
        return new RemoteServiceSyncProxy(moduleBaseURL,
    ...
    private String computeBinaryClassName(Class type) {
        ...
        if (type.isArray()) {
            if (PRIMITIVE_TYPES.contains(type.getComponentType())) {
                ...
                if (type.getComponentType().isArray()) {
                    return "["
                            + computeBinaryClassName(type.getComponentType());
                } else {
                    return "[L"
                            + computeBinaryClassName(type.getComponentType())
                            + ';';
        ...
        if (enclosingClass != null) {
            return computeBinaryClassName(enclosingClass) + "$"
                    + type.getSimpleName();
        }
    ...
    public void deserializePayload(Class returnType, String payload,
            AsyncCallback callback) {
        try {
            RemoteServiceSyncProxy syncProxy = createSyncProxy();
            payload = payload.substring(4);
            ResponseReader readerFor = getReaderFor(returnType);
            Object result = readerFor
                    .read(syncProxy.createStreamReader(payload));
            if (callback != null) {
