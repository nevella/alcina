...
//cc.alcina.extras.misc.ie.override.
...
public class SimpleEventBus extends EventBus {
	...
	private native boolean isEventBusExceptionCatchDisabled() /*-{
	...
	private Boolean exceptionCatchDisabled;

	private void ensureIsExceptionCatch() {
	...
	private <H> void doFire(Event<H> event, Object source) {
		...
		try {
			...
			ListIterator<H> it = isReverseOrder ? handlers
					.listIterator(handlers.size()) : handlers.listIterator();
