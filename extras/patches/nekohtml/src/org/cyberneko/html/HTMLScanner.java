...
public class HTMLScanner
	...
	/**
	 * HTML 4.01 frameset public identifier ( "-//W3C//DTD HTML 4.01
	 * Frameset//EN").
	 */
	...
	/**
	 * Strip XHTML CDATA delimiters ("&lt;![CDATA[" and "]]&gt;") from SCRIPT
	 * tag contents.
	 */
	...
	/** Recognized features. */
	private static final String[] RECOGNIZED_FEATURES = { AUGMENTATIONS,
			REPORT_ERRORS, NOTIFY_CHAR_REFS, NOTIFY_XML_BUILTIN_REFS,
			NOTIFY_HTML_BUILTIN_REFS, FIX_MSWINDOWS_REFS,
			SCRIPT_STRIP_CDATA_DELIMS, SCRIPT_STRIP_COMMENT_DELIMS,
			STYLE_STRIP_CDATA_DELIMS, STYLE_STRIP_COMMENT_DELIMS,
			IGNORE_SPECIFIED_CHARSET, CDATA_SECTIONS, OVERRIDE_DOCTYPE,
			INSERT_DOCTYPE, NORMALIZE_ATTRIBUTES, PARSE_NOSCRIPT_CONTENT, };
	...
	private static final String[] RECOGNIZED_PROPERTIES = { NAMES_ELEMS,
			NAMES_ATTRS, DEFAULT_ENCODING, ERROR_REPORTER, DOCTYPE_PUBID,
			DOCTYPE_SYSID, };

	/** Recognized properties defaults. */
	private static final Object[] RECOGNIZED_PROPERTIES_DEFAULTS = { null, null,
			"Windows-1252", null, HTML_4_01_TRANSITIONAL_PUBID,
	 ...
	 * marker
	 */
	static void reduceToContent(XMLStringBuffer buffer, String startMarker,
			String endMarker) {
		int i = 0;
		int startContent = -1;
		...
		while (i < buffer.length - l1) {
			...
			if (Character.isWhitespace(c)) {
				++i;
			} else if (c == startMarker.charAt(0) && startMarker
					.equals(new String(buffer.ch, buffer.offset + i, l1))) {
				startContent = i + l1;
				break;
			} else {
		...
		while (i > startContent + l2) {
			...
			if (Character.isWhitespace(c)) {
				--i;
			} else if (c == endMarker.charAt(l2 - 1) && endMarker.equals(
					new String(buffer.ch, buffer.offset + i - l2 + 1, l2))) {
	 ...
	 * 
	 * @param closeall
	 *            Close all streams, including the original. This is used in
	...
	protected int load(int offset) throws IOException {
		...
		if (offset == fCurrentEntity.buffer.length) {
			...
			System.arraycopy(fCurrentEntity.buffer, 0, array, 0,
					fCurrentEntity.length);
			fCurrentEntity.buffer = array;
		}
		// read a block of characters
		int count = fCurrentEntity.stream.read(fCurrentEntity.buffer, offset,
				fCurrentEntity.buffer.length - offset);
	...
	protected int scanEntityRef(XMLStringBuffer str, boolean content)
			...
			try {
				...
				if (content && fDocumentHandler != null
					...
					if (fNotifyCharRefs) {
						...
						fDocumentHandler.startGeneralEntity(name, id, encoding,
								locationAugs());
					}
					str.clear();
				...
				if (fReportErrors) {
					fErrorReporter.reportError("HTML1005",
							new Object[] { name });
				}
				if (content && fDocumentHandler != null
						&& fElementCount >= fElementDepth) {
		...
		if (content && fDocumentHandler != null
			...
			fEndLineNumber = fCurrentEntity.lineNumber;
			fEndColumnNumber = fCurrentEntity.columnNumber;
			boolean notify = fNotifyHtmlBuiltinRefs
					|| (fNotifyXmlBuiltinRefs && builtinXmlRef(name));
			if (notify) {
				XMLResourceIdentifier id = resourceId();
				String encoding = null;
				fDocumentHandler.startGeneralEntity(name, id, encoding,
	...
	protected String scanName() throws IOException {
		...
		while (true) {
			...
			if (fCurrentEntity.offset == fCurrentEntity.length) {
				...
				System.arraycopy(fCurrentEntity.buffer, offset,
						fCurrentEntity.buffer, 0, length);
		...
		String name = length > 0
				? new String(fCurrentEntity.buffer, offset, length)
				: null;
		if (DEBUG_BUFFER) {
			System.out.print(")scanName: ");
			printBuffer();
	...
	protected void setScanner(Scanner scanner) {
		...
		if (DEBUG_SCANNER) {
			...
			System.out.print(
					scanner != null ? scanner.getClass().getName() : "null");
			System.out.println(");");
		}
	} // setScanner(Scanner)
	...
	protected boolean skip(String s, boolean caseSensitive) throws IOException {
		...
		for (int i = 0; i < length; i++) {
			if (fCurrentEntity.offset == fCurrentEntity.length) {
				System.arraycopy(fCurrentEntity.buffer,
						fCurrentEntity.offset - i, fCurrentEntity.buffer, 0, i);
				if (load(i) == -1) {
	...
	public class ContentScanner implements Scanner {
		...
		public boolean scan(boolean complete) throws IOException {
			...
			do {
				try {
					...
					case STATE_MARKUP_BRACKET: {
						...
						if (c == '!') {
							if (skip("--", false)) {
								...
								if (fReportErrors) {
									fErrorReporter.reportError("HTML1002",
											null);
								}
								skipMarkup(true);
							}
							...
							final String ename = scanStartElement(
									fSingleBoolean);
							if ("script".equalsIgnoreCase(ename)) {
								scanScriptContent();
							} else if (!fParseNoScriptContent
									&& "noscript".equalsIgnoreCase(ename)) {
								scanNoScriptContent();
							} else if (ename != null && !fSingleBoolean[0]
									&& HTMLElements.getElement(ename)
											.isSpecial()
									&& (!ename.equalsIgnoreCase("TITLE")
											|| isEnded(ename))) {
								setScanner(
										fSpecialScanner.setElementName(ename));
								setScannerState(STATE_CONTENT);
								return true;
							}
						}
						...
						if (fDocumentHandler != null
							...
							XercesBridge.getInstance()
									.XMLDocumentHandler_startDocument(
											fDocumentHandler, locator, encoding,
											nscontext, augs);
						}
						if (fInsertDoctype && fDocumentHandler != null) {
							String root = HTMLElements
									.getElement(HTMLElements.HTML).name;
							root = modifyName(root, fNamesElems);
					...
					case STATE_END_DOCUMENT: {
						if (fDocumentHandler != null
								&& fElementCount >= fElementDepth && complete) {
							if (DEBUG_CALLBACKS) {
					...
					default: {
						throw new RuntimeException(
								"unknown scanner state: " + fScannerState);
					}
					}
					...
					if (fCurrentEntityStack.empty()) {
						...
						fCurrentEntity = (CurrentEntity) fCurrentEntityStack
								.pop();
					}
					next = true;
				}
		 ...
		 * @param empty
		 *            Is used for a second return value to indicate whether the
		 ...
		 *            start element tag is empty (e.g. "/&gt;").
		 * @param endc
		 *            The end character that appears before the closing angle
		 *            bracket ('>').
		 */
		protected boolean scanAttribute(XMLAttributesImpl attributes,
			...
			if (c == '=') {
				...
				if (c != '\'' && c != '"') {
					...
					attributes.setNonNormalizedValue(lastattr,
							fNonNormAttr.toString());
					if (fAugmentations) {
						addLocationItem(attributes, attributes.getLength() - 1);
					}
					return true;
				...
				do {
					boolean acceptSpace = !fNormalizeAttributes
							|| (!isStart && !prevSpace);
					c = read();
					if (c == -1) {
						if (fReportErrors) {
		...
		protected void scanCharacters() throws IOException {
			...
			int newlines = skipNewlines();
			if (newlines == 0
					&& fCurrentEntity.offset == fCurrentEntity.length) {
				if (DEBUG_BUFFER) {
					System.out.print(")scanCharacters: ");
					printBuffer();
					System.out.println();
			...
			if (fCurrentEntity.offset > offset && fDocumentHandler != null
					&& fElementCount >= fElementDepth) {
				fString.setValues(fCurrentEntity.buffer, offset,
						fCurrentEntity.offset - offset);
				if (DEBUG_CALLBACKS) {
		...
		protected String scanStartElement(boolean[] empty) throws IOException {
			...
			if (fByteStream != null && fElementDepth == -1) {
				if (ename.equalsIgnoreCase("META")) {
					...
					String charset = getValue(fAttributes, "charset");
					if ((httpEquiv != null
							&& httpEquiv.equalsIgnoreCase("content-type"))
							|| charset != null) {
						if (DEBUG_CHARSET) {
							System.out.println(
									"+++ @content-type: \"" + httpEquiv + '"');
						}
						String content = getValue(fAttributes, "content");
						int index1 = content != null
								? content.toLowerCase().indexOf("charset=")
								: -1;
						if ((charset != null || index1 != -1)
								&& !fIgnoreSpecifiedCharset) {
							if (charset == null) {
								int index2 = content.indexOf(';', index1);
								charset = index2 != -1
										? content.substring(index1 + 8, index2)
										: content.substring(index1 + 8);
							}
							try {
								String ianaEncoding = charset;
								String javaEncoding = EncodingMap
										.getIANA2JavaMapping(
												ianaEncoding.toUpperCase());
								if (DEBUG_CHARSET) {
									System.out.println("+++ ianaEncoding: "
											+ ianaEncoding);
									System.out.println("+++ javaEncoding: "
											+ javaEncoding);
								}
								if (javaEncoding == null) {
									javaEncoding = ianaEncoding;
									if (fReportErrors) {
										fErrorReporter.reportError("HTML1001",
												new Object[] { ianaEncoding });
									}
								}
								// patch: Marc Guillemot
								if (!javaEncoding.equals(fJavaEncoding)) {
									if (!isEncodingCompatible(javaEncoding,
											fJavaEncoding)) {
										if (fReportErrors) {
											fErrorReporter.reportError(
													"HTML1015",
													new Object[] { javaEncoding,
															fJavaEncoding });
										}
									}
									// change the charset
										...
										fCurrentEntity.stream = new InputStreamReader(
												fByteStream, javaEncoding);
								...
								if (fReportErrors) {
									fErrorReporter.reportError("HTML1010",
											new Object[] { charset });
								}
					...
					HTMLElements.Element element = HTMLElements
							.getElement(ename);
			...
			if (fDocumentHandler != null && fElementCount >= fElementDepth) {
				...
				if (DEBUG_CALLBACKS) {
					System.out.println(
							"startElement(" + fQName + ',' + fAttributes + ")");
				}
				fEndLineNumber = fCurrentEntity.lineNumber;
				fEndColumnNumber = fCurrentEntity.columnNumber;
				if (empty[0]) {
					fDocumentHandler.emptyElement(fQName, fAttributes,
							locationAugs());
				} else {
					fDocumentHandler.startElement(fQName, fAttributes,
							locationAugs());
				}
			}
			return ename;
	...
	public static class CurrentEntity {
		...
		public CurrentEntity(Reader stream, String encoding, String publicId,
				String baseSystemId, String literalSystemId,
	 ...
	 * read from the underlying input stream and play the bytes back later. This
	 * class is used by the HTML scanner to switch encodings when a &lt;meta&gt;
	 * tag is detected that specifies a different encoding.
	 * <p>
	 ...
	 * be reclaimed.
	 * <p>
	 * <strong>Note:</strong> If the buffer is never played back or cleared,
	 * this input stream will continue to buffer the entire stream. Therefore,
	 * it is very important to use this stream correctly.
	 * 
	 * @author Andy Clark
	 */
	public static class PlaybackInputStream extends FilterInputStream {
		...
		public int read(byte[] array, int offset, int length)
			...
			if (fPushbackOffset < fPushbackLength) {
				...
				System.arraycopy(fByteBuffer, fPushbackOffset, array, offset,
						count);
			...
			if (fPlayback) {
				...
				System.arraycopy(fByteBuffer, fByteOffset, array, offset,
						length);
				fByteOffset += length;
				if (fByteOffset == fByteLength) {
					fCleared = true;
			...
			if (count != -1) {
				...
				System.arraycopy(array, offset, fByteBuffer, fByteLength,
						count);
				fByteLength += count;
			}
			if (DEBUG_PLAYBACK) {
				System.out.println(
						")read(" + offset + ',' + length + ") -> " + count);
			}
			return count;
		} // read(byte[]):int
	...
	public interface Scanner {
		 ...
		 * 
		 * @param complete
		 *            True if the scanner should not return until scanning is
		 *            complete.
		 * 
	...
	public class SpecialScanner implements Scanner {
							...
							if (ename != null) {
								if (ename.equalsIgnoreCase(fElementName)) {
									if (read() == '>') {
										...
										if (fDocumentHandler != null
												&& fElementCount >= fElementDepth) {
											fQName.setValues(null, ename, ename,
													null);
											if (DEBUG_CALLBACKS) {
											...
											fEndLineNumber = fCurrentEntity.lineNumber;
											fEndColumnNumber = fCurrentEntity.columnNumber;
											fDocumentHandler.endElement(fQName,
													locationAugs());
										}
										setScanner(fContentScanner);
										setScannerState(STATE_CONTENT);
		...
		protected void scanCharacters(XMLStringBuffer buffer, int delimiter)
			...
			if (DEBUG_BUFFER) {
				System.out.print(
						"(scanCharacters, delimiter=" + delimiter + ": ");
				printBuffer();
				System.out.println();
			}
			boolean strip = (fStyle
					&& (fStyleStripCommentDelims || fStyleStripCDATADelims));
			while (true) {
				int c = read();
