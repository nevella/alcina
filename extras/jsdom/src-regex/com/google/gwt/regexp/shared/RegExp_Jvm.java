...
public class RegExp_Jvm implements IRegExp {
	...
	private static final Pattern REPLACEMENT_BACKSLASH = Pattern
			.compile("\\\\");
	...
	// $0 before passing it to Java. However, we have to watch out for $$&,
	// which
	// is actually a Javascript $$ (see below) followed by a & with no special
	// meaning, and must not get translated.
	private static final Pattern REPLACEMENT_DOLLAR_AMPERSAND = Pattern
			.compile("((?:^|\\G|[^$])(?:\\$\\$)*)\\$&");
	...
	// rejected.
	private static final Pattern REPLACEMENT_DOLLAR_APOSTROPHE = Pattern
			.compile("(?:^|[^$])(?:\\$\\$)*\\$[`']");
	...
	private static final Pattern REPLACEMENT_DOLLAR_DOLLAR = Pattern
			.compile("\\$\\$");
	 ...
	 *            the Javascript regular expression pattern to compile
	 * @param flags
	 *            the flags string, containing at most one occurrence of {@code
	...
	public static RegExp compile(String pattern, String flags) {
		...
		for (char flag : parseFlags(flags)) {
			...
			case 'g':
				...
				break;
			case 'i':
				javaPatternFlags |= Pattern.CASE_INSENSITIVE
	...
	private static Set<Character> parseFlags(String flags) {
		...
		for (int flagIndex = 0; flagIndex < flags.length(); flagIndex++) {
			...
			if (!flagsSet.add(flag)) {
				throw new IllegalArgumentException(
						"Flag cannot be specified twice: '" + flag + "'");
			}
		}
		return flagsSet;
	}
	...
	private RegExp_Jvm(String source, Pattern pattern, boolean globalFlag) {
		this.source = source;
	...
	public MatchResult exec(String input) {
		...
		if (matcher != null) {
			...
			return new MatchResult(
					new MatchResult_Jvm(matcher.start(), input, groups));
		} else {
			// No match
			if (globalFlag) {
				lastIndex = 0;
	 ...
	 * all matches of the regular expression. Otherwise, replaces the first
	 * match of the regular expression. As per Javascript semantics, backslashes
	 * in the replacement string get no special treatment, but the replacement
	 * string can use the following special patterns:
	 * <ul>
	...
	public String replace(String input, String replacement) {
		...
		// Replace the Javascript-ese $& in the replacement with Java-ese $0,
		// but
		// watch out for $$&, which should stay $$&, to be changed to \$& below.
		replacement = REPLACEMENT_DOLLAR_AMPERSAND.matcher(replacement)
				.replaceAll(REPLACEMENT_DOLLAR_AMPERSAND_FOR_JAVA);
		// Test for Javascript-ese $` and $', which we do not support in the
		// pure
		// Java version.
		...
		replacement = REPLACEMENT_DOLLAR_DOLLAR.matcher(replacement)
	 ...
	 * constituent characters. If the regular expression is not empty but
	 * matches an empty string, the results are not well defined.
	 *
	 * @param input
	 ...
	 * it is delegated to the browser, and no browser follows the spec
	 * completely. A major difference is that IE will exclude empty strings in
	 * the result.
	 *
	 * @param input
	 *            the string to be split.
	 ...
	 *            ignoring the rest of the input string. If negative, there is
	 *            no limit.
	 * @return the strings split off, any of which may be empty.
	 */
	public SplitResult split(String input, int limit) {
		String[] result;
		if (source.length() == 0) {
			// Javascript split using a completely empty regular expression
			// splits the
		...
		return new SplitResult(new SplitResult_Jvm(result));
	}

	/**
	 * Determines if the regular expression matches the given string. This call
	 * affects the value returned by {@link #getLastIndex()} if the global flag
	 * is set. Equivalent to: {@code exec(input) != null}
	 *
	 * @param input
	 *            the string to apply the regular expression to
