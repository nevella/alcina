...
public final class SafeHtmlUtils {
	...
	/**
	 * Returns a {@link SafeHtml} constructed from a safe string, i.e., without
	 * escaping the string.
	 *
	 * <p>
	 * <b>Important</b>: For this method to be able to honor the
	 * {@link SafeHtml} contract, all uses of this method must satisfy the
	 ...
	 *
	 * <p>
	 * The first constraint provides a sufficient condition that the argument
	 * (and any HTML markup contained in it) originates from a trusted source.
	 * The second constraint ensures the composability of {@link SafeHtml}
	 * values.
	 *
	 * <p>
	 ...
	 * Production Mode on the client, and with assertions disabled on the
	 * server.
	 *
	 * @param s
	 ...
	 *             if not running in Production Mode and {@code html} violates
	...
	/**
	 * HTML-escapes a character. HTML meta characters will be escaped as
	 * follows:
	 *
	 * <pre>
	 * &amp; - &amp;amp;
	 ...
	 *            the string to be escaped
	 * @return the input string, with all occurrences of HTML meta-characters
	 *         replaced with their corresponding HTML Entity References, with
	 *         the exception that ampersand characters are not double-escaped if
	 *         they form the start of an HTML Entity Reference
	 */
	public static String htmlEscapeAllowEntities(String text) {
		...
		for (String segment : text.split("&", -1)) {
			...
			if (entityEnd > 0 && segment.substring(0, entityEnd)
					.matches(HTML_ENTITY_REGEX)) {
	...
	public static String htmlEscapeNoQuotes(String s) {
		if (!HTML_CHARS_RE.test(s)) {
			return s;
		}
		if (s.indexOf("&") != -1) {
