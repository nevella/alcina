...
public final class Impl {
    static {
        if (GWT.isScript() && StackTraceCreator.collector != null) {
            // Just enforces loading of StackTraceCreator early on, nothing else
            // to do here...
        }
    }
     ...
     * implement in Development Mode. If the requested member is a method, the
     * method will not be devirtualized, inlined or prunned.
     *
     * @param jsniIdent
    ...
    public static void reportUncaughtException(Throwable e) {
        ...
        if (handler != null) {
            ...
            // TODO(goktug): Handler might throw an exception but catching and
            // reporting it to browser
            // here breaks assumptions of some existing hybrid apps that uses
            // UCE for exception
            // conversion. We don't have an alternative functionality (yet) and
            // it is too risky to include
     ...
     * i.e. {@link GWT#getUncaughtExceptionHandler()} will still return null if
     * a handler is not set via {@link GWT#setUncaughtExceptionHandler}.
     */
