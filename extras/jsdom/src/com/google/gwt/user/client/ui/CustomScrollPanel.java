...
public class CustomScrollPanel extends ScrollPanel {
	 ...
	 * will fire for position 100 after the scroller has already moved to
	 * position 200. If we do not ignore the scroll events, we can end up with a
	 * loop where the scrollbars update the scroll position, and vice versa.
	 */
	...
	public CustomScrollPanel(Resources resources) {
		...
		/*
		 * Apply the inline block style to the container element so it resizes
		 * with the content.
		 */
		Element containerElem = getContainerElement();
		containerElem.setClassName(CommonResources.getInlineBlockStyle());
		/*
		 * Attach the scrollable element with the container. The scrollable
		 * element always shows its scrollbars, but they are hidden beneath the
		 * root element.
		 */
		 ...
		 * element's dimensions are large enough that the scrollbars are outside
		 * of the root element, and the root element is set to hide overflow,
		 * making the scrollbars invisible. The scrollable elements dimensions
		 * are set after the widget is attached to the document in
		 * hideNativeScrollbars().
		 */
		getElement().getStyle().setOverflow(Overflow.HIDDEN);
		/*
		 * Create a corner element that appears at the gap where the vertical
		 * and horizontal scrollbars meet (when both are visible). This prevents
		 * the content from peeking out from this gap.
		 */
		...
		NativeHorizontalScrollbar.Resources hResources = GWT
				.create(NativeHorizontalScrollbar.ResourcesTransparant.class);
		// boolean useLocalDom = LocalDomImpl.useLocalImpl;
		// LocalDomBridge.get().useJsoDom();
		// NativeHorizontalScrollbar h_scrollbar = new
		// NativeHorizontalScrollbar(hResources);
		...
		// NativeVerticalScrollbar v_scrollbar = new
		// NativeVerticalScrollbar(vResources);
		...
		/*
		 * Listen for scroll events from the root element and the scrollable
		 * element so we can align the scrollbars with the content. Scroll
		 * events usually come from the scrollable element, but they can also
		 * come from the root element if the user clicks and drags the content,
		 * which reveals the hidden scrollbars.
		 */
		Event.sinkEvents(getElement(),
				Event.getEventsSunk(getElement()) | Event.ONSCROLL);
		Event.sinkEvents(getScrollableElement(),
				Event.getEventsSunk(getScrollableElement()) | Event.ONSCROLL);
	}
	 ...
	 * 
	 * @param w
	 *            the widget to add, or null to clear the widget
	 * @param toReplace
	 *            the widget to replace
	 * @param layer
	 *            the layer in which the existing widget is placed
	...
	/**
	 * Hide the native scrollbars. We call this after attaching to ensure that
	 * we inherit the direction (rtl or ltr).
	 */
	...
	private void maybeUpdateScrollbarPositions() {
		...
		/*
		 * Ensure that the viewport is anchored to the corner. If the user
		 * clicks and drags the content, its possible to shift the viewport and
		 * reveal the hidden scrollbars.
		 */
	 ...
	 * 
	 * <p>
	 * If only the vertical scrollbar is present, it takes up the entire height
	 * of the right side. If only the horizontal scrollbar is present, it takes
	 * up the entire width of the bottom. If both scrollbars are present, the
	 * vertical scrollbar extends from the top to just above the horizontal
	 * scrollbar, and the horizontal scrollbar extends from the left to just
	 * right of the vertical scrollbar, leaving a small square in the bottom
	 * right corner.
	 * 
	 * <p>
	...
	private void maybeUpdateScrollbars() {
		...
		/*
		 * Measure the height and width of the content directly. Note that
		 * measuring the height and width of the container element (which should
		 * be the same) doesn't work correctly in IE.
		 */
		Widget w = getWidget();
		...
		if (hScrollbar != null && (alwaysShowScrollbars
				|| getElement().getClientWidth() < contentWidth)) {
			// Horizontal scrollbar is defined and required.
			realScrollbarHeight = hScrollbarHeight;
		}
		if (vScrollbar != null && (alwaysShowScrollbars
				|| getElement().getClientHeight() < contentHeight)) {
			// Vertical scrollbar is defined and required.
			realScrollbarWidth = vScrollbarWidth;
		}
		/*
		 * Add some padding to the so bottom we can scroll to the bottom without
		 * the content being hidden beneath the horizontal scrollbar.
		 */
		if (w != null) {
			if (realScrollbarHeight > 0) {
				w.getElement().getStyle().setMarginBottom(realScrollbarHeight,
						Unit.PX);
		...
		if (realScrollbarHeight > 0) {
			...
			if (isRtl) {
				hScrollbarLayer.setLeftRight(realScrollbarWidth, Unit.PX, 0.0,
						Unit.PX);
			} else {
				hScrollbarLayer.setLeftRight(0.0, Unit.PX, realScrollbarWidth,
						Unit.PX);
			}
			hScrollbarLayer.setBottomHeight(0.0, Unit.PX, realScrollbarHeight,
					Unit.PX);
			hScrollbar.setScrollWidth(
					Math.max(0, contentWidth - realScrollbarWidth));
		} else if (hScrollbarLayer != null) {
			hScrollbarLayer.setVisible(false);
		}
		if (realScrollbarWidth > 0) {
			vScrollbarLayer.setVisible(true);
			vScrollbarLayer.setTopBottom(0.0, Unit.PX, realScrollbarHeight,
					Unit.PX);
			if (isRtl) {
				vScrollbarLayer.setLeftWidth(0.0, Unit.PX, realScrollbarWidth,
						Unit.PX);
			} else {
				vScrollbarLayer.setRightWidth(0.0, Unit.PX, realScrollbarWidth,
						Unit.PX);
			}
			vScrollbar.setScrollHeight(
					Math.max(0, contentHeight - realScrollbarHeight));
		...
		if (isRtl) {
			...
			cornerLayer.setRightWidth(0.0, Unit.PX, realScrollbarWidth,
					Unit.PX);
		}
