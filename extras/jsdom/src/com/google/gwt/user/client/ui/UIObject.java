...
public abstract class UIObject implements HasVisibility {
	 ...
	 * with third-party libraries and test tools. If elem already has an ID,
	 ...
	 * account only the "display" style, ignoring CSS and Aria roles, thus may
	 * not accurately reflect whether the element is actually visible in the
	 * browser.
	 * </p>
	 */
	 ...
	 * clears the "display" style to its default value if {@code visible} is
	 * true, else forces the style to "none". If the "display" style is set to
	 * "none" via CSS style sheets, the element remains invisible after a call
	 * to {@code setVisible(elem, true)}.
	 * </p>
	 */
	...
	public void setHeight(String height) {
		...
		assert extractLengthValue(height.trim().toLowerCase(
				Locale.ROOT)) >= 0 : "CSS heights should not be negative";
	...
	public void setWidth(String width) {
		...
		// where
		// it won't accept negative numbers in length measurements
		assert extractLengthValue(width.trim().toLowerCase(
				Locale.ROOT)) >= 0 : "CSS widths should not be negative";
	...
	public void sinkEvents(int eventBitsToAdd) {
		DOM.sinkEvents(getElement(),
				eventBitsToAdd | DOM.getEventsSunk(getElement()));
	}
