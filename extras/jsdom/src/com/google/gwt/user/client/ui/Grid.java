 ...
 * </p>
 * <p>
 * <h3>Example</h3> {@example com.google.gwt.examples.GridExample}
 * </p>
 * 
 * <h3>Use in UiBinder Templates</h3>
 * <p>
 * Grid widget consists of &lt;g:row> elements. Each &lt;g:row> element can
 * contain one or more &lt;g:cell> or &lt;g:customCell> elements. Using
 * &lt;g:cell> attribute it is possible to place pure HTML content.
 * &lt;g:customCell> is used as a container for
 * {@link com.google.gwt.user.client.ui.Widget} type objects. (Note that the
 * tags of the row, cell and customCell elements are not capitalized. This is
 * meant to signal that the item is not a runtime object, and so cannot have a
...
public class Grid extends HTMLTable {
	...
	/**
	 * Inserts a new row into the table. If you want to add multiple rows at
	 * once, use {@link #resize(int, int)} or {@link #resizeRows(int)} as they
	 * are more efficient.
	 * 
	 * @param beforeRow
	...
	public void resizeColumns(int columns) {
		...
		if (columns < 0) {
			throw new IndexOutOfBoundsException(
					"Cannot set number of columns to " + columns);
		}
		if (numColumns > columns) {
	...
	public void resizeRows(int rows) {
		...
		if (rows < 0) {
			throw new IndexOutOfBoundsException(
					"Cannot set number of rows to " + rows);
		}
		if (numRows < rows) {
