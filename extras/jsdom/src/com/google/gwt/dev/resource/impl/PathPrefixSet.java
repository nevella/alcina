...
public class PathPrefixSet {
	/*
	 * (1) TODO(amitmanjhi): Improve the api of the PathPrefixSet so that with
	 * one trie-traversal, it could be found out which resources rooted at a
	 * directory are allowed?
	 */
	/**
	 * Whether or not to merge prefixes that are added. Merged prefixes perform
	 * better during resource scanning but at the cost of not being able to tell
	 * which module(s) are responsible for a particular resource inclusion.
	 */
	...
	public boolean add(PathPrefix prefix) {
		...
		/*
		 * An empty prefix means we have no prefix requirement, but we do attach
	 ...
	 *
	 * @param dirPath
	 *            must be a valid abstract directory name (must not be an empty
	 *            string)
	...
	public boolean includesDirectory(String dirPath) {
		 ...
		 *
		 * (1) The empty string was specified as a prefix, which causes
		 * everything to be included.
		 *
		 * (2) As we walk the parts of dirPath, we see a path prefix attached to
		 * one of the trie nodes we encounter. This means that there was a
		 * specified prefix that this dirPath falls underneath, so it is
		 * included.
		 *
		 * (3) dirPath is longer than the trie, but we never encounter a path
		 * prefix as we walk the trie. This indicates that this directory
		 * doesn't fall into any of the specified prefixes.
		 *
		 * (4) dirPath is not longer than the trie and stays on the trie the
		 * whole time, which means it is included (since at least some longer
		 * prefix includes it).
		 */
	...
	/**
	 * Dives down the package hierarchy looking for the most specific package
	 * that applies to this resource. The filter of the most specific package is
	 * the final determiner of inclusion/exclusion, such that more specific
	 * subpackages can override the filter settings on less specific
	 * superpackages.
	 */
	public ResourceResolution includesResource(String resourceAbstractPathName,
		...
		for (String part : parts) {
			...
			if (childNode.hasPrefixes) {
				List<PathPrefix> moreSpecificPrefixes = childNode
						.getPathPrefixes();
				// If PathPrefix->Module associations are accurate because
				// PathPrefixes haven't been merged.
				if (!mergePathPrefixes) {
					// Record the module name of every PathPrefix that would
					// allow this
					// resource. This enables detailed dependency validity
					// checking.
					for (PathPrefix candidatePrefix : moreSpecificPrefixes) {
						if (candidatePrefix
								.getJudgement(resourceAbstractPathName)
								.isInclude()) {
		...
		for (PathPrefix candidatePrefix : mostSpecificPrefixes) {
			Judgement judgement = candidatePrefix
					.getJudgement(resourceAbstractPathName);
