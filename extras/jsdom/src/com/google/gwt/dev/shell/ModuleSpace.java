...
public abstract class ModuleSpace implements ShellJavaScriptHost {
	 ...
	 * since this class will have been loaded from a ClassLoader other than the
	 * session's thread.
	 */
	protected static RuntimeException createJavaScriptException(ClassLoader cl,
		...
		try {
			...
			return (RuntimeException) ctor
					.newInstance(new Object[] { exception, message });
		...
		throw new RuntimeException("Error creating JavaScriptException",
				caught);
	}
	 ...
	 * gets the object wrapped by a JavaScriptException. We have to do this
	 * reflectively, since the JavaScriptException object is from an arbitrary
	 * classloader. If the object is not a JavaScriptException, or is not from
	 * the given ClassLoader, or the exception is not set we'll return exception
	 * itself.
	 */
	static Object getThrownObject(ClassLoader cl, Object exception) {
		...
		try {
			...
			Method isThrownSet = javaScriptExceptionClass
					.getMethod("isThrownSet");
	...
	public void exceptionCaught(Object exception) {
		...
		if (thrown != null && isExceptionSame(thrown, exception)) {
			...
			caught = createJavaScriptException(getIsolatedClassLoader(),
					exception);
			...
			assert trace[1].getClassName()
					.equals(JavaScriptHost.class.getName());
	...
	public char invokeNativeChar(String name, Object jthis, Class<?>[] types,
		...
		Character value = JsValueGlue.get(result, null, Character.TYPE,
				msgPrefix);
		if (value == null) {
			throw new HostedModeException(msgPrefix
	 ...
	 */
	public final void onLoad(TreeLogger logger)
			throws UnableToCompleteException {
		Event moduleSpaceLoadEvent = SpeedTracerLogger
				.start(DevModeEventType.MODULE_SPACE_LOAD);
		...
		try {
			...
			invokeNativeVoid("__defineStatic", null,
					new Class[] { Object.class },
					new Object[] { staticDispatch });
		} catch (Throwable e) {
			logger.log(TreeLogger.ERROR,
					"Unable to initialize static dispatcher", e);
		...
		try {
			...
			Class<?> implClass = loadClassFromSourceName(
			...
			if (entryPoints.length > 0) {
				try {
					for (int i = 0; i < entryPoints.length; i++) {
						...
						Object module;
						// Try to initialize EntryPoint, else throw up glass
						// panel
						try {
							Class<?> clazz = loadClassFromSourceName(
									entryPointTypeName);
							try {
								onModuleLoad = clazz.getMethod("onModuleLoad");
								if (!Modifier.isStatic(
										onModuleLoad.getModifiers())) {
									// it's non-static, so we need to rebind the
									// class
									onModuleLoad = null;
								}
							} catch (NoSuchMethodException e) {
								// okay, try rebinding it; maybe the rebind
								// result will have one
							}
							module = null;
							if (onModuleLoad == null) {
								module = rebindAndCreate(entryPointTypeName);
								onModuleLoad = module.getClass()
										.getMethod("onModuleLoad");
								// Record the rebound name of the class for
								// stats (below).
								entryPointTypeName = module.getClass().getName()
										.replace('$', '.');
							}
						} catch (Throwable e) {
							displayErrorGlassPanel(
									"EntryPoint initialization exception",
									entryPointTypeName, e);
							throw e;
						}
						...
						try {
							...
							invokeNativeVoid("fireOnModuleLoadStart", null,
									...
									new Object[] { entryPointTypeName });
							Event onModuleLoadEvent = SpeedTracerLogger
									.start(DevModeEventType.ON_MODULE_LOAD);
							try {
					...
					Method exit = implClass.getDeclaredMethod("exit",
							boolean.class);
			...
			// plugin issue
			logger.log(TreeLogger.INFO, unableToLoadMessage);
	...
	public <T> T rebindAndCreate(String requestedClassName)
		...
		Event moduleSpaceRebindAndCreate = SpeedTracerLogger
				.start(DevModeEventType.MODULE_SPACE_REBIND_AND_CREATE);
		try {
			...
			moduleSpaceRebindAndCreate.addData("Requested Class",
					requestedClassName, "Result Name", resultName);
	...
	private void scrubStackTrace(Throwable thrown) {
		...
		for (ListIterator<StackTraceElement> it = trace.listIterator(); it
				.hasNext();) {
					...
					if (it.hasNext()) {
						...
						if (next.getLineNumber() == -1) {
							next = new StackTraceElement(next.getClassName(),
									next.getMethodName(), next.getFileName(),
									-2);
							it.set(next);
						}
					}
