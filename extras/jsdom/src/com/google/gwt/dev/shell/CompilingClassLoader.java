...
public final class CompilingClassLoader extends ClassLoader
	 ...
	 * classpath.
	 */
	private static final ClassLoader bootstrapClassLoader = ClassLoader
	...
	private static final Class<?>[] BRIDGE_CLASSES = new Class<?>[] {
			...
			com.google.gwt.core.shared.GWTBridge.class };

	private static final boolean CLASS_DUMP = Boolean
			.getBoolean("gwt.dev.classDump");

	private static final String CLASS_DUMP_PATH = System
			.getProperty("gwt.dev.classDumpPath", "rewritten-classes");
	...
	static {
		 ...
		 * that changes in the future, this code would need to be updated as
		 * well.
		 */
		try {
		...
		try {
			Class<?> jacoco = Class.forName(JACOCO_ENTRYPOINT, false,
					Thread.currentThread().getContextClassLoader());
			String offlineName = (String) jacoco
					.getDeclaredField("RUNTIMEPACKAGE").get(jacoco)
					+ ".Offline";
			Class<?> offlineBridge = Class.forName(offlineName, false,
					Thread.currentThread().getContextClassLoader());
	...
	/**
	 * Magic: {@link JavaScriptHost} was never compiled because it's a part of
	 * the hosted mode infrastructure. However, unlike {@link #BRIDGE_CLASSES},
	 * {@code JavaScriptHost} needs a separate copy per inside the ClassLoader
	 * for each module.
	 */
	...
	private static JClassType findImplementingTypeForMethod(JClassType type,
		...
		while (type != null) {
			for (JMethod candidate : type.getMethods()) {
				if (hasMatchingErasedSignature(method, methodParamTypes,
						candidate)) {
					return type;
				}
			}
	...
	private final Map<Object, Object> weakJavaWrapperCache = new MapMaker()
	...
	public CompilingClassLoader(TreeLogger logger,
		...
		if (jsoType != null) {
			...
			for (JClassType type : jsoTypes) {
				...
				// 1- We shouldn't need re-write those as they will not be used
				// in dev mode.
			...
			classRewriter = new HostedModeClassRewriter(jsoTypeNames,
					jsoSuperTypes, singleJsoImplData, mapper);
		} else {
	...
	protected Class<?> findClass(String className)
		...
		if (className.equals("com.google.gwt.core.ext.debug.JsoEval")) {
			// In addition to the system ClassLoader, we let JsoEval be
			// available
		...
		try {
			...
			if (HasAnnotation.hasAnnotation(classBytes, GwtScriptOnly.class)) {
				 ...
				 * scriptOnlyClassLoader. This prevents deadlock conditions when
				 * a class from scriptOnlyClassLoader ends up trying to call
				 * back into this classloader from another thread.
				 */
				loadLock.unlock();
			 ...
			 * have circular dependencies on one another via JSNI and
			 * inheritance. This check ensures that a class's supertype can
			 * refer to the subtype (static members, etc) via JSNI references by
			 * ensuring that the Class for the subtype will have been defined
			 * before injecting the JSNI for the supertype.
			 */
			 ...
			 * dispId assignment is based around reflection on Class objects.
			 * Don't inject JSNI when loading a JSO interface class; just wait
			 * until the implementation class is loaded.
			 */
			if (!classRewriter.isJsoIntf(className)) {
				CompilationUnit unit = getUnitForClassName(
			...
			if (localInjection) {
				try {
					/*
					 * Can't use an iterator here because calling injectJsniFor
					 * may cause additional entries to be added.
					 */
	...
	protected Class<?> loadClass(String name, boolean resolve)
			throws ClassNotFoundException {
		// at design time we want to provide parent ClassLoader, so keep default
		// implementation
	...
	private final class DispatchClassInfoOracle {
		 ...
		 * name. It is important to avoid initializing the class because this
		 * would potentially cause initializers to be run in a different order
		 * than in web mode. Moreover, we may not have injected all of the JSNI
		 * code required to initialize the class.
		 *
		...
		/**
		 * Returns the {@link java.lang.Class} object for a class that matches
		 * the source or binary name given.
		 *
		 * @param className
		 *            binary or source name
		...
		private Class<?> getClassFromBinaryOrSourceName(String className) {
			...
			JClassType type = typeOracle
					.findType(SourceOrBinaryName.toSourceName(className));
			if (type != null) {
		 ...
		 * permutations of the class name with regards to source or binary forms
		 * map on the same {@link DispatchClassInfo}.
		 *
		 * @param className
		 *            binary or source name for a class
		 * @return {@link DispatchClassInfo} associated with the binary or
		 *         source class name; null if there is none
		 */
		private DispatchClassInfo getClassInfoFromClassName(String className) {
			DispatchClassInfo dispClassInfo = classNameToClassInfo
					.get(className);
			...
			if (cls == null) {
				/*
				 * default to return null; mask the specific error and let the
				 * caller handle it
				 */
				return null;
			}
			...
			/*
			 * we need to create a new DispatchClassInfo since we have never
			 * seen this class before under any source or binary class name
			 */
			int classId = classIdToClassInfo.size();
			dispClassInfo = new DispatchClassInfo(cls, classId);
			classIdToClassInfo.add(dispClassInfo);
			/*
			 * Whether we created a new DispatchClassInfo or not, we need to add
			 * a mapping for this name
			 */
	...
	private static class MultiParentClassLoader extends ClassLoader {
		...
		protected Class<?> loadClass(String name, boolean resolve)
			...
			try {
				...
				// Make a second attempt not only on ClassNotFoundExceptions,
		...
		public MyInstanceMethodOracle(Set<JClassType> jsoTypes,
			...
			for (JClassType type : jsoTypes) {
				for (JMethod method : type.getMethods()) {
					if (!method.isStatic()) {
						assert !method
								.isAbstract() : "Abstract method in JSO type "
										+ method;
						add(type, method);
					}
				}
			...
			/*
			 * Record the implementing types for methods defined in
			 * SingleJsoImpl interfaces. We have to make this pass because of
			 * possible variance in the return types between the abstract method
			 * declaration in the interface and the concrete method.
			 */
			for (String intfName : jsoData.getSingleJsoIntfTypes()) {
				// We only store the name in the data block to keep it
				// lightweight
				JClassType intf = typeOracle
						.findType(Name.InternalName.toSourceName(intfName));
				JClassType jso = typeOracle.getSingleJsoImpl(intf);
				for (JMethod method : intf.getMethods()) {
					JClassType implementingJso = findImplementingTypeForMethod(
							jso, method);
			...
			for (JMethod method : javaLangObject.getMethods()) {
				if (!method.isStatic()) {
					...
					signatureToDeclaringClasses.put(signature,
							declaringClasses);
	 ...
	 * interfaces with methods. This includes the set of SingleJsoImpl
	 * interfaces actually implemented by a JSO type, the mangled method names,
	 * and the names of the Methods that should actually implement the virtual
	 * functions.
	 *
	...
	private class MySingleJsoImplData implements SingleJsoImplData {
		...
		private final Set<String> unmodifiableIntfNames = Collections
				.unmodifiableSet(singleJsoImplTypes);

		private final SortedSet<String> unmodifiableNames = Collections
				.unmodifiableSortedSet(mangledNames);

		public MySingleJsoImplData() {
			// Loop over all interfaces with JSO implementations
			typeLoop: for (JClassType type : typeOracle
				 ...
				 * method could be called, we greatly simplify the logic
				 * necessary to rewrite the call-site.
				 *
				 * interface A {void m();}
				 *
				...
				for (JMethod intfMethod : type.getOverridableMethods()) {
					assert intfMethod
							.isAbstract() : "Expecting only abstract methods";
					/*
					 ...
					 *
					 * Methods inherited from interfaces A and B must be
					 * dispatched to their respective JSO implementations.
					 */
					JClassType implementingType = typeOracle
							.getSingleJsoImpl(intfMethod.getEnclosingType());
					if (implementingType == null || implementingType
							.isAnnotationPresent(GwtScriptOnly.class)) {
						/*
						 * This means that there is no concrete implementation
						 * of the interface by a JSO. Any implementation that
						 * might be created by a Generator won't be a JSO
						 * subtype, so we'll just ignore it as an actionable
						 * type. Were Generators ever able to create new JSO
						 * subtypes, we'd have to speculatively rewrite the
						 * callsite.
						 */
						continue typeLoop;
					}
					 ...
					 */
					singleJsoImplTypes
							.add(canonicalizeClassName(getBinaryName(type)));
					/*
					 * The mangled name adds the current interface like
					 *
					 * com_foo_Bar_methodName
					 */
					String mangledName = getBinaryName(type).replace('.', '_')
							+ "_" + intfMethod.getName();
					mangledNames.add(mangledName);
					/*
					 * Handle virtual overrides by finding the method that we
					 * would normally invoke and using its declaring class as
					 * the dispatch target.
					 */
					...
					// implementingmethod and implementingType cannot be null
					// here
					/*
					 * Create a pseudo-method declaration for the interface
					 * method. This should look something like
					 *
					 ...
					 */
					{
						String decl = getBinaryOrPrimitiveName(
								intfMethod.getReturnType().getErasedType())
								+ " " + intfMethod.getName() + "(";
						for (JParameter param : intfMethod.getParameters()) {
							decl += ",";
							decl += getBinaryOrPrimitiveName(
									param.getType().getErasedType());
						}
						decl += ")";
						org.objectweb.asm.commons.Method declaration = org.objectweb.asm.commons.Method
								.getMethod(decl);
						addToMap(mangledNamesToDeclarations, mangledName,
								declaration);
					}
					/*
					 * Cook up the a pseudo-method declaration for the concrete
					 * type. This should look something like
					 *
					 ...
					 */
					{
						String returnName = getBinaryOrPrimitiveName(
								implementingMethod.getReturnType()
										.getErasedType());
						String jsoName = getBinaryOrPrimitiveName(
								implementingType);
						String decl = returnName + " " + intfMethod.getName()
								+ "$ (" + jsoName;
						for (JParameter param : implementingMethod
								.getParameters()) {
							decl += ",";
							decl += getBinaryOrPrimitiveName(
									param.getType().getErasedType());
						...
						decl += ")";
						org.objectweb.asm.commons.Method toImplement = org.objectweb.asm.commons.Method
								.getMethod(decl);
			...
			if (logger.isLoggable(TreeLogger.SPAM)) {
				...
				for (Map.Entry<String, List<org.objectweb.asm.commons.Method>> entry : mangledNamesToImplementations
						.entrySet()) {
					dumpLogger.log(TreeLogger.SPAM,
		...
		private JMethod findOverloadUsingErasure(JClassType implementingType,
			...
			for (int i = 0; i < numParams; i++) {
				erasedTypes[i] = intfMethod.getParameters()[i].getType()
						.getErasedType();
			}
			outer: for (JMethod method : implementingType
					.getOverloads(intfMethod.getName())) {
