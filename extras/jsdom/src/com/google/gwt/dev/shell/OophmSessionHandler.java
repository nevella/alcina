...
public class OophmSessionHandler extends SessionHandlerServer {
	...
	public void freeValue(BrowserChannelServer channel, int[] ids) {
		ServerObjectsTable localObjects = channel
		...
		try {
			...
			TreeLogger branch = logger.branch(TreeLogger.SPAM,
					"Client special invoke of getProperty(" + dispId + " ["
							+ ccl.getClassInfoByDispId(dispId).getMember(dispId)
							+ "]) on " + obj.toString(),
					null);
	...
	public ExceptionOrReturnValue invoke(BrowserChannelServer channel,
		...
		Event jsToJavaCallEvent = SpeedTracerLogger.start(
				channel.getDevModeSession(), DevModeEventType.JS_TO_JAVA_CALL);
		ServerObjectsTable localObjects = channel
				.getJavaObjectsExposedInBrowser();
		ModuleSpace moduleSpace = moduleMap.get(channel);
		ModuleHandle moduleHandle = moduleHandleMap.get(channel);
		...
		if (SpeedTracerLogger.jsniCallLoggingEnabled()) {
			DispatchClassInfo clsInfo = cl
					.getClassInfoByDispId(methodDispatchId);
			if (clsInfo != null) {
		...
		if (logger.isLoggable(TreeLogger.SPAM)) {
			...
			DispatchClassInfo classInfo = cl
					.getClassInfoByDispId(methodDispatchId);
			if (classInfo != null) {
	...
	public synchronized TreeLogger loadModule(BrowserChannelServer channel,
		...
		Event moduleInit = SpeedTracerLogger.start(channel.getDevModeSession(),
				DevModeEventType.MODULE_INIT, "Module Name", moduleName);
		ModuleHandle moduleHandle = host.createModuleLogger(moduleName,
				userAgent, url, tabKey, sessionKey, channel, userAgentIcon);
		TreeLogger logger = moduleHandle.getLogger();
		...
		try {
			...
			if (logger.isLoggable(TreeLogger.SPAM)) {
				logger.log(TreeLogger.SPAM,
						"Client special invoke of setProperty(id=" + dispId
	...
	public void unloadModule(BrowserChannelServer channel, String moduleName) {
		...
		if (moduleSpace == null || moduleHandle == null) {
			topLogger.log(TreeLogger.ERROR,
