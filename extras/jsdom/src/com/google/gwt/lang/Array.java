...
/**
 * This is an intrinsic class that contains the implementation details for Java
 * arrays.
 * <p>
 *
...
public final class Array {
	// Array element type classes. Needs to be in sync with enums in
	// TypeCategory.java.
	...
	// The purpose of this method is to avoid introducing a public api to {@link
	// java.lang.Class}.
	private static native <T> Class<T>
			getClassLiteralForArrayImpl(Class<?> clazz, int dimensions) /*-{
	...
	private static native int getElementTypeCategory(Object array) /*-{
																	return array.__elementTypeCategory$ == null ? @Array::TYPE_JS_UNKNOWN_NATIVE
	...
	private static Object initMultidimensionalArray(Class<?> leafClassLiteral,
			JavaScriptObject[] castableTypeMapExprs,
			JavaScriptObject[] elementTypeIds, int leafElementTypeCategory,
			int[] dimExprs, int index, int count) {
		int length = dimExprs[index];
		boolean isLastDimension = (index == (count - 1));
		// All dimensions but the last are plain reference types.
		int elementTypeCategory = isLastDimension ? leafElementTypeCategory
				: TYPE_JAVA_OBJECT;
		Object result = initializeArrayElementsWithDefaults(elementTypeCategory,
				length);
		if (leafElementTypeCategory != TYPE_JS_UNKNOWN_NATIVE) {
			stampJavaTypeInfo(
					getClassLiteralForArray(leafClassLiteral, count - index),
					castableTypeMapExprs[index], elementTypeIds[index],
					elementTypeCategory, result);
		}
		if (!isLastDimension) {
			// Recurse to next dimension.
			++index;
			for (int i = 0; i < length; ++i) {
				set(result, i, initMultidimensionalArray(leafClassLiteral,
						castableTypeMapExprs, elementTypeIds,
						leafElementTypeCategory, dimExprs, index, count));
			}
		}
		return result;
	}
