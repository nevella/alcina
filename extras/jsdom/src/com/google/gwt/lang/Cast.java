 ...
 * require code.<br />
 *
 * The cast operations are only as accurate as the contents of the
 * castableTypeMaps and should not be used directly by user code. The compiler
 * takes care to record most cast operations in user code so that it can build
 * limited but accurate castableTypeMaps.
 */
final class Cast {
	/**
	 * As plain JavaScript Strings (not monkey patched) are used to model Java
	 * Strings, {@code  stringCastMap} stores runtime type info for cast
	 * purposes for string objects. Same applies to Double and Boolean.
	 *
	 * NOTE: it is important that the field is left uninitialized so that Cast
	 * does not require a clinit.
	 */
	// NOTE: if any of these three are edited, update JProgram.DispatchType's
	// constructor
	...
	static Object castTo(Object src, JavaScriptObject dstId) {
		...
		// return src;
		return castToAllowJso(src, dstId);
	}
	...
	static Object castToAllowJso(Object src, JavaScriptObject dstId) {
		checkType(
				src == null || isJavaScriptObject(src) || canCast(src, dstId));
		return src;
	}
	...
	/**
	 * Allow a cast to (untyped) array. This case covers single and
	 * multidimensional JsType arrays.
	 */
	...
	/**
	 * Allow a dynamic cast to a native GLOBAL.Object if it is JavaScript
	 * object.
	 */
	...
	/**
	 * Returns true if Object can dispatch instance methods and does not need a
	 * compiler provided trampoline.
	 *
	 * Java non primitive objects fall into 3 classes: Strings, arrays (of
	 * primitive or non primitive types) and regular Java Objects (all others).
	 *
	 * Only regular Java object have dynamic instance dispatch, strings and
	 * arrays need compiler generated trampolines to implement instance
	 * dispatch.
	 */
	 ...
	 * that could be a String, 'unboxed' Double, or 'unboxed' Boolean.
	 */
