...
class DOMImplIE9 extends DOMImplStandardBase {
	...
	protected native String getNumericStyleProperty(StyleRemote style,
			String name) /*-{
        return typeof (style[name]) == "number" ? "" + style[name]
                : style[name];
	}-*/;
	...
	protected native int getTabIndex(Element elem) /*-{
        return elem.tabIndex < 65535 ? elem.tabIndex
                : -(elem.tabIndex % 65535) - 1;
	}-*/;
	...
	@Override
	protected native void selectRemoveOption(Element select,
			int index) /*-{
        try {
