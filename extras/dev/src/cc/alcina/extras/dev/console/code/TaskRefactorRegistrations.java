...
public class TaskRefactorRegistrations
	...
	protected void performAction0(TaskRefactorRegistrations task)
			throws Exception {
		// body removed due to refactored class removal
	}
	...
	// Optional<MemberValuePair> outImplementation = implementation ==
	...
	// (ClassOrInterfaceDeclaration) location
	// .getParentNode().get();
	...
	// if (!node.isInterface()) {
	// CompilationUnits.ClassOrInterfaceDeclarationWrapper declaration = new
	// CompilationUnits.ClassOrInterfaceDeclarationWrapper(
	// unit, node);
