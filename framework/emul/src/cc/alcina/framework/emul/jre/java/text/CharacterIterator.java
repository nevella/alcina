 ...
 */
/*
 * (C) Copyright Taligent, Inc. 1996, 1997 - All Rights Reserved (C) Copyright
 * IBM Corp. 1996 - 1998 - All Rights Reserved
 *
 * The original version of this source code and documentation is copyrighted and
 * owned by Taligent, Inc., a wholly-owned subsidiary of IBM. These materials
 * are provided under terms of a License Agreement between Taligent and Sun.
 * This technology is protected by multiple US and International patents.
 *
 * This notice and attribution to Taligent may not be removed. Taligent is a
 * registered trademark of Taligent, Inc.
 *
 */
package java.text;
 ...
 * getBeginIndex() to getEndIndex(); the value getEndIndex() is included to
 * allow handling of zero-length text ranges and for historical reasons. The
 * current index can be retrieved by calling getIndex() and set directly by
 ...
 * 	for (char c = iter.last(); c != CharacterIterator.DONE; c = iter
 * 			.previous()) {
 * 		processChar(c);
 * 	}
 * }
 * }</pre>
 *
 * Traverse both forward and backward from a given position in the text. Calls
 * to notBoundary() in this example represents some additional stopping
 * criteria.
 * 
 * <pre>{@code
...
public interface CharacterIterator extends Cloneable {
	/**
	 * Constant that is returned when the iterator has reached either the end or
	 * the beginning of the text. The value is '\\uFFFF', the "not a character"
