...
public final class Class<T> implements Type {
	 ...
	 * <p>
	 *
	 * NOTE: this method is accessed through JSNI (Violator pattern) to avoid
	 * changing the public API.
	 */
	private static native <T> Class<T>
			getClassLiteralForArray(Class<?> leafClass, int dimensions) /*-{
  var arrayLiterals = leafClass.@Class::arrayLiterals = leafClass.@Class::arrayLiterals
      || [];
  return arrayLiterals[dimensions]
      || (arrayLiterals[dimensions] = leafClass.@Class::createClassLiteralForArray(I)(dimensions));
}-*/;
	 ...
	 */
	@DoNotInline
	static <T> Class<T> createForClass(String packageName,
			String compoundClassName, JavaScriptObject typeId,
			Class<? super T> superclass) {
		Class<T> clazz = createClassObject(packageName, compoundClassName,
				typeId);
	...
	/**
	 * Create a Class object for an anonymous class. Produces no extra string
	 * literals, so compacts nicely
	 *
	 * @skip
	 */
	@DoNotInline
	static <T> Class<T> createForAnonymousClass(Class<? super T> superclass,
		...
		return createForClass(superclass.getPackage().getName(),
				"anon_" + typeId.toString(), typeId, superclass);
	}
	 ...
	 */
	@DoNotInline
	static <T> Class<T> createForEnum(String packageName,
			String compoundClassName, JavaScriptObject typeId,
	 ...
	 */
	@DoNotInline
	static <T> Class<T> createForInterface(String packageName,
			String compoundClassName) {
		Class<T> clazz = createClassObject(packageName, compoundClassName,
				null);
	 ...
	 */
	@DoNotInline
	static Class<?> createForPrimitive(String className,
  ...
  return !!typeId;
	 ...
	 */
	private static <T> Class<T> createClassObject(String packageName,
			String compoundClassName, JavaScriptObject typeId) {
		Class<T> clazz = new Class<T>();
	 ...
	 */
	private static native void maybeSetClassLiteral(JavaScriptObject typeId,
			Class<?> clazz) /*-{
  var proto;
  if (!typeId) {
  ...
  prototype.@java.lang.Object::___clazz = clazz;
	 ...
	 * Written in JSNI to minimize dependencies (on String.+).
	 */
	private static native String join(String separator,
  ...
  return result;
	 ...
	 */
	static native void synthesizeClassNamesFromTypeId(Class<?> clazz,
			JavaScriptObject typeId) /*-{
