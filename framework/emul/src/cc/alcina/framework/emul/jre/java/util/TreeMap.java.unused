...
public class TreeMap<K, V> extends AbstractMap<K, V> implements
	...
	private final class EntryIterator implements Iterator<Entry<K, V>> {
		 ...
		 * 
		 * @param fromKey
		 *            the first key to return in the iterator.
		 * @param toKey
		 *            the upper bound of keys to return.
		 */
	...
	private final class EntrySet extends AbstractSet<Entry<K, V>> {
		...
		public boolean remove(Object o) {
			/*
			 * TODO(jat): is this safe since we can copy a predecessor's data to
			 * an interior node when it is deleted? I think so since we can only
			 * go through the iterator in ascending order, so we will have
			 * passed the one that was copied by the time we can delete a node
			 * that will make that copy.
			 */
			if (!(o instanceof Map.Entry)) {
				return false;
			}
	...
	private static class Node<K, V> implements Entry<K, V> {
		/*
		 * The children are kept in an array to minimize the normal duplication
		 * of code.
		 */
		 ...
		 * @param value
		 * @param isRed
		 *            true if this should be a red node, false for black
		 */
		@SuppressWarnings("unchecked")
	 ...
	 * All uses make use of the done flag to indicate when no further
	 * rebalancing of the tree is required. Remove methods use the found flag to
	 * indicate when the desired key has been found. value is used both to
	 * return the value of a removed node as well as to pass in a value which
	 * must match (used for entrySet().remove(entry)), and the matchValue flag
	 * is used to request this behavior.
	 * 
	 * @param <V>
	 *            value type
	 */
	private static class State<V> {
		...
		@Override
		public String toString() {
			return "State: mv=" + matchValue + " value=" + value + " done="
					+ done + " found=" + found;
		}
	}
		...
		public Set<java.util.Map.Entry<K, V>> entrySet() {
			return new AbstractSet<Entry<K, V>>() {
				...
				public boolean contains(Object o) {
					...
					Map.Entry<K, V> entry = (Entry<K, V>) o; // suppress
																// unchecked
					K key = entry.getKey();
					if (!inRange(key)) {
						return false;
					...
					return lookupEntry != null
							&& Utility.equalsWithNullCheck(
									lookupEntry.getValue(), entry.getValue());
				}
				...
				public boolean remove(Object o) {
					...
					Map.Entry<K, V> entry = (Map.Entry<K, V>) o; // suppress
																	// unchecked
	...
	public V get(Object k) {
		K key = (K) k; // suppress unchecked
		/*
		 * Don't bother validating the key as getEntry does that internally if
		 * the map is non-empty. This is against the spec but matches JRE 1.5
		 * behavior.
		 */
	 ...
	 *             if any correctness errors are detected.
	 */
	 ...
	 * @param isRed
	 *            true if the parent of this node is red.
	 * @return the black height of this subtree.
	 * @throws RuntimeException
	 *             if this RB-tree is not valid.
	 */
	 ...
	 * 
	 * @param tree
	 *            subtree to insert into
	 * @param newNode
	 *            new node to insert
	 * @param state
	 *            result of the insertion: state.found true if the key already
	 *            existed in the tree state.value the old value if the key
	 *            existed
	 * @return the new subtree root
	 */
	private Node<K, V> insert(Node<K, V> tree, Node<K, V> newNode,
			State<V> state) {
		if (tree == null) {
			return newNode;
		} else {
			...
			if (isRed(tree.child[childNum])) {
				if (isRed(tree.child[otherChild(childNum)])) {
					// both children are red (nulls are black), make both black
					// and me red
					tree.isRed = true;
	...
	private boolean removeWithState(K key, State<V> state) {
		...
		// create a fake tree root to minimize special cases for changing the
		// root
				...
				if (isRed(node.child[otherChild(dir)])) {
					...
					if (sibling != null) {
						if (!isRed(sibling.child[otherChild(last)])
							...
							int dir2 = grandparent.child[RIGHT] == parent ? RIGHT
									: LEFT;
							if (isRed(sibling.child[last])) {
								grandparent.child[dir2] = rotateDouble(parent,
										last);
							} else if (isRed(sibling.child[otherChild(last)])) {
								grandparent.child[dir2] = rotateSingle(parent,
										last);
							}
		...
		if (found != null) {
			...
			/**
			 * put the "node" values in "found" (the node with key K) and cut
			 * "node" out. However, we do not want to corrupt "found" -- issue
			 * 3423. So create a new node "newNode" to replace the "found" node.
			 * 
			 * TODO: (jat's suggestion) Consider using rebalance to move the
			 * deleted node to a leaf to avoid the extra traversal in
			 * replaceNode.
			 */
			if (node != found) {
			...
			// cut "node" out
			parent.child[parent.child[RIGHT] == node ? RIGHT : LEFT] = node.child[node.child[LEFT] == null ? RIGHT
					: LEFT];
			size--;
		}
	 ...
	 */
	private void replaceNode(Node<K, V> head, Node<K, V> node,
			Node<K, V> newNode) {
		Node<K, V> parent = head;
		int direction = (parent.key == null || cmp
				.compare(node.key, parent.key) > 0) ? RIGHT : LEFT; // parent.key
																	// == null
																	// handles
																	// the fake
																	// root node
	 ...
	 *            the direction to rotate: 0=left, 1=right
	...
	private Node<K, V> rotateDouble(Node<K, V> tree, int rotateDirection) {
		...
		tree.child[otherChild(rotateDirection)] = rotateSingle(
				tree.child[otherChild(rotateDirection)],
				otherChild(rotateDirection));
		return rotateSingle(tree, rotateDirection);
	}

	/**
	 * Perform a single rotation, pushing the root of the subtree to the
	 * specified direction.
	 * 
	 * <pre>
	 *      A                                              B
	 ...
	 * @param rotateDirection
	 *            the direction to rotate: 0=left rotation, 1=right
	 * @return the new root of the rotated subtree
	 */
