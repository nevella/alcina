...
class ClassSourceFileComposer implements SourceWriter {
	...
	ClassSourceFileComposer(GeneratorContext ctx, PrintWriter printWriter,
		...
		if (targetPackageName == null) {
			throw new IllegalArgumentException(
					"Cannot supply a null package name to"
		...
		if (imports != null && imports.length > 0) {
			println();
			Arrays.stream(imports).sorted()
					.forEach(elem -> println("import " + elem + ";"));
		}
		if (classJavaDocComment != null) {
			...
			// beginJavaDocComment adds its own leading newline, make up for it
			// here.
			println();
		}
		...
		if (category == JavaSourceCategory.CLASS) {
			...
			emitInterfaceDecl(targetClassShortName, superClassName,
					interfaceNames);
		}
		println(" {");
		indent();
	...
	public void print(String s) {
		...
		for (;;) {
			...
			int nl = s.indexOf('\n', pos);
			// If there's no newline or it's at the end of the string, just
			// write
			...
			// Otherwise, write up to and including the newline, note that we
			// just
