 ...
 * WITHOUT WARRANTY OF ANY KIND, either express oqr implied. See the License for
 ...
 *
 * Contributor(s): Mike Ang Mike McCabe
 *
...
public class Parser {
	...
	private Object memberExprTail(TokenStream ts, boolean allowCallSyntax,
		...
		while ((tt = ts.getToken()) > ts.EOF) {
			if (tt == ts.DOT) {
				...
				/*
				 * pn = nf.createBinary(ts.DOT, pn, memberExpr(ts)) is the
				 * version in Brendan's IR C version. Not in ECMA... does it
				 * reflect the 'new' operator syntax he mentioned?
				 */
				lastExprEndLine = ts.getLineno();
			} else if (tt == ts.LB) {
	...
	private Object mulExpr(TokenStream ts)
		...
		while ((tt = ts.peekToken()) == ts.MUL || tt == ts.DIV
				|| tt == ts.MOD) {
			tt = ts.getToken();
			sourceAdd((char) tt);
				...
				do {
					...
					if (tt == ts.COMMA) {
						nf.addChildToBack(pn,
								nf.createLeaf(ts.PRIMARY, ts.UNDEFINED));
					} else {
	...
	private void reportError(TokenStream ts, String messageId)
		 ...
		 * JavaScriptException here even though it is really a different use of
		 * the exception than it is usually used for.
		 */
	 ...
	 * Source information is returned from the parser as a String associated
	 * with function nodes and with the toplevel script. When saved in the
	 * constant pool of a class, this string will be UTF-8 encoded, and token
	 * values will occupy a single byte.
	 * 
	 * Source is saved (mostly) as token numbers. The tokens saved pretty much
	 * correspond to the token stream of a 'canonical' representation of the
	 * input program, as directed by the parser. (There were a few cases where
	 * tokens could have been left out where decompiler could easily reconstruct
	 * them, but I left them in for clarity). (I also looked adding source
	 * collection to TokenStream instead, where I could have limited the changes
	 * to a few lines in getToken... but this wouldn't have saved any space in
	 * the resulting source representation, and would have meant that I'd have
	 * to duplicate parser logic in the decompiler to disambiguate situations
	 * where newlines are important.) NativeFunction.decompile expands the
	 * tokens back into their string representations, using simple lookahead to
	 * correct spacing and indentation.
	 * 
	 * Token types with associated ops (ASSIGN, SHOP, PRIMARY, etc.) are saved
	 * as two-token pairs. Number tokens are stored inline, as a NUMBER token, a
	 * character representing the type, and either 1 or 4 characters
	 * representing the bit-encoding of the number. String types NAME, STRING
	 * and OBJECT are currently stored as a token type, followed by a character
	 * giving the length of the string (assumed to be less than 2^16), followed
	 * by the characters of the string inlined into the source string. Changing
	 * this to some reference to to the string in the compiled class' constant
	 * pool would probably save a lot of space... but would require some method
	 * of deriving the final constant pool entry from information available at
	 * parse time.
	 * 
	 ...
	 * functions are encoded as FUNCTION followed by a function number (encoded
	 * as a character), which is enough information to find the proper generated
	...
	private void sourceAddNumber(double n) {
		...
		/*
		 * encode the number in the source stream. Save as NUMBER type (char |
		 * char char char char) where type is 'D' - double, 'S' - short, 'J' -
		 * long.
		 * 
		 ...
		 * tries to present 1.0 to Java as a float/double) OPT: This is no
		 * longer true. We could compress the format.
		 * 
		 * This may not be the most space-efficient encoding; the chars created
		 * below may take up to 3 bytes in constant pool UTF-8 encoding, so a
		 * Double could take up to 12 bytes.
		 */
		long lbits = (long) n;
		if (lbits != n) {
	...
	private Object statementHelper(TokenStream ts)
			...
			if (peek == ts.CATCH) {
				while (ts.matchToken(ts.CATCH)) {
					...
					nf.addChildToBack(catchblocks, nf.createCatch(varName,
							catchCond, statements(ts), ts.getLineno()));
					mustMatchToken(ts, ts.RC, "msg.no.brace.after.body");
					sourceAdd((char) ts.RC);
					sourceAdd((char) ts.EOL);
				}
		...
		default: {
			...
			if (ts.peekToken() == ts.COLON) {
				/*
				 * check that the last thing the tokenizer returned was a NAME
				 * and that only one token was consumed.
				 */
				if (lastExprType != ts.NAME || (ts.getTokenno() != tokenno))
					reportError(ts, "msg.bad.label");
				ts.getToken(); // eat the COLON
				/*
				 * in the C source, the label is associated with the statement
				 * that follows: nf.addChildToBack(pn, statement(ts));
				 */
				String name = ts.getString();
			 ...
			 * problem with function expressions; the ts.getLineno() == lineno
			 * check was firing after a function definition even though the next
			 * statement was on a new line, because speculative getToken calls
			 * advanced the line number even when they didn't succeed.
			 */
			if (ts.getLineno() == lineno || (lastExprType == ts.FUNCTION
	...
	private Object unaryExpr(TokenStream ts)
		...
		default:
			 ...
			 * 
			 * the rhino scanner seems to work differently than the js scanner
			 * here; in js, it works to have the line number check precede the
			 * peekToken calls. It'd be better if they had similar behavior...
			 */
			int peeked;
	...
	private boolean wellTerminated(TokenStream ts, int lastExprType)
		...
		if (tt != ts.EOF && tt != ts.EOL && tt != ts.SEMI && tt != ts.RC) {
			...
			if ((tt == ts.FUNCTION || lastExprType == ts.FUNCTION)
				...
				/*
				 * Checking against version < 1.2 and version >= 1.0 in the
				 * above line breaks old javascript, so we keep it this way for
				 * now... XXX warning needed?
				 */
				return true;
			} else {
