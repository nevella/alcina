...
public abstract class JClassType
	...
	public JParameterizedType asParameterizationOf(
		...
		for (JClassType supertype : supertypes) {
			...
			if (isParameterized != null
					&& isParameterized.getBaseType() == type) {
	 ...
	 * preferentially over those found in the superinterface hierarchy. Note
	 * that the annotation does not need to be tagged with {@code @Inherited} in
	 * order to be returned from the superclass chain.
	 *
	...
	public <T extends Annotation> T
		...
		while (!searchTypes.isEmpty()) {
			...
			if (toReturn != null) {
				 ...
				 * annotation if it is present on multiple supertypes.
				 */
				break;
			}
	 ...
	 * those found in superclasses and superinterfaces. A method is inheritable
	 * if its accessibility is <code>public</code>, <code>protected</code>, or
	 * package protected.
	 *
	 ...
	 * <ol>
	 * <li>Array: subtypes are those array types with the same number of
	 * dimensions as this type, and whose base element type is a subtype of this
	 * type's element type.</li>
	 * <li>Wildcards: if ? extends X, subtypes are subtypes of X. If ? super X:
	 * no subtypes.
	 * <li>Named type parameter: subtypes are (1) those subtypes of the "base"
	 * type that are assignable to this, plus (2) the first bound of the
	 * parameter, if the bound is both assignable to this and not an interface.
	 * The "base" is the type itself, except for delegating types, where the
	 * base is the result of <code>getBaseType()</code>.</li>
	 * <li>Real class: subtypes are those real classes that are subtypes of this
	 * class in the class hierarchy. Interfaces aren't subtypes of
	 * {@code Object}.</li>
	 * <li>Parameterized type: subtypes are those subtypes of the generic type
	 * of this type which can be parameterized with the right types so that they
	 * have this type as a supertype. The subtypes are parameterized as
	 * described, with minimal parameterization.</li>
	 * <li>Raw type: subtypes are the subtypes of the generic type of this type,
	 * as raw types if they themselves are generic.</li>
	 * </ol>
	 */
	@Override
	 ...
	 *
	 * @param possibleSubtype
	 *            possible subtype of this {@link JClassType}
	 * @return <code>true</code> if this {@link JClassType} is assignable from
	 *         the specified {@link JClassType} parameter
	 *
	 * @throws NullPointerException
	 *             if <code>possibleSubtype</code> is <code>null</code>
	 */
	@Override
	public boolean isAssignableFrom(
		...
		return new AssignabilityChecker()
				.isAssignable((JClassType) possibleSubtype, this);
	}
	 ...
	 *            possible supertype of this {@link JClassType}
	 * @return <code>true</code> if this {@link JClassType} is assignable to the
	 *         specified {@link JClassType} parameter
	 *
	 * @throws NullPointerException
	 *             if <code>possibleSupertype</code> is <code>null</code>
	 */
	@Override
	public boolean isAssignableTo(
		...
		return new AssignabilityChecker().isAssignable(this,
