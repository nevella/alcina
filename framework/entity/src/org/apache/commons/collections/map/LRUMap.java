...
/**
 * A <code>Map</code> implementation with a fixed maximum size which removes the
 ...
 * change the order. Queries such as containsKey and containsValue or access via
 * views also do not change the order.
 * <p>
 ...
 * least recently used to most recently used. Iterators from map views can also
 * be cast to <code>OrderedIterator</code> if required.
 * <p>
 ...
 * {@link java.util.Collections#synchronizedMap(Map)}. This class may throw
 * <code>NullPointerException</code>'s when accessed by concurrent threads.
 *
 * @since Commons Collections 3.0 (previously in main package v1.0)
 * @version $Revision: 348299 $ $Date: 2005-11-22 23:51:45 +0000 (Tue, 22 Nov
 *          2005) $
 *
...
public class LRUMap extends AbstractLinkedMap
	 ...
	 *
	 * @param maxSize
	 *            the maximum size of the map, -1 for no limit,
	 * @param loadFactor
	 *            the load factor
	 * @throws IllegalArgumentException
	 *             if the maximum size is less than one
	 ...
	 * <p>
	 * This implementation checks the LRU size and determines whether to discard
	 * an entry or not using {@link #removeLRU(AbstractLinkedMap.LinkEntry)}.
	 * <p>
	...
	protected void addMapping(int hashIndex, int hashCode, Object key,
		...
		if (isFull()) {
			...
			if (scanUntilRemovable) {
				...
				if (reuse == null) {
					throw new IllegalStateException(
							"Entry.after=null, header.after" + header.after
									+ " header.before" + header.before + " key="
									+ key + " value=" + value + " size=" + size
									+ " maxSize=" + maxSize
			...
			if (removeLRUEntry) {
				if (reuse == null) {
					throw new IllegalStateException("reuse=null, header.after="
							+ header.after + " header.before" + header.before
	 ...
	 * in the map has been passed. If the scanUntilRemovable flag is false, the
	 * map will exceed the maximum size.
	 * <p>
	...
	protected void reuseMapping(LinkEntry entry, int hashIndex, int hashCode,
		...
		try {
			...
			if (loop == null) {
				throw new IllegalStateException(
						"Entry.next=null, data[removeIndex]="
								+ data[removeIndex] + " previous=" + previous
								+ " key=" + key + " value=" + value + " size="
								+ size + " maxSize=" + maxSize
								+ " Please check that your keys are immutable, and that you have used synchronization properly."
								+ " If so, then please report this to commons-dev@jakarta.apache.org as a bug.");
			...
			throw new IllegalStateException("NPE, entry=" + entry
					+ " entryIsHeader=" + (entry == header) + " key=" + key
					+ " value=" + value + " size=" + size + " maxSize="
					+ maxSize
					+ " Please check that your keys are immutable, and that you have used synchronization properly."
					+ " If so, then please report this to commons-dev@jakarta.apache.org as a bug.");
		}
