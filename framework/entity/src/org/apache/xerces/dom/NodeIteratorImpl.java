 ...
 * before the actual DOM transformation. If not called by the DOM, the client
...
public class NodeIteratorImpl implements NodeIterator {
	...
	/**
	 * When TRUE, the children of entites references are returned in the
	 * iterator.
	 */
	 ...
	 * depth-first order which also passes the filter, and whatToShow. If there
	 * is no next node which passes these criteria, then return null.
	 */
	public Node nextNode() {
		...
		accepted_loop: while (!accepted) {
			...
			fForward = true; // REVIST: should direction be set forward before
								// null check?
	...
	public void removeNode(Node node) {
		...
		// if (!fForward)
		{
			...
			if (next != null) {
				// normal case: there _are_ nodes following this in the
				// iterator.
				fCurrentNode = next;
			} else {
