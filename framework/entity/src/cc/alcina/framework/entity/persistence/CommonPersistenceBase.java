...
public abstract class CommonPersistenceBase implements CommonPersistenceLocal {
     ...
     * @param onlyIfOptimal
     */
    public DetachedEntityCache cacheEntities(List<DomainTransformEvent> items, boolean fixWithPrecreate, boolean onlyCacheIfWouldOptimiseCalls) {
        ...
        for (DomainTransformEvent dte : items) {
            ...
            if (CommonUtils.compareWithNullMinusOne(precreDate, dte.getUtcDate()) > 0) {
        ...
        for (Entry<Class, Set<Long>> entry : lkp.entrySet()) {
            ...
            if (clazz == null || (ids.size() < 2 && onlyCacheIfWouldOptimiseCalls)) {
            ...
            if (storageClass != null) {
                for (int i = 0; i < ids.size(); i += PRECACHE_RQ_SIZE) {
                    List<Long> idsSlice = ids.subList(i, Math.min(ids.size(), i + PRECACHE_RQ_SIZE));
                    if (ids.size() > PRECACHE_RQ_SIZE) {
                        logger.info("Transform precache - {} - {}", clazz.getSimpleName(), new IntPair(i, i + idsSlice.size()));
                    }
                    Query query = getEntityManager().createQuery(String.format("from %s where id in %s", storageClass.getSimpleName(), EntityPersistenceHelper.toInClause(idsSlice)));
                ...
                if (fixWithPrecreate && storageClass == clazz) {
                    ...
                    for (Long lv : entry.getValue()) {
                        System.out.println(String.format("tp: create object: %10s %s", lv, clazz.getSimpleName()));
                        ThreadlocalTransformManager.get().newInstance(clazz, lv, 0);
                    }
                }
            }
        }
    ...
    public <T> T ensure(Class<T> clazz, String key, Object value) {
        String eql = Ax.format("from %s where %s = ?1", clazz.getSimpleName(), key);
        ...
        if (list.isEmpty()) {
            ...
            Reflections.at(instance.getClass()).property(key).set(instance, value);
            return instance;
        } else {
    ...
    public void ensurePublicationCounters() {
        try (Connection conn = Registry.impl(CommonPersistenceConnectionProvider.class).getConnection()) {
            Statement statement = conn.createStatement();
            statement.execute("create index publicationcounter_user_id on publicationcounter using btree(user_id)");
        } catch (Exception e) {
            e.printStackTrace();
        }
        List<Object[]> objects = getEntityManager().createQuery(Ax.format("select max(userPublicationId), user.id from %s j group by user.id", getImplementationSimpleClassName(Publication.class))).getResultList();
        for (Object[] array : objects) {
            long max = (long) array[0];
            long userId = (long) array[1];
            Class<? extends PublicationCounter> clazz = PersistentImpl.getImplementation(PublicationCounter.class);
            Class<? extends IUser> iUserClass = PersistentImpl.getImplementation(IUser.class);
    ...
    public void expandExceptionInfo(DomainTransformLayerWrapper wrapper) {
        ...
        try {
            ...
            for (DomainTransformException ex : wrapper.response.getTransformExceptions()) {
    ...
    @Override
    public Integer getHighestPersistedRequestIdForClientInstance(long clientInstanceId) {
        String eql = String.format("select max(dtrq.requestId) as maxId " + "from %s dtrq where dtrq.clientInstance.id=%s ", getImplementation(DomainTransformRequestPersistent.class).getSimpleName(), clientInstanceId);
        Integer result = (Integer) getEntityManager().createQuery(eql).getSingleResult();
        return result;
    }

    @Override
    public <T> T getItemByKeyValueKeyValue(Class<T> clazz, String key1, Object value1, String key2, Object value2) {
        String eql = String.format("from %s where %s=?1 and %s=?2 order by id desc", clazz.getSimpleName(), key1, key2);
        Query q = getEntityManager().createQuery(eql).setParameter(1, value1).setParameter(2, value2);
    ...
    public long getLastTransformId() {
        String eql = String.format("select max(dtep.id) from %s dtep ", getImplementationSimpleClassName(DomainTransformEventPersistent.class));
    ...
    public EntityLocatorMap getLocatorMap(Long clientInstanceId) {
        ...
        EntityManager entryEntityManager = ThreadlocalTransformManager.get().getEntityManager();
        ...
        try {
            ...
            ClientInstance clientInstanceImpl = PersistentImpl.getNewImplementationInstance(ClientInstance.class);
            ...
            ThreadlocalTransformManager.get().setEntityManager(entryEntityManager);
        }
        return map;
    }
    ...
    long getNextPublicationIdForUser(IUser user) {
        Query query = getEntityManager().createNativeQuery("SELECT  id, counter from publicationCounter where user_id=?1 FOR UPDATE").setParameter(1, user.getId());
        List<Object[]> resultList = query.getResultList();
        if (resultList.isEmpty()) {
            logger.warn("No publication counter for user {} - creating... ", user.toIdNameString());
			 ...
			 */
            if (resultList.isEmpty()) {
                List<BigInteger> resultList2 = getEntityManager().createNativeQuery("SELECT nextval('publicationCounter_id_seq');").getResultList();
                getEntityManager().createNativeQuery("INSERT INTO publicationcounter (id,optlock,creationdate,lastmodificationdate,counter,user_id) VALUES (?1,?2,?3,?4,?5,?6);").setParameter(1, resultList2.get(0).longValue()).setParameter(2, 1).setParameter(3, new Timestamp(System.currentTimeMillis())).setParameter(4, new Timestamp(System.currentTimeMillis())).setParameter(5, 0).setParameter(6, user.getId()).executeUpdate();
            }
            resultList = query.getResultList();
        }
        long id = ((BigInteger) resultList.get(0)[0]).longValue();
        long counter = ((BigInteger) resultList.get(0)[1]).longValue();
        getEntityManager().createNativeQuery("update publicationCounter set counter=?1  where id=?2").setParameter(1, ++counter).setParameter(2, id).executeUpdate();
        return counter;
    }

    @Override
    public List<DomainTransformRequestPersistent> getPersistentTransformRequests(long fromId, long toId, Collection<Long> specificIds, boolean mostRecentOnly, boolean populateTransformSourceObjects, Logger logger) {
        ...
        if (mostRecentOnly) {
            ...
            String eql = String.format("select dte.domainTransformRequestPersistent.id  " + "from %s dte " + "order by dte.id desc", getImplementation(DomainTransformEventPersistent.class).getSimpleName(), fromId);
            ...
            DomainTransformRequestPersistent instance = PersistentImpl.getNewImplementationInstance(DomainTransformRequestPersistent.class);
            instance.setId(id);
            dtrps.add(instance);
            ...
            DurationCounter dc = new DurationCounter();
            String idFilter = specificIds == null ? String.format("dtrp.id>=%s and dtrp.id<=%s", fromId, toId) : String.format("dtrp.id in %s", EntityPersistenceHelper.toInClause(specificIds));
            String eql = String.format("select distinct dtrp " + "from %s dtrp " + "inner join fetch dtrp.events " + "inner join fetch dtrp.clientInstance " + " where %s " + "order by dtrp.id", getImplementation(DomainTransformRequestPersistent.class).getSimpleName(), idFilter);
            query = getEntityManager().createQuery(eql);
            dtrps = new ArrayList<DomainTransformRequestPersistent>(query.getResultList());
            if (logTransformReadMetrics) {
                dc.endWithLogger(logger, "dtrp-get-dtrps - %s ms");
            }
        }
        dtrps.stream().forEach(dtrp -> dtrp.getEvents().removeIf(DomainTransformEvent::provideNotApplicableToVmDomain));
        if (populateTransformSourceObjects) {
            DurationCounter dc = new DurationCounter();
            List<DomainTransformEvent> events = (List) DomainTransformRequest.allEvents(dtrps);
            DetachedEntityCache cache = cacheEntities(events, false, false);
            for (DomainTransformEvent event : events) {
                event.setSource((Entity) cache.get(event.getObjectClass(), event.getObjectId()));
            }
            if (logTransformReadMetrics) {
                dc.endWithLogger(logger, "populate transform source events - %s ms");
            }
        }
        DetachedEntityCache cache = new DetachedEntityCache();
        GraphProjectionDataFilter filter = Registry.impl(JPAImplementation.class).getResolvingFilter(Registry.impl(JPAImplementation.class).getClassrefInstantiator(), cache, true);
        GraphProjectionFieldFilter allowSourceFilter = new GraphProjectionFieldFilter() {
            ...
            @Override
            public boolean permitField(Field field, Set<Field> perObjectPermissionFields, Class clazz) {
                return true;
            }
    ...
    public List<Long> listRecentClientInstanceIds(String iidKey) {
        Class<? extends ClientInstance> clientInstanceImpl = (Class<? extends ClientInstance>) getImplementation(ClientInstance.class);
        return getEntityManager().createQuery(String.format("select ci.id from %s ci where ci.iid = ?1 order by id desc", clientInstanceImpl.getSimpleName())).setParameter(1, iidKey).setMaxResults(99).getResultList();
    }
    ...
    public void persistClientLogRecords(List<ClientLogRecords> recordsList) {
        ...
        for (ClientLogRecord clr : records) {
            ClientLogRecordPersistent clrp = PersistentImpl.getNewImplementationInstance(ClientLogRecordPersistent.class);
    ...
    public void persistInternalMetrics(List<InternalMetric> toPersist) {
        ...
        for (InternalMetric metric : toPersist) {
            if (metric.getId() != 0) {
                InternalMetric managed = getEntityManager().find(metric.getClass(), metric.getId());
    ...
    public void ping() {
        MethodContext.instance().withMetricKey("ping").run(() -> {
            List list = getEntityManager().createNativeQuery("select 1").getResultList();
            Ax.out(list.get(0));
        });
    }
    ...
    public EntityLocatorMap reconstituteEntityMap(long clientInstanceId) {
        ...
        try {
            ...
            ClientInstance clientInstance = PersistentImpl.getNewImplementationInstance(ClientInstance.class);
    ...
    public SearchResultsBase search(SearchDefinition def) {
        ...
        Searcher searcher = (Searcher) Registry.get().instantiateSingle(Searcher.class, def.getClass());
    ...
    @Override
    public void setField(Class clazz, Long id, String key, Object value) throws Exception {
        AppPersistenceBase.checkNotReadOnly();
        Object inst = getEntityManager().find(clazz, id);
        PropertyDescriptor descriptor = SEUtilities.getPropertyDescriptorByName(clazz, key);
        descriptor.getWriteMethod().invoke(inst, value);
        getEntityManager().merge(inst);
    }

    @Override
    public DomainTransformLayerWrapper transformInPersistenceContext(TransformPersisterToken transformPersisterToken, TransformPersistenceToken token, DomainTransformLayerWrapper wrapper) {
        AppPersistenceBase.checkNotReadOnly();
        new TransformPersisterInPersistenceContext().transformInPersistenceContext(transformPersisterToken, token, this, getEntityManager(), wrapper);
        return wrapper;
    }

    @Override
    public void updatePublicationMimeMessageId(Long publicationId, String mimeMessageId) {
        getEntityManager().createQuery(Ax.format("update %s set mimeMessageId=?1 where id=?2", getImplementationSimpleClassName(Publication.class))).setParameter(1, mimeMessageId).setParameter(2, publicationId).executeUpdate();
    }
    ...
    public TransformCache warmupTransformCache() {
        ...
        if (!recentTransforms.isEmpty()) {
            ...
            recentTransforms = getRecentTransforms(getPerUserTransformClasses(), 0, result.cacheValidFrom);
    ...
    private void tryAddSourceObjectName(DomainTransformException transformException) {
        if (transformException.getEvent() == null) {
            return;
        }
        try {
            Entity object = TransformManager.get().getObject(transformException.getEvent(), true);
            if (object != null) {
                transformException.setSourceObjectName(HasDisplayName.displayName(object));
            }
        } catch (Exception e) {
    ...
    protected List<DomainTransformEventPersistent> getRecentTransforms(Collection<Class> sourceObjectClasses, int maxTransforms, long sinceId) {
        ...
        String eql = String.format("select dtep from %s dtep " + "where  dtep.id>?1 and dtep.objectClassRef.id in %s " + "order by dtep.id desc", getImplementationSimpleClassName(DomainTransformEventPersistent.class), EntityPersistenceHelper.toInClause(classRefIds));
        Query query = getEntityManager().createQuery(eql).setParameter(1, sinceId);
        if (sinceId == 0) {
    ...
    protected SearchResultsBase projectSearchResults(SearchResultsBase results) {
     ...
     */
    boolean removeProcessedRequests(TransformPersistenceToken persistenceToken) {
        return new TransformPersisterInPersistenceContext().removeProcessedRequests(this, persistenceToken);
    }
}
