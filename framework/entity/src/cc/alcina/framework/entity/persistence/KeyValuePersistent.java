...
public abstract class KeyValuePersistent<T extends KeyValuePersistent> extends VersionableEntity<T> {
    ...
    public static <KVP extends KeyValuePersistent> Optional<KVP> byKey(String key) {
        return byKey(key, true);
    }

    @MvccAccess(type = MvccAccessType.VERIFIED_CORRECT)
    public static <KVP extends KeyValuePersistent> Optional<KVP> byKey(String key, boolean populate) {
        DomainQuery<? extends KeyValuePersistent> query = Domain.query(implementation());
        if (populate) {
            query.contextTrue(LazyPropertyLoadTask.CONTEXT_POPULATE_STREAM_ELEMENT_LAZY_PROPERTIES);
        }
        return (Optional<KVP>) query.filter("key", keyMapper.apply(key)).optional();
    }

    @MvccAccess(type = MvccAccessType.VERIFIED_CORRECT)
    public static <KVP extends KeyValuePersistent> List<KVP> byParentKey(String parentKey) {
        return (List<KVP>) Domain.query(implementation()).contextTrue(LazyPropertyLoadTask.CONTEXT_POPULATE_STREAM_ELEMENT_LAZY_PROPERTIES).filter("parentKey", keyMapper.apply(parentKey)).list();
    }
    ...
    public static void persist(String key, String value) {
        KeyValuePersistent writeable = (KeyValuePersistent) Domain.ensure(implementation(), "key", keyMapper.apply(key));
    ...
    public void setParentKey(String parentKey) {
        ...
        propertyChangeSupport().firePropertyChange("parentKey", old_parentKey, parentKey);
    }
