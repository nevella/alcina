...
public class TransformPersister {
    ...
    public DomainTransformLayerWrapper transformExPersistenceContext(TransformPersistenceToken token) {
        try {
            ...
            DomainTransformLayerWrapper wrapper = new DomainTransformLayerWrapper(token);
            ...
            while (perform) {
                ...
                try {
                    ...
                    if (AppPersistenceBase.isInstanceReadOnly()) {
                        wrapper = new ReadonlyInMemoryPersister().commitInMemoryTransforms(persisterToken, token, wrapper);
                    } else {
                        wrapper = Registry.impl(CommonPersistenceProvider.class).getCommonPersistence().transformInPersistenceContext(persisterToken, token, wrapper);
                    }
                    ...
                    if (ex instanceof DeliberatelyThrownWrapperException) {
                        ...
                        TransformPersisterInPersistenceContext.putExceptionInWrapper(token, ex, wrapper);
                    }
                } finally {
                    LooseContext.pop();
                }
    ...
    public static class TransformPersisterPeer {
        ...
        public static TransformPersister.TransformPersisterPeer get() {
            return Registry.impl(TransformPersister.TransformPersisterPeer.class);
        }
