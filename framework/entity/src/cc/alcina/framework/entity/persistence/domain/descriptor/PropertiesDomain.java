...
public class PropertiesDomain {
    ...
    public <P extends UserPropertyPersistable> P getProperties(Class<P> clazz) {
        IUser user = PersistentSingleton.class.isAssignableFrom(clazz) ? new UserlandProvider().getSystemUser() : PermissionsManager.get().getUser();
        Optional<UserProperty> byUserClass = UserProperty.byUserClass(user, clazz);
        if (!byUserClass.isPresent()) {
            ...
            try {
                ...
                if (!byUserClass.isPresent()) {
                    ...
                    UserProperty property = UserProperty.ensure(user, clazz.getName());
                    property.ensureUserPropertySupport().setPersistable(instance);
        ...
        return (P) byUserClass.get().ensureUserPropertySupport().getPersistable();
    }
