...
public class InternalMetricSliceOracle {
    ...
    public boolean noSliceBecauseNoLongRunningMetrics(Collection<InternalMetricData> values) {
        deadlockedThreadIds = new ArrayList<>();
        return values.stream().allMatch(imd -> imd.isFinished() || !shouldSlice(imd));
    }
    ...
    public boolean shouldSlice(InternalMetricData imd) {
        ...
        long timeSinceLastSlice = System.currentTimeMillis() - imd.lastSliceTime;
        if (deadlockedThreadIds.contains(imd.thread.getId())) {
            return true;
        }
        if (imd.type == InternalMetricTypeAlcina.client) {
            long initialClientDelay = ResourceUtilities.getLong(InternalMetricSliceOracle.class, "initialClientDelay");
