...
public class AuthenticationPersistence {
    ...
    public AuthenticationSession createAuthenticationSession(Iid iid, Date startDate, String sessionId, IUser user, String authenticationType) {
        Class<? extends AuthenticationSession> clazz = PersistentImpl.getImplementation(AuthenticationSession.class);
    ...
    public void createBootstrapClientInstance() {
        BootstrapCreationResult bootstrapInstanceResult = callWithEntityManager(new BootstrapInstanceCreator());
        ...
        bootstrapInstanceResult.createdDetached.stream().forEach(e -> TransformManager.get().objectsToDtes(Collections.singletonList(e.domain().domainVersion()), e.entityClass(), false).forEach(events::add));
        /*
		 * 
		 */
        events.stream().filter(DomainTransformEvent::provideIsCreationTransform).forEach(TransformManager.get()::addTransform);
        events.stream().filter(event -> !event.provideIsCreationTransform()).forEach(TransformManager.get()::addTransform);
        Transactions.getEnqueuedLazyLoads().clear();
        MethodContext.instance().withContextTrue(TransformPersisterInPersistenceContext.CONTEXT_REPLAYING_FOR_LOGS).run(() -> Transaction.commit());
    ...
    public ClientInstance createClientInstance(AuthenticationSession session, String userAgent, String remoteAddress, String referrer, String url) {
        Class<? extends ClientInstance> clazz = PersistentImpl.getImplementation(ClientInstance.class);
        ...
        clientInstance.setReferrer(CommonUtils.trimToWsChars(referrer, 240, true));
        clientInstance.setUrl(CommonUtils.trimToWsChars(url, 240, true));
        clientInstance.setBotUserAgent(EntityLayerUtils.isBotUserAgent(userAgent));
        return clientInstance;
    }

    public Iid createIid(String instanceId) {
        Class<? extends Iid> clazz = PersistentImpl.getImplementation(Iid.class);
    ...
    public AuthenticationSession getAuthenticationSession(String sessionId) {
        Class<? extends AuthenticationSession> clazz = PersistentImpl.getImplementation(AuthenticationSession.class);
        Optional<? extends AuthenticationSession> domainEntity = Domain.query(clazz).filter("sessionId", sessionId).optional();
        if (domainEntity.isPresent()) {
            return domainEntity.get();
        }
        String query = Ax.format("select authenticationSession.id from %s authenticationSession where %s='%s'", clazz.getSimpleName(), "sessionId", sessionId);
        /*
        ...
        // return null;
        // }
        Long id = Ax.first((List<Long>) callWithEntityManager(em -> em.createQuery(query).getResultList()));
        if (id == null) {
            return null;
        }
    ...
    public Iid getIid(String instanceId) {
        Class<? extends Iid> clazz = PersistentImpl.getImplementation(Iid.class);
        Optional<? extends Iid> domainEntity = Domain.query(clazz).filter("instanceId", instanceId).optional();
        if (domainEntity.isPresent()) {
            return domainEntity.get();
        }
        String query = Ax.format("select id from %s where %s='%s'", clazz.getSimpleName(), "instanceId", instanceId);
        /*
		 * caching issue if not in postProcess - see DomainStore.find
		 */
        // if (lookupQueries.put(query, query) != null) {
    ...
    public void putSession(ClientInstance instance, AuthenticationSession session) {
        instance.setAuthenticationSession(session);
        Transaction.commit();
    }

    public boolean validateClientInstance(long clientInstanceId, int clientInstanceAuth) {
        return Optional.ofNullable(getClientInstance(clientInstanceId)).map(ci -> ci.getAuth() == clientInstanceAuth).orElse(false);
    }
    ...
    private <V> V callWithEntityManager(ThrowingFunction<EntityManager, V> function) {
        return CommonPersistenceProvider.get().getCommonPersistence().callWithEntityManager(function);
    }

    private BootstrapCreationResult createBootstrapClientInstance(EntityManager em, String hostName) {
        ...
        Iid iid = (Iid) Ax.first(em.createQuery(Ax.format("select iid from %s iid where instanceId = '%s'", PersistentImpl.getImplementationSimpleClassName(Iid.class), iidUid)).getResultList());
        if (iid == null) {
        ...
        AuthenticationSession authenticationSession = (AuthenticationSession) Ax.first(em.createQuery(Ax.format("select authenticationSession from %s authenticationSession where sessionId = '%s'", PersistentImpl.getImplementationSimpleClassName(AuthenticationSession.class), authenticationSessionUid)).getResultList());
        if (authenticationSession == null) {
            authenticationSession = PersistentImpl.getNewImplementationInstance(AuthenticationSession.class);
            authenticationSession.setSessionId(authenticationSessionUid);
            authenticationSession.setStartTime(new Date());
            authenticationSession.setUser((IUser) persistentImpl(em, (Entity) PermissionsManager.get().getUser()));
        ...
        Iid detachedIid = PersistentImpl.getNewImplementationInstance(Iid.class);
        detachedIid.setId(iid.getId());
        if (createdObjects.contains(iid)) {
            result.createdDetached.add(detachedIid);
        }
        AuthenticationSession detachedSession = PersistentImpl.getNewImplementationInstance(AuthenticationSession.class);
        ...
        ClientInstance detachedInstance = PersistentImpl.getNewImplementationInstance(ClientInstance.class);
    ...
    public static class BootstrapInstanceCreator implements ThrowingFunction<EntityManager, BootstrapCreationResult> {
        ...
        public BootstrapCreationResult apply(EntityManager em) {
            return AuthenticationPersistence.get().createBootstrapClientInstance(em, hostName);
        }
    }
}
