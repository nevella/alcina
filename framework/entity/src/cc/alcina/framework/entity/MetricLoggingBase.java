...
public class MetricLoggingBase {
    ...
    public void average(String key) {
        if (averageCount.containsKey(key)) {
            String message = Ax.format("Metric: %s avg %sms", key, sum.get(key) / averageCount.get(key));
    ...
    private synchronized void end(String key, String extraInfo, Logger overrideLogger) {
        ...
        long delta = ticksSum.containsKey(key) ? ticksSum.get(key) / 1000000 : System.currentTimeMillis() - metricStart.get(key);
        ticksSum.remove(key);
        String units = "ms";
        String message = Ax.format("Metric: %s - %s %s%s", key, delta, units, CommonUtils.isNullOrEmpty(extraInfo) ? "" : " - " + extraInfo);
        if (!muted) {
    ...
    private synchronized String keyWithParents(String key, boolean end) {
        ...
        for (String parentKey : metricStart.keySet()) {
            ...
            if (!terminated.contains(parentKey) && (tid.equals(thisLoggerThreadId) || tid.equals(getCurrentThreadId()))) {
