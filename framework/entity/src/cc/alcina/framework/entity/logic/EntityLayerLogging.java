...
public class EntityLayerLogging {
    ...
    public static void log(LogMessageType componentKey, String message) {
        EntityLayerObjects.get().getPersistentLogger().info(componentKey + " - " + message);
    }

    public static void log(LogMessageType componentKey, String message, Throwable throwable) {
        ...
        if (LooseContext.is(CONTEXT_MUTE_PERSISTENT_LOGGING)) {
            Ax.out("*** persistent log muted => %s :: %s", componentKey, message);
            if (throwable == null) {
            } else {
    ...
    public static long persistentLog(Enum componentKey, Throwable t) {
        if (ResourceUtilities.is("useCommonPersistence")) {
            return CommonPersistenceProvider.get().getCommonPersistence().log(SEUtilities.getFullExceptionMessage(t), componentKey.toString());
        } else {
    ...
    private static org.slf4j.Logger logger = LoggerFactory.getLogger(EntityLayerLogging.class);

    public static void persistentLog(Exception e, Object logMessageType) {
        try {
            if (ResourceUtilities.is("useCommonPersistence")) {
                CommonPersistenceProvider.get().getCommonPersistence().log(SEUtilities.getFullExceptionMessage(e), logMessageType.toString());
            }
    ...
    public static void persistentLog(String message, Object componentKey) {
        Registry.impl(CommonPersistenceProvider.class).getCommonPersistence().log(message, componentKey.toString());
    }
    ...
    public static void setLevel(org.slf4j.Logger slf4jlogger, Level level) {
        ...
        if (!Ax.isTest() && slf4jlogger.getClass().getName().equals("org.slf4j.impl.Slf4jLogger")) {
            // jboss/wildfly
            try {
                Field loggerField = SEUtilities.getFieldByName(slf4jlogger.getClass(), "logger");
                loggerField.setAccessible(true);
                java.util.logging.Logger jblmLogger = (java.util.logging.Logger) loggerField.get(slf4jlogger);
