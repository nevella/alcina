...
public class ImgUtilities {
	...
	public static void writeJpeg(BufferedImage image, OutputStream os,
			Float compressionQuality) throws Exception {
		try {
			writeWIthImageIo(image, os, compressionQuality);
		} catch (Exception e) {
			// https://stackoverflow.com/questions/3432388/imageio-not-able-to-write-a-jpeg-file
			if (e.getMessage()
					.contains("Invalid argument to native writeImage")) {
				Ax.out("Retrying image write with different color space");
