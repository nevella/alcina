...
public class TransformPropagationPolicy {
    ...
    public long getProjectedPersistentCount(Stream<DomainTransformEvent> events) {
    ...
    public boolean shouldPersistEventRecord(DomainTransformEvent event) {
        ...
        switch(propagation.value()) {
			 ...
			 */
                return propagation.persistNonRoot() && !PermissionsManager.get().isRoot();
            default:
    ...
    public boolean shouldPropagate(DomainTransformEvent event) {
        ...
        Predicate<DomainTransformEvent> propagationFilter = LooseContext.get(CONTEXT_PROPAGATION_FILTER);
        if (propagationFilter != null && !propagationFilter.test(event)) {
            return false;
        }
        return propagation.value() == PropagationType.PERSISTENT || propagation.value() == PropagationType.NON_PERSISTENT;
    }

    private DomainTransformPropagation resolvePropagation(DomainTransformEvent event) {
        ...
        // low-priority
        AnnotationLocation location = new AnnotationLocation(event.getObjectClass(), event.getPropertyName() == null ? null : Reflections.at(event.getObjectClass()).property(event.getPropertyName()));
        DomainTransformPropagation annotation = location.getAnnotation(DomainTransformPropagation.class);
        return annotation;
    }
