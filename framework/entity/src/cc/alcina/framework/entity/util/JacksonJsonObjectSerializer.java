...
public class JacksonJsonObjectSerializer implements JsonObjectSerializer {
    ...
    private static CachingMap<JacksonJsonObjectSerializer, ObjectMapperPool> objectMappersPool = new CachingConcurrentMap<>(serializer -> new ObjectMapperPool(serializer), 10);
    ...
    public JacksonJsonObjectSerializer() {
        maxLength = ResourceUtilities.getInteger(JacksonJsonObjectSerializer.class, "maxLength", 10000000);
    }
    ...
    public <T> T deserialize(String deserJson, Class<T> clazz) {
        return runWithObjectMapper(mapper -> {
            ...
            try {
                ...
                if (withDuplicateIdRefCheck && hasDuplicateIds(mapper.readTree(json))) {
                } else {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof JacksonJsonObjectSerializer) {
            ...
            return CommonUtils.equals(withIdRefs, o.withIdRefs, withTypeInfo, o.withTypeInfo, withAllowUnknownProperties, o.withAllowUnknownProperties, withBase64Encoding, o.withBase64Encoding, maxLength, o.maxLength, withPrettyPrint, o.withPrettyPrint, withDefaults, o.withDefaults, withWrapRootValue, o.withWrapRootValue);
        } else {
            return false;
        }
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(withIdRefs, withTypeInfo, withAllowUnknownProperties, withBase64Encoding, maxLength, withPrettyPrint, withDefaults, withWrapRootValue);
    }
    ...
    public String serialize(Object object) {
        return runWithObjectMapper(mapper -> {
            try {
                ...
                if (withBase64Encoding) {
                    json = Base64.getEncoder().encodeToString(json.getBytes(StandardCharsets.UTF_8));
                }
                return json;
            } catch (Exception e) {
                throw new WrappedRuntimeException(e);
    ...
    public JacksonJsonObjectSerializer withDuplicateIdRefCheck(boolean withDuplicateIdRefCheck) {
    ...
    private <T> T runWithObjectMapper(Function<ObjectMapper, T> mapperFunction) {
    ...
    ObjectMapper createObjectMapper() {
        ...
        if (withIdRefs) {
            mapper.setVisibility(mapper.getSerializationConfig().getDefaultVisibilityChecker().withFieldVisibility(JsonAutoDetect.Visibility.ANY).withGetterVisibility(JsonAutoDetect.Visibility.ANY).withSetterVisibility(JsonAutoDetect.Visibility.NONE).withCreatorVisibility(JsonAutoDetect.Visibility.NONE));
    ...
    public static class AddIdAnnotationIntrospector extends JacksonAnnotationIntrospector {
        ...
        public ObjectIdInfo findObjectIdInfo(Annotated ann) {
            ...
            if (annotatedResult == null && Entity.class.isAssignableFrom(ann.getRawType())) {
                ...
                return new ObjectIdInfo(name, Object.class, EntityStringIdGenerator.class, com.fasterxml.jackson.annotation.SimpleObjectIdResolver.class);
            } else {
    ...
    public static class LengthLimitedStringWriter extends StringWriter {
        ...
        private boolean checkLength(int len) {
            ...
            if (getBuffer().length() + len > maxLength) {
                ...
                List<StackTraceElement> frames = Arrays.asList(new Exception().getStackTrace());
                int fromIndex = Math.max(0, frames.size() - 200);
                List<StackTraceElement> topOfTrace = frames.subList(fromIndex, frames.size());
                throw Ax.runtimeException("Limited writer overflow - %s bytes ::\n (0-1000): \n%s\n(last 1000)" + ":\n%s\n\ntop of stack:\n%s", maxLength, first, last, CommonUtils.joinWithNewlines(topOfTrace));
            } else {
                return true;
            }
        }
    ...
    static class ObjectMapperPool {
        ...
        class ObjectMapperFactory extends BasePooledObjectFactory<ObjectMapper> {
