...
public class JaxbUtils {
    ...
    public static synchronized List<Class> ensureJaxbSubclasses(Class addClass) {
        if (jaxbSubclasses == null) {
            jaxbSubclasses = Registry.get().lookup(JaxbContextRegistration.class);
        }
        if (addClass == null) {
    ...
    public static JAXBContext getContext(Collection<Class> classes) throws JAXBException {
    ...
    public static <T> T xmlDeserialize(Class<T> clazz, String xmlStr) {
        ...
        try {
            String preProcessed = Registry.optional(JaxbUtils.PreProcessor.class).map(pp -> pp.preprocess(xmlStr)).orElse(xmlStr);
    ...
    public static String xmlSerialize(Object object, Collection<Class> jaxbClasses) throws JAXBException {
        return xmlSerialize(object, jaxbClasses, false);
    }

    public static String xmlSerialize(Object object, Collection<Class> jaxbClasses, boolean tight) throws JAXBException {
    ...
    protected static <T> List<Class> getContextClasses(Class<T> clazz) {
        ...
        if (!LooseContext.containsKey(JaxbUtils.CONTEXT_CLASSES)) {
            ...
            classes = new ArrayList<>((List) LooseContext.get(JaxbUtils.CONTEXT_CLASSES));
        }
        return classes;
    }
    ...
    private JAXBContext getContext0(Collection<Class> classes) throws JAXBException {
        if (jc == null || !jcClasses.containsAll(classes)) {
            synchronized (jcClasses) {
                ...
                Class[] clazzez = (Class[]) jcClasses.toArray(new Class[jcClasses.size()]);
                try {
