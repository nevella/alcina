 ...
 * unknown entity, the node has no children and its replacement value, when used
 * by <code>Attr.value</code> for example, is empty.
 * <p>
 * As for <code>Entity</code> nodes, <code>EntityReference</code> nodes and all
 * their descendants are readonly.
 * <p >
 * <b>Note:</b> <code>EntityReference</code> nodes may cause element content and
 * attribute value normalization problems when, such as in XML 1.0 and XML
 * Schema, the normalization is performed after entity reference are expanded.
 * <p>
 * See also the
