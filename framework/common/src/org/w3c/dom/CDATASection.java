 ...
 * <code>CharacterData</code> interface through the <code>Text</code> interface.
 * Adjacent <code>CDATASection</code> nodes are not merged by use of the
 * <code>normalize</code> method of the <code>Node</code> interface.
 * <p>
 ...
 * the content, which is illegal in a CDATA section per section 2.7 of
 * [<a href='http://www.w3.org/TR/2004/REC-xml-20040204'>XML 1.0</a>]. The
 * presence of this character sequence must generate a fatal error during
 * serialization or the cdata section must be splitted before the serialization
 * (see also the parameter <code>"split-cdata-sections"</code> in the
 * <code>DOMConfiguration</code> interface).
 * <p >
 * <b>Note:</b> Because no markup is recognized within a
 * <code>CDATASection</code>, character numeric references cannot be used as an
 * escape mechanism when serializing. Therefore, action needs to be taken when
 * serializing a <code>CDATASection</code> with a character encoding where some
 * of the contained characters cannot be represented. Failure to do so would not
 * produce well-formed XML.
 * <p >
 * <b>Note:</b> One potential solution in the serialization process is to end
 * the CDATA section before the character, output the character using a
 ...
 * is not corrupted on serialization more difficult.
 * <p>
 * See also the
