...
public interface Document extends Node {
	 ...
	 * this returns <code>null</code>. For HTML documents, a
	 ...
	 * <br>
	 * This provides direct access to the <code>DocumentType</code> node, child
	 * node of this <code>Document</code>. This node can be set at document
	 ...
	 * implementations may instantiate different types of <code>Document</code>
	 ...
	 * Level 2 HTML</a>] , based on the <code>DocumentType</code> specified at
	 * creation time, changing it afterwards is very unlikely to result in a
	 * change of the features supported.
	 * 
	 * @version DOM Level 3
	 */
	 ...
	 * <p >
	 * <b>Note:</b> If any descendant of the <code>Entity</code> node has an
	 * unbound namespace prefix, the corresponding descendant of the created
	 * <code>EntityReference</code> node is also unbound; (its
	 * <code>namespaceURI</code> is <code>null</code>). The DOM Level 2 and 3 do
	 ...
	 * related to namespaces (<code>prefix</code>, <code>localName</code>, and
	 * <code>namespaceURI</code>). As in the <code>cloneNode</code> operation,
	 * the source node is not altered. User data associated to the imported node
	 * is not carried over. However, if any <code>UserDataHandlers</code> has
	 * been specified along with the associated data these handlers will be
	 * called with the appropriate parameters before this method returns. <br>
	 ...
	 * <dd>The <code>ownerElement</code> attribute is set to <code>null</code>
	 * and the <code>specified</code> flag is set to <code>true</code> on the
	 ...
	 * they always carry their children with them when imported.</dd>
	 * <dt>DOCUMENT_FRAGMENT_NODE</dt>
	 * <dd>If the <code>deep</code> option was set to <code>true</code>, the
	 ...
	 * <dd><code>DocumentType</code> nodes cannot be imported.</dd>
	 * <dt>ELEMENT_NODE</dt>
	 ...
	 * generated <code>Element</code>. Default attributes are <em>not</em>
	 ...
	 * <code>true</code>, the descendants of the source element are recursively
	 * imported and the resulting nodes reassembled to form the corresponding
	 * subtree.</dd>
	 * <dt>ENTITY_NODE</dt>
	 * <dd><code>Entity</code> nodes can be imported, however in the current
	 ...
	 * <dd>Only the <code>EntityReference</code> itself is copied, even if a
	 ...
	 * being imported into provides a definition for this entity name, its value
	 * is assigned.</dd>
	 * <dt>NOTATION_NODE</dt>
	 ...
	 * <code>data</code> values from those of the source node.Note that the
	 * <code>deep</code> parameter has no effect on this type of nodes since
	 * they cannot have any children.</dd>
	 ...
	 * those of the source node.Note that the <code>deep</code> parameter has no
	 * effect on these types of nodes since they cannot have any children.</dd>
	 * </dl>
	 * 
	 * @param importedNode
	 *            The node to import.
	 ...
	 *                [<a href=
	 *                'http://www.w3.org/TR/2004/REC-xml11-20040204/'>XML
	 *                1.1</a>] element into an XML 1.0 document, for instance.
	 * @since DOM Level 2
	 */
	 ...
	 *         </tr>
	 *         <tr>
	 *         <td valign='top' rowspan='1' colspan=
	 *         '1'><code>Node.nodeName</code></td>
	 *         <td valign='top' rowspan='1' colspan='1'>
	 *         <code>qualifiedName</code></td>
	 *         </tr>
	 *         <tr>
	 *         <td valign='top' rowspan='1' colspan=
	 *         '1'><code>Node.namespaceURI</code></td>
	 *         <td valign='top' rowspan='1' colspan='1'>
	 *         <code>namespaceURI</code></td>
	 *         </tr>
	 *         <tr>
	 *         <td valign='top' rowspan='1' colspan=
	 *         '1'><code>Node.prefix</code></td>
	 ...
	 *         <tr>
	 *         <td valign='top' rowspan='1' colspan=
	 *         '1'><code>Node.localName</code></td>
	 *         <td valign='top' rowspan='1' colspan='1'>local name, extracted
	 *         from <code>qualifiedName</code></td>
	 *         </tr>
	 *         <tr>
	 *         <td valign='top' rowspan='1' colspan=
	 *         '1'><code>Element.tagName</code></td>
	 *         <td valign='top' rowspan='1' colspan='1'>
	 *         <code>qualifiedName</code></td>
	 *         </tr>
	 *         </table>
	 ...
	 *                http://www.w3.org/XML/1998/namespace</a>" [<a href=
	 ...
	 *                or if the <code>namespaceURI</code> is "<a href=
	 *                'http://www.w3.org/2000/xmlns/'>http://www.w3.org/2000/xmlns/</a>"
	 ...
	 *         <tr>
	 *         <td valign='top' rowspan='1' colspan=
	 *         '1'><code>Attr.name</code></td>
	 *         <td valign='top' rowspan='1' colspan='1'>
	 *         <code>qualifiedName</code></td>
	 *         </tr>
	 *         <tr>
	 *         <td valign='top' rowspan='1' colspan=
	 *         '1'><code>Node.nodeValue</code></td>
	 ...
	 *                the <code>namespaceURI</code> is different from "<a href=
	 *                'http://www.w3.org/2000/xmlns/'>http://www.w3.org/2000/xmlns/</a>",
	 *                or if the <code>namespaceURI</code> is "<a href=
	 *                'http://www.w3.org/2000/xmlns/'>http://www.w3.org/2000/xmlns/</a>"
	 *                and neither the <code>qualifiedName</code> nor its prefix
	 ...
	 * <a href='http://www.w3.org/TR/2004/REC-xml-20040204#NT-XMLDecl'>XML
	 * declaration</a>, the encoding of this document. This is <code>null</code>
	 * when unspecified or when it is not known, such as when the
	 ...
	 * <p >
	 * <b>Note:</b> No verification is done on the value when setting this
	 * attribute. Applications should use
	 * <code>Document.normalizeDocument()</code> with the "validate" parameter
	 * to verify if the value matches the
	 ...
	 * declaration and if this document supports the "XML" feature, the value is
	 * <code>"1.0"</code>. If this document does not support the "XML" feature,
	 * the value is always <code>null</code>. Changing this attribute will
	 * affect methods that check for invalid characters in XML names.
	 * Application should invoke <code>Document.normalizeDocument()</code> in
	 * order to check for invalid characters in the <code>Node</code>s that are
	 * already part of this <code>Document</code>. <br>
	 * DOM applications may use the
	 * <code>DOMImplementation.hasFeature(feature, version)</code> method with
	 * parameter values "XMLVersion" and "1.0" (respectively) to determine if an
	 ...
	 * [<a href='http://www.w3.org/TR/2004/REC-xml11-20040204/'>XML 1.1</a>]. In
	 * both cases, in order to support XML, an implementation must also support
	 ...
	 * <code>NOT_SUPPORTED_ERR</code> exception for the same version number when
	 ...
	 * possible error case normally defined on DOM operations, and not raise any
	 * <code>DOMException</code> on DOM operations or report errors while using
	 * <code>Document.normalizeDocument()</code>. In case of error, the behavior
	 ...
	 * precedence over this attribute when computing <code>Node.baseURI</code>.
	 * 
	 * @since DOM Level 3
	 */
	 ...
	 * supported, it changes the <code>ownerDocument</code> of the source node,
	 * its children, as well as the attached attribute nodes if there are any.
	 * If the source node has a parent it is first removed from the child list
	 * of its parent. This effectively allows moving a subtree from one document
	 * to another (unlike <code>importNode()</code> which create a copy of the
	 * source node instead of moving it). When it fails, applications should use
	 * <code>Document.importNode()</code> instead. Note that if the adopted node
	 * is already part of this document (i.e. the source and target document are
	 * the same), this method still has the effect of removing the source node
	 * from the child list of its parent, if any. The following list describes
	 ...
	 * <dt>ELEMENT_NODE</dt>
	 * <dd><em>Specified</em> attribute nodes of the source element are adopted.
	 ...
	 * <dt>ENTITY_REFERENCE_NODE</dt>
	 * <dd>Only the <code>EntityReference</code> node itself is adopted, the
	 * descendants are discarded, since the source and destination documents
	 * might have defined the entity differently. If the document being imported
	 * into provides a definition for this entity name, its value is
	 * assigned.</dd>
	 * <dt>NOTATION_NODE</dt>
	 ...
	 * <code>Document.importNode()</code> method, this method does not raise an
	 * <code>INVALID_CHARACTER_ERR</code> exception, and applications should use
	 * the <code>Document.normalizeDocument()</code> method to check if an
	 ...
	 * method updates the replacement tree of <code>EntityReference</code> nodes
	 * and normalizes <code>Text</code> nodes, as defined in the method
	 * <code>Node.normalize()</code>. <br>
	 ...
	 * actually take place. Noticeably this method could also make the document
	 * namespace well-formed according to the algorithm described in , check the
	 * character normalization, remove the <code>CDATASection</code> nodes, etc.
	 * See <code>DOMConfiguration</code> for details.
	 * 
	 * <pre>
	 ...
	 * to update a read-only node or a <code>Node.nodeName</code> contains an
	 * invalid character according to the XML version in use, errors or warnings
	 * (<code>DOMError.SEVERITY_ERROR</code> or
	 ...
	 * node with the new node as described below. <br>
	 * If simply changing the name of the given node is not possible, the
	 * following operations are performed: a new node is created, any registered
	 * event listener is registered on the new node, any user data attached to
	 * the old node is removed from that node, the old node is removed from its
	 * parent if it has one, the children are moved to the new node, if the
	 * renamed node is an <code>Element</code> its attributes are moved to the
	 * new node, the new node is inserted at the position the old node used to
	 * have in its parent's child nodes list if it has one, the user data that
	 * was attached to the old node is attached to the new node. <br>
	 * When the node being renamed is an <code>Element</code> only the specified
	 ...
	 * modifying the existing node or creating a new one as described above, it
	 * is put back. <br>
	 * In addition,
	 * <ul>
	 ...
	 * @exception DOMException
	 *                NOT_SUPPORTED_ERR: Raised when the type of the specified
