...
public interface DOMError {
	 ...
	 * warning. A <code>SEVERITY_WARNING</code> will not cause the processing to
	...
	/**
	 * The severity of the error described by the <code>DOMError</code> is
	 * error. A <code>SEVERITY_ERROR</code> may not cause the processing to stop
	...
	/**
	 * The severity of the error described by the <code>DOMError</code> is fatal
	 * error. A <code>SEVERITY_FATAL_ERROR</code> will cause the normal
	 ...
	 * <code>relatedData</code>. Users should refer to the specification of the
	 * error in order to find its <code>DOMString</code> type and
	 * <code>relatedData</code> definitions if any.
	 * <p >
	 * <b>Note:</b> As an example, <code>Document.normalizeDocument()</code>
	 * does generate warnings when the "split-cdata-sections" parameter is in
	 * use. Therefore, the method generates a <code>SEVERITY_WARNING</code> with
	 ...
	 * is returned by the <code>relatedData</code> attribute.
	 */
