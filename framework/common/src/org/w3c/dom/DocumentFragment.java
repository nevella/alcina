 ...
 * <code>Document</code> object. It is very common to want to be able to extract
 * a portion of a document's tree or to create a new fragment of a document.
 * Imagine implementing a user command like cut or rearranging a document by
 * moving fragments around. It is desirable to have an object which can hold
 * such fragments and it is quite natural to use a Node for this purpose. While
 * it is true that a <code>Document</code> object could fulfill this role, a
 ...
 * a very lightweight object. <code>DocumentFragment</code> is such an object.
 * <p>
 ...
 * <p>
 * When a <code>DocumentFragment</code> is inserted into a <code>Document</code>
 * (or indeed any other <code>Node</code> that may take children) the children
 * of the <code>DocumentFragment</code> and not the
 * <code>DocumentFragment</code> itself are inserted into the <code>Node</code>.
 * This makes the <code>DocumentFragment</code> very useful when the user wishes
 * to create nodes that are siblings; the <code>DocumentFragment</code> acts as
 * the parent of these nodes so that the user can use the standard methods from
 * the <code>Node</code> interface, such as <code>Node.insertBefore</code> and
