 ...
 * represents the textual content (termed
 * <a href='http://www.w3.org/TR/2004/REC-xml-20040204#syntax'>character
 * data</a> in XML) of an <code>Element</code> or <code>Attr</code>. If there is
 * no markup inside an element's content, the text is contained in a single
 * object implementing the <code>Text</code> interface that is the only child of
 * the element. If there is markup, it is parsed into the information items
 * (elements, comments, etc.) and <code>Text</code> nodes that form the list of
...
public interface Text extends CharacterData {
	 ...
	 * <code>offset</code> point, is returned. If the original node had a parent
	 * node, the new node is inserted as the next sibling of the original node.
	 * When the <code>offset</code> is equal to the length of this node, the new
	 * node has no data.
	 * 
	 * @param offset
	 ...
	 * whitespace". The text node is determined to contain whitespace in element
	 * content during the load of the document or if validation occurs while
	 * using <code>Document.normalizeDocument()</code>.
	 * 
	 * @since DOM Level 3
	 */
	...
	/**
	 * Returns all text of <code>Text</code> nodes logically-adjacent text nodes
	 * to this node, concatenated in document order. <br>
	 * For instance, in the example below <code>wholeText</code> on the
	 * <code>Text</code> node that contains "bar" returns "barfoo", while on the
	 ...
	 * of the replacement.</li>
	 * </ul>
	 * <br>
	 * For instance, in the above example calling <code>replaceWholeText</code>
	 * on the <code>Text</code> node that contains "bar" with "yo" in argument
	 ...
	 * <code>EntityReference</code> node "ent" contains an <code>Element</code>
