 ...
 * </tr>
 * <tr>
 * <td valign='top' rowspan='1' colspan='1'><code>Attr</code></td>
 * <td valign='top' rowspan='1' colspan='1'>same as <code>Attr.name</code></td>
 * <td valign='top' rowspan='1' colspan='1'>same as <code>Attr.value</code></td>
 ...
 * <code>CharacterData.data</code>, the content of the CDATA Section</td>
 * <td valign='top' rowspan='1' colspan='1'><code>null</code></td>
 * </tr>
 * <tr>
 ...
 * <td valign='top' rowspan='1' colspan='1'><code>"#comment"</code></td>
 * <td valign='top' rowspan='1' colspan='1'>same as
 * <code>CharacterData.data</code>, the content of the comment</td>
 * <td valign='top' rowspan='1' colspan='1'><code>null</code></td>
 * </tr>
 * <tr>
 ...
 * </tr>
 * <tr>
 * <td valign='top' rowspan='1' colspan='1'><code>DocumentFragment</code></td>
 * <td valign='top' rowspan='1' colspan=
 * '1'><code>"#document-fragment"</code></td>
 * <td valign='top' rowspan='1' colspan='1'><code>null</code></td>
 * <td valign='top' rowspan='1' colspan='1'><code>null</code></td>
 * </tr>
 * <tr>
 ...
 * <td valign='top' rowspan='1' colspan='1'>same as
 * <code>Element.tagName</code></td>
 * <td valign='top' rowspan='1' colspan='1'><code>null</code></td>
 * <td valign='top' rowspan='1' colspan='1'><code>NamedNodeMap</code></td>
 * </tr>
 * <tr>
 ...
 * <tr>
 * <td valign='top' rowspan='1' colspan=
 * '1'><code>ProcessingInstruction</code></td>
 * <td valign='top' rowspan='1' colspan='1'>same as
 * <code>ProcessingInstruction.target</code></td>
 * <td valign='top' rowspan='1' colspan='1'>same as
 ...
 * <td valign='top' rowspan='1' colspan='1'><code>"#text"</code></td>
 * <td valign='top' rowspan='1' colspan='1'>same as
 * <code>CharacterData.data</code>, the content of the text node</td>
 * <td valign='top' rowspan='1' colspan='1'><code>null</code></td>
 * </tr>
 * </table>
 * <p>
...
public interface Node {
	 ...
	 *                readonly or if the parent of the node being inserted is
	 *                readonly. <br>
	 *                NOT_FOUND_ERR: Raised if <code>refChild</code> is not a
	 *                child of this node. <br>
	 *                NOT_SUPPORTED_ERR: if this node is of type
	 *                <code>Document</code>, this exception might be raised if
	 *                the DOM implementation doesn't support the insertion of a
	 *                <code>DocumentType</code> or <code>Element</code> node.
	 * @version DOM Level 3
	 */
	 ...
	 *                one of this node's ancestors or this node itself, or if
	 *                this node is of type <code>Document</code> and the result
	 ...
	 *                NOT_SUPPORTED_ERR: if this node is of type
	 *                <code>Document</code>, this exception might be raised if
	 *                the DOM implementation doesn't support the replacement of
	 ...
	 * <code>parentNode</code> is <code>null</code>) and no user data. User data
	 * associated to the imported node is not carried over. However, if any
	 * <code>UserDataHandlers</code> has been specified along with the
	 ...
	 * attributes, but this method does not copy any children it contains unless
	 * it is a deep clone. This includes text contained in an the
	 * <code>Element</code> since the text is contained in a child
	 * <code>Text</code> node. Cloning an <code>Attr</code> directly, as opposed
	 * to be cloned as part of an <code>Element</code> cloning operation,
	 * returns a specified attribute (<code>specified</code> is
	 * <code>true</code>). Cloning an <code>Attr</code> always clones its
	 * children, since they represent its value, no matter whether this is a
	 * deep clone or not. Cloning an <code>EntityReference</code> automatically
	 * constructs its subtree if a corresponding <code>Entity</code> is
	 * available, no matter whether this is a deep clone or not. Cloning any
	 * other type of node simply returns a copy of this node. <br>
	 * Note that cloning an immutable subtree results in a mutable copy, but the
	 * children of an <code>EntityReference</code> clone are readonly . In
	 ...
	 * [<a href=
	 * 'http://www.w3.org/TR/2003/REC-xptr-framework-20030325/'>XPointer</a>]
	 * lookups) that depend on a particular document tree structure are to be
	 * used. If the parameter "normalize-characters" of the
	 ...
	 * such as <code>Document.createElement()</code>, this is always
	 * <code>null</code>.
	 * <p >
	 * <b>Note:</b> Per the <em>Namespaces in XML</em> Specification
	 * [<a href='http://www.w3.org/TR/1999/REC-xml-names-19990114/'>XML
	 * Namespaces</a>] an attribute does not inherit its namespace from the
	 * element it is attached to. If an attribute is not explicitly given a
	 ...
	 * <code>nodeName</code> attribute, which holds the qualified name, as well
	 * as the <code>tagName</code> and <code>name</code> attributes of the
	 * <code>Element</code> and <code>Attr</code> interfaces, when applicable.
	 * <br>
	 * Setting the prefix to <code>null</code> makes it unspecified, setting it
	 * to an empty string is implementation dependent. <br>
	 ...
	 * the original prefix appear, since the <code>namespaceURI</code> and
	 ...
	 *                in use specified in the <code>Document.xmlVersion</code>
	 *                attribute. <br>
	 *                NO_MODIFICATION_ALLOWED_ERR: Raised if this node is
	 *                readonly. <br>
	 *                NAMESPACE_ERR: Raised if the specified <code>prefix</code>
	 *                is malformed per the Namespaces in XML specification, if
	 ...
	 *                from "<a href='http://www.w3.org/XML/1998/namespace'>
	 *                http://www.w3.org/XML/1998/namespace</a>", if this node is
	 *                an attribute and the specified prefix is "xmlns" and the
	 ...
	 * Level 2 HTML</a>] , the base URI is computed using first the value of the
	 ...
	 *         node.
	 * @exception DOMException
	 *                NOT_SUPPORTED_ERR: when the compared nodes are from
	 *                different DOM implementations that do not coordinate to
	 ...
	 * When it is defined to be <code>null</code>, setting it has no effect. On
	 * setting, any possible children this node may have are removed and, if it
	 * the new string is not empty or <code>null</code>, replaced by a single
	 * <code>Text</code> node containing the string this attribute is set to.
	 * <br>
	 ...
	 * the attribute <code>Text.isElementContentWhitespace</code>). Similarly,
	 * on setting, no parsing is performed either, the input string is taken as
	 * pure textual content. <br>
	 ...
	 * </tr>
	 * <tr>
	 * <td valign='top' rowspan='1' colspan='1'>ELEMENT_NODE, ATTRIBUTE_NODE,
	 * ENTITY_NODE, ENTITY_REFERENCE_NODE, DOCUMENT_FRAGMENT_NODE</td>
	 ...
	 * COMMENT_NODE and PROCESSING_INSTRUCTION_NODE nodes. This is the empty
	 * string if the node has no children.</td>
	 * </tr>
	 * <tr>
	 * <td valign='top' rowspan='1' colspan='1'>TEXT_NODE, CDATA_SECTION_NODE,
	 ...
	 * references returned by the implementation reference the same object. When
	 * two <code>Node</code> references are references to the same object, even
	 * if through a proxy, the references may be used completely
	 ...
	 *            <code>null</code>, the method will return the default
	 ...
	 * equal, though the reverse may not be true. <br>
	 * Two nodes are equal if and only if the following conditions are
	 * satisfied:
	 * <ul>
	 * <li>The two nodes are of the same type.</li>
	 * <li>The following string attributes are equal: <code>nodeName</code>,
	 * <code>localName</code>, <code>namespaceURI</code>, <code>prefix</code>,
	 * <code>nodeValue</code> . This is: they are both <code>null</code>, or
	 * they have the same length and are character for character identical.</li>
	 * <li>The <code>attributes</code> <code>NamedNodeMaps</code> are equal.
	 * This is: they are both <code>null</code>, or they have the same length
	 * and for each node that exists in one map there is a node that exists in
	 * the other map and is equal, although not necessarily at the same index.
	 * </li>
	 * <li>The <code>childNodes</code> <code>NodeLists</code> are equal. This
	 * is: they are both <code>null</code>, or they have the same length and
	 * contain equal nodes at the same index. Note that normalization can affect
	 * equality; to avoid this, nodes should be normalized before being
	 * compared.</li>
	 * </ul>
	 * <br>
	 * For two <code>DocumentType</code> nodes to be equal, the following
	 * conditions must also be satisfied:
	 * <ul>
	 * <li>The following string attributes are equal: <code>publicId</code>,
	 * <code>systemId</code>, <code>internalSubset</code>.</li>
	 ...
	 * <code>parentNode</code> attributes, the <code>specified</code> attribute
	 * for <code>Attr</code> nodes, the <code>schemaTypeInfo</code> attribute
	 * for <code>Attr</code> and <code>Element</code> nodes, the
	 ...
	 * first have been set to this node by calling <code>setUserData</code> with
	 * the same key.
	 * 
	 * @param key
