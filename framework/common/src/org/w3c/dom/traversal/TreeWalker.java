 ...
 * view. For instance, consider a <code>NodeFilter</code> that skips all nodes
 * except for Text nodes and the root node of a document. In the logical view
 * that results, all text nodes will be siblings and appear as direct children
 * of the root node, no matter how deeply nested the structure of the original
 * document.
 * <p>
 * See also the <a href=
 * 'http://www.w3.org/TR/2000/REC-DOM-Level-2-Traversal-Range-20001113'>Document
...
public interface TreeWalker {
	 ...
	 * <code>whatToShow</code> will be skipped, but their children may still be
	 ...
	 * they and their descendants will be rejected. Note that this rejection
	 * takes precedence over <code>whatToShow</code> and the filter, if any.
	 * <br>
	 ...
	 * <code>currentNode</code> may also be explicitly set to any node, whether
	 * or not it is within the subtree specified by the <code>root</code> node
	 * or would be accepted by the filter and <code>whatToShow</code> flags.
	 * Further traversal occurs relative to <code>currentNode</code> even if it
	 * is not part of the current view, by applying the filters in the requested
	 * direction; if no traversal is possible, <code>currentNode</code> is not
	 * changed.
	 * 
	 ...
	 * node. If the search for <code>parentNode</code> attempts to step upward
	 * from the <code>TreeWalker</code>'s <code>root</code> node, or if it fails
	 * to find a visible ancestor node, this method retains the current position
	 * and returns <code>null</code>.
	 * 
	 * @return The new parent node, or <code>null</code> if the current node has
	 *         no parent in the <code>TreeWalker</code>'s logical view.
	 */
	 ...
	 * document order relative to the current node, and returns the new node. If
	 ...
	 * step upward from the <code>TreeWalker</code>'s <code>root</code> node,
