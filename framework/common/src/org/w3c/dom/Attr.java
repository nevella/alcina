 ...
 * <code>Element</code> object. Typically the allowable values for the attribute
 * are defined in a schema associated with the document.
 * <p>
 ...
 * not consider them part of the document tree. Thus, the <code>Node</code>
 * attributes <code>parentNode</code>, <code>previousSibling</code>, and
 * <code>nextSibling</code> have a <code>null</code> value for <code>Attr</code>
 ...
 * default attributes associated with all elements of a given type. Furthermore,
 ...
 * the <code>Node.nodeValue</code> attribute on the <code>Attr</code> instance
 ...
 * but has a default value provided by the schema associated with the document,
 * an attribute node will be created with <code>specified</code> set to
 * <code>false</code>. Removing attribute nodes for which a default value is
 * defined in the schema generates a new attribute node with the default value
 * and <code>specified</code> set to <code>false</code>. If validation occurred
 * while invoking <code>Document.normalizeDocument()</code>, attribute nodes
 * with <code>specified</code> equals to <code>false</code> are recomputed
 * according to the default attribute values provided by the schema. If no
 * default value is associate with this attribute in the schema, the attribute
 * node is discarded.
 * <p>
 * In XML, where the value of an attribute can contain entity references, the
 * child nodes of the <code>Attr</code> node may be either <code>Text</code> or
 * <code>EntityReference</code> nodes (when these are in use; see the
 ...
 * Typically, the <code>value</code> and <code>nodeValue</code> attributes of an
 * <code>Attr</code> node initially returns the normalized value given by the
 * parser. It is also the case after <code>Document.normalizeDocument()</code>
 * is called (assuming the right options have been set). But this may not be the
 * case after mutation, independently of whether the mutation is performed by
 * setting the string value directly or by changing the <code>Attr</code> child
 * nodes. In particular, this is true when
 * <a href='http://www.w3.org/TR/2004/REC-xml-20040204#dt-charref'>character
 * references</a> are involved, given that they are not represented in the DOM
 * and they impact attribute value normalization. On the other hand, if the
 * implementation knows about the schema in use when the attribute value is
 * changed, and it is of a different type than CDATA, it may normalize it again
 * at that time. This is especially true of specialized DOM implementations,
 * such as SVG DOM implementations, which store attribute values in an internal
 ...
 * </tr>
 * <tr>
 * <td valign='top' rowspan='1' colspan='1'>Character reference</td>
 ...
 * </tr>
 * <tr>
 * <td valign='top' rowspan='1' colspan='1'>Entity <code>e</code> with literal
 * newline</td>
 * <td valign='top' rowspan='1' colspan='1'>
 * 
 * <pre>
 ...
 * 
 * </td>
 * <td valign='top' rowspan='1' colspan='1'><em>Dependent on Implementation and
 * Load Options</em></td>
 * <td valign='top' rowspan='1' colspan='1'><em>Dependent on Implementation and
 * Load/Save Options</em></td>
 * </tr>
 * </table>
 * <p>
 * See also the
...
public interface Attr extends Node {
	 ...
	 * same value as the default value) then it is set to <code>true</code>. The
	 * implementation may handle attributes with default values from other
	 ...
	 * See also the method <code>getAttribute</code> on the <code>Element</code>
	 * interface. <br>
	 ...
	 * implementations, may do normalization automatically, even after mutation;
	 * in such case, the value on retrieval may differ from the value on
	 * setting.
	 */
	 ...
	 * unique, the <code>ownerElement</code> of this attribute can be retrieved
	 ...
	 * if this attribute is a schema-determined ID attribute using the <a href=
	 * 'http://www.w3.org/TR/2003/REC-xptr-framework-20030325/#term-sdi'>
	 * schema-determined ID</a> definition in [<a href=
	 ...
	 * while invoking <code>Document.normalizeDocument()</code>, the infoset
	 * <b>[type definition]</b> value is used to determine if this attribute is
	 * a DTD-determined ID attribute using the <a href=
	 * 'http://www.w3.org/TR/2003/REC-xptr-framework-20030325/#term-ddi'>
	 * DTD-determined ID</a> definition in [<a href=
	 * 'http://www.w3.org/TR/2003/REC-xptr-framework-20030325/'>XPointer</a>] .
	 * </li>
	 * <li>from the use of the methods <code>Element.setIdAttribute()</code>,
	 * <code>Element.setIdAttributeNS()</code>, or
	 * <code>Element.setIdAttributeNode()</code>, i.e. it is an user-determined
	 * ID attribute;
	 * <p >
	 * <b>Note:</b> XPointer framework (see section 3.2 in [<a href=
	 * 'http://www.w3.org/TR/2003/REC-xptr-framework-20030325/'>XPointer</a>] )
	 * consider the DOM user-determined ID attribute as being part of the
	 * XPointer externally-determined ID definition.</li>
	 * <li>using mechanisms that are outside the scope of this specification, it
	 * is then an externally-determined ID attribute. This includes using schema
