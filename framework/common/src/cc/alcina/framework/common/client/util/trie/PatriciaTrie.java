 ...
 * 
 * <p>
 * A PATRICIA {@link Trie} is a compressed {@link Trie}. Instead of storing all
 * data at the edges of the {@link Trie} (and having empty internal nodes),
 * PATRICIA stores data in every node. This allows for very efficient traversal,
 * insert, delete, predecessor, successor, prefix, range, and
 * {@link #select(Object)} operations. All operations are performed at worst in
 * O(K) time, where K is the number of bits in the largest item in the tree. In
 * practice, operations actually take O(A(K)) time, where A(K) is the average
 * number of bits of all items in the tree.
 * 
 * <p>
 * Most importantly, PATRICIA requires very few comparisons to keys while doing
 * any operation. While performing a lookup, each comparison (at most K of them,
 * described above) will perform a single bit comparison against the given key,
 * instead of comparing the entire key to another key.
 * 
 * <p>
 ...
 * {@link Trie} can also scan for items that are 'bitwise' (using an XOR metric)
 * by the 'select' method. Bitwise closeness is determined by the
 * {@link KeyAnalyzer} returning true or false for a bit being set or not in a
 * given key.
 * 
 * <p>
 * Any methods here that take an {@link Object} argument may throw a
 * {@link ClassCastException} if the method is expecting an instance of K and it
 * isn't K.
 * 
 * @see <a href="http://en.wikipedia.org/wiki/Radix_tree">Radix Tree</a>
 * @see <a href=
 *      "http://www.csse.monash.edu.au/~lloyd/tildeAlgDS/Tree/PATRICIA">PATRICIA</a>
...
public class PatriciaTrie<K, V> extends AbstractPatriciaTrie<K, V>
	...
	private TrieEntry<K, V> followRight(TrieEntry<K, V> node) {
		...
		while (node.getRight().getBitIndex() > node.getBitIndex()) {
			node = node.getRight();
		}
	 ...
	 * none).
	 * 
	 * This follows four simple branches: - If the uplink that returned us was a
	 * right uplink: - If predecessor's left is a valid uplink from predecessor,
	 * return it. - Else, follow the right path from the predecessor's left. -
	 * If the uplink that returned us was a left uplink: - Loop back through
	 * parents until we encounter a node where node != node.parent.left. - If
	 * node.parent.left is uplink from node.parent: - If node.parent.left is not
	 * root, return it. - If it is root & root isEmpty, return null. - If it is
	 * root & root !isEmpty, return root. - If node.parent.left is not uplink
	 * from node.parent: - Follow right path for first right child from
	 * node.parent.left
	 * 
	 * @param start
	 */
	private TrieEntry<K, V> previousEntry(TrieEntry<K, V> start) {
		...
		if (start.getPredecessor().getRight() == start) {
			if (isValidUplink(start.getPredecessor().getLeft(),
					start.getPredecessor())) {
				return start.getPredecessor().getLeft();
			} else {
				return followRight(start.getPredecessor().getLeft());
			}
		} else {
			TrieEntry<K, V> node = start.getPredecessor();
			while (node.getParent() != null
					&& node == node.getParent().getLeft()) {
				node = node.getParent();
			}
			if (node.getParent() == null) { // can be null if we're looking up
											// root.
				return null;
			}
			if (isValidUplink(node.getParent().getLeft(), node.getParent())) {
				if (node.getParent().getLeft() == root) {
					if (root.isEmpty()) {
						return null;
					} else {
						return root;
				...
				return followRight(node.getParent().getLeft());
			}
		}
	}
	...
	private TrieEntry<K, V> subtree(K prefix) {
		...
		while (true) {
			if (current.getBitIndex() <= path.getBitIndex()
					|| lengthInBits < current.getBitIndex()) {
				break;
			}
			path = current;
		...
		if (isBitSet(prefix, lengthInBits) != isBitSet(entry.getKey(),
				lengthInBits)) {
			return null;
		}
	...
	private final class PrefixRangeEntrySet extends RangeEntrySet {
		...
		public Iterator<Map.Entry<K, V>> iterator() {
			...
			} else if (lengthInBits(delegate.prefix) >= prefixStart
					.getBitIndex()) {
		...
		private final class EntryIterator
			...
			/**
			 * Starts iteration at the given entry & search only within the
			 * given subtree.
			 */
			...
			public void remove() {
				...
				int bitIdx = subtree.getBitIndex();
				if (current == subtree) {
					needsFixing = true;
				}
				super.remove();
				...
				if (bitIdx != subtree.getBitIndex() || needsFixing) {
	...
	private class PrefixRangeMap extends RangeMap {
		...
		/**
		 * This method does two things. It determinates the FROM and TO range of
		 * the {@link PrefixRangeMap} and the number of elements in the range.
		 * This method must be called every time the {@link Trie} has changed.
		 */
		private int fixup() {
			...
			if (size == -1 || PatriciaTrie.this.modCount != expectedModCount) {
				...
				if (fromKey != null) {
					TrieEntry<K, V> prior = previousEntry(
							(TrieEntry<K, V>) entry);
		...
		/**
		 * Returns true if this {@link PrefixRangeMap}'s key is a prefix of the
		 * provided key.
		 */
		@Override
	...
	private class RangeEntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public boolean contains(Object o) {
			...
			return node != null
					&& Tries.areEqual(node.getValue(), entry.getValue());
		}
		...
		private final class EntryIterator
			...
			public boolean hasNext() {
				return next != null
						&& !Tries.areEqual(next.getKey(), excludedKey);
			}
