...
public class LongPair implements Comparable<LongPair>, Predicate<Long>, TreeSerializable {
    ...
    public static List<LongPair> provideUncovered(List<LongPair> covered, LongPair container) {
        ...
        for (int idx = 0; idx <= covered.size(); idx++) {
            ...
            long to = idx == covered.size() ? container.l2 : covered.get(idx).l1;
            if (from != to) {
    ...
    public int compareTo(LongPair ip) {
        return l1 < ip.l1 ? -1 : l1 > ip.l1 ? 1 : l2 < ip.l2 ? -1 : l2 > ip.l2 ? 1 : 0;
    }
    ...
    public LongPair intersection(LongPair other) {
        LongPair result = new LongPair(Math.max(l1, other.l1), Math.min(l2, other.l2));
