...
public abstract class Entity<T extends Entity> extends Bindable implements HasVersionNumber, HasId, HasEntity {
        ...
        if (hash == 0) {
            ...
            if (getLocalId() != 0) {
                ...
                if (getId() == 0) {
                    ...
                    hash = TransformManager.replaceWithCreatedLocalObjectHash(domainIdentity(), hash);
                }
            }
            if (hash == 0) {
                hash = -1;
    ...
    public final String toStringEntity() {
        try {
            ...
            return Ax.format("Unable to return locator - class %s - id %s", getClass().getSimpleName(), id);
        }
    }
    ...
    public class DomainSupport {
        ...
        public <V extends Entity> void addToProperty(String propertyName, V v) {
            TransformManager.get().modifyCollectionProperty(Entity.this, propertyName, v, CollectionModificationType.ADD);
        }

        public <V extends Entity> V checkPermission(PermissionRule rule) throws PermissionsException {
        ...
        public <V extends Entity> void removeFromProperty(String propertyName, V v) {
            TransformManager.get().modifyCollectionProperty(Entity.this, propertyName, v, CollectionModificationType.REMOVE);
        }
    ...
    public static interface Ownership {
        ...
        public static Stream<Property> getOwnerReflectors(Class<?> beanClass) {
            return Reflections.at(beanClass).properties().stream().filter(pr -> pr.has(DomainProperty.class)).filter(pr -> pr.annotation(DomainProperty.class).owner());
        }
        ...
        public static List<Entity> getOwningEntities(Entity entity) {
            ...
            while (true) {
                ...
                List<Entity> values = (List) getOwnerReflectors(cursor.entityClass()).map(pr -> pr.get(f_cursor)).collect(Collectors.toList());
                if (values.size() > 1) {
                    if (entity instanceof HasLogicalParent) {
                        Entity logicalParent = ((HasLogicalParent) entity).provideLogicalParent();
    ...
    public interface PropertyEnum {
        ...
        default Predicate<? super DomainTransformEvent> transformFilter() {
            return event -> Objects.equals(event.getPropertyName(), ((Enum) this).name());
        }
    }
}
