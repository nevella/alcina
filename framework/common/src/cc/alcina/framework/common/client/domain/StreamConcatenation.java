...
public final class StreamConcatenation {
	 ...
	 * all the elements of the first input stream followed by all the elements
	 * of the second input stream, and so on.
	 *
	 * <p>
	 ...
	 *             if the argument array or any of the input streams are
	 *             {@code null}
	 */
	public static DoubleStream concat(DoubleStream... streams) {
		return concatInternal(streams, Spliterator.OfDouble[]::new,
				DoubleStream::spliterator, ConcatSpliterator.OfDouble::new,
	...
	public static IntStream concat(IntStream... streams) {
		return concatInternal(streams, Spliterator.OfInt[]::new,
				IntStream::spliterator, ConcatSpliterator.OfInt::new,
	...
	public static LongStream concat(LongStream... streams) {
		return concatInternal(streams, Spliterator.OfLong[]::new,
				LongStream::spliterator, ConcatSpliterator.OfLong::new,
	 ...
	 */
	@SafeVarargs
	// @SuppressWarnings({ "unchecked", "varargs" }) // TODO: Explain why this
	// is
	// ok.
	public static <T> Stream<T> concat(Stream<? extends T>... streams) {
		return concatInternal((Stream<T>[]) streams,
				size -> (Spliterator<T>[]) new Spliterator<?>[size],
				Stream::spliterator, ConcatSpliterator.OfRef::new,
	...
	private static <T, T_SPLITR extends Spliterator<T>, T_STREAM extends BaseStream<T, T_STREAM>>
	...
	abstract static class ConcatSpliterator<T, T_SPLITR extends Spliterator<T>>
		...
		public int characteristics() {
			int i = low; // ignore the cursor; iteration can't affect
							// characteristics
			if (i >= high) {
				// TODO(perf): This may report fewer characteristics than it
				// should.
				return Spliterators.emptySpliterator().characteristics();
			}
			if (i == high - 1) {
				// note for getComparator(): this is the only time we might be
				// SORTED
				return spliterators[i].characteristics();
			}
			//
			// DISTINCT and SORTED are *not* safe to inherit. Imagine our input
			// spliterators each contain the elements [1, 2] (distinct and
			// sorted) and
			// so we are [1, 2, 1, 2] (neither distinct nor sorted).
			//
			// SIZED and SUBSIZED might be safe to inherit, but we have to
			// account for
			// arithmetic overflow. If we're unable to produce a correct sum in
			// estimateSize() due to overflow, we must not report SIZED (or
			// SUBSIZED).
			//
			// ORDERED, NONNULL, IMMUTABLE, and CONCURRENT are safe to inherit.
			//
			// We assume that all other characteristics not listed here (that do
			// not
			// exist yet at the time of this writing) are *not* safe to inherit.
			// False negatives generally result in degraded performance, while
			// false
		...
		public Comparator<? super T> getComparator() {
			...
			if (i == high - 1) {
				// this is the only time we might be SORTED; see
				// characteristics()
		...
		public T_SPLITR trySplit() {
			...
			//
			// 1) Rather than splitting our *spliterators* in half, we could try
			// to
			// split our *elements* in half.
			//
			// 2) We could refuse to split if our total element count is lower
			// than
			// some threshold.
			//
			int i = cursor;
		...
		static final class OfDouble extends
			...
			OfDouble(Spliterator.OfDouble[] spliterators, int fromIndex,
					int toIndex) {
			...
			Spliterator.OfDouble
					invokeTrySplit(Spliterator.OfDouble spliterator) {
				return spliterator.trySplit();
			}

			@Override
			Spliterator.OfDouble slice(int fromIndex, int toIndex) {
				return new ConcatSpliterator.OfDouble(spliterators, fromIndex,
						toIndex);
			}
		}
		...
		static final class OfInt extends
			...
			OfInt(Spliterator.OfInt[] spliterators, int fromIndex,
					int toIndex) {
				super(spliterators, fromIndex, toIndex);
			}
			...
			Spliterator.OfInt slice(int fromIndex, int toIndex) {
				return new ConcatSpliterator.OfInt(spliterators, fromIndex,
						toIndex);
			}
		}
		...
		static final class OfLong extends
			...
			OfLong(Spliterator.OfLong[] spliterators, int fromIndex,
					int toIndex) {
				super(spliterators, fromIndex, toIndex);
			}
			...
			Spliterator.OfLong slice(int fromIndex, int toIndex) {
				return new ConcatSpliterator.OfLong(spliterators, fromIndex,
						toIndex);
			}
		}
		...
		abstract static class OfPrimitive<T, T_CONS, T_SPLITR extends Spliterator.OfPrimitive<T, T_CONS, T_SPLITR>>
			...
			// TODO: Is there a good way to share this logic with the base
			// class?
			@Override
		...
		static final class OfRef<T>
			...
			Spliterator<T> slice(int fromIndex, int toIndex) {
				return new ConcatSpliterator.OfRef<>(spliterators, fromIndex,
						toIndex);
			}
		}
	}
