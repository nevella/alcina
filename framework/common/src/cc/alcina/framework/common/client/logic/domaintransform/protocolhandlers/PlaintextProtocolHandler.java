...
public class PlaintextProtocolHandler implements DTRProtocolHandler {
    ...
    public void appendTo(DomainTransformEvent domainTransformEvent, StringBuffer sb) {
        boolean full = GWT.isClient() || !LooseContext.is(DTRProtocolSerializer.CONTEXT_EXCEPTION_DEBUG);
        String newStringValue = domainTransformEvent.getNewStringValue();
        if (!full && newStringValue != null && newStringValue.length() > 1000) {
            newStringValue = Ax.format("Truncated - %s chars - %s", newStringValue.length(), newStringValue.substring(0, 1000));
        }
        ...
        sb.append(SRC);
        sb.append(domainTransformEvent.getObjectClass() == null ? domainTransformEvent.getObjectClassName() : domainTransformEvent.getObjectClass().getName());
        sb.append(",");
        sb.append(SimpleStringParser.toString(domainTransformEvent.getObjectId()));
        sb.append(",");
        sb.append(SimpleStringParser.toString(domainTransformEvent.getObjectLocalId()));
        sb.append(",");
        sb.append(SimpleStringParser.toStringOrNullNonNegativeInteger(domainTransformEvent.getObjectVersionNumber()));
        sb.append(newlineTab);
        sb.append(PARAMS);
        ...
        sb.append(",");
        sb.append(SimpleStringParser.toString(domainTransformEvent.getUtcDate() == null ? System.currentTimeMillis() : domainTransformEvent.getUtcDate().getTime()));
        sb.append(newlineTab);
        sb.append(STRING_VALUE);
        sb.append(ns);
        sb.append(newlineTab);
        sb.append(TGT);
        sb.append(domainTransformEvent.getValueClass() == null ? domainTransformEvent.getValueClassName() : domainTransformEvent.getValueClass().getName());
        sb.append(",");
        sb.append(SimpleStringParser.toString(domainTransformEvent.getValueId()));
        sb.append(",");
        sb.append(SimpleStringParser.toString(domainTransformEvent.getValueLocalId()));
        sb.append(",");
        sb.append(SimpleStringParser.toStringOrNullNonNegativeInteger(domainTransformEvent.getValueVersionNumber()));
        sb.append("\n");
    }
    ...
    public String deserialize(String serializedEvents, List<DomainTransformEvent> events, int maxCount) {
        ...
        int i = 0;
        String s;
        while ((s = asyncParser.read(getDomainTransformEventMarker(), getDomainTransformEventMarker(), true, false)) != null) {
    ...
    private DomainTransformEvent fromString(String s) {
        ...
        if (dte.getTransformType() != TransformType.CHANGE_PROPERTY_SIMPLE_VALUE && dte.getNewStringValue().equals("null")) {
