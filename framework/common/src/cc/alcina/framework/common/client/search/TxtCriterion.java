...
public class TxtCriterion extends SearchCriterion implements HasValue<String> {
    ...
    public String toString() {
        ...
        if (string.length() > 0 && getOperator() != StandardSearchOperator.CONTAINS) {
            string = Ax.format("%s '%s'", Ax.friendly(getOperator()), string);
        }
        return string.length() == 0 ? "" : Ax.isBlank(getDisplayName()) ? Ax.format("\"%s\"", string) : getDisplayName() + ": " + string;
    }
