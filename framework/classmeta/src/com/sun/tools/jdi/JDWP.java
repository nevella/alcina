...
public class JDWP {
	static class VirtualMachine {
		...
		/**
		 * Returns the JDWP version implemented by the target VM. The version
				...
				if (vm.traceReceives) {
					vm.printTrace("Receiving Command(id=" + ps.pkt.id
							...
							+ (ps.pkt.flags != 0 ? ", FLAGS=" + ps.pkt.flags
									: "")
							+ (ps.pkt.errorCode != 0
									? ", ERROR CODE=" + ps.pkt.errorCode
									: ""));
				}
				description = ps.readString();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
		 ...
		 * which match the given signature. Multple reference types will be
		 * returned if two or more class loaders have loaded a class of the same
		 * name. The search is confined to loaded classes only; no attempt is
		 * made to load a class of the given signature.
		 */
		static class ClassesBySignature {
			static final int COMMAND = 2;

			static ClassesBySignature process(VirtualMachineImpl vm,
					String signature) throws JDWPException {
			...
			static ClassesBySignature waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			static class ClassInfo {
				...
				private ClassInfo(VirtualMachineImpl vm, PacketStream ps) {
					...
					if (vm.traceReceives) {
						vm.printReceiveTrace(5,
								"refTypeTag(byte): " + refTypeTag);
					}
					typeID = ps.readClassRef();
					if (vm.traceReceives) {
						vm.printReceiveTrace(5,
								"typeID(long): " + "ref=" + typeID);
					}
					status = ps.readInt();
					if (vm.traceReceives) {
		...
		static class AllClasses {
			...
			static AllClasses waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		...
		/**
		 * Returns all threads currently running in the target VM . The returned
		 * list contains threads created through java.lang.Thread, all native
		 * threads attached to the target VM through JNI, and system threads
		 * created by the target VM. Threads that have not yet been started and
		 * threads that have completed their execution are not included in the
		 * returned list.
		 */
		static class AllThreads {
			...
			static AllThreads waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		...
		/**
		 * Returns all thread groups that do not have a parent. This command may
		 * be used as the first step in building a tree (or trees) of the
		...
		static class TopLevelThreadGroups {
			...
			static TopLevelThreadGroups waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private TopLevelThreadGroups(VirtualMachineImpl vm,
					PacketStream ps) {
				if (vm.traceReceives) {
					vm.printTrace("Receiving Command(id=" + ps.pkt.id
							+ ") JDWP.VirtualMachine.TopLevelThreadGroups"
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"groups(ThreadGroupReferenceImpl[]): " + "");
				}
				...
				for (int i = 0; i < groupsCount; i++) {
					...
					if (vm.traceReceives) {
						vm.printReceiveTrace(5,
								"groups[i](ThreadGroupReferenceImpl): "
										+ (groups[i] == null ? "NULL"
												: "ref=" + groups[i].ref()));
					}
				}
		...
		/**
		 * Invalidates this virtual machine mirror. The communication channel to
		 * the target VM is closed, and the target VM prepares to accept another
		 * subsequent connection from this debugger or another debugger,
		 * including the following tasks:
		 * <ul>
		 * <li>All event requests are cancelled.
		 * <li>All threads suspended by the thread-level
		 * <a href="#JDWP_ThreadReference_Resume">resume</a> command or the
		 * VM-level <a href="#JDWP_VirtualMachine_Resume">resume</a> command are
		 ...
		 * continued after the disconnection. Upon completion of any such method
		 * invocation, the invoking thread continues from the location where it
		 ...
		 * returned values indicate the number of bytes used by the identifiers
		...
		static class IDSizes {
			...
			private IDSizes(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"methodIDSize(int): " + methodIDSize);
				}
				objectIDSize = ps.readInt();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"objectIDSize(int): " + objectIDSize);
				}
				referenceTypeIDSize = ps.readInt();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"referenceTypeIDSize(int): " + referenceTypeIDSize);
				}
		 ...
		 * it must be resumed through the
		 * <a href="#JDWP_VirtualMachine_Resume">VM-level resume</a> command or
		 * the <a href="#JDWP_ThreadReference_Resume">thread-level resume</a>
		 ...
		 * event has stopped it. Suspensions of the Virtual Machine and
		 * individual threads are counted. If a particular thread is suspended n
		 * times, it must resumed n times before it will continue.
		 */
		static class Resume {
		...
		/**
		 * Terminates the target VM with the given exit code. On some platforms,
		 * the exit code might be truncated, for example, to the low order 8
		 * bits. All ids previously returned from the target VM become invalid.
		 * Threads running in the VM are abruptly terminated. A thread death
		 * exception is not thrown and finally blocks are not run.
		 */
		static class Exit {
			static final int COMMAND = 10;

			static Exit process(VirtualMachineImpl vm, int exitCode)
					throws JDWPException {
			...
			static PacketStream enqueueCommand(VirtualMachineImpl vm,
				...
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace("Sending:                 exitCode(int): "
							+ exitCode);
				}
		...
		static class CreateString {
			...
			static CreateString process(VirtualMachineImpl vm, String utf)
					throws JDWPException {
			...
			static CreateString waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private CreateString(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"stringObject(StringReferenceImpl): "
									+ (stringObject == null ? "NULL"
											: "ref=" + stringObject.ref()));
				}
			}
		}
		...
		static class Capabilities {
			...
			static Capabilities waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private Capabilities(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canWatchFieldAccess(boolean): "
							+ canWatchFieldAccess);
				}
				canGetBytecodes = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"canGetBytecodes(boolean): " + canGetBytecodes);
				}
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canGetOwnedMonitorInfo(boolean): "
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"canGetMonitorInfo(boolean): " + canGetMonitorInfo);
				}
			}
		}
		...
		static class ClassPaths {
			...
			static ClassPaths waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private ClassPaths(VirtualMachineImpl vm, PacketStream ps) {
				...
				for (int i = 0; i < classpathsCount; i++) {
					...
					if (vm.traceReceives) {
						vm.printReceiveTrace(5,
								"classpaths[i](String): " + classpaths[i]);
					}
				}
				if (vm.traceReceives) {
				...
				for (int i = 0; i < bootclasspathsCount; i++) {
					...
					if (vm.traceReceives) {
						vm.printReceiveTrace(5, "bootclasspaths[i](String): "
								+ bootclasspaths[i]);
					}
				}
			}
		 ...
		 * this command. This description assumes reference counting, a back-end
		 * may use any implementation which operates equivalently.
		 */
		static class DisposeObjects {
			...
			static class Request {
				 ...
				 * received from the back-end. An accurate count prevents the
				 * object ID from being freed on the back-end if it is part of
				 * an incoming packet, not yet handled by the front-end.
				 */
				final int refCnt;
				...
				private void write(PacketStream ps) {
					if ((ps.vm.traceFlags
							& VirtualMachineImpl.TRACE_SENDS) != 0) {
						ps.vm.printTrace(
								"Sending:                     object(ObjectReferenceImpl): "
										+ (object == null ? "NULL"
												: "ref=" + object.ref()));
					}
			...
			static DisposeObjects process(VirtualMachineImpl vm,
					Request[] requests) throws JDWPException {
			...
			static DisposeObjects waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * they are held until a subsequent ReleaseEvents command is sent. This
		 * command is useful to control the number of events sent to the
		 ...
		 * already being held, this command is ignored.
		 */
		static class HoldEvents {
			...
			static HoldEvents waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		...
		static class ReleaseEvents {
			...
			static ReleaseEvents waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * as booleans, each indicating the presence or absence of a capability.
		 * The commands associated with each capability will return the
		 * NOT_IMPLEMENTED error if the cabability is not available. Since JDWP
		 * version 1.4.
		 */
		static class CapabilitiesNew {
			...
			static CapabilitiesNew waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private CapabilitiesNew(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canRedefineClasses(boolean): "
							+ canRedefineClasses);
				}
				canAddMethod = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"canAddMethod(boolean): " + canAddMethod);
				}
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"canPopFrames(boolean): " + canPopFrames);
				}
				canUseInstanceFilters = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canUseInstanceFilters(boolean): "
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canRequestVMDeathEvent(boolean): "
							+ canRequestVMDeathEvent);
				}
				canSetDefaultStratum = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canSetDefaultStratum(boolean): "
							+ canSetDefaultStratum);
				}
				canGetInstanceInfo = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canGetInstanceInfo(boolean): "
							+ canGetInstanceInfo);
				}
				canRequestMonitorEvents = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canRequestMonitorEvents(boolean): "
							+ canRequestMonitorEvents);
				}
				canGetMonitorFrameInfo = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canGetMonitorFrameInfo(boolean): "
							+ canGetMonitorFrameInfo);
				}
				canUseSourceNameFilters = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canUseSourceNameFilters(boolean): "
							+ canUseSourceNameFilters);
				}
				canGetConstantPool = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canGetConstantPool(boolean): "
							+ canGetConstantPool);
				}
				canForceEarlyReturn = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "canForceEarlyReturn(boolean): "
							+ canForceEarlyReturn);
				}
				reserved22 = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"reserved22(boolean): " + reserved22);
				}
				reserved23 = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"reserved23(boolean): " + reserved23);
				}
				reserved24 = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"reserved24(boolean): " + reserved24);
				}
				reserved25 = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"reserved25(boolean): " + reserved25);
				}
				reserved26 = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"reserved26(boolean): " + reserved26);
				}
				reserved27 = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"reserved27(boolean): " + reserved27);
				}
				reserved28 = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"reserved28(boolean): " + reserved28);
				}
				reserved29 = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"reserved29(boolean): " + reserved29);
				}
				reserved30 = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"reserved30(boolean): " + reserved30);
				}
				reserved31 = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"reserved31(boolean): " + reserved31);
				}
				reserved32 = ps.readBoolean();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"reserved32(boolean): " + reserved32);
				}
			}
		}

		/**
		 * Installs new class definitions. If there are active stack frames in
		 * methods of the redefined classes in the target VM then those active
		 * frames continue to run the bytecodes of the original method. These
		 * methods are considered obsolete - see
		 ...
		 * original method ID refers to the redefined method. All breakpoints in
		 * the redefined classes are cleared. If resetting of stack frames is
		 * desired, the <a href="#JDWP_StackFrame_PopFrames">PopFrames</a>
		 ...
		 * classes, or the canUnrestrictedlyRedefineClasses to redefine classes
		 * in arbitrary ways.
		 */
		static class RedefineClasses {
			...
			static class ClassDef {
				...
				private void write(PacketStream ps) {
					if ((ps.vm.traceFlags
							& VirtualMachineImpl.TRACE_SENDS) != 0) {
						ps.vm.printTrace(
								"Sending:                     refType(ReferenceTypeImpl): "
										+ (refType == null ? "NULL"
												: "ref=" + refType.ref()));
					}
			...
			static RedefineClasses process(VirtualMachineImpl vm,
					ClassDef[] classes) throws JDWPException {
			...
			static RedefineClasses waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		...
		/**
		 * Set the default stratum. Requires canSetDefaultStratum capability -
		 * see
		 * <a href="#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
		 */
		static class SetDefaultStratum {
			...
			static SetDefaultStratum process(VirtualMachineImpl vm,
					String stratumID) throws JDWPException {
			...
			static SetDefaultStratum waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * signature attribute section in <cite>The Java&trade; Virtual Machine
		 * Specification</cite>. Since JDWP version 1.5.
		 */
		static class AllClassesWithGeneric {
			...
			static AllClassesWithGeneric waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			static class ClassInfo {
				...
				/**
				 * The generic signature of the loaded reference type or an
				...
				private ClassInfo(VirtualMachineImpl vm, PacketStream ps) {
					...
					if (vm.traceReceives) {
						vm.printReceiveTrace(5, "genericSignature(String): "
								+ genericSignature);
					}
					status = ps.readInt();
					if (vm.traceReceives) {
			...
			private AllClassesWithGeneric(VirtualMachineImpl vm,
					PacketStream ps) {
				if (vm.traceReceives) {
					vm.printTrace("Receiving Command(id=" + ps.pkt.id
							+ ") JDWP.VirtualMachine.AllClassesWithGeneric"
		 ...
		 * list. Only instances that are reachable for the purposes of garbage
		 * collection are counted. If a reference type is invalid, eg. it has
		 * been unloaded, zero is returned for its instance count.
		 * <p>
		...
		static class InstanceCounts {
			...
			static InstanceCounts process(VirtualMachineImpl vm,
					ReferenceTypeImpl[] refTypesCount) throws JDWPException {
			...
			static PacketStream enqueueCommand(VirtualMachineImpl vm,
				...
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 refTypesCount(ReferenceTypeImpl[]): "
									+ "");
				}
				...
				for (int i = 0; i < refTypesCount.length; i++) {
					...
					if ((ps.vm.traceFlags
							& VirtualMachineImpl.TRACE_SENDS) != 0) {
						ps.vm.printTrace(
								"Sending:                     refTypesCount[i](ReferenceTypeImpl): "
										+ (refTypesCount[i] == null ? "NULL"
												: "ref=" + refTypesCount[i]
														.ref()));
					}
			...
			static InstanceCounts waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
	...
	static class ReferenceType {
		...
		/**
		 * Returns the JNI signature of a reference type. JNI signature formats
		 * are described in the <a href=
		 * "http://java.sun.com/products/jdk/1.2/docs/guide/jni/index.html">Java
		 * Native Inteface Specification</a>
		 * <p>
		...
		static class Signature {
			...
			static Signature process(VirtualMachineImpl vm,
					ReferenceTypeImpl refType) throws JDWPException {
			...
			static Signature waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * loader, the returned object ID is null.
		 */
		static class ClassLoader {
			...
			static ClassLoader waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * type. The returned bit mask contains information on the declaration
		 * of the reference type. If the reference type is an array or a
		 * primitive class (for example, java.lang.Integer.TYPE), the value of
		 * the returned bit mask is undefined.
		 */
		static class Modifiers {
			...
			static Modifiers waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			/**
			 * Modifier bits as defined in Chapter 4 of <cite>The Java&trade;
		...
		/**
		 * Returns information for each field in a reference type. Inherited
		 * fields are not included. The field list will include any synthetic
		 * fields created by the compiler. Fields are returned in the order they
		...
		static class Fields {
			...
			static class FieldInfo {
				 ...
				 * Individual flag values are defined in Chapter 4 of <cite>The
				 * Java&trade; Virtual Machine Specification</cite>. In
				 * addition, The <code>0xf0000000</code> bit identifies the
				 * field as synthetic, if the synthetic attribute
				 * <a href="#JDWP_VirtualMachine_Capabilities">capability</a> is
				 * available.
				 */
				final int modBits;
		 ...
		 * methods are not included. The list of methods will include
		 * constructors (identified with the name "&lt;init&gt;"), the
		 * initialization method (identified with the name "&lt;clinit&gt;") if
		 * present, and any synthetic methods created by the compiler. Methods
		 * are returned in the order they occur in the class file.
		 */
		static class Methods {
			...
			static class MethodInfo {
				 ...
				 * Java&trade; Virtual Machine Specification</cite>. In
				 * addition, The <code>0xf0000000</code> bit identifies the
				 * method as synthetic, if the synthetic attribute
				 * <a href="#JDWP_VirtualMachine_Capabilities">capability</a> is
				 * available.
				 */
				final int modBits;
		...
		static class GetValues {
			...
			static GetValues process(VirtualMachineImpl vm,
					ReferenceTypeImpl refType, Field[] fields)
					throws JDWPException {
			...
			static GetValues waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			/**
			 * The number of values returned, always equal to fields, the number
			 * of values to get.
			 */
		...
		static class SourceFile {
			...
			static SourceFile waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		...
		static class NestedTypes {
			...
			static NestedTypes waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * initialized, as described in section 2.1.6 of <cite>The Java&trade;
		 * Virtual Machine Specification</cite>. If the class is linked the
		 * PREPARED and VERIFIED bits in the returned status bits will be set.
		 * If the class is initialized the INITIALIZED bit in the returned
		 * status bits will be set. If an error occured during initialization
		 * then the ERROR bit in the returned status bits will be set. The
		 * returned status bits are undefined for array types and for primitive
		...
		static class Status {
			...
			/**
			 * <a href="#JDWP_ClassStatus">Status</a> bits: See
			 * <a href="#JDWP_ClassStatus">JDWP.ClassStatus</a>
			 */
			final int status;
		...
		static class Interfaces {
			...
			static Interfaces waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private Interfaces(VirtualMachineImpl vm, PacketStream ps) {
				...
				for (int i = 0; i < interfacesCount; i++) {
					...
					if (vm.traceReceives) {
						vm.printReceiveTrace(5,
								"interfaces[i](InterfaceTypeImpl): "
										+ (interfaces[i] == null ? "NULL"
												: "ref=" + interfaces[i]
														.ref()));
					}
				}
		...
		static class ClassObject {
			...
			static ClassObject waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private ClassObject(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"classObject(ClassObjectReferenceImpl): "
									+ (classObject == null ? "NULL"
											: "ref=" + classObject.ref()));
				}
			}
		}
		...
		/**
		 * Returns the value of the SourceDebugExtension attribute. Since JDWP
		 * version 1.4. Requires canGetSourceDebugExtension capability - see
		 * <a href="#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
		 */
		static class SourceDebugExtension {
			...
			static SourceDebugExtension waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private SourceDebugExtension(VirtualMachineImpl vm,
					PacketStream ps) {
				if (vm.traceReceives) {
					vm.printTrace("Receiving Command(id=" + ps.pkt.id
							+ ") JDWP.ReferenceType.SourceDebugExtension"
		...
		static class SignatureWithGeneric {
			...
			static SignatureWithGeneric waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private SignatureWithGeneric(VirtualMachineImpl vm,
					PacketStream ps) {
				if (vm.traceReceives) {
					vm.printTrace("Receiving Command(id=" + ps.pkt.id
							+ ") JDWP.ReferenceType.SignatureWithGeneric"
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"genericSignature(String): " + genericSignature);
				}
			}
		}
		 ...
		 * field in a reference type. Inherited fields are not included. The
		 * field list will include any synthetic fields created by the compiler.
		 * Fields are returned in the order they occur in the class file.
		 * Generic signatures are described in the signature attribute section
		 * in <cite>The Java&trade; Virtual Machine Specification</cite>. Since
		 * JDWP version 1.5.
		 */
		static class FieldsWithGeneric {
			...
			static FieldsWithGeneric waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			static class FieldInfo {
				...
				/**
				 * The generic signature of the field, or an empty string if
				 * there is none.
				 */
				final String genericSignature;
		...
		/**
		 * Returns information, including the generic signature if any, for each
		 * method in a reference type. Inherited methodss are not included. The
		 * list of methods will include constructors (identified with the name
		 * "&lt;init&gt;"), the initialization method (identified with the name
		 * "&lt;clinit&gt;") if present, and any synthetic methods created by
		 * the compiler. Methods are returned in the order they occur in the
		 * class file. Generic signatures are described in the signature
		 * attribute section in <cite>The Java&trade; Virtual Machine
		 * Specification</cite>. Since JDWP version 1.5.
		 */
		static class MethodsWithGeneric {
			...
			static MethodsWithGeneric waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * reachable for the purposes of garbage collection are returned.
		 * <p>
		 * Since JDWP version 1.6. Requires canGetInstanceInfo capability - see
		 * <a href="#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
		 */
		static class Instances {
			...
			static Instances process(VirtualMachineImpl vm,
					ReferenceTypeImpl refType, int maxInstances)
					throws JDWPException {
			...
			static Instances waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private Instances(VirtualMachineImpl vm, PacketStream ps) {
				...
				for (int i = 0; i < instancesCount; i++) {
					...
					if (vm.traceReceives) {
						vm.printReceiveTrace(5,
								"instances[i](ObjectReferenceImpl): "
										+ (instances[i] == null ? "NULL"
												: "ref=" + instances[i].ref()));
					}
				}
			}
		...
		/**
		 * Returns the class file major and minor version numbers, as defined in
		 * the class file format of the Java Virtual Machine specification.
		 * <p>
		...
		static class ClassFileVersion {
			...
			static ClassFileVersion waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * constant_pool item of the Class File Format in <cite>The Java&trade;
		 * Virtual Machine Specification</cite>.
		 * <p>
		...
		static class ConstantPool {
			...
			static ConstantPool waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			/**
			 * Total number of constant pool entries plus one. This corresponds
			 * to the constant_pool_count item of the Class File Format in
	...
	static class ClassType {
		...
		static class Superclass {
			...
			static Superclass process(VirtualMachineImpl vm,
					ClassTypeImpl clazz) throws JDWPException {
			...
			static Superclass waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * member of the class type or one of its superclasses, superinterfaces,
		 * or implemented interfaces. Access control is not enforced; for
		 * example, the values of private fields can be set. Final fields cannot
		 * be set. For primitive values, the value's type must match the field's
		 * type exactly. For object values, there must exist a widening
		...
		static class SetValues {
			...
			static SetValues process(VirtualMachineImpl vm, ClassTypeImpl clazz,
					FieldValue[] values) throws JDWPException {
				PacketStream ps = enqueueCommand(vm, clazz, values);
				return waitForReply(vm, ps);
			}

			static PacketStream enqueueCommand(VirtualMachineImpl vm,
					ClassTypeImpl clazz, FieldValue[] values) {
				PacketStream ps = new PacketStream(vm, COMMAND_SET, COMMAND);
				if ((vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					vm.printTrace("Sending Command(id=" + ps.pkt.id
			...
			static SetValues waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * or one of its superclasses, superinterfaces, or implemented
		 * interfaces. Access control is not enforced; for example, private
		 ...
		 * argument list. The method invocation is synchronous; the reply packet
		 * is not sent until the invoked method returns in the target VM. The
		 * return value (possibly the void value) is included in the reply
		 * packet. If the invoked method throws an exception, the exception
		 * object ID is set in the reply packet; otherwise, the exception object
		 * ID is null.
		 * <p>
		 ...
		 * <p>
		 * By default, all threads in the target VM are resumed while the method
		 * is being invoked if they were previously suspended by an event or by
		 * command. This is done to prevent the deadlocks that will occur if any
		 * of the threads own monitors that will be needed by the invoked
		 ...
		 * during the invocation. By default, when the invocation completes, all
		 * threads in the target VM are suspended, regardless their state before
		 * the invocation.
		 * <p>
		 * The resumption of other threads during the invoke can be prevented by
		 * specifying the INVOKE_SINGLE_THREADED bit flag in the
		 * <code>options</code> field; however, there is no protection against
		 * or recovery from the deadlocks described above, so this option should
		 * be used with great caution. Only the specified thread will be resumed
		 * (as described for all threads above). Upon completion of a single
		 * threaded invoke, the invoking thread will be suspended once again.
		 * Note that any threads started during the single threaded invocation
		...
		static class InvokeMethod {
			...
			static InvokeMethod process(VirtualMachineImpl vm,
					ClassTypeImpl clazz, ThreadReferenceImpl thread,
					long methodID, ValueImpl[] arguments, int options)
					throws JDWPException {
				PacketStream ps = enqueueCommand(vm, clazz, thread, methodID,
						arguments, options);
				return waitForReply(vm, ps);
			}
			...
			static PacketStream enqueueCommand(VirtualMachineImpl vm,
				...
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 thread(ThreadReferenceImpl): "
									+ (thread == null ? "NULL"
											: "ref=" + thread.ref()));
				}
				ps.writeObjectRef(thread.ref());
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace("Sending:                 methodID(long): "
							+ methodID);
				}
				ps.writeMethodRef(methodID);
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 arguments(ValueImpl[]): "
									+ "");
				}
				...
				for (int i = 0; i < arguments.length; i++) {
					...
					if ((ps.vm.traceFlags
							& VirtualMachineImpl.TRACE_SENDS) != 0) {
						ps.vm.printTrace(
								"Sending:                     arguments[i](ValueImpl): "
										+ arguments[i]);
					}
					ps.writeValue(arguments[i]);
				}
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace("Sending:                 options(int): "
							+ options);
				}
				ps.writeInt(options);
				ps.send();
			...
			static InvokeMethod waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private InvokeMethod(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"returnValue(ValueImpl): " + returnValue);
				}
				exception = ps.readTaggedObjectReference();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"exception(ObjectReferenceImpl): "
									+ (exception == null ? "NULL"
											: "ref=" + exception.ref()));
				}
			}
		 ...
		 * constructor. The constructor method ID must be a member of the class
		 * type.
		 * <p>
		 * Instance creation will occur in the specified thread. Instance
		 * creation can occur only if the specified thread has been suspended by
		 * an event. Method invocation is not supported when the target VM has
		 * been suspended by the front-end.
		 * <p>
		 ...
		 * the reply packet is not sent until the invoked method returns in the
		 * target VM. The return value (possibly the void value) is included in
		 * the reply packet. If the constructor throws an exception, the
		...
		static class NewInstance {
			...
			static NewInstance waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
	...
	static class ArrayType {
		...
		static class NewInstance {
			...
			static NewInstance process(VirtualMachineImpl vm,
					ArrayTypeImpl arrType, int length) throws JDWPException {
				PacketStream ps = enqueueCommand(vm, arrType, length);
				return waitForReply(vm, ps);
			}

			static PacketStream enqueueCommand(VirtualMachineImpl vm,
					ArrayTypeImpl arrType, int length) {
				PacketStream ps = new PacketStream(vm, COMMAND_SET, COMMAND);
				if ((vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					vm.printTrace("Sending Command(id=" + ps.pkt.id
	...
	static class Method {
		 ...
		 * installed using
		 * <a href="#JDWP_VirtualMachine_RedefineClasses">RedefineClasses</a>.
		 */
		static class LineTable {
			static final int COMMAND = 1;

			static LineTable process(VirtualMachineImpl vm,
					ReferenceTypeImpl refType, long methodID)
					throws JDWPException {
			...
			static LineTable waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			static class LineInfo {
				...
				private LineInfo(VirtualMachineImpl vm, PacketStream ps) {
					...
					if (vm.traceReceives) {
						vm.printReceiveTrace(5,
								"lineCodeIndex(long): " + lineCodeIndex);
					}
			...
			/**
			 * Lowest valid code index for the method, >=0, or -1 if the method
			 * is native
			 */
			final long start;

			/**
			 * Highest valid code index for the method, >=0, or -1 if the method
			 * is native
			 */
			final long end;
		...
		static class VariableTable {
			...
			static VariableTable waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			static class SlotInfo {
				 ...
				 * Used in conjunction with <code>length</code>. The variable
				 * can be get or set only when the current
				 * <code>codeIndex</code> <= current frame code index <
				 * <code>codeIndex + length</code>
				 */
				 ...
				 * <code>codeIndex</code>. The variable can be get or set only
				 * when the current <code>codeIndex</code> <= current frame code
				 * index < <code>code index + length</code>
				 */
				final int length;
		...
		/**
		 * Retrieve the method's bytecodes as defined in <cite>The Java&trade;
		 * Virtual Machine Specification</cite>. Requires canGetBytecodes
		 * capability - see
		 * <a href="#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
		 */
		static class Bytecodes {
			...
			static Bytecodes waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		...
		/**
		 * Determine if this method is obsolete. A method is obsolete if it has
		 * been replaced by a non-equivalent method using the
		 * <a href="#JDWP_VirtualMachine_RedefineClasses">RedefineClasses</a>
		 * command. The original and redefined methods are considered equivalent
		 * if their bytecodes are the same except for indices into the constant
		...
		static class IsObsolete {
			...
			static IsObsolete waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			/**
			 * true if this method has been replaced by a non-equivalent method
			 * using the RedefineClasses command.
			 */
		 ...
		 * "this" reference is included in the table. Also, synthetic variables
		 * may be present. Generic signatures are described in the signature
		 * attribute section in <cite>The Java&trade; Virtual Machine
		 * Specification</cite>. Since JDWP version 1.5.
		 */
		static class VariableTableWithGeneric {
			...
			static VariableTableWithGeneric waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private VariableTableWithGeneric(VirtualMachineImpl vm,
					PacketStream ps) {
				if (vm.traceReceives) {
					vm.printTrace("Receiving Command(id=" + ps.pkt.id
							+ ") JDWP.Method.VariableTableWithGeneric"
	...
	static class ObjectReference {
		...
		/**
		 * Returns the runtime type of the object. The runtime type will be a
		 * class or an array.
		 */
		static class ReferenceType {
			static final int COMMAND = 1;
			...
			static ReferenceType process(VirtualMachineImpl vm,
					ObjectReferenceImpl object) throws JDWPException {
			...
			static ReferenceType waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * member of the object's type or one of its superclasses,
		...
		static class GetValues {
			...
			static GetValues process(VirtualMachineImpl vm,
					ObjectReferenceImpl object, Field[] fields)
					throws JDWPException {
			 ...
			 * the same order as corresponding fieldIDs in the command.
			 */
			final ValueImpl[] values;
		...
		/**
		 * Sets the value of one or more instance fields. Each field must be
		 * member of the object's type or one of its superclasses,
		 * superinterfaces, or implemented interfaces. Access control is not
		 ...
		 * conversion from the value's type to the field's type and the field's
		 * type must be loaded.
		 */
		static class SetValues {
			static final int COMMAND = 3;
			...
			static SetValues process(VirtualMachineImpl vm,
					ObjectReferenceImpl object, FieldValue[] values)
					throws JDWPException {
		...
		/**
		 * Returns monitor information for an object. All threads int the VM
		 * must be suspended. Requires canGetMonitorInfo capability - see
		 * <a href="#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
		 */
		static class MonitorInfo {
			...
			static MonitorInfo waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private MonitorInfo(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "owner(ThreadReferenceImpl): "
							+ (owner == null ? "NULL" : "ref=" + owner.ref()));
				}
		...
		/**
		 * Invokes a instance method. The method must be member of the object's
		...
		static class InvokeMethod {
			...
			static InvokeMethod process(VirtualMachineImpl vm,
					ObjectReferenceImpl object, ThreadReferenceImpl thread,
					ClassTypeImpl clazz, long methodID, ValueImpl[] arguments,
					int options) throws JDWPException {
				PacketStream ps = enqueueCommand(vm, object, thread, clazz,
						methodID, arguments, options);
				return waitForReply(vm, ps);
			}
		 ...
		 * VM is running. A call to this command guarantees that the object will
		 ...
		 * will occur because all threads are suspended. The typical examination
		 * of variables, fields, and arrays during the suspension is safe
		 * without explicitly disabling garbage collection.
		 * <p>
		...
		static class DisableCollection {
			...
			static DisableCollection waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		...
		/**
		 * Permits garbage collection for this object. By default all objects
		 * returned by JDWP may become unreachable in the target VM, and hence
		 * may be garbage collected. A call to this command is necessary only if
		 * garbage collection was previously disabled with the <a href=
		 * "#JDWP_ObjectReference_DisableCollection">DisableCollection</a>
		 * command.
		 */
		static class EnableCollection {
			...
			static EnableCollection waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		...
		static class IsCollected {
			...
			static IsCollected waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		...
		/**
		 * Returns objects that directly reference this object. Only objects
		 * that are reachable for the purposes of garbage collection are
		 * returned. Note that an object can also be referenced in other ways,
		...
		static class ReferringObjects {
			...
			static ReferringObjects process(VirtualMachineImpl vm,
					ObjectReferenceImpl object, int maxReferrers)
					throws JDWPException {
			...
			static ReferringObjects waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private ReferringObjects(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"referringObjects(ObjectReferenceImpl[]): " + "");
				}
				...
				for (int i = 0; i < referringObjectsCount; i++) {
					...
					if (vm.traceReceives) {
						vm.printReceiveTrace(5,
								"referringObjects[i](ObjectReferenceImpl): "
										+ (referringObjects[i] == null ? "NULL"
												: "ref=" + referringObjects[i]
														.ref()));
					}
				}
			}
	...
	static class StringReference {
		...
		static class Value {
			...
			static Value process(VirtualMachineImpl vm,
					ObjectReferenceImpl stringObject) throws JDWPException {
			...
			static PacketStream enqueueCommand(VirtualMachineImpl vm,
				...
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 stringObject(ObjectReferenceImpl): "
									+ (stringObject == null ? "NULL"
											: "ref=" + stringObject.ref()));
				}
	...
	static class ThreadReference {
		...
		static class Name {
			...
			static Name process(VirtualMachineImpl vm,
					ThreadReferenceImpl thread) throws JDWPException {
		 ...
		 * <a href="#JDWP_ThreadReference_Status">ThreadStatus</a> command.) For
		 * example, if it was Running, it will still appear running to other
		 * threads.
		 */
		static class Suspend {
			static final int COMMAND = 2;
		 ...
		 * indicates the thread status the last time it was running. the suspend
		 * status provides information on the thread's suspension, if any.
		 */
		static class Status {
			static final int COMMAND = 4;
			 ...
			 * <a href="#JDWP_ThreadStatus">JDWP.ThreadStatus</a>
			 */
			final int threadStatus;

			/**
			 * One of the suspend status codes See
			 * <a href="#JDWP_SuspendStatus">JDWP.SuspendStatus</a>
			 */
			...
			private Status(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"threadStatus(int): " + threadStatus);
				}
				suspendStatus = ps.readInt();
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"suspendStatus(int): " + suspendStatus);
				}
			}
		}
		...
		static class ThreadGroup {
			...
			static ThreadGroup waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private ThreadGroup(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4, "group(ThreadGroupReferenceImpl): "
							+ (group == null ? "NULL" : "ref=" + group.ref()));
				}
			}
		}
		 ...
		 * frames starts with the currently executing frame, followed by its
		 * caller, and so on. The thread must be suspended, and the returned
		...
		static class Frames {
			...
			static Frames process(VirtualMachineImpl vm,
					ThreadReferenceImpl thread, int startFrame, int length)
					throws JDWPException {
				PacketStream ps = enqueueCommand(vm, thread, startFrame,
						length);
				return waitForReply(vm, ps);
			}
		...
		/**
		 * Returns the count of frames on this thread's stack. The thread must
		 * be suspended, and the returned count is valid only while the thread
		 * is suspended. Returns JDWP.Error.errorThreadNotSuspended if not
		 * suspended.
		 */
		static class FrameCount {
			...
			static FrameCount waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * canGetOwnedMonitorInfo capability - see
		 * <a href="#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
		 */
		static class OwnedMonitors {
			...
			static OwnedMonitors waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * information is relevant only while the thread is suspended. Requires
		 * canGetCurrentContendedMonitor capability - see
		 * <a href="#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
		 */
		static class CurrentContendedMonitor {
			...
			static CurrentContendedMonitor waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private CurrentContendedMonitor(VirtualMachineImpl vm,
					PacketStream ps) {
				if (vm.traceReceives) {
					vm.printTrace("Receiving Command(id=" + ps.pkt.id
							+ ") JDWP.ThreadReference.CurrentContendedMonitor"
		...
		static class Stop {
			...
			static Stop process(VirtualMachineImpl vm,
					ThreadReferenceImpl thread, ObjectReferenceImpl throwable)
					throws JDWPException {
			...
			static PacketStream enqueueCommand(VirtualMachineImpl vm,
				...
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 throwable(ObjectReferenceImpl): "
									+ (throwable == null ? "NULL"
											: "ref=" + throwable.ref()));
				}
		...
		static class Interrupt {
			...
			static Interrupt waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		...
		static class SuspendCount {
			...
			static SuspendCount waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
			...
			private SuspendCount(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"suspendCount(int): " + suspendCount);
				}
			}
		}

		/**
		 * Returns monitor objects owned by the thread, along with stack depth
		 * at which the monitor was acquired. Returns stack depth of -1 if the
		...
		static class OwnedMonitorsStackDepthInfo {
			...
			static OwnedMonitorsStackDepthInfo
					waitForReply(VirtualMachineImpl vm, PacketStream ps)
							throws JDWPException {
				ps.waitForReply();
			...
			private OwnedMonitorsStackDepthInfo(VirtualMachineImpl vm,
					PacketStream ps) {
				if (vm.traceReceives) {
					vm.printTrace("Receiving Command(id=" + ps.pkt.id
							+ ") JDWP.ThreadReference.OwnedMonitorsStackDepthInfo"
		 ...
		 * Frames section in <cite>The Java&trade; Virtual Machine
		 * Specification</cite>) for the specified thread at the time this
		 * command is received.
		 * <p>
		...
		static class ForceEarlyReturn {
			...
			static ForceEarlyReturn process(VirtualMachineImpl vm,
					ThreadReferenceImpl thread, ValueImpl value)
					throws JDWPException {
			...
			static ForceEarlyReturn waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
	...
	static class ThreadGroupReference {
		...
		static class Name {
			...
			static Name process(VirtualMachineImpl vm,
					ThreadGroupReferenceImpl group) throws JDWPException {
			...
			static PacketStream enqueueCommand(VirtualMachineImpl vm,
				...
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 group(ThreadGroupReferenceImpl): "
									+ (group == null ? "NULL"
											: "ref=" + group.ref()));
				}
		...
		static class Parent {
			...
			/**
			 * The parent thread group object, or null if the given thread group
			...
			private Parent(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"parentGroup(ThreadGroupReferenceImpl): "
									+ (parentGroup == null ? "NULL"
											: "ref=" + parentGroup.ref()));
				}
			}
		}
		 ...
		 * in this thread group. Threads and thread groups in child thread
		 * groups are not included. A thread is alive if it has been started and
		 * has not yet been stopped. See <a
		 * href=../../../api/java/lang/ThreadGroup.html>java.lang.ThreadGroup
		 * </a> for information about active ThreadGroups.
		 */
		static class Children {
			...
			private Children(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"childThreads(ThreadReferenceImpl[]): " + "");
				}
				...
				for (int i = 0; i < childThreadsCount; i++) {
					...
					if (vm.traceReceives) {
						vm.printReceiveTrace(5,
								"childThreads[i](ThreadReferenceImpl): "
										+ (childThreads[i] == null ? "NULL"
												: "ref=" + childThreads[i]
														.ref()));
					}
				}
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"childGroups(ThreadGroupReferenceImpl[]): " + "");
				}
				...
				for (int i = 0; i < childGroupsCount; i++) {
					...
					if (vm.traceReceives) {
						vm.printReceiveTrace(5,
								"childGroups[i](ThreadGroupReferenceImpl): "
										+ (childGroups[i] == null ? "NULL"
												: "ref=" + childGroups[i]
														.ref()));
					}
				}
	...
	static class ArrayReference {
		...
		static class Length {
			...
			static Length process(VirtualMachineImpl vm,
					ArrayReferenceImpl arrayObject) throws JDWPException {
			...
			static PacketStream enqueueCommand(VirtualMachineImpl vm,
				...
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 arrayObject(ArrayReferenceImpl): "
		...
		static class GetValues {
			...
			static GetValues process(VirtualMachineImpl vm,
					ArrayReferenceImpl arrayObject, int firstIndex, int length)
					throws JDWPException {
				PacketStream ps = enqueueCommand(vm, arrayObject, firstIndex,
						length);
				return waitForReply(vm, ps);
			}

			static PacketStream enqueueCommand(VirtualMachineImpl vm,
					ArrayReferenceImpl arrayObject, int firstIndex,
					int length) {
				PacketStream ps = new PacketStream(vm, COMMAND_SET, COMMAND);
				if ((vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					vm.printTrace("Sending Command(id=" + ps.pkt.id
		...
		/**
		 * Sets a range of array components. The specified range must be within
		 * the bounds of the array. For primitive values, each value's type must
		 * match the array component type exactly. For object values, there must
		 * be a widening reference conversion from the value's type to the array
		...
		static class SetValues {
			...
			static SetValues process(VirtualMachineImpl vm,
					ArrayReferenceImpl arrayObject, int firstIndex,
					ValueImpl[] values) throws JDWPException {
				PacketStream ps = enqueueCommand(vm, arrayObject, firstIndex,
						values);
				return waitForReply(vm, ps);
			}
	...
	static class ClassLoaderReference {
		 ...
		 * <i>initiating</i> class loader for each class in the returned list.
		 * The list contains each reference type defined by this loader and any
		 * types for which loading was delegated by this class loader to another
		 * class loader.
		 * <p>
		 * The visible class list has useful properties with respect to the type
		 * namespace. A particular type name will occur at most once in the
		 * list. Each field or variable declared with that type name in a class
		 * defined by this class loader must be resolved to that single type.
		 * <p>
		...
		static class VisibleClasses {
			...
			static PacketStream enqueueCommand(VirtualMachineImpl vm,
				...
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 classLoaderObject(ClassLoaderReferenceImpl): "
									+ (classLoaderObject == null ? "NULL"
											: "ref=" + classLoaderObject
													.ref()));
				}
			...
			static VisibleClasses waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
	...
	static class EventRequest {
		 ...
		 * occurs, an <a href="#JDWP_Event">event</a> is sent from the target
		 * VM. If an event occurs that has not been requested then it is not
		 * sent from the target VM. The two exceptions to this are the VM Start
		 * Event and the VM Death Event which are automatically generated events
		 * - see <a href="#JDWP_Event_Composite">Composite Command</a> for
		 * further details.
		 */
		static class Set {
			static final int COMMAND = 1;
			...
			static class Modifier {
				 ...
				 * given number of occurrences. The event is not reported the
				 * first <code>count - 1</code> times this filter is reached. To
				 * request a one-off event, call this method with a count of 1.
				 * <p>
				 * Once the count reaches 0, any subsequent filters in this
				 * request are applied. If none of those filters cause the event
				 * to be suppressed, the event is reported. Otherwise, the event
				 * is not reported. In either case subsequent events are never
				 * reported for this request. This modifier can be used with any
				 * event kind.
				 */
				 ...
				 * modifier can be used with any event kind except for class
				 * unload.
				 */
				 ...
				 * this request to be the preparation of the given reference
				 * type and any subtypes. For monitor wait and waited events,
				 * restricts the events generated by this request to those whose
				 * monitor object is of the given reference type or any of its
				 * subtypes. For other events, restricts the events generated by
				 * this request to those whose location is in the given
				 * reference type or any of its subtypes. An event will be
				 * generated for any location in a reference type that can be
				 * safely cast to the given reference type. This modifier can be
				 * used with any event kind except class unload, thread start,
				 * and thread end.
				 */
				static class ClassOnly extends ModifierCommon {
					...
					void write(PacketStream ps) {
						if ((ps.vm.traceFlags
								& VirtualMachineImpl.TRACE_SENDS) != 0) {
							ps.vm.printTrace(
									"Sending:                         clazz(ReferenceTypeImpl): "
											+ (clazz == null ? "NULL"
													: "ref=" + clazz.ref()));
						}
						ps.writeClassRef(clazz.ref());
				 ...
				 * matches the given restricted regular expression. For class
				 * prepare events, the prepared class name is matched. For class
				 * unload events, the unloaded class name is matched. For
				 * monitor wait and waited events, the name of the class of the
				 * monitor object is matched. For other events, the class name
				 * of the event's location is matched. This modifier can be used
				 * with any event kind except thread start and thread end.
				 */
				static class ClassMatch extends ModifierCommon {
					static final byte ALT_ID = 5;

					static Modifier create(String classPattern) {
						return new Modifier(ALT_ID,
								new ClassMatch(classPattern));
					}

					/**
					 * Required class pattern. Matches are limited to exact
					 * matches of the given class pattern and matches of
					 * patterns that begin or end with '*'; for example, "*.Foo"
					 * or "java.*".
					 */
				...
				/**
				 * Restricts reported events to those for classes whose name
				 * does not match the given restricted regular expression. For
				 * class prepare events, the prepared class name is matched. For
				 * class unload events, the unloaded class name is matched. For
				 * monitor wait and waited events, the name of the class of the
				 * monitor object is matched. For other events, the class name
				 * of the event's location is matched. This modifier can be used
				 * with any event kind except thread start and thread end.
				...
				static class ClassExclude extends ModifierCommon {
					...
					static Modifier create(String classPattern) {
						return new Modifier(ALT_ID,
								new ClassExclude(classPattern));
					}

					/**
					 * Disallowed class pattern. Matches are limited to exact
					 * matches of the given class pattern and matches of
					 * patterns that begin or end with '*'; for example, "*.Foo"
					 * or "java.*".
					 */
				 ...
				 * location. This modifier can be used with breakpoint, field
				 ...
				 * are caught or uncaught. This modifier can be used with
				 * exception event kinds only.
				 */
				static class ExceptionOnly extends ModifierCommon {
					static final byte ALT_ID = 8;

					static Modifier create(ReferenceTypeImpl exceptionOrNull,
							boolean caught, boolean uncaught) {
						return new Modifier(ALT_ID, new ExceptionOnly(
								exceptionOrNull, caught, uncaught));
					}

					/**
					 * Exception to report. Null (0) means report exceptions of
					 * all types. A non-null type restricts the reported
					 * exception events to exceptions of the given type or any
					 * of its subtypes.
					 */
					 ...
					 * uncaught at the time it is thrown. See the exception
					 * event catch location under
					 * <a href="#JDWP_Event_Composite">composite events</a> for
					 * more information.
					 */
					final boolean uncaught;

					ExceptionOnly(ReferenceTypeImpl exceptionOrNull,
							boolean caught, boolean uncaught) {
					...
					void write(PacketStream ps) {
						if ((ps.vm.traceFlags
								& VirtualMachineImpl.TRACE_SENDS) != 0) {
							ps.vm.printTrace(
									"Sending:                         exceptionOrNull(ReferenceTypeImpl): "
											+ (exceptionOrNull == null ? "NULL"
													: "ref=" + exceptionOrNull
															.ref()));
						}
				 ...
				 * modification event kinds only.
				 */
				static class FieldOnly extends ModifierCommon {
					static final byte ALT_ID = 9;

					static Modifier create(ReferenceTypeImpl declaring,
							long fieldID) {
						return new Modifier(ALT_ID,
								new FieldOnly(declaring, fieldID));
					}
					...
					void write(PacketStream ps) {
						if ((ps.vm.traceFlags
								& VirtualMachineImpl.TRACE_SENDS) != 0) {
							ps.vm.printTrace(
									"Sending:                         declaring(ReferenceTypeImpl): "
											+ (declaring == null ? "NULL"
													: "ref=" + declaring
															.ref()));
						}
				...
				/**
				 * Restricts reported step events to those which satisfy depth
				 * and size constraints. This modifier can be used with step
				 * event kinds only.
				 */
				static class Step extends ModifierCommon {
					static final byte ALT_ID = 10;

					static Modifier create(ThreadReferenceImpl thread, int size,
							int depth) {
						return new Modifier(ALT_ID,
								new Step(thread, size, depth));
					}
					 ...
					 * <a href="#JDWP_StepSize">JDWP.StepSize</a>
					 */
					final int size;

					/**
					 * relative call stack limit. See
					 * <a href="#JDWP_StepDepth">JDWP.StepDepth</a>
					 */
					final int depth;
				 ...
				 * is the given object. Match value is the null object for
				 * static methods. This modifier can be used with any event kind
				 * except class prepare, class unload, thread start, and thread
				 * end. Introduced in JDWP version 1.4.
				 */
				static class InstanceOnly extends ModifierCommon {
					...
					void write(PacketStream ps) {
						if ((ps.vm.traceFlags
								& VirtualMachineImpl.TRACE_SENDS) != 0) {
							ps.vm.printTrace(
									"Sending:                         instance(ObjectReferenceImpl): "
											+ (instance == null ? "NULL"
													: "ref=" + instance.ref()));
						}
				...
				/**
				 * Restricts reported class prepare events to those for
				 * reference types which have a source name which matches the
				 * given restricted regular expression. The source names are
				 * determined by the reference type's
				 * <a href="#JDWP_ReferenceType_SourceDebugExtension">
				 * SourceDebugExtension</a>. This modifier can only be used with
				 * class prepare events. Since JDWP version 1.6. Requires the
				 * canUseSourceNameFilters capability - see <a href=
				 * "#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
				 */
				static class SourceNameMatch extends ModifierCommon {
					...
					static Modifier create(String sourceNamePattern) {
						return new Modifier(ALT_ID,
								new SourceNameMatch(sourceNamePattern));
					}

					/**
					 * Required source name pattern. Matches are limited to
					 * exact matches of the given pattern and matches of
					 * patterns that begin or end with '*'; for example, "*.Foo"
					 * or "java.*".
					 */
					...
					void write(PacketStream ps) {
						if ((ps.vm.traceFlags
								& VirtualMachineImpl.TRACE_SENDS) != 0) {
							ps.vm.printTrace(
									"Sending:                         sourceNamePattern(String): "
											+ sourceNamePattern);
						}
			...
			static Set process(VirtualMachineImpl vm, byte eventKind,
					byte suspendPolicy, Modifier[] modifiers)
					throws JDWPException {
				PacketStream ps = enqueueCommand(vm, eventKind, suspendPolicy,
						modifiers);
				return waitForReply(vm, ps);
			}

			static PacketStream enqueueCommand(VirtualMachineImpl vm,
					byte eventKind, byte suspendPolicy, Modifier[] modifiers) {
				PacketStream ps = new PacketStream(vm, COMMAND_SET, COMMAND);
				if ((vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					vm.printTrace("Sending Command(id=" + ps.pkt.id
				...
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 eventKind(byte): "
									+ eventKind);
				}
				ps.writeByte(eventKind);
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 suspendPolicy(byte): "
									+ suspendPolicy);
				}
				ps.writeByte(suspendPolicy);
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 modifiers(Modifier[]): "
									+ "");
				}
		 ...
		 * <a href="#JDWP_EventKind">JDWP.EventKind</a> for a complete list of
		 * events that can be cleared. Only the event request matching the
		 * specified event kind and requestID is cleared. If there isn't a
		 * matching event request the command is a no-op and does not result in
		 * an error. Automatically generated events do not have a corresponding
		 * event request and may not be cleared using this command.
		 */
		static class Clear {
			static final int COMMAND = 2;

			static Clear process(VirtualMachineImpl vm, byte eventKind,
					int requestID) throws JDWPException {
			...
			static PacketStream enqueueCommand(VirtualMachineImpl vm,
				...
				ps.writeByte(eventKind);
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace("Sending:                 requestID(int): "
							+ requestID);
				}
		...
		static class ClearAllBreakpoints {
			...
			static ClearAllBreakpoints waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
				return new ClearAllBreakpoints(vm, ps);
			}

			private ClearAllBreakpoints(VirtualMachineImpl vm,
					PacketStream ps) {
				if (vm.traceReceives) {
					vm.printTrace("Receiving Command(id=" + ps.pkt.id
							+ ") JDWP.EventRequest.ClearAllBreakpoints"
	...
	static class StackFrame {
		 ...
		 * Each variable must be visible at the frame's code index. Even if
		 * local variable information is not available, values can be retrieved
		 * if the front-end is able to determine the correct local variable
		 * index. (Typically, this index can be determined for method arguments
		...
		static class GetValues {
			...
			static class SlotInfo {
				...
				/**
				 * A <a href="#JDWP_Tag">tag</a> identifying the type of the
				 * variable
				 */
				final byte sigbyte;
			...
			static GetValues process(VirtualMachineImpl vm,
					ThreadReferenceImpl thread, long frame, SlotInfo[] slots)
					throws JDWPException {
			...
			static PacketStream enqueueCommand(VirtualMachineImpl vm,
				...
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 frame(long): " + frame);
				}
				ps.writeFrameRef(frame);
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 slots(SlotInfo[]): "
									+ "");
				}
			...
			/**
			 * The number of values retrieved, always equal to slots, the number
			 * of values to get.
			 */
			final ValueImpl[] values;
		 ...
		 * value's type must match the variable's type exactly. For object
		 * values, there must be a widening reference conversion from the
		 * value's type to the variable's type and the variable's type must be
		 * loaded.
		 * <p>
		 * Even if local variable information is not available, values can be
		 * set, if the front-end is able to determine the correct local variable
		 * index. (Typically, this index can be determined for method arguments
		 * from the method signature without access to the local variable table
		 * information.)
		 */
		static class SetValues {
			...
			static SetValues process(VirtualMachineImpl vm,
					ThreadReferenceImpl thread, long frame,
					SlotInfo[] slotValues) throws JDWPException {
		...
		static class ThisObject {
			...
			static ThisObject waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
		 ...
		 * command. The top-most stack frames are discarded and the stack frame
		 * previous to 'frame' becomes the current frame. The operand stack is
		 * restored -- the argument values are added back and if the invoke was
		 * not <code>invokestatic</code>, <code>objectref</code> is added back
		 * as well. The Java virtual machine program counter is restored to the
		 * opcode of the invoke instruction.
		 * <p>
		...
		static class PopFrames {
			...
			static PopFrames waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
	...
	static class ClassObjectReference {
		...
		static class ReflectedType {
			...
			static ReflectedType process(VirtualMachineImpl vm,
					ClassObjectReferenceImpl classObject) throws JDWPException {
			...
			static PacketStream enqueueCommand(VirtualMachineImpl vm,
				...
				if ((ps.vm.traceFlags & VirtualMachineImpl.TRACE_SENDS) != 0) {
					ps.vm.printTrace(
							"Sending:                 classObject(ClassObjectReferenceImpl): "
									+ (classObject == null ? "NULL"
											: "ref=" + classObject.ref()));
				}
			...
			static ReflectedType waitForReply(VirtualMachineImpl vm,
					PacketStream ps) throws JDWPException {
				ps.waitForReply();
	...
	static class Event {
		 ...
		 * composite event. For uniformity, a composite event is always used to
		 * deliver events, even if there is only one event to report.
		 * <P>
		 ...
		 * </UL>
		 * <LI>Only with other access watchpoint events for the same field
		 * access:
		 * <UL>
		 ...
		 * </UL>
		 * <LI>Only with other Monitor contended enter events for the same
		 * monitor object:
		 * <UL>
		 * <LI>Monitor Contended Enter Event
		 * </UL>
		 * <LI>Only with other Monitor contended entered events for the same
		 * monitor object:
		 * <UL>
		 ...
		 * </UL>
		 * <LI>Only with other Monitor waited events for the same monitor
		 * object:
		 * <UL>
		 * <LI>Monitor Waited Event
		 * </UL>
		 * <LI>Only with other ExceptionEvents for the same exception
		 * occurrance:
		 * <UL>
		 * <LI>ExceptionEvent
		 * </UL>
		 * <LI>Only with other members of this group, at the same location and
		 * in the same thread:
		 * <UL>
		 * <LI>Breakpoint Event
		 * <LI>Step Event
		 ...
		 * <a href="#JDWP_EventRequest_Set">EventRequest.Set</a> command. The VM
		 * Start event signals the completion of VM initialization. The VM Death
		 * event signals the termination of the VM. If there is a debugger
		 * connected at the time when an automatically generated event occurs it
		 * is sent from the target VM. Automatically generated events may also
		 * be requested using the EventRequest.Set command and thus multiple
		 * events of the same event kind will be sent from the target VM when an
		 * event occurs. Automatically generated events are sent with the
		 * requestID field in the Event Data set to 0. The value of the
		 * suspendPolicy field in the Event Data depends on the event. For the
		 * automatically generated VM Start Event the value of suspendPolicy is
		 * not defined and is therefore implementation or configuration
		 * specific. In the Sun implementation, for example, the suspendPolicy
		 * is specified as an option to the JDWP agent at launch-time. The
		 * automatically generated VM Death Event will have the suspendPolicy
		 * set to NONE.
		 */
		static class Composite {
			...
			static class Events {
				 ...
				 * of system classes have been loaded. This event is always
				 ...
				 * event is generated before the code at its location is
				 * executed.
				 */
				 ...
				 * executed. Method entry events are generated for both native
				 * and non-native methods.
				 * <P>
				 * In some VMs method entry events can occur for a particular
				 * thread before its thread start event occurs if methods are
				 * called as part of the thread's initialization.
				 */
				 ...
				 * is generated after all code in the method has executed, but
				 * the location of this event is the last executed location in
				 * the method. Method exit events are generated for both native
				 * and non-native methods. Method exit events are not generated
				...
				static class MethodExitWithReturnValue extends EventsCommon {
					...
					MethodExitWithReturnValue(VirtualMachineImpl vm,
							PacketStream ps) {
						requestID = ps.readInt();
						if (vm.traceReceives) {
							vm.printReceiveTrace(6,
				 ...
				 * Requires canRequestMonitorEvents capability - see <a href=
				 * "#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
				 * <p>
				 * Since JDWP version 1.6.
				 */
				static class MonitorContendedEnter extends EventsCommon {
					...
					MonitorContendedEnter(VirtualMachineImpl vm,
							PacketStream ps) {
						requestID = ps.readInt();
						if (vm.traceReceives) {
							vm.printReceiveTrace(6,
				...
				/**
				 * Notification of a thread in the target VM is entering a
				 * monitor after waiting for it to be released by another
				 * thread. Requires canRequestMonitorEvents capability - see
				 * <a href=
				 * "#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
				 * <p>
				 * Since JDWP version 1.6.
				...
				static class MonitorContendedEntered extends EventsCommon {
					...
					MonitorContendedEntered(VirtualMachineImpl vm,
							PacketStream ps) {
						requestID = ps.readInt();
						if (vm.traceReceives) {
							vm.printReceiveTrace(6,
				 ...
				 * <a href=
				 * "#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>. a
				 * monitor object.
				 * <p>
				 * Since JDWP version 1.6.
				 */
				...
				/**
				 * Notification of an exception in the target VM. If the
				 * exception is thrown from a non-native method, the exception
				 * event is generated at the location where the exception is
				 * thrown. If the exception is thrown from a native method, the
				 * exception event is generated at the first non-native location
				...
				static class Exception extends EventsCommon {
					 ...
					 * considered to be caught if, at the point of the throw,
					 * the current location is dynamically enclosed in a try
					 * statement that handles the exception. (See the JVM
					 * specification for details). If there is such a try
					 * statement, the catch location is the first location in
					 * the appropriate catch clause.
					 * <p>
					 * If there are native methods in the call stack at the time
					 * of the exception, there are important restrictions to
					 * note about the returned catch location. In such cases, it
					 * is not possible to predict whether an exception will be
					 * handled by some native method on the call stack. Thus, it
					 * is possible that exceptions considered uncaught here
					 * will, in fact, be handled by a native method and not
					 * cause termination of the target VM. Furthermore, it
					 * cannot be assumed that the catch location returned here
					 * will ever be reached by the throwing thread. If there is
					 * a native frame between the current location and the catch
					 * location, the exception might be handled and cleared in
					 * that native method instead.
					 * <p>
					 * Note that compilers can generate try-catch blocks in some
					 * cases where they are not explicit in the source code; for
					 * example, the code generated for <code>synchronized</code>
					 * and <code>finally</code> blocks can contain implicit
					 * try-catch blocks. If such an implicitly generated
					 * try-catch is present on the call stack at the time of the
					 * throw, the exception will be considered caught even
					 * though it appears to be uncaught from examination of the
					 * source code.
					 */
					...
					Exception(VirtualMachineImpl vm, PacketStream ps) {
						...
						if (vm.traceReceives) {
							vm.printReceiveTrace(6, "catchLocation(Location): "
									+ catchLocation);
						}
					}
				}
				 ...
				 * new thread can be the result of a call to
				 * <code>java.lang.Thread.start</code> or the result of
				 * attaching a new thread to the VM though JNI. The notification
				 * is generated by the new thread some time before its execution
				 * starts. Because of this timing, it is possible to receive
				 * other events for the thread before this event is received.
				 * (Notably, Method Entry Events and Method Exit Events might
				 * occur during thread initialization. It is also possible for
				 * the <a href="#JDWP_VirtualMachine_AllThreads">VirtualMachine
				 * AllThreads</a> command to return a thread before its thread
				 * start event is received.
				 * <p>
				 * Note that this event gives no information about the creation
				 * of the thread object which may have happened much earlier,
				 * depending on the VM being debugged.
				 */
				 ...
				 * notification is generated by the dying thread before it
				 * terminates. Because of this timing, it is possible for
				 * {@link VirtualMachine#allThreads} to return this thread after
				 * this event is received.
				 * <p>
				 * Note that this event gives no information about the lifetime
				 * of the thread object. It may or may not be collected soon
				 * depending on what references exist in the target VM.
				 */
				 ...
				 * specification for a definition of class preparation. Class
				 * prepare events are not generated for primtiive classes (for
				...
				static class ClassPrepare extends EventsCommon {
					...
					/**
					 * Preparing thread. In rare cases, this event may occur in
					 * a debugger system thread within the target VM. Debugger
					 * threads take precautions to prevent these events, but
					 * they cannot be avoided under some conditions, especially
					 * for some subclasses of java.lang.Error. If the event was
					 * generated by a debugger system thread, the value returned
					 * by this method is null, and if the requested
					 * <a href="#JDWP_SuspendPolicy">suspend policy</a> for the
					 * event was EVENT_THREAD all threads will be suspended
					 * instead, and the composite event's suspend policy will
					 * reflect this change.
					 * <p>
					 * Note that the discussion above does not apply to system
					 * threads created by the target VM during its normal
					 * (non-debug) operation.
					 */
					final ThreadReferenceImpl thread;
					 ...
					 * <a href="#JDWP_TypeTag">JDWP.TypeTag</a>
					 */
					final byte refTypeTag;
				 ...
				 * canWatchFieldAccess capability - see <a href=
				 * "#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
				 */
				 ...
				 * Requires canWatchFieldModification capability - see <a href=
				 * "#JDWP_VirtualMachine_CapabilitiesNew">CapabilitiesNew</a>.
				 */
			...
			Composite(VirtualMachineImpl vm, PacketStream ps) {
				...
				if (vm.traceReceives) {
					vm.printReceiveTrace(4,
							"suspendPolicy(byte): " + suspendPolicy);
				}
				if (vm.traceReceives) {
	...
	/**
	 * The invoke options are a combination of zero or more of the following bit
	 * flags:
	 */
