...
class ServerConsoleAnswers implements AppSuggestor.AnswerSupplier {
	public ServerConsoleAnswers() {
	}
	...
	public void begin(Invocation invocation) {
		...
		request.commandContexts.add(ServerConsoleBrowser.CommandContext.class);
		List<AppSuggestion> suggestions = new ArrayList<>();
		{
			// add filter selection
			AppSuggestionEntry suggestion = new AppSuggestionEntry();
			suggestion.match = Ax.format("Filter: '%s'", query);
			suggestion.modelEvent = ServerConsoleEvents.FilterElements.class;
		...
		}
		{
			...
			commandNodes.stream().map(ServerConsoleAnswers::createSuggestion)
