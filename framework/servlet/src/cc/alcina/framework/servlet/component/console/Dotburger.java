...
import cc.alcina.framework.gwt.client.dirndl.model.edit.CheckboxInput;
...
class Dotburger extends Model.Fields {
	...
	static class Menu extends Model.All implements ValueChange.Container {
		...
		Heading section1 = new Heading("Compact report UI");
		...
		Link keyboardShortcuts = Link
				.of(ServerConsoleBrowserCommand.ShowKeyboardShortcuts.class);

		Menu() {
			from(ServerConsoleSettings.get().properties()
					.compactServerConsoleUi())
							.to(properties().compactServerConsoleUi()).bidi();
		}
	}
