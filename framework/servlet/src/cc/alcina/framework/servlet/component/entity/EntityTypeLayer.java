...
class EntityTypeLayer extends Layer<EntityTypesLayer.TypeSelection> {
	@Override
	public void process(EntityTypesLayer.TypeSelection selection)
			throws Exception {
		if (!EntityGraphView.peer().isSelected(selection)) {
			return;
		}
		Domain.stream(selection.get()).limit(10)
				.map(e -> new EntitySelection(selection, e))
				.forEach(this::select);
	}
