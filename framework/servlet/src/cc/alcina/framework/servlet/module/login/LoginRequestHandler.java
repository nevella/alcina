...
public abstract class LoginRequestHandler<U extends IUser> {
    ...
    protected void handle0() {
        try {
            if (!validateLoginAttempt()) {
                loginResponse.getStates().add(LoginResponseState.Account_locked_out);
                return;
            }
            if (!validateUserName()) {
            ...
            if (!validateAccount()) {
                loginResponse.getStates().add(LoginResponseState.Account_cannot_login);
                return;
            }
            TwoFactorAuthResult twoFactorAuthResult = validateTwoFactorAuth();
            if (twoFactorAuthResult.requiresTwoFactorAuth) {
                loginResponse.getStates().add(LoginResponseState.Two_factor_code_required);
                loginResponse.setTwoFactorAuthQRCode(twoFactorAuthResult.qrCode);
                if (twoFactorAuthResult.requiresTwoFactorQrCode) {
                    loginResponse.getStates().add(LoginResponseState.Two_factor_qr_code_required);
                }
                return;
            }
            processLogin();
    ...
    protected void postRequestHandled() {
        ...
        if (loginResponse.isOk()) {
            ...
            logger.warn("Login failed: {} {}", loginRequest.getUserName(), loginResponse.getStates());
        }
    }
    ...
    protected void processLogin() throws Exception {
        authenticator.processValidLogin(loginResponse, loginModel.user.getUserName(), true);
    }

    protected boolean validateAccount() throws Exception {
        authenticator.validateAccount(loginModel.loginResponse, loginModel.user.getUserName());
