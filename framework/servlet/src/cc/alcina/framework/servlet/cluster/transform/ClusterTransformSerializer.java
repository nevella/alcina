...
public class ClusterTransformSerializer {
    ...
    public ClusterTransformRequest deserialize(byte[] data, Class<? extends TransformCommitLog> requestorClass) {
        ...
        synchronized (packets) {
            ...
            if (packets.get(packet.sequenceIdx).size() == packet.chunkCount) {
                ...
                packets.remove(packet.sequenceIdx).stream().sorted(Comparator.comparing(p -> p.chunkIdx)).forEach(p -> {
                    try {
                        out.write(p.bytes);
                    } catch (IOException e) {
        ...
        try {
            InputStream inputStream = new GZIPInputStream(new ByteArrayInputStream(assembled));
            inputStream = preProcessJson(inputStream, requestorClass);
            ClusterTransformRequest request = JacksonUtils.deserialize(inputStream, ClusterTransformRequest.class);
            inputStream.close();
            return request;
        } catch (Exception e) {
    ...
    public List<byte[]> serialize(DomainTransformRequestPersistent request, State state) {
        ...
        try {
            ...
            new JacksonJsonObjectSerializer().withIdRefs().withTypeInfo().withMaxLength(Integer.MAX_VALUE).serializeToStream(clusterRequest, outputStream);
            zipped = out.toByteArray();
            if (state == State.PRE_COMMIT) {
                if (zipped == null || zipped.length > 100000 || request.getEvents().size() > 1000) {
                    logger.info("Large serialized request :: {} :: {} events ::  {} bytes zipped", request.getId(), request.getEvents().size(), zipped.length);
                }
            }
            /*
            ...
            String sequenceIdx = Ax.format("%s::%s", EntityLayerUtils.getLocalHostName(), localSequenceCounter.getAndIncrement());
            for (int idx = 0; idx < chunkCount; idx += 1) {
                ...
                int endArrayPos = Math.min(startArrayPos + CHUNK_SIZE, zipped.length);
                packet.dataLength = endArrayPos - startArrayPos;
                packet.bytes = new byte[packet.dataLength];
                System.arraycopy(zipped, startArrayPos, packet.bytes, 0, packet.dataLength);
                ByteArrayOutputStream outBytes = new ByteArrayOutputStream(packet.dataLength + 100);
    ...
    private DomainTransformRequestPersistent projectRequest(DomainTransformRequestPersistent request) {
        DomainTransformRequestPersistent result = Reflections.newInstance(request.getClass());
        result.setId(request.getId());
        result.setEvents(request.getEvents().stream().collect(Collectors.toList()));
        result.setChunkUuidString(request.getChunkUuidString());
        ClientInstance originalClientInstance = request.getClientInstance();
        ClientInstance clientInstance = (ClientInstance) Reflections.newInstance(originalClientInstance.entityClass());
        ...
        AuthenticationSession originalAuthenticationSession = originalClientInstance.getAuthenticationSession();
        AuthenticationSession authenticationSession = (AuthenticationSession) Reflections.newInstance(originalAuthenticationSession.entityClass());
        authenticationSession.setId(originalAuthenticationSession.getId());
        IUser originalUser = originalClientInstance.provideUser();
        IUser user = (IUser) Reflections.newInstance(((Entity) originalUser).entityClass());
	 ...
	 */
    protected InputStream preProcessJson(InputStream inputStream, Class<? extends TransformCommitLog> requestorClass) throws Exception {
        return inputStream;
    }
