...
public class BasicGridPublisher {
    ...
    @Registration({ ContentModelHandler.class, BasicGridContentDefinition.class })
    public static class BasicExcelPublisherContentHandler extends ContentModelHandler<BasicGridContentDefinition, BasicGridPublicationModel, BasicGridRequest> {
        ...
        protected void prepareContent() throws Exception {
            ...
            defName = Ax.blankTo(defName, () -> Ax.format("%s-%s", def.getClass().getSimpleName(), CommonUtils.formatDate(new Date(), DateStyle.TIMESTAMP)));
            if (def instanceof SingleTableSearchDefinition) {
                deliveryModel.setSuggestedFileName(SEUtilities.sanitiseFileName(defName.replace(" ", "_")));
                SearchResultsBase results = Registry.impl(CommonRemoteServletProvider.class).getCommonRemoteServiceServlet().search(def);
                ...
                deliveryModel.setSuggestedFileName(SEUtilities.sanitiseFileName(defName.replace(" ", "_")));
                defName = Ax.blankTo(defName, () -> Ax.format("%s-%s", bdef.getClass().getSimpleName(), CommonUtils.formatDate(new Date(), DateStyle.TIMESTAMP)));
                ModelSearchResults modelSearchResults = Registry.impl(CommonRemoteServletProvider.class).getCommonRemoteServiceServlet().searchModel(bdef);
    ...
    public static class BasicGridPublicationModelRenderer extends ContentRenderer<BasicGridContentDefinition, BasicGridPublicationModel, ContentRequestBase> {
        ...
        @Override
        protected void renderContent(long publicationId, long publicationUserId) throws Exception {
            ...
            exporter.addCollectionToBook(wr.pc.resultRows, doc, "query_results");
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            OutputStreamWriter writer = new OutputStreamWriter(baos, "UTF-8");
            XmlUtils.streamXML(doc, writer);
            if (deliveryModel.provideTargetFormat() == FormatConversionTarget.XLSX) {
                results.bytes = baos.toByteArray();
            } else if (deliveryModel.provideTargetFormat() == FormatConversionTarget.HTML) {
                results.bytes = baos.toByteArray();
            ...
            } else if (deliveryModel.provideTargetFormat() == FormatConversionTarget.CSV) {
                CsvCols csvCols = new CsvCols((List) exporter.getCellList());
                results.bytes = csvCols.toCsv().getBytes(StandardCharsets.UTF_8);
            } else if (deliveryModel.provideTargetFormat() == FormatConversionTarget.JSON) {
                List<List> cellList = exporter.getCellList();
