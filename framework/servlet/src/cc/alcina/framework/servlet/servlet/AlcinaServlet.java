...
public abstract class AlcinaServlet extends HttpServlet {
    ...
    public void writeAndClose(String s, HttpServletResponse response) throws IOException {
    ...
    protected Boolean parseBooleanParam(HttpServletRequest request, String param) {
        try {
    ...
    protected Integer parseIntegerParam(HttpServletRequest request, String param) {
        try {
    ...
    protected void wrapRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        AlcinaServletContext alcinaContext = null;
        try {
            String threadName = Ax.format("task-%s:%s", getClass().getSimpleName(), callCounter.incrementAndGet());
            alcinaContext = new AlcinaServletContext().withRootPermissions(isRunWithRootPermissions());
            alcinaContext.begin(request, response, threadName);
            if (trackMetrics()) {
                MetricLogging.get().start(getClass().getSimpleName());
                InternalMetrics.get().startTracker(request, () -> Ax.format("Alcina servlet %s", getClass().getSimpleName()), InternalMetricTypeAlcina.servlet, Thread.currentThread().getName(), () -> true);
            }
            handleRequest(request, response);
        } catch (Throwable t) {
            topicApplicationThrowables().publish(t);
            logger.warn("Alcina servlet request issue - user {} - url {}", PermissionsManager.get().getUser().toIdNameString(), request.getRequestURI());
            ...
            try {
                ...
                writeTextResponse(response, throwDetailedExceptions() ? SEUtilities.getFullExceptionMessage(t) : "Sorry, that's a 500");
            } catch (IOException e) {
                e.printStackTrace();
            }
            return;
    ...
    protected void writeHtmlResponse(HttpServletResponse response, String string) throws IOException {
        if (response == null) {
    ...
    protected void writeTextResponse(HttpServletResponse response, String string) throws IOException {
        if (response == null) {
            System.out.println(CommonUtils.trimToWsChars(string, 1000));
        } else {
            try {
