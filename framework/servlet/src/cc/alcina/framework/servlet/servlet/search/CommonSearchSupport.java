...
public class CommonSearchSupport {
    ...
    public static final transient String CONTEXT_DO_NOT_PROJECT_SEARCH = CommonSearchSupport.class.getName() + ".CONTEXT_DO_NOT_PROJECT_SEARCH";
    ...
    public <T extends Entity> ModelSearchResults getForClass(String className, long objectId) {
        try {
            T t = null;
    ...
    public ModelSearchResults getModelSearchResults(List<? extends Entity> queried, EntitySearchDefinition def) {
        ...
        if (LooseContext.is(CONTEXT_DO_NOT_PROJECT_SEARCH)) {
            ...
            if (def == null) {
                ...
                modelSearchResults.setResultClassName(Optional.ofNullable(first).map(e -> e.entityClass().getName()).orElse(null));
                if (first != null) {
                ...
                modelSearchResults.setFilteringEntities((List) filterPlaces.stream().map(EntityPlace::asLocator).map(Domain::find).collect(Collectors.toList()));
                /*
				 * Just get non-entity properties, essentially - for things like
				 * breadcrumbs client-side
				 */
                Set<Entity> alsoProject = modelSearchResults.getFilteringEntities().stream().map(Entity.Ownership::getOwningEntities).flatMap(Collection::stream).collect(Collectors.toSet());
                CollectionProjectionFilterWithCache dataFilter = new CollectionProjectionFilterWithCache() {

                    @Override
                    public <T> T filterData(T original, T projected, GraphProjectionContext context, GraphProjection graphProjection) throws Exception {
                        if (original instanceof Set || original instanceof Map) {
                            return null;
                        }
                        if (original instanceof Entity && !(modelSearchResults.getFilteringEntities().contains(original)) && !(alsoProject.contains(original))) {
                            return null;
                        }
                        return super.filterData(original, projected, context, graphProjection);
                    }
                };
                modelSearchResults.setFilteringEntities(GraphProjections.defaultProjections().dataFilter(dataFilter).project(modelSearchResults.getFilteringEntities()));
            }
        }
    ...
    public ModelSearchResults searchModel(BindableSearchDefinition def) {
        ...
        String key = LooseContext.runWithTrue(SearchCriterion.CONTEXT_ENSURE_DISPLAY_NAME, () -> "searchModel - " + def);
        try {
            LooseContext.push();
            ...
            Optional<CustomSearchHandler> customSearchHandler = Registry.optional(CustomSearchHandler.class, def.getClass());
            if (customSearchHandler.isPresent()) {
                customSearchHandler.get().prepareContext(searchContext);
                ModelSearchResults searchModel = customSearchHandler.get().searchModel(searchContext);
                if (searchModel != null) {
                    return searchModel;
                }
            }
            Stream<? extends Entity> search = new DomainSearcher().search(def, clazz, searchContext.orders);
            Registry.impl(SearcherCollectionSource.class).beforeQuery(clazz, def);
            // FIXME - 2022 - there may be places where we can get result set
            // size without collecting (i.e. index-only)
            ObjectWrapper<Stream<? extends Entity>> mutableStream = ObjectWrapper.of(search);
            List<Entity> rows = DomainStore.queryPool().call(() -> mutableStream.get().collect(Collectors.toList()), mutableStream);
            IntPair range = new IntPair(def.getResultsPerPage() * (def.getPageNumber()), def.getResultsPerPage() * (def.getPageNumber() + 1));
            range = range.intersection(new IntPair(0, rows.size()));
            searchContext.queried = range == null ? new ArrayList<Entity>() : new ArrayList<Entity>(rows.subList(range.i1, range.i2));
            if (searchContext.groupingParameters == null) {
                searchContext.modelSearchResults = getModelSearchResults(searchContext.queried, (EntitySearchDefinition) def);
            } else {
                searchContext.modelSearchResults = new ModelSearchResults();
                GroupingHandler groupingHandler = Registry.impl(GroupingHandler.class, searchContext.groupingParameters.getClass());
                searchContext.modelSearchResults.setGroupedResult(groupingHandler.process(searchContext.queried, searchContext.groupingParameters, def));
                groupingHandler.sort(searchContext.modelSearchResults.getGroupedResult(), searchContext.groupingParameters);
                ...
                if (LooseContext.is(CONTEXT_DO_NOT_PROJECT_SEARCH)) {
                    ...
                    searchContext.modelSearchResults = GraphProjections.defaultProjections().project(searchContext.modelSearchResults);
                }
            }
            ...
            searchContext.modelSearchResults.setTransformLogPosition(DomainStore.stores().writableStore().getPersistenceEvents().getQueue().getTransformCommitPosition());
    ...
    public ModelSearchResults toModelSearchResults(SearchResultsBase searchResultsBase, BindableSearchDefinition toSearchDefinition) {
        ...
        result.setResultClassName(toSearchDefinition.queriedBindableClass().getName());
        return result;
    }

    private <T> T project(T object, ModelSearchResults modelSearchResults, boolean projectAsSingleEntityDataObjects) {
        SearchResultProjector projector = Registry.impl(SearchResultProjector.class);
        projector.setProjectAsSingleEntityDataObjects(projectAsSingleEntityDataObjects);
        Class<? extends Bindable> projectedClass = projector.getProjectedClass(modelSearchResults);
    ...
    public static class SearchResultProjector {
        ...
        public void setProjectAsSingleEntityDataObjects(boolean projectAsSingleEntityDataObjects) {
