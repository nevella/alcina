...
public class GeolocationResolver_Ipstack implements GeolocationResolver {
    ...
    private CachingMap<String, String> ipToLocation = new CachingMap<>(s -> getLocation0(s));
    ...
    private String getLocation0(String ipAddress) {
        ...
        try {
            String apiKey = ResourceUtilities.get(GeolocationResolver_Ipstack.class, "apiKey");
            String url = Ax.format("http://api.ipstack.com/%s?access_key=%s&output=json", ipAddress, apiKey);
            ...
            return Ax.format("%s/%s", Ax.blankTo(node.get("city").asText(), "(No city)"), node.get("country_name").asText());
        } catch (Exception e) {
            e.printStackTrace();
