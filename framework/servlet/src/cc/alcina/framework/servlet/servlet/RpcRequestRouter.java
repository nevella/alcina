...
public class RpcRequestRouter {
    ...
    public String invokeAndEncodeResponse(Object target, RPCRequest rpcRequest) throws SerializationException {
        String requestHandlerUrl = ResourceUtilities.get(RpcRequestRouter.class, "requestHandlerUrl");
        if (Ax.notBlank(requestHandlerUrl) && !forceServerHandler(rpcRequest)) {
            try {
                ...
                return RPC.invokeAndEncodeResponse(proxy, rpcRequest.getMethod(), rpcRequest.getParameters(), rpcRequest.getSerializationPolicy());
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                LooseContext.pop();
    ...
    private Object wrapProxy(Object invocationTarget, RPCRequest rpcRequest) {
        for (Class clazz : invocationTarget.getClass().getInterfaces()) {
            if (RemoteService.class.isAssignableFrom(clazz) && rpcRequest.getMethod().getDeclaringClass().isAssignableFrom(clazz)) {
                Object[] args = rpcRequest.getParameters();
                String methodName = rpcRequest.getMethod().getName();
                Method method = new SEUtilities.MethodFinder().findMethod(clazz, args, methodName);
                if (method != null) {
                    RemoteInvocationProxy proxy = new RemoteInvocationProxy();
                    proxy.setRemoteAddress(ResourceUtilities.get("requestHandlerUrl"));
    ...
    protected boolean forceServerHandler(RPCRequest rpcRequest) {
        if (rpcRequest.getMethod().getName().matches("performAction|performAdminAction")) {
		 ...
		 * (reflective rpc)
		 */
        if (rpcRequest.getMethod().getName().matches("callRpc") && rpcRequest.getParameters()[0].toString().contains("\"methodName\":\"login\"")) {
            return true;
        }
        /*
