...
public class RemoteInvocation {
    ...
    public void hookParams(String methodName, Object[] args, RemoteInvocationParameters params) {
        for (RemoteInvocationProxyInterceptor interceptor : Registry.impls(RemoteInvocationProxyInterceptor.class)) {
            interceptor.hookParams(methodName, args, params);
        }
    }

    public Object invoke(String methodName, Object[] args, RemoteInvocationParameters params) throws Exception {
        try {
            ...
            String address = Ax.blankTo(getRemoteAddress(), ResourceUtilities.getBundledString(RemoteInvocation.class, "address"));
            ...
            boolean transformMethod = methodName.equals("transformInPersistenceContext");
            ...
            LooseContext.getContext().properties.forEach((k, v) -> {
                if (v == null || GraphProjection.isPrimitiveOrDataClass(v.getClass())) {
                    params.context.put(k, v);
                }
            });
            List<NameValuePair> qparams = new ArrayList<NameValuePair>();
            qparams.add(new BasicNameValuePair(RemoteInvocationServlet.REMOTE_INVOCATION_PARAMETERS, KryoUtils.serializeToBase64(params)));
            ...
            if (transformMethod) {
                ...
                if (!LooseContext.is(TransformPersisterInPersistenceContext.CONTEXT_REPLAYING_FOR_LOGS)) {
                    EntityLocatorMap returned = ((DomainTransformLayerWrapper) container.get(0)).locatorMap;
                    EntityLocatorMap sent = ((TransformPersistenceToken) params.args[1]).getLocatorMap();
                    sent.merge(returned);
                }
            }
    ...
    public boolean tryInterception(Object proxy, Method method, Object[] args) throws Throwable {
        for (RemoteInvocationProxyInterceptor interceptor : Registry.impls(RemoteInvocationProxyInterceptor.class)) {
