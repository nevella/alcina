...
public class Knowns {
    ...
    public static Object fromStringValue(String path, String value, Field field, ValueType valueType) throws ParseException {
        Class type = field.getType();
        return fromStringValue(path, value, field.getName(), type, valueType);
    }

    public static Object fromStringValue(String path, String value, String fieldName, Class type, ValueType valueType) throws ParseException {
        if (value == null) {
            return null;
        }
    ...
    public static void handleStatusRule(KnownRenderableNode node) {
        ...
        if (node.getField() != null) {
            ...
            Registry.get().lookupImplementation(KnownStatusRuleHandler.class, rule.name(), "ruleName", true).handleRule(field, node, rule);
            ...
            if (node.getNodeMetadata() != null) {
                ...
                Registry.get().lookupImplementation(KnownStatusRuleHandler.class, rule.name(), "ruleName", true).handleRule(node.getNodeMetadata(), node, rule);
                Registry.impl(KnownNodeAppLogic.class).processNodeRule(node, rule);
            }
        }
    }

    public static void mapToRenderablePropertyNode(KnownRenderableNode parent, String value, Object typedValue, String name, Field field, KnownNodeProperty propertyMetadata) {
        try {
            ...
            if (propertyMetadata != null) {
                Class typeClass = Class.forName(propertyNode.getPropertyMetadata().getTypeName());
                if (typeClass == Date.class) {
                    propertyNode.setDateValue((Date) fromStringValue(parent.path(), value, name, typeClass, ValueType.DATA_TYPE));
                } else if (typeClass == OpStatus.class) {
                    propertyNode.setOpStatusValue((OpStatus) fromStringValue(parent.path(), value, name, typeClass, ValueType.DATA_TYPE));
                }
            }
        } catch (Exception e) {
    ...
    public static void reconcile(KnownNode node, boolean fromPersistent) {
        ...
        try {
            ...
            if (registryProvider instanceof ClassLoaderAwareRegistryProvider) {
                ...
                LooseContext.set(KryoUtils.CONTEXT_OVERRIDE_CLASSLOADER, clRegistryProvider.getServletLayerClassloader());
            }
            if (fromPersistent) {
                fromPersistent(node);
            } else {
    ...
    public static String toStringValue(Object value, Field field, ValueType valueType) {
        if (value == null) {
            return null;
        }
        if (valueType == ValueType.KRYO_PERSISTABLE) {
        ...
        if (value.getClass() == Integer.class || value.getClass() == String.class || value.getClass() == Double.class || value.getClass() == Float.class || value.getClass() == Short.class || value.getClass() == Boolean.class || value.getClass() == Long.class || value instanceof Enum) {
            return value.toString();
        } else if (value.getClass() == Date.class) {
            ZonedDateTime zdt = ZonedDateTime.ofInstant(((Date) value).toInstant(), ZoneId.of("UTC"));
            return DateTimeFormatter.ISO_DATE_TIME.format(zdt);
        }
        throw new RuntimeException("not implemented");
    }

    private static synchronized KnownRenderableNode fromPersistent(KnownNode node) {
    ...
    public static class KnownNodeAppLogic {
        ...
        public void processNodeRule(KnownRenderableNode node, KnownStatusRule rule) {
            // No-op
        }
    }
