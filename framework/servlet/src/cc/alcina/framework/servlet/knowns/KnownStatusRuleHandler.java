...
public abstract class KnownStatusRuleHandler {
    ...
    public abstract void handleRule(Field field, KnownRenderableNode node, KnownStatusRule rule);

    public abstract void handleRule(KnownNodeMetadata nodeMetadata, KnownRenderableNode node, KnownStatusRule rule);
    ...
    public static class KnownStatusRuleHandler_Max_Age extends KnownStatusRuleHandler {
        ...
        @Override
        public void handleRule(Field field, KnownRenderableNode node, KnownStatusRule rule) {
            ...
            if (status == KnownTagAlcina.Status_Error) {
                ...
                if (lastOkDate != null) {
                    ...
                    if (System.currentTimeMillis() - lastOkDate.getTime() > rule.warnValue() * TimeConstants.ONE_HOUR_MS) {
                        status = KnownTagAlcina.Status_Warn;
                    }
                    if (System.currentTimeMillis() - lastOkDate.getTime() > rule.errorValue() * TimeConstants.ONE_HOUR_MS) {
                        status = KnownTagAlcina.Status_Error;
                    }
                }
            }
        ...
        @Override
        public void handleRule(KnownNodeMetadata nodeMetadata, KnownRenderableNode node, KnownStatusRule rule) {
            KnownTagAlcina status = null;
            ...
            if (status == KnownTagAlcina.Status_Error) {
                ...
                if (node.hasProperty("lastOk") || node.hasProperty("lastRun")) {
                    Date lastOkDate = node.hasProperty("lastOk") ? node.typedChildValue(Date.class, "lastOk") : node.typedChildValue(Date.class, "lastRun");
                    if (lastOkDate != null) {
                        status = KnownTagAlcina.Status_Ok;
                        if (System.currentTimeMillis() - lastOkDate.getTime() > rule.warnValue() * TimeConstants.ONE_HOUR_MS) {
                            status = KnownTagAlcina.Status_Warn;
    ...
    public static class KnownStatusRuleHandler_Run_By extends KnownStatusRuleHandler {
        ...
        public void handleRule(KnownNodeMetadata nodeMetadata, KnownRenderableNode node, KnownStatusRule rule) {
            ...
            if (node.hasProperty("lastOk") || node.hasProperty("lastRun")) {
                lastOkDate = node.hasProperty("lastOk") ? node.typedChildValue(Date.class, "lastOk") : node.typedChildValue(Date.class, "lastRun");
            }
        ...
        private KnownTagAlcina testRule(OpStatus opStatus, KnownStatusRule rule, Date lastOkDate) {
        ...
        private KnownTagAlcina checkDateTime(KnownStatusRule rule, Date lastOkDate) {
            KnownTagAlcina status = KnownTagAlcina.Status_Ok;
            LocalDateTime now = LocalDateTime.now();
            LocalDateTime lastOkLt = LocalDateTime.ofInstant(lastOkDate.toInstant(), ZoneId.systemDefault());
