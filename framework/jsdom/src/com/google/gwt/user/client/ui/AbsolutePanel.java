...
public class AbsolutePanel extends ComplexPanel
	 ...
	 * substitute another element. The element is presumed to be a &lt;div&gt;.
	 * 
	 * @param elem
	 ...
	 * attached to the document BODY. Note that the current use of this method
	 * is not comprehensive, since we can only verify the offsetParent if both
	 * parent (AbsolutePanel) and child widget are both visible and attached to
	 * the DOM when this test is executed.
	 *
	 * @param child
	 *            the widget whose position and placement should be tested
	 */
	private void verifyPositionNotStatic(Widget child) {
		 ...
		 * then no explicit position:relative is needed as children are already
		 ...
		 * mode, the offsetParent, for elements whose parent is the document
		 * BODY, is the HTML element, not the BODY element.
		 */
		if ("body"
				.equals(getElement().getNodeName().toLowerCase(Locale.ROOT))) {
			return;
		}
		/*
		 * Warn the developer, but allow the execution to continue in case
		 * legacy apps depend on broken CSS.
		 */
