...
public final class JsValueGlue {
	...
	public static Object createJavaScriptObject(JsValue value,
		...
		Throwable caught;
		try {
			// See if there's already a wrapper object (assures identity
			// comparison).
			Object jso = classLoader
					.getCachedJso(value.getJavaScriptObjectPointer());
			if (jso != null) {
				return jso;
			}
	 ...
	 *             if the JavaScript object is not assignable to the supplied
	 *             type.
	 */
	...
	public static <T> T get(JsValue value, CompilingClassLoader cl,
			...
			if (type == Boolean.TYPE) {
				if (!value.isBoolean()) {
					throw new HostedModeException(msgPrefix
							+ ": JS value of type " + value.getTypeString()
							+ ", expected boolean");
				}
				return (T) Boolean.valueOf(value.getBoolean());
			} else if (type == Byte.TYPE) {
				return (T) Byte.valueOf((byte) getIntRange(value,
						Byte.MIN_VALUE, Byte.MAX_VALUE, "byte", msgPrefix));
			} else if (type == Character.TYPE) {
				return (T) Character
						.valueOf((char) getIntRange(value, Character.MIN_VALUE,
				...
				// actual end point value, in case it is being used as a
				// sentinel
				// value. This test works by the subtraction result rounding off
				// to
				// zero if the delta is not representable in a float.
				// TODO(jat): add similar test for MAX_VALUE if we have a JS
				// platform that alters the value while converting to/from
				// strings.
				...
				if (Float.isInfinite(floatVal)
						&& !Double.isInfinite(doubleVal)) {
					// in this case we had overflow from the double value which
					// was
					// outside the range of supported float values, and the cast
					// converted it to infinity. Since this lost data, we treat
					// this
					// as an error in hosted mode.
					throw new HostedModeException(msgPrefix + ": JS value "
							+ doubleVal + " out of range for a float");
				}
				...
				return (T) Short.valueOf((short) getIntRange(value,
						Short.MIN_VALUE, Short.MAX_VALUE, "short", msgPrefix));
			}
		}
		...
		// handle (NR) js-side auto-unboxing
		if (value.isBoolean()) {
			return (T) Boolean.valueOf(value.getBoolean());
		}
		if (value.isNumber()) {
		 ...
		 * catching a RuntimeException in user test
	 ...
	 */
	public static void set(JsValue value, CompilingClassLoader cl,
		...
		if (type.isPrimitive()) {
			if (type == Boolean.TYPE) {
				...
				throw new HostedModeException(
						"Cannot marshal primitive type " + type);
			}
		} else if (obj == null) {
			value.setNull();
		} else {
			...
			try {
				...
				// Ignore the exception, if we can't find the class then
				// obviously we
