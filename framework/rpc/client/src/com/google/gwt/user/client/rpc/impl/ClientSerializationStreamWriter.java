...
public final class ClientSerializationStreamWriter
	...
	private static String quotingRegexString;

	public static String quoteString(String str) {
		if (GWT.isScript()) {
			return quoteStringScript(str);
		} else {
	...
	public static String quoteStringHosted(String str) {
		if (quotingRegexString == null) {
			String pattern = getQuotingRegex().toString();
			// - /regex/g => regex
			quotingRegexString = pattern.substring(1, pattern.length() - 2);
		}
		RegExp regExp = RegExp.compile(quotingRegexString, "g");
		int idx = 0;
	 ...
	 */
	public static native String quoteStringScript(String str) /*-{
    //var regex = @com.google.gwt.user.client.rpc.impl.ClientSerializationStreamWriter::getQuotingRegex;
    //nick - not that big an impact (rpc not that frequent), and doesn't play nice currently with jscodeserver
    var regex = @com.google.gwt.user.client.rpc.impl.ClientSerializationStreamWriter::getQuotingRegex()
        ();
    var idx = 0;
    var out = "";
