...
public final class ServerSerializationStreamReader
	...
	/**
	 * A basic sanity check for strong names. Only allow certain characters to
	 * protect serialization policy providers that use it blindly to load files.
	 * (This is normally a hex string, but we allow a few more safe characters
	 * in case it's useful for testing.)
	 * 
	 * @see com.google.gwt.dev.util.Util#computeStrongName
	 */
	private static final Pattern ALLOWED_STRONG_NAME = Pattern
			.compile("[a-zA-Z0-9_]+");
	...
	private enum VectorReader {
		BOOLEAN_VECTOR {
			@Override
			protected Object
					readSingleValue(ServerSerializationStreamReader stream)
							throws SerializationException {
				return stream.readBoolean();
			}
		...
		OBJECT_VECTOR {
			...
			protected Object readSingleValue(
					ServerSerializationStreamReader stream, Type expectedType,
					DequeMap<TypeVariable<?>, Type> resolvedTypes)
					throws SerializationException {
				return stream.readObject(expectedType, resolvedTypes);
			}
		...
		Object read(ServerSerializationStreamReader stream,
				BoundedList<Object> instance, Type expectedType,
				DequeMap<TypeVariable<?>, Type> resolvedTypes)
				throws SerializationException {
			for (int i = 0, n = instance.getExpectedSize(); i < n; ++i) {
				instance.add(
						readSingleValue(stream, expectedType, resolvedTypes));
			}
			return toArray(instance.getComponentType(), instance);
		}
	}
	...
	public Object deserializeValue(Class<?> rpcType)
			throws SerializationException {
	...
	public Object deserializeValue(Class<?> rpcType, Type methodType,
		...
		if (valueReader != null) {
			...
			// Arrays of primitive or reference types need to go through
			// readObject.
			return ValueReader.OBJECT.readValue(this, methodType,
					resolvedTypes);
		}
	}
	...
	public void prepareToRead(String encodedTokens)
		...
		stringTable = null;
		int idx = 0, nextIdx;
		while (-1 != (nextIdx = encodedTokens.indexOf(RPC_SEPARATOR_CHAR,
				idx))) {
		...
		if (!areFlagsValid()) {
			throw new IncompatibleRemoteServiceException(
					"Got an unknown flag from " + "client: " + getFlags());
		}
	...
	public byte readByte() throws SerializationException {
		...
		try {
			...
			throw getNumberFormatException(value, "byte", Byte.MIN_VALUE,
					Byte.MAX_VALUE);
		}
	}
	...
	public int readInt() throws SerializationException {
		...
		try {
			...
			throw getNumberFormatException(value, "int", Integer.MIN_VALUE,
					Integer.MAX_VALUE);
		}
	}
	...
	public Object readObject(Type expectedType,
			DequeMap<TypeVariable<?>, Type> resolvedTypes)
			throws SerializationException {
		int token = readInt();
		if (token < 0) {
		...
		if (isSimpleClass(expectedType) && !resolvedTypes.isEmpty()) {
			// Start a new scope for resolving type variables. This is a
			// workaround for false sharing
			// because the type checker doesn't implement type variables
			// properly.
			//
			// For example, if expectedType is Serializable and the actual value
			// is a LinkedHashMap, we
			// don't want any of LinkedHashMap, HashMap, or Map to inherit any
			// bindings for their K,V
	...
	public short readShort() throws SerializationException {
		...
		try {
			...
			throw getNumberFormatException(value, "short", Short.MIN_VALUE,
					Short.MAX_VALUE);
		}
	}
	...
	protected Object deserialize(String typeSignature, Type expectedType,
		...
		try {
			...
			if (resolvedTypes == null) {
				// We can find ourselves with a null resolvedTypes map if a
				// class
				// has a non-type-checking serializer that tries to deserialize
				// a field.
				// In such cases there is field type information from the class,
				// but no
				// resolvedTypes because we did not pass it through the non-type
				// checking serializer. Create a map, so that from this point
				// forward
				// we have some way of type checking.
				resolvedTypes = new DequeMap<TypeVariable<?>, Type>();
			}
			// Try to determine the type for the generic type parameters of the
			// instance class, based upon the expected type and any class
			// hierarchy
			// type information.
			// In looking for the expected parameter types, we also find out the
			// way in which the instance meets the expected type, and hence can
			// find
			// out when it does not meet expectations.
			TypeVariable<?>[] instanceParameterTypes = instanceClass
					.getTypeParameters();
			Type[] expectedParameterTypes = null;
			if (expectedType != null) {
				SerializabilityUtil.resolveTypes(expectedType, resolvedTypes);
				expectedParameterTypes = SerializabilityUtil
						.findExpectedParameterTypes(instanceClass, expectedType,
								resolvedTypes);
				if (expectedParameterTypes == null) {
					throw new SerializedTypeViolationException(
							"Attempt to deserialize an object of type "
									+ instanceClass.toString()
									+ " when an object of type "
									+ SerializabilityUtil.findActualType(
											expectedType, resolvedTypes)
											.toString()
									+ " is expected");
				}
				// Add mappings from the instance type variables to the resolved
				// types
				// map.
				for (int i = 0; i < instanceParameterTypes.length; ++i) {
					resolvedTypes.add(instanceParameterTypes[i],
			...
			Class<?> customSerializer = SerializabilityUtil
					.hasServerCustomFieldSerializer(instanceClass);
			...
			Object replacement = deserializeImpl(customSerializer,
					instanceClass, instance, expectedType,
					expectedParameterTypes, resolvedTypes);
	 ...
	 * 
	 * @param instanceClass
	 *            the class we are deserializing
	 * @param instance
	 ...
	 * 
	 * Will default to deserializing as an Object vector if the instance is not
	 * a primitive vector.
	 * 
	 * @param instanceClass
	 *            the class we are deserializing
	 ...
	 *            the object to deserialize into
	 * @param expectedType
	 *            the parameterized type the system is expected to find
	 * @throws SerializationException
	 */
	@SuppressWarnings("unchecked")
	private Object deserializeArray(Class<?> instanceClass, Object instance,
		...
		VectorReader instanceReader = CLASS_TO_VECTOR_READER.get(instanceClass);
		if (instanceReader != null) {
			return instanceReader.read(this, buffer, expectedType,
					resolvedTypes);
		} else {
			return VectorReader.OBJECT_VECTOR.read(this, buffer, expectedType,
					resolvedTypes);
		}
	}

	private void deserializeClass(Class<?> instanceClass, Object instance,
			Type expectedType, Type[] expectedParameterTypes,
			DequeMap<TypeVariable<?>, Type> resolvedTypes)
		...
		for (Field declField : serializableFields) {
			 ...
			 */
			if ((setters != null)
					&& ((setter = setters.get(fieldName)) != null)) {
		...
		if (serializationPolicy.shouldDeserializeFields(superClass)) {
			Type[] superParameterTypes = SerializabilityUtil
					.findExpectedParameterTypes(superClass, superClass,
							resolvedTypes);
			deserializeImpl(
					SerializabilityUtil.hasServerCustomFieldSerializer(
							superClass),
					superClass, instance, expectedType, superParameterTypes,
					resolvedTypes);
		}
	}

	private Object deserializeImpl(Class<?> customSerializer,
			Class<?> instanceClass, Object instance, Type expectedType,
		...
		if (customSerializer != null) {
			...
			if (customFieldSerializer == null) {
				deserializeWithCustomFieldDeserializer(customSerializer,
						instanceClass, instance, expectedParameterTypes,
						resolvedTypes);
				...
				serverCFS.deserializeInstance(this, instance,
						expectedParameterTypes, resolvedTypes);
			} else {
			...
			Type actualExpectedType = SerializabilityUtil
					.findActualType(expectedType, resolvedTypes);
			if (actualExpectedType instanceof GenericArrayType) {
				Type arrayType = ((GenericArrayType) actualExpectedType)
						.getGenericComponentType();
				return deserializeArray(instanceClass, instance, arrayType,
						resolvedTypes);
			} else if (((Class<?>) actualExpectedType)
					.getComponentType() != null) {
				Class<?> arrayType = ((Class<?>) actualExpectedType)
						.getComponentType();
				return deserializeArray(instanceClass, instance, arrayType,
						resolvedTypes);
			}
			...
			deserializeClass(instanceClass, instance, expectedType,
					expectedParameterTypes, resolvedTypes);
		}
		return instance;
	}
	...
	private void deserializeStringTable() throws SerializationException {
			...
			if (idx >= 0) {
				...
				while (idx >= 0) {
					...
					case '\\':
						buf.append(ch);
						break;
					// added (Nick) to handle json blobs from client
					case '"':
					case 't':
					case 'r':
					case 'n':
					...
					case 'u':
						try {
							ch = (char) Integer.parseInt(
									str.substring(idx + 1, idx + 5), 16);
						} catch (NumberFormatException e) {
							throw new SerializationException(
									"Invalid Unicode escape sequence in \""
											+ str + "\"");
						}
						buf.append(ch);
						pos += 4;
					...
					default:
						throw new SerializationException(
								"Unexpected escape character " + ch
										+ " after backslash: \"" + str + "\"");
					}
					idx = str.indexOf('\\', pos);
				}
		...
		if (buffer.size() != buffer.getExpectedSize()) {
			throw new SerializationException("Expected "
					+ buffer.getExpectedSize()
		...
		if (expectedParameterTypes != null) {
			for (Method method : customSerializer.getMethods()) {
				if ("deserializeChecked".equals(method.getName())) {
					method.invoke(null, this, instance, expectedParameterTypes,
							resolvedTypes);
					return;
				}
			}
	...
	private String extract() throws SerializationException {
		try {
			...
			throw new SerializationException("Too few tokens in RPC request",
					e);
		}
	}

	/**
	 * Returns a suitable NumberFormatException with an explanatory message when
	 * a numerical value cannot be parsed according to its expected type.
	 * 
	 * @param value
	 *            the value as read from the RPC stream
	 * @param type
	 *            the name of the expected type
	 * @param minValue
	 *            the smallest valid value for the expected type
	 * @param maxValue
	 *            the largest valid value for the expected type
	...
	private NumberFormatException getNumberFormatException(String value,
		...
		try {
			// Check the field contents in order to produce a more
			// comprehensible
			// error message
		...
		return new NumberFormatException("Expected type '" + type
				+ "' but received " + message + ": " + value);
	}
		...
		synchronized (settersByClass) {
			...
			if (setters == null) {
				...
				for (Field field : fields) {
					// Consider non-static, non-transient (or @GwtTransient)
					// fields only
	...
	private Object instantiate(Class<?> customSerializer,
			Class<?> instanceClass, Type[] expectedParameterTypes,
			DequeMap<TypeVariable<?>, Type> resolvedTypes)
		...
		if (customSerializer != null) {
			CustomFieldSerializer<?> customFieldSerializer = SerializabilityUtil
					.loadCustomFieldSerializer(customSerializer);
			if (customFieldSerializer == null) {
				Object result = instantiateWithCustomFieldInstantiator(
						customSerializer, expectedParameterTypes,
						resolvedTypes);
				if (result != null) {
					return result;
				}
				...
				if (expectedParameterTypes != null
					...
					return serverCFS.instantiateInstance(this,
							expectedParameterTypes, resolvedTypes);
				} else {
	...
	private Object instantiateWithCustomFieldInstantiator(
			Class<?> customSerializer, Type[] expectedParameterTypes,
			DequeMap<TypeVariable<?>, Type> resolvedTypes)
		...
		if (expectedParameterTypes != null) {
			for (Method method : customSerializer.getMethods()) {
				if ("instantiateChecked".equals(method.getName())) {
					return method.invoke(null, this, expectedParameterTypes,
							resolvedTypes);
				}
			}
		}
	...
	/**
	 * Returns true if the given type is a top-level class or interface with no
	 * type parameters.
	 */
	...
	private void validateTypeVersions(Class<?> instanceClass,
		...
		if (clientTypeSignature.length() == 0) {
			throw new SerializationException(
					"Missing type signature for " + instanceClass.getName());
		}
		...
		if (!clientTypeSignature.equals(serverTypeSignature)) {
			throw new SerializationException(
					"Invalid type signature for " + instanceClass.getName());
		}
	}
}
