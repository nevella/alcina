...
public class DeltaStore {
    ...
    public void deserializeTranches(final AsyncCallback playerCallback, final List<DomainModelDeltaTransport> transports) {
        for (DomainModelDeltaTransport transport : transports) {
            DomainModelDeltaSignature sig = DomainModelDeltaSignature.parseSignature(transport.getSignature());
            String nvs = sig.nonVersionedSignature();
            DomainModelDeltaSignature versioned = cache.nonVersionedSignatures.get(nvs);
            if (cache.deltaCache.get(nvs) == null && !hasNoSerializedContent(nvs)) {
    ...
    public void getDelta(DomainModelDeltaSignature sig, final AsyncCallback<DomainModelDelta> callback) {
        ...
        final AsyncCallback<DomainModelDelta> assignCallback = new AsyncCallbackStd<DomainModelDelta>() {
            ...
            @Override
            public void onSuccess(DomainModelDelta result) {
                cache.deltaCache.put(result.getSignature().nonVersionedSignature(), result);
        ...
        AsyncCallback<String> valueCallback = new AsyncCallbackStd<String>() {
            ...
            public void onSuccess(String result) {
                try {
                    ...
                    Registry.impl(RpcDeserialiser.class).deserialize(DomainModelDelta.class, result, assignCallback);
    ...
    public void getDeltaForNonVersionSignature(DomainModelDeltaSignature sig, final AsyncCallback<DomainModelDelta> callback) {
        DomainModelDeltaSignature versioned = cache.nonVersionedSignatures.get(sig.nonVersionedSignature());
        if (versioned == null) {
    ...
    public DomainModelDeltaMetadata getDomainObjectsMetadata() {
        ...
        return cache.metadataCache.values().stream().filter(hasDomainObjectsFilter).findFirst().orElse(null);
    }
    ...
    public DomainModelDeltaSignature getExistingVersionedSignature(DomainModelDeltaSignature sig) {
    ...
    public Long getUserId() {
        ...
        DomainModelDeltaSignature sig = CommonUtils.first(cache.metadataCache.keySet());
        return sig == null ? null : (Long) sig.getUserId();
    }

    public boolean hasInstantiatedContentFor(DomainModelDeltaSignature sig) {
        return getExistingVersionedSignature(sig) != null && cache.contentCache.get(getExistingVersionedSignature(sig)) != null;
    }
    ...
    public void mergeResponse(final LoadObjectsResponse response, boolean deserializeTranches, boolean removeUnusedTranches, final AsyncCallback<Void> callback) {
        MergeResponseConsort mergeResponseConsort = new MergeResponseConsort(response, deserializeTranches, removeUnusedTranches, callback);
        new SubconsortSupport().maybeAttach(callback, mergeResponseConsort, false);
    }
    ...
    public void refreshCache(AsyncCallback callback) {
        ...
        if (callback instanceof Player) {
            ...
            new SubconsortSupport().run(player.getConsort(), ensureCacheConsort, player);
        } else {
    ...
    private void persistTranches(List<DomainModelDeltaTransport> deltaTransports, AsyncCallback callback) {
        ...
        for (DomainModelDeltaTransport transport : deltaTransports) {
            if (!transport.provideIsCacheReference()) {
                ...
                cache.deltaCache.put(sig.nonVersionedSignature(), transport.getDelta());
    ...
    void removeUnusedTranches(List<String> preserveClientDeltaSignatures, final AsyncCallback callback) {
        ...
        for (String signature : preserveClientDeltaSignatures) {
            preserveKeys.add(getKey(DomainModelDeltaSignature.parseSignature(signature), true));
            preserveKeys.add(getKey(DomainModelDeltaSignature.parseSignature(signature), false));
        }
                ...
                case LIST_METADATA:
                    ...
                    for (String sigWithMarker : newCache.existingKeys) {
                        if (sigWithMarker.startsWith(META)) {
                            String signatureString = sigWithMarker.substring(META.length());
                ...
                case MERGE:
                    ...
                    try {
                        for (Entry<String, String> e : mdKvs.entrySet()) {
                            DomainModelDeltaSignature signature = DomainModelDeltaSignature.parseSignature(e.getKey().substring(META.length()));
                            newCache.metadataCache.put(signature, (DomainModelDeltaMetadata) Registry.impl(AlcinaBeanSerializer.class).deserialize(e.getValue()));
                        }
                        cache = newCache;
                        wasPlayed(player);
                    } catch (Exception e) {
                        Registry.impl(ClientNotifications.class).log("Problem deserialising delta store - " + e.getMessage());
    ...
    class MergeResponseConsort extends AllStatesConsort<MergeResponsePhase> {
        ...
        public MergeResponseConsort(LoadObjectsResponse response, boolean deserializeTranches, boolean removeUnusedTranches, AsyncCallback<Void> callback) {
        ...
        public void runPlayer(AllStatesPlayer player, MergeResponsePhase next) {
            switch(next) {
                case ENSURE_CACHE:
                    new SubconsortSupport().maybeAttach(player, new EnsureCacheConsort(player), false);
                    break;
                case REMOVE_UNUSED:
                    if (removeUnusedTranches) {
                        removeUnusedTranches(response.getPreserveClientDeltaSignatures(), player);
                    } else {
    ...
    enum MergeResponsePhase {
        ...
        ENSURE_CACHE, REMOVE_UNUSED, PERSIST_TRANCHES, DESERIALIZE_TRANCHES, RELOAD_CACHE
    }
}
