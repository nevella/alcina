...
public class DirectedActivity<P extends BasePlace> extends Model implements Activity {
    ...
    public static Activity forPlace(Place place) {
        ...
        Optional<Provider> provider = Registry.optional(DirectedActivity.Provider.class, place.getClass());
        if (provider.isPresent()) {
            ...
            if (place instanceof EntityPlace) {
                ...
                if (action instanceof PlaceAction) {
                    AppController.get().goToPlaceReplaceCurrent(((PlaceAction) action).getTargetPlace());
