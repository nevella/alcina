...
public class AppController {
    ...
    public void doCreate(Class<? extends Entity> clazz) {
        EntityPlace target = (EntityPlace) RegistryHistoryMapper.get().getPlaceByModelClass(clazz);
    ...
    public void doEdit(VersionableEntity object) {
        ...
        EntityPlace target = (EntityPlace) RegistryHistoryMapper.get().getPlaceByModelClass(object.getClass());
    ...
    public void doSearch(EntitySearchDefinition def) {
        ...
        EntityPlace target = (EntityPlace) RegistryHistoryMapper.get().getPlaceByModelClass(def.queriedEntityClass());
        target.action = EntityAction.VIEW;
        target.def = def;
        Client.goTo(target);
    }
    ...
    public void export(DomainStoreDataProvider dataProvider, GroupingParameters exportDefinition, RowExportContentDefinition exportContentDefinition, Set<Long> selectedIds) {
    ...
    public void goToPlace(Place place, boolean fireHistoryEvent) {
        try {
            LooseContext.pushWithBoolean(PlaceHistoryHandler.CONTEXT_IGNORE_NEXT_TOKEN, !fireHistoryEvent);
            Client.goTo(place);
        } finally {
            LooseContext.pop();
        }
    ...
    public void toggleCurrentPlaceEditing(boolean edit) {
        ...
        ((HasEntityAction) copy).setAction(edit ? EntityAction.EDIT : EntityAction.VIEW);
        Client.goTo(copy);
    }

    private void afterDeleteSuccess(VersionableEntity object) {
        MessageManager.get().showMessage(Ax.format("Deleted %s %s", object.getClass().getSimpleName(), object.getId()));
    ...
    protected void flushPostDelete(VersionableEntity object) {
        CommitToStorageTransformListener.flushAndRun(() -> afterDeleteSuccess(object));
    }

    protected void maybeSetId(BindablePlace place, Set<? extends VersionableEntity> collection) {
