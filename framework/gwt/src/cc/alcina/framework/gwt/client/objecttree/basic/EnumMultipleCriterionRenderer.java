...
@Registration({ TreeRenderer.class, EnumMultipleCriterion.class })
public class EnumMultipleCriterionRenderer<T extends EnumMultipleCriterion> extends SearchCriterionRenderer<T> {
    ...
    public BoundWidgetProvider renderCustomiser() {
        ListBoxEnumProvider provider = new ListBoxEnumProvider(getRenderable().enumClass()) {

            @Override
            public SetBasedListBox get() {
                SetBasedListBox widget = super.get();
                Binding binding = BindingBuilder.bind(widget).onLeftProperty("value").toRight(getRenderable()).onRightProperty(renderablePropertyName()).toBinding();
