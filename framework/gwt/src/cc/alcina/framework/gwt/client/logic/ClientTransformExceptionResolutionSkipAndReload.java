...
public class ClientTransformExceptionResolutionSkipAndReload implements ClientTransformExceptionResolver, TopicListener<CommitToStorageTransformListener.State>, PermissibleActionListener {
    ...
    @Override
    public void resolve(DomainTransformRequestException dtre, Callback<ClientTransformExceptionResolutionToken> callback) {
        ...
        fp = new FlowPanel() {
            ...
            protected void onAttach() {
                super.onAttach();
                storage.topicStateChanged().add(ClientTransformExceptionResolutionSkipAndReload.this);
            }

            @Override
            protected void onDetach() {
                storage.topicStateChanged().remove(ClientTransformExceptionResolutionSkipAndReload.this);
                super.onDetach();
            }
        };
        this.callback = callback;
        boolean reloadRequired = false;
        List<DomainTransformException> exceptions = dtre.getDomainTransformResponse().getTransformExceptions();
        ...
        for (DomainTransformException dte : exceptions) {
            ...
            if (view.recommendedAction != RecommendedAction.OK || !dte.isSilent() || !PermissionsManager.get().isAdmin()) {
                adapters.add(view);
            }
        ...
        Field[] fields = GwittirBridge.get().fieldsForReflectedObjectAndSetupWidgetFactory(Reflections.at(DTEView.class).templateInstance(), factory, false, true);
        ...
        NiceWidthBoundTable table = new NiceWidthBoundTable(mask, factory, fields, cp);
        ...
        final String buttonText = irresolvable == null && Registry.impl(CommitToStorageTransformListener.class).isAllowPartialRetryRequests() ? "Retry" : "Reload application";
        dialog = new OkCancelDialogBox("Problems saving changes", fp, this) {
            ...
            protected void onOkButtonClicked() {
                ...
                if (vetoableActionListener != null) {
                    vetoableActionListener.vetoableAction(new PermissibleActionEvent(this, OkAction.INSTANCE));
                }
            }
        };
    }
    ...
    public void vetoableAction(PermissibleActionEvent evt) {
        if (status.isIrresolvable() || !Registry.impl(CommitToStorageTransformListener.class).isAllowPartialRetryRequests()) {
            Window.Location.reload();
        } else {
    ...
    public static class DTEView extends Bindable {
        ...
        @Custom(customiserClass = ExpandableLabelCustomiser.class, parameters = { @NamedParameter(name = ExpandableLabelCustomiser.FORCE_COLUMN_WIDTH, booleanValue = true), @NamedParameter(name = ExpandableLabelCustomiser.MAX_WIDTH, intValue = 30) })
    ...
    private class StatusWidget extends Composite {
        ...
        public StatusWidget() {
            ...
            blurb = new Label("Problems occurred saving changes. " + "The changes you made will be altered to allow your work to be saved.");
            blurb.setStyleName("pad-15 italic");
            blurb.setVisible(Registry.impl(CommitToStorageTransformListener.class).isAllowPartialRetryRequests());
            fp.add(blurb);
            fp.add(html);
            initWidget(fp);
