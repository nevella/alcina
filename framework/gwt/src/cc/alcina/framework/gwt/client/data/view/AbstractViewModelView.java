...
public abstract class AbstractViewModelView<VM extends ViewModel>
    ...
    protected <C extends DataDomainBase> void asyncSelect(Class<C> clazz,
    ...
    protected ContentViewSections getBuilder(HasDataAction place) {
        ContentViewSections builder = new ContentViewSections();
        DataAction action = place.getAction();
        boolean editable = action != null && action.isEditable();
        if (editable) {
            builder.editable();
        }
        boolean create = action == DataAction.CREATE;
    ...
    protected boolean isEditing() {
        return model != null && model.getAction() == DataAction.EDIT;
    }
