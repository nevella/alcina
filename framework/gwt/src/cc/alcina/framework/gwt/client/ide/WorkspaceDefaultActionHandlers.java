...
public class WorkspaceDefaultActionHandlers {
    ...
    public static abstract class BaseViewActionHandler extends WorkspaceDefaultActionHandlerBase {
        ...
        public void performAction(PermissibleActionEvent event, Object node, Object object, Workspace workspace, Class nodeObjectClass) {
            Widget view = null;
            if (object instanceof Collection) {
                view = workspace.createMultipleBeanView((Collection) object, nodeObjectClass, editView(), workspace, isAutoSave(), false);
            } else {
                PaneWrapperWithObjects paneWrapper = getContentViewFactory().createBeanView(object, editView(), workspace, isAutoSave(), false);
                view = paneWrapper;
                Widget widge = getContentViewFactory().createExtraActionsWidget(object);
                if (widge != null) {
    ...
    public static class DefaultCloneActionHandler extends WorkspaceDefaultActionHandlerBase implements CloneActionHandler {

        @Override
        public void performAction(final PermissibleActionEvent event, Object node, Object object, final Workspace workspace, Class nodeObjectClass) {
            ...
            try {
                ...
                if (isAutoSave()) {
                    ClientTransformManager.cast().promoteToDomainObject(cloner.getProvisionalObjects());
                ...
                provisionals.addAll(ClientTransformManager.cast().prepareObject(newObj, isAutoSave(), true, false));
                TextProvider.get().setDecorated(true);
                String newName = TextProvider.get().getObjectName(newObj) + " (copy)";
            ...
            Widget view = getContentViewFactory().editable(true).actionListener(workspace).autoSave(isAutoSave()).doNotClone(false).additionalProvisional(provisionals).doNotPrepare(true).createBeanView(newObj);
            workspace.getVisualiser().setContentWidget(view);
            workspace.fireVetoableActionEvent(new PermissibleActionEvent(newObj, event.getAction()));
            return;
        }
    }
    ...
    public static class DefaultCreateActionHandler extends WorkspaceDefaultActionHandlerBase implements CreateActionHandler {
        ...
        public void performAction(PermissibleActionEvent event, Object node, Object object, Workspace workspace, Class nodeObjectClass) {
            try {
                CollectionModificationSupport.queue(true);
                newObj = isAutoSave() ? TransformManager.get().createDomainObject(nodeObjectClass) : TransformManager.get().createProvisionalObject(nodeObjectClass);
                handleParentLinks(workspace, node, newObj);
            ...
            workspace.getVisualiser().selectNodeForObject(newObj, true);
            PaneWrapperWithObjects view = getContentViewFactory().createBeanView(newObj, true, workspace, isAutoSave(), false);
            workspace.getVisualiser().setContentWidget(view);
            workspace.fireVetoableActionEvent(new PermissibleActionEvent(newObj, event.getAction()));
        }
    }
    ...
    public static class DefaultDeleteActionHandler extends WorkspaceDefaultActionHandlerBase implements DeleteActionHandler {

        @Override
        public void performAction(final PermissibleActionEvent event, Object node, Object object, final Workspace workspace, Class nodeObjectClass) {
            final Entity entity = (Entity) object;
            Registry.impl(ClientNotifications.class).confirm("Are you sure you want to delete the selected object", new DoDeleteCallback(event, workspace, entity));
        }
        ...
        private class DoDeleteCallback implements OkCallback {
            ...
            private DoDeleteCallback(PermissibleActionEvent event, Workspace workspace, Entity entity) {
                this.event = event;
            ...
            public void ok() {
                AsyncCallback deleteObjectCallback = new AsyncCallback<Void>() {
                    ...
                    private void finish() {
                        entity.delete();
                        if (workspace != null) {
                            workspace.getVisualiser().setContentWidget(new HorizontalPanel());
                            workspace.fireVetoableActionEvent(new PermissibleActionEvent(entity, event.getAction()));
                        }
                    }
                };
    ...
    public static class DefaultEditActionHandler extends BaseViewActionHandler implements EditActionHandler {
    ...
    public static class DefaultViewActionHandler extends BaseViewActionHandler implements ViewActionHandler {
    ...
    public abstract static class WorkspaceDefaultActionHandlerBase {
        ...
        protected void handleParentLinks(Workspace workspace, Object node, Entity newObj) {
