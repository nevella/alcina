...
/**
 * This BoundWidget renders a Calendar on the screen, and supports a value
 * property of Date.
 * 
 * @author cooper
 */
...
public class Calendar extends AbstractBoundWidget implements Renderers,
	...
	private Date renderDate = new Date(new Date().getYear(), new Date()
			.getMonth(), 1);
	...
	public Calendar() {
		...
		this.grid.addTableListener(new TableListener() {
			public void onCellClicked(SourcesTableEvents sender, int row,
				...
				for (Iterator it = new ArrayList(eventListeners).iterator(); it
						.hasNext();) {
					if (!((CalendarListener) it.next()).onDateClicked(instance,
							currentDates[row - 1][cell])) {
						cancelled = true;
						break;
					}
				}
	...
	public CalendarDrawListener[] getCalendarDrawListeners() {
		return (CalendarDrawListener[]) this.drawEventListeners
				.toArray(new CalendarDrawListener[this.drawEventListeners
						.size()]);
	}
	...
	public CalendarListener[] getCalendarListeners() {
		return (CalendarListener[]) this.eventListeners
				.toArray(new CalendarListener[this.eventListeners.size()]);
	}

	/**
	 * Replaces the Label containing the date in the Calendar's table with the
	 * widget provided.
	 * 
	 * @param date
	 *            The date to overwrite
	 * @param widget
	 *            The widget to place in the table.
	 * @return boolean indicating if the date is currently drawn, and the
	 *         operation succeeded.
	 */
	...
	/**
	 * Sets a date value that represents the month and year the calendar will be
	 * rendering.
	 * 
	 * @param renderDate
	 *            Date to render around.
	 */
	...
	/**
	 * Returns a date value that represents the month and year the calendar will
	 * be rendering.
	 * 
	...
	/**
	 * Sets the date value of the calendar. Will reset the render date to this
	 * value as well.
	 * 
	 * @param value
	 *            Date balue of the Calendar
	 */
	public void setValue(Object value) {
		Date old = this.value;
		this.value = (Date) value;
		this.setRenderDate(new Date(this.value.getYear(),
	 ...
	 */
	public void render() {
		this.renderDate = new Date(getRenderDate().getYear(), getRenderDate()
				.getMonth(), getRenderDate().getDate());
		// int month = getRenderDate().getMonth();
		Date tempDate = new Date(getRenderDate().getYear(), getRenderDate()
				.getMonth(), 1);
		...
		for (int row = 0; row < 6; row++) {
			for (int col = 0; col < 7; col++) {
				...
				if (tempDate.getMonth() == this.getRenderDate().getMonth()) {
					this.grid.setWidget(row + 1, col, new Label(Integer
							.toString(tempDate.getDate())));
					this.grid.getCellFormatter().setStyleName(row + 1, col,
							"date");
					for (Iterator it = this.drawEventListeners.iterator(); it
							.hasNext();) {
						String altStyle = ((CalendarDrawListener) it.next())
								.onCalendarDrawEvent(new CalendarDrawEvent(
										this, tempDate));
						if (altStyle != null) {
							this.grid.getCellFormatter().addStyleName(row + 1,
									col, altStyle);
						}
					}
				} else {
				...
				tempDate = new Date(tempDate.getYear(), tempDate.getMonth(),
						tempDate.getDate() + 1);
			}
		}
	}
