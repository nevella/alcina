...
/**
 * This is an abstract class for creating Adapters for @Introspectable types
 * that do not source PropertyChangeEvents.
 * 
 * @author <a href="mailto:kebernet@gmail.com">Robert "kebernet" Cooper</a>
 */
public abstract class BindableAdapter
		implements SourcesPropertyChangeEvents, SelfDescribed {
			...
			for (int i = 0; i < this.properties.length; i++) {
				wrappedProps[i] = new Property(this.properties[i].getName(),
						...
						(this.properties[i].getAccessorMethod() != null)
								? new MethodWrapper(
										this.properties[i].getAccessorMethod())
								: null,
	...
	public void update() {
		for (Property p : properties) {
			...
			try {
				...
				Object cur = p.getAccessorMethod().invoke(this.getWatched(),
						null);
	...
	protected void initValues() {
		try {
			for (Property p : this.properties) {
				if (p.getAccessorMethod() != null) {
					values.put(p.getName(), p.getAccessorMethod()
							.invoke(this.getWatched(), null));
				}
			}
		} catch (Exception e) {
	...
	protected static class MethodWrapper implements Method {
		...
		public Object invoke(Object target, Object[] args) throws Exception {
			return internal.invoke(((BindableAdapter) target).getWatched(),
					args);
		}
	}
}
