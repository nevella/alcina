...
/**
 * A FlowContext is a registry of BoundWidgets or BoundWidgetProviders and
 * Actions to activity names. Each .add() method returns this, so you can use a
 * builder pattern: FlowContext context = new FlowContext() .add() .add()
 * .add();
 *
...
public class FlowContext {
	 ...
	 * 
	 * @param name
	 *            The activity name to register
	 * @param widget
	 *            The BoundWidget to use
	 ...
	 *            The BoundWidget to use
	 * @param action
	 *            The Action to register with the BoundWidget
	 * @return this (builder pattern)
	 */
	...
	/**
	 * Adds a BoundWidgetProvider for a given activity name. This method should
	 * be used where the BoundWidget needs to be recreated with each activity
	 * call (not a singleton). Optionally you can use a Singleton provider to
	 * delay initialization of the BoundWidget until it is needed.
	 * 
	 * @param name
	 *            The activity name to register
	 * @param provider
	 *            The BoundWidgetProvider to use
	 * @return this (builder pattern)
	 */
	...
	public FlowEventListener[] getFlowEventListeners() {
		FlowEventListener[] listeners = new FlowEventListener[this.listeners
				.size()];
	...
	FlowEvent createEvent(Widget managedWidget, String toName,
			BoundWidget<?> toWidget, BoundWidget<?> fromWidget) {
		FlowEvent e = new FlowEvent(this, managedWidget, fromWidget,
				fromWidget == null ? null : fromWidget.getModel(),
				this.fromName, toWidget,
				toWidget == null ? null : toWidget.getModel(), toName);
	...
	private static class SimpleProvider<T extends BoundWidget<?>>
