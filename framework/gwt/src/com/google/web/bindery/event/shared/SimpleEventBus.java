...
public class SimpleEventBus extends EventBus {
  ...
  /*
   * Added - NR - to allow one-off setting of handlers (dirndl nodeevent dispatch)
   */
  public void fireEventFromSource(Event<?> event, Object source,  List handlers) {
    if (source == null) {
      throw new NullPointerException("Cannot fire from a null source");
    }
  ...
  private <H> void doFire(Event<H> event, Object source) {
		doFire(event, source, null);
  }
  
  /*
   * Added - NR - to allow one-off setting of handlers (dirndl nodeevent dispatch)
   */
  private <H> void doFire(Event<H> event, Object source, List<H> handlers ) {
    if (event == null) {
    ...
    try {
      ...
      if (source != null) {
		...
		handlers = handlers != null ? handlers
				: getDispatchList(event.getAssociatedType(), source);
