...
class DomStylesBuilder implements StylesBuilder {
	 ...
	 * example, in the phrase background-url, the pattern matches "background"
	 * and "-url".
	 */
	...
	static String toCamelCaseForm(String name) {
		...
		if (hyphenatedMap == null) {
			...
			maybeHyphenatedWord = RegExp.compile("([-]?)([a-z])([a-z0-9]*)",
					"g");
		}
		...
		if (camelCase == null) {
			/*
			 * Strip of any leading hyphens, which are used in browser specified
			 * style properties such as "-webkit-border-radius". In this case,
			 * the first word "webkit" should remain lowercase.
			 */
	 ...
	 * 
	 * @param name
	 *            the user specified style name
	 * @return the camelCase name, or null if not set
	 */
	private static native String getCamelCaseName(JavaScriptObject map,
			String name) /*-{
	 ...
	 *            the camelCase name
	 */
	private static native void putCamelCaseName(JavaScriptObject map,
			String name, String camelCase) /*-{
    map[name] = camelCase;
  }-*/;
	...
	public StylesBuilder opacity(double value) {
		delegate.assertCanAddStyleProperty().setOpacity(String.valueOf(value));
		return this;
	}
	...
	@Override
	public StylesBuilder trustedBackgroundImage(@IsSafeUri
	String value) {
