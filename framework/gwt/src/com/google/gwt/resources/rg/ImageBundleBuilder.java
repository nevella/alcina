...
class ImageBundleBuilder {
	...
	static class BestFitArranger implements Arranger {
			...
			for (int i = 1, n = rectsOrderedByHeight.size(); i < n; i++) {
				...
				for (int j = i; j < n; j++) {
					...
					if (!current.hasBeenPositioned()
						...
						// Set the horizontal position here, the top field will
						// be set in
						...
						// Keep the ImageRects in this column in decreasing
						// order by width.
						final int pos = Collections.binarySearch(rectsInColumn,
								current, decreasingWidthComparator);
						assert pos < 0;
				...
				// vertical,
				// now we'll scan the remaining ImageRects to try to fit some in
				// the
				// horizontal gaps.
				if (!rectsInColumn.isEmpty()) {
					arrangeColumn(rectsInColumn, rectsOrderedByWidth);
				}
				// We're done with that column, so move the horizontal
				// accumulator by
				// the
		 ...
		 * Decreasing Width. The basic strategy is to sort the remaining
		 * rectangles by decreasing width and try to fit them to the left of
		 * each of the rectangles we've already picked for this column.
		 *
		 * @param rectsInColumn
		 *            the ImageRects that were already selected for this column
		 * @param remainingRectsOrderedByWidth
		 *            the sub list of ImageRects that may not have been
		 *            positioned yet
		 */
		private void arrangeColumn(List<ImageRect> rectsInColumn,
			...
			for (int i = 1, m = rectsInColumn.size(); i < m; i++) {
				...
				// The ImageRect was previously positioned horizontally, now set
				// the top
				// field.
				...
				for (int j = 0,
						n = remainingRectsOrderedByWidth.size(); j < n; j++) {
					final ImageRect current = remainingRectsOrderedByWidth
							.get(j);
					if (!current.hasBeenPositioned()
				...
				// Update the vertical accumulator so we'll know where to place
				// the next
				// ImageRect.
				curY += r.getHeight();
			}
	...
	static class ImageRect {
		...
		public void setHeight(int height) {
			...
			if (width <= 0) {
				width = (int) Math.round(
						(double) height / intrinsicHeight * intrinsicWidth);
			}
		}
		...
		public void setWidth(int width) {
			...
			if (height <= 0) {
				height = (int) Math.round(
						(double) width / intrinsicWidth * intrinsicHeight);
			}
		}
		...
		public AffineTransform transform() {
			...
			if (height > 0) {
				toReturn.scale((double) height / intrinsicHeight,
						(double) width / intrinsicWidth);
			}
		...
		private boolean checkTransform(AffineTransform tx) {
			...
			assert out[3] >= 0;
			// Check scaling
			if (!Boolean.getBoolean("ImageBundleBuilder.ignoreChecks")) {
				// NR -- issue with headless hosted mode, ignore -- FIXME - gwt
				assert getWidth() == Math
						.round(Math.abs(out[0] - out[2])) : "Width "
								+ getWidth() + " != "
								+ Math.round(Math.abs(out[0] - out[2]));
				assert getHeight() == Math.round(
						Math.abs(out[1] - out[3])) : "Height " + getHeight()
								+ "!=" + Math.round(Math.abs(out[1] - out[3]));
			}
			return true;
		}
	}
	...
	/**
	 * Performs a simple vertical arrangement of rectangles. Images will be
	 * tiled horizontally to fill the full width of the image.
	 */
	...
	private static final int IMAGE_MAX_SIZE = Integer
			.getInteger("gwt.imageResource.maxBundleSize", 256);
	...
	public static void main(String[] args) {
		...
		if (args.length < 2) {
			logger.log(TreeLogger.ERROR,
					ImageBundleBuilder.class.getSimpleName()
		...
		for (int i = 1, j = args.length; i < j; i++) {
			...
			if (ex != null) {
				loopLogger.log(TreeLogger.ERROR, "Unable to assimilate image",
						ex);
				fail = true;
			}
		}
		...
		try {
			BufferedImage bundledImage = builder
					.drawBundledImage(new BestFitArranger());
	...
	private static byte[] createImageBytes(TreeLogger logger,
		...
		try {
			...
			boolean writerAvailable = ImageIO.write(bundledImage,
					BUNDLE_FILE_TYPE, byteOutputStream);
			if (!writerAvailable) {
				logger.log(TreeLogger.ERROR,
						"No " + BUNDLE_FILE_TYPE + " writer available");
	...
	public ImageBundleBuilder(ImageBundleBuilder other) {
		for (Map.Entry<String, ImageRect> entry : other.imageNameToImageRectMap
				.entrySet()) {
			imageNameToImageRectMap.put(entry.getKey(),
					new ImageRect(entry.getValue()));
		}
	}
	 ...
	 * @param resource
	 *            the URL from which the image data wil be loaded
	 * @throws UnableToCompleteException
	 *             if the image with name <code>imageName</code> cannot be added
	 *             to the master composite image
	 */
	public ImageRect assimilate(TreeLogger logger, String imageName,
			URL resource)
	...
	private ImageRect addImage(TreeLogger logger, String imageName,
		...
		try (InputStream is = imageUrl.openStream();
			 ...
			 * particulars of the implementation, so just choose the first
			 * ImageReader.
			 */
				...
				if (numImages == 0) {
					...
					try {
						...
						if (metadata != null && metadata
							...
							Node data = metadata.getAsTree(
									IIOMetadataFormatImpl.standardMetadataFormatName);
							metadata: for (int i = 0, j = data.getChildNodes()
									.getLength(); i < j; i++) {
								Node child = data.getChildNodes().item(i);
								if (child.getLocalName()
										.equalsIgnoreCase("compression")) {
									for (int k = 0, l = child.getChildNodes()
											.getLength(); k < l; k++) {
										Node child2 = child.getChildNodes()
												.item(k);
										if (child2.getLocalName()
												.equalsIgnoreCase("lossless")) {
											Node value = child2.getAttributes()
													.getNamedItem("value");
											if (value == null) {
												// The default is true,
		...
		if (image == null) {
			logger.log(TreeLogger.ERROR, "Unrecognized image file format",
					null);
			throw new UnableToCompleteException();
		}
	...
	private BufferedImage drawBundledImage(Arranger arranger) {
		 ...
		 * position the ImageRects in a deterministic fashion, even though we
		 * might paint them in a non-deterministic order.
		 */
		...
		SpeedTracerLogger.Event graphicsEvent = SpeedTracerLogger.start(
				CompilerEventType.GRAPHICS_INIT, "java.awt.headless",
				System.getProperty("java.awt.headless"));
		Graphics2D g2d = bundledImage.createGraphics();
		setBetterRenderingQuality(g2d);
