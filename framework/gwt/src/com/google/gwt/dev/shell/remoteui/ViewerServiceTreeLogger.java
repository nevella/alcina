...
public final class ViewerServiceTreeLogger extends AbstractTreeLogger {
	 ...
	 *
	 * @param viewerServiceClient
	 *            An object that can be used to make requests to a viewer
	 *            service server.
	 */
	...
	protected void doCommitBranch(AbstractTreeLogger childBeingCommitted,
		...
		if (isSent()) {
			...
			pending = Lists.add(pending,
					new PendingBranch(child, type, msg, caught, helpInfo));
		}
	}
	...
	protected void doLog(int indexOfLogEntry, Type type, String msg,
		...
		if (caught == null) {
			Ax.out("(GWT.log) - %s", msg);
		} else {
			Ax.err("(GWT.log) - %s", msg);
		...
		if (isSent()) {
			...
			pending = Lists.add(pending, new PendingLog(indexOfLogEntry, type,
					msg, caught, helpInfo));
		}
	}
		...
		viewerServiceClient.addLogBranch(branch.getBranchedIndex(), type, msg,
				caught, helpInfo, logHandle, new Callback<Integer>() {
					...
					public void onError(Throwable t) {
						System.err.println(
								"An error occurred while attempting to add a log branch.");
	...
	private abstract class Pending {
		...
		public Pending(Type type, String msg, Throwable caught,
				HelpInfo helpInfo) {
			this.caught = caught;
			this.msg = msg;
			this.type = type;
	...
	private class PendingBranch extends Pending {
		...
		public PendingBranch(ViewerServiceTreeLogger branch, Type type,
