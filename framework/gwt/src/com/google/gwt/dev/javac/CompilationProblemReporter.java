...
public class CompilationProblemReporter {
	 ...
	 */
	public static void indexErrors(
			CompilationErrorsIndex compilationErrorsIndex,
			List<CompilationUnit> units) {
		for (CompilationUnit unit : units) {
			if (unit.isError()) {
				Dependencies dependencies = unit.getDependencies();
				compilationErrorsIndex.add(unit.getTypeName(),
						unit.getResourceLocation(), dependencies.getApiRefs(),
						CompilationProblemReporter.formatErrors(unit));
			}
		}
	}
	 ...
	 *
	 * @param logger
	 *            logger used to report errors to the console
	 * @param e
	 *            the exception to analyze and log
	 * @return Always returns an instance of {@link UnableToCompleteException}
	 *         so that the calling method can declare a more narrow 'throws
	 *         UnableToCompleteException'
	 */
	public static UnableToCompleteException
			logAndTranslateException(TreeLogger logger, Throwable e) {
		if (e instanceof UnableToCompleteException) {
			...
			List<NodeInfo> nodeTrace = ((InternalCompilerException) e)
					.getNodeTrace();
			for (NodeInfo nodeInfo : nodeTrace) {
				...
				TreeLogger nodeBranch = topBranch.branch(TreeLogger.ERROR, msg,
						null);
	...
	public static int logErrorTrace(TreeLogger logger, Type logLevel,
			CompilerContext compilerContext, List<CompilationUnit> units,
			boolean hint) {
		int errorCount = 0;
		for (CompilationUnit unit : units) {
			if (unit.isError()) {
				logErrorTrace(logger, logLevel, compilerContext,
						unit.getTypeName(), hint);
				errorCount++;
	...
	/**
	 * Provides a meaning error message and hints for a type that failed to
	 * compile or is otherwise missing.
	 */
	public static void logErrorTrace(TreeLogger logger, Type logLevel,
			CompilerContext compilerContext, String typeSourceName,
			boolean hint) {
	...
	public static int logWarnings(TreeLogger logger, Type logLevelForWarnings,
		...
		int warningCount = 0;
		for (CompilationUnit unit : units) {
			if (CompilationProblemReporter.logWarnings(logger,
					logLevelForWarnings, unit)) {
				warningCount++;
			}
		}
		return warningCount++;
	...
	public static boolean reportErrors(TreeLogger logger, CompilationUnit unit,
		...
		for (CategorizedProblem problem : problems) {
			...
			if (branch == null) {
				...
				branch = logger.branch(branchType,
						branchString + " in '" + fileName + "'", null);
			}
			branch.log(logLevel, toMessageWithLineNumber(problem), null,
					helpInfo);
		}
		if (branch != null && branch.isLoggable(TreeLogger.INFO)) {
			if (unit instanceof GeneratedCompilationUnit) {
				...
				CompilationProblemReporter.maybeDumpSource(branch,
						generatedUnit.getSource(), unit.getTypeName());
			}
		}
		return branch != null;
	}
