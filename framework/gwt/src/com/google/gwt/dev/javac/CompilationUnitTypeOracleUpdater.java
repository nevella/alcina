...
public class CompilationUnitTypeOracleUpdater extends TypeOracleUpdater {
	...
	static class TypeData {
		...
		protected TypeData(String packageName, String sourceName,
				String internalName, byte[] byteCode, long lastModifiedTime) {
		 ...
		 * TypeOracle data structures. This is used to make the initial shallow
		 * identity pass for creating JRealClassType/JGenericType objects.
		 */
	...
	private class CompilationUnitTypeOracleResolver implements Resolver {
		...
		public CompilationUnitTypeOracleResolver(
				TypeOracleBuildContext context) {
			this.context = context;
		}

		@Override
		public void addImplementedInterface(JRealClassType type,
				JClassType intf) {
			CompilationUnitTypeOracleUpdater.this.addImplementedInterface(type,
					intf);
		}
		...
		public JRealClassType findByInternalName(String internalName) {
			return CompilationUnitTypeOracleUpdater.this
		...
		public void newParameter(JAbstractMethod method, JType argType,
			...
			CompilationUnitTypeOracleUpdater.this.newParameter(method, argType,
					argName, declaredAnnotations, argNamesAreReal);
		}

		@Override
		public JRealClassType newRealClassType(JPackage pkg,
				String enclosingTypeName, boolean isLocalType,
				String simpleSourceName, boolean isInterface) {
			return CompilationUnitTypeOracleUpdater.this.newRealClassType(pkg,
					enclosingTypeName, simpleSourceName, isInterface);
		}
		...
		public boolean resolveClass(TreeLogger logger, JRealClassType type) {
			return CompilationUnitTypeOracleUpdater.this.resolveClass(logger,
					type, context);
		}

		@Override
		public void setReturnType(JAbstractMethod method, JType returnType) {
			CompilationUnitTypeOracleUpdater.this.setReturnType(method,
					returnType);
		}

		@Override
		public void setSuperClass(JRealClassType type, JClassType superType) {
			CompilationUnitTypeOracleUpdater.this.setSuperClass(type,
					superType);
		}
	}
	...
	private static JTypeParameter[]
			getTypeParametersForClass(CollectClassData classData) {
	...
	private static Class<?> getWrapperClass(Class<?> primitiveClass) {
		...
		if (primitiveClass.equals(Integer.TYPE)) {
			...
			throw new IllegalArgumentException(
					primitiveClass.toString() + " not a primitive class");
		}
	}
	...
	private final Map<String, JRealClassType> typesByInternalName = Maps
			.newHashMap();
	...
	private static ExecutorService executor = new ThreadPoolExecutor(0,
			...
			// Make sure this executor lets the whole process terminate
			// correctly even if there
	...
	private static void prefechTypeData(Collection<TypeData> typeDataList) {
		// Parse bytecode in parallel by calling {@code
		// TypeData.getCollectClassData()} in parallel.
		try {
			...
			// InterruptedException can be safely ignored here as the threads
			// interrupted are only
			// precomputing data in parallel that will otherwise be computed
			// later sequentially if
	...
	private Annotation createAnnotation(TreeLogger logger,
		...
		Map<String, Object> values = Maps
				.newHashMap(annotationData.getValues());
		logger = logger.branch(TreeLogger.TRACE,
				"Resolving annotation for " + annotationClass.getName());
		for (Map.Entry<String, Object> entry : values.entrySet()) {
			Method method = null;
			Throwable caught = null;
			try {
				method = annotationClass.getMethod(entry.getKey());
				entry.setValue(resolveAnnotationValue(logger,
						method.getReturnType(), entry.getValue()));
			...
			if (caught != null) {
				logger.log(TreeLogger.WARN,
						"Exception resolving "
								...
								+ entry.getKey() + " : " + caught.getMessage());
				return null;
			}
		}
	...
	private JRealClassType createType(TypeData typeData,
			CollectClassData collectClassData,
			CollectClassData enclosingClassData) {
		...
		if ((access & Opcodes.ACC_ANNOTATION) != 0) {
			...
			JTypeParameter[] typeParams = getTypeParametersForClass(
					collectClassData);
			if ((typeParams != null && typeParams.length > 0)
					|| nonStaticInsideGeneric(collectClassData,
							enclosingClassData)) {
				type = new JGenericType(typeOracle, pkg, enclosingSimpleName,
						simpleName, isInterface, typeParams);
			} else {
				type = newRealClassType(pkg, enclosingSimpleName, simpleName,
						isInterface);
			}
		}
	...
	private JRealClassType createType(TypeData typeData,
		...
		CollectClassData classData = context.classDataByInternalName
				.get(typeData.internalName);
		String enclosingClassInternalName = classData
				.getEnclosingInternalName();
		CollectClassData enclosingClassData = null;
		if (enclosingClassInternalName != null) {
			enclosingClassData = context.classDataByInternalName
					.get(enclosingClassInternalName);
			if (enclosingClassData == null) {
				// if our enclosing class was skipped, skip this one too
				return null;
			}
		}
		JRealClassType realClassType = createType(typeData, classData,
				enclosingClassData);
	...
	private Class<? extends Annotation> getAnnotationClass(TreeLogger logger,
		...
		try {
			Class<?> clazz = Class.forName(binaryName, false,
					Thread.currentThread().getContextClassLoader());
			if (!Annotation.class.isAssignableFrom(clazz)) {
				logger.log(TreeLogger.ERROR,
						"Type " + binaryName + " is not an annotation");
				return null;
			}
			...
			logger.log(level,
					"Ignoring unresolvable annotation type " + binaryName);
			return null;
		}
	}
	 ...
	 *
	 * @param mapping
	 *            int array containing a sequence of from/to pairs, each from
	...
	private boolean resolveAnnotation(TreeLogger logger,
			CollectAnnotationData annotationVisitor,
			Map<Class<? extends Annotation>, Annotation> declaredAnnotations) {
		...
		Annotation annotation = createAnnotation(logger, annotationClass,
				annotationData);
		if (annotation == null) {
			return false;
		}
	...
	private boolean resolveAnnotations(TreeLogger logger,
		...
		if (annotationVisitors != null) {
			for (CollectAnnotationData annotationVisitor : annotationVisitors) {
				succeeded &= resolveAnnotation(logger, annotationVisitor,
						declaredAnnotations);
			}
		}
		return succeeded;
	...
	private Object resolveAnnotationValue(TreeLogger logger,
		...
		if (expectedType.isArray()) {
			...
			if (!value.getClass().isArray()) {
				logger.log(TreeLogger.WARN,
						"Annotation error: expected array of "
			...
			for (int i = 0; i < n; ++i) {
				...
				Object resolvedValue = resolveAnnotationValue(logger,
						componentType, valueElement);
				if (resolvedValue == null || !componentType
						.isAssignableFrom(resolvedValue.getClass())) {
			...
			if (!(value instanceof AnnotationEnum)) {
				logger.log(TreeLogger.ERROR,
						"Annotation error: expected an enum value,"
								+ " but got " + value);
				return null;
			}
			AnnotationEnum annotEnum = (AnnotationEnum) value;
			Class<? extends Enum> enumType = expectedType
					.asSubclass(Enum.class);
			try {
				return Enum.valueOf(enumType, annotEnum.getValue());
			} catch (IllegalArgumentException e) {
				logger.log(TreeLogger.WARN,
						"Unable to resolve annotation value '"
			...
			if (Class.class.equals(expectedType)) {
				if (!(value instanceof Type)) {
					logger.log(TreeLogger.WARN,
							"Annotation error: expected a class "
	...
	static {
		ImmutableMap.Builder<String, Class<?>> builder = ImmutableMap
				.<String, Class<?>> builder().put("Z", boolean.class)
				.put("B", byte.class).put("C", char.class).put("S", short.class)
				.put("I", int.class).put("F", float.class)
				.put("D", double.class).put("J", long.class)
				.put("V", void.class);
	...
	// TODO(stalcup): refactor this recursive resolveFoo() process. At the
	// moment there is a recursive
	// tree of resolveFoo() calls. Most of them return a boolean, but some do
	// not. Some of the
	// booleans are read and acted upon, some are not. Some functions do their
	// own logging and some
	// return a false to indicate that the caller should log. Sometimes the
	// boolean return value is an
	// indication of whether logging should occur and other times it's an
	// indication of whether
	// exploration should continue. It's a mess. Some ideas that would probably
	// make this more sane:
	// be consistent about SPAM logging throughout and WARN logging only at the
	// tip, and process types
	// in a queue instead of with recursion.
	private boolean resolveClass(TreeLogger logger,
			JRealClassType unresolvedType, TypeOracleBuildContext context) {
		assert unresolvedType != null;
		// Avoid cycles and useless computation.
		if (resolvedTypeSourceNames
				.contains(unresolvedType.getQualifiedSourceName())) {
			return true;
		}
		resolvedTypeSourceNames.add(unresolvedType.getQualifiedSourceName());
		// Make sure our enclosing type is resolved first.
		if (unresolvedType.getEnclosingType() != null && !resolveClass(logger,
				unresolvedType.getEnclosingType(), context)) {
			return false;
		}
		...
		CollectClassData classData = context.classDataByType
				.get(unresolvedType);
		assert classData != null;
		int access = classData.getAccess();
		assert (!classData.getClassType().hasNoExternalName());
		logger = logger.branch(TreeLogger.SPAM,
				"Found type '" + unresolvedType.getQualifiedSourceName() + "'",
				null);
		// Handle package-info classes.
		if (isPackageInfoTypeName(unresolvedType.getSimpleSourceName())) {
			return resolvePackage(logger, unresolvedType,
					classData.getAnnotations());
		}
		// Resolve annotations
		Map<Class<? extends Annotation>, Annotation> declaredAnnotations = Maps
				.newHashMap();
		resolveAnnotations(logger, classData.getAnnotations(),
				declaredAnnotations);
		addAnnotations(unresolvedType, declaredAnnotations);
		String signature = classData.getSignature();
		/*
		 * Note: Byte code from the OpenJDK compiler doesn't contain a type
		 * signature for non-static inner classes of parameterized types that do
		 * not contain new parameters (but JDT compiled byte code does). That
		 * can cause some differences in the way generic types are represented
		 * in the type oracle.
		 *
		 * These differences also show up when using java.lang.reflect to look
		 * at types.
		 */
		boolean isInterface = (access & Opcodes.ACC_INTERFACE) != 0;
		if (signature != null) {
			...
			if (unresolvedType.getSuperclass() != null && !resolveClass(logger,
					unresolvedType.getSuperclass(), context)) {
				logger.log(TreeLogger.WARN, "Unable to resolve supertype "
						+ unresolvedType.getSuperclass().getName());
				return false;
			}
		} else {
			...
			if (!isInterface) {
				...
				if (superInternalName != null) {
					JClassType superType = findByInternalName(
							superInternalName);
					if (superType == null
							|| !resolveClass(logger, superType, context)) {
						logger.log(TreeLogger.WARN,
								"Unable to resolve supertype "
										+ superInternalName);
						return false;
					}
					setSuperClass(unresolvedType,
							(JClassType) possiblySubstituteRawType(superType));
				}
			}
			// Set interfaces
			for (String interfaceInternalName : classData
					.getInterfaceInternalNames()) {
				JClassType interfaceType = findByInternalName(
						interfaceInternalName);
				if (interfaceType == null
						|| !resolveClass(logger, interfaceType, context)) {
					logger.log(TreeLogger.WARN, "Unable to resolve interface "
							+ interfaceInternalName);
					return false;
				}
				addImplementedInterface(unresolvedType,
						(JClassType) possiblySubstituteRawType(interfaceType));
			}
		}
		...
		for (CollectMethodData method : classData.getMethods()) {
			TreeLogger branch = logger.branch(TreeLogger.SPAM,
					"Resolving method " + method.getName());
			// TODO(rluble): Allow the users to ask for Java 8 features. For now
			// these are filtered out.
			if (isInterface && isJava8InterfaceMethod(method)) {
				logger.log(TreeLogger.Type.SPAM,
						"Ignoring Java 8 interface method " + method.getName());
				continue;
			}
			if (!resolveMethod(branch, unresolvedType, method, typeParamLookup,
					context)) {
				// Already logged.
				return false;
			}
		...
		for (CollectFieldData field : classData.getFields()) {
			TreeLogger branch = logger.branch(TreeLogger.SPAM,
					"Resolving field " + field.getName());
			if (!resolveField(branch, unresolvedType, field, typeParamLookup,
					nextEnumOrdinal, context)) {
				// Already logged.
				return false;
			}
		}
	...
	private boolean isJava8InterfaceMethod(CollectMethodData method) {
		// (Normal) interface methods are abstract. Java 8 introduced the
		// ability to declare default
		// methods and static methods both of which are exposed as non abstract
		// methods.
		return (method.getAccess() & Opcodes.ACC_ABSTRACT) == 0;
	}

	private boolean resolveClass(TreeLogger logger, JType unresolvedType,
			TypeOracleBuildContext context) {
		...
		if (unresolvedType instanceof JRealClassType) {
			return resolveClass(logger, (JRealClassType) unresolvedType,
					context);
		}
		if (unresolvedType instanceof JArrayType) {
			return resolveClass(logger,
					((JArrayType) unresolvedType).getComponentType(), context);
		}
		if (unresolvedType instanceof JParameterizedType) {
			return resolveClass(logger,
					((JParameterizedType) unresolvedType).getBaseType(),
					context);
		}
		if (unresolvedType instanceof JRawType) {
			return resolveClass(logger,
					((JRawType) unresolvedType).getBaseType(), context);
		}
	...
	private boolean resolveEnclosingClass(TreeLogger logger,
			JRealClassType unresolvedType, TypeOracleBuildContext context) {
		assert unresolvedType != null;
		...
		if (enclosingClassInternalName != null) {
			...
			if (enclosingType != null) {
				...
				if (enclosingType.isGenericType() != null && (classData
						.getAccess()
						& (Opcodes.ACC_STATIC | Opcodes.ACC_INTERFACE)) != 0) {
					// If the inner class doesn't have access to it's enclosing
					// type's
					// type variables, the enclosing type must be the raw type
					// instead
					// of the generic type.
	...
	private boolean resolveField(TreeLogger logger,
			JRealClassType unresolvedType, CollectFieldData field,
		...
		if ((field.getAccess() & Opcodes.ACC_ENUM) != 0) {
			...
			jfield = newEnumConstant(unresolvedType, name, declaredAnnotations,
					nextEnumOrdinal[0]++);
		} else {
			JField newField = newField(unresolvedType, name,
					declaredAnnotations);
			jfield = newField;
		}
		// Get modifiers.
		addModifierBits(jfield,
				mapBits(ASM_TO_SHARED_MODIFIERS, field.getAccess()));
		String signature = field.getSignature();
		JType fieldJType;
		if (signature != null) {
			SignatureReader reader = new SignatureReader(signature);
			JType[] fieldTypeRef = new JType[1];
			reader.acceptType(new ResolveTypeSignature(context.resolver, logger,
					fieldTypeRef, typeParamLookup, null));
			fieldJType = fieldTypeRef[0];
			if (fieldJType == null) {
				logger.log(TreeLogger.ERROR,
						"Unable to resolve type in field signature "
								+ signature);
				return false;
			}
		} else {
			...
			if (fieldJType == null) {
				logger.log(TreeLogger.ERROR,
						"Unable to resolve type " + fieldType.getInternalName()
	...
	private boolean resolveMethod(TreeLogger logger,
			JRealClassType unresolvedType, CollectMethodData methodData,
		...
		// Declare the type parameters. We will pass them into the constructors
		// for
		// JConstructor/JMethod/JAnnotatedMethod. Then, we'll do a second pass
		// to
		// resolve the bounds on each JTypeParameter object.
		JTypeParameter[] typeParams = collectTypeParams(
		...
		if ("<init>".equals(name)) {
			...
			method = newConstructor(unresolvedType, name, declaredAnnotations,
					typeParams);
			hasReturnType = false;
		} else {
			if (unresolvedType.isAnnotation() != null) {
				// TODO(jat): actually resolve the default annotation value.
				method = newAnnotationMethod(unresolvedType, name,
						declaredAnnotations, typeParams, null);
			} else {
				method = newMethod(unresolvedType, name, declaredAnnotations,
						typeParams);
			}
		}
		addModifierBits(method,
				mapBits(ASM_TO_SHARED_MODIFIERS, methodData.getAccess()));
		...
		if (signature != null) {
			...
			ResolveMethodSignature methodResolver = new ResolveMethodSignature(
					...
					hasReturnType, methodData, methodData.getArgTypes(),
					methodData.getArgNames(), methodData.hasActualArgNames(),
					context.allMethodArgs);
	...
	private boolean resolveParameters(TreeLogger logger, JAbstractMethod method,
		...
		if (!argNamesAreReal) {
			String[] lookupNames = context.allMethodArgs.lookup(method,
					methodData);
			if (lookupNames != null) {
		...
		List<CollectAnnotationData>[] paramAnnot = methodData
				.getArgAnnotations();
		for (int i = 0; i < argTypes.length; ++i) {
			...
			if (argJType == null) {
				logger.log(TreeLogger.ERROR,
						"Unable to resolve type " + argType.getInternalName()
								+ " of argument "
								+ methodData.getArgNames()[i]);
				return false;
			}
			...
			newParameter(method, argJType, argNames[i], declaredAnnotations,
					argNamesAreReal);
		}
		return true;
	}
