...
import static com.google.gwt.dev.js.JsUtils.*;
...
public class GenerateJavaScriptAST {
	private static final ImmutableList<String> METHODS_PROVIDED_BY_PREAMBLE = ImmutableList
			.of("Class.createForClass", "Class.createForPrimitive",
	 ...
	 * other useful information that is used in subsequent passes.
	 *
	 * @param logger
	 *            a TreeLogger
	 * @param program
	 *            a Java AST
	 * @param jsProgram
	 *            an (empty) JavaScript AST
	 * @param symbolTable
	 *            an (empty) symbol table that will be populated here
	 *
	 * @return A pair containing a JavaToJavaScriptMap and a Set of JsFunctions
	 *         that need to be considered for inlining.
	 */
	public static Pair<JavaToJavaScriptMap, Set<JsNode>> exec(TreeLogger logger,
			JProgram program, JsProgram jsProgram,
			CompilerContext compilerContext, TypeMapper<?> typeMapper,
			Map<StandardSymbolData, JsName> symbolTable,
			PermutationProperties props) {
		Event event = SpeedTracerLogger
				.start(CompilerEventType.GENERATE_JS_AST);
		try {
			GenerateJavaScriptAST generateJavaScriptAST = new GenerateJavaScriptAST(
					logger, program, jsProgram, compilerContext, typeMapper,
					symbolTable, props);
	...
	private final Map<JClassType, JsScope> classScopes = Maps
			.newIdentityHashMap();
	...
	/**
	 * Classes that could potentially see uninitialized values for fields that
	 * are initialized in the declaration.
	 */
	 ...
	 * hashCode, and toString. All other class scopes have this scope as an
	 * ultimate parent.
	 */
	private final JsScope objectScope;

	private final Map<JMethod, JsName> polymorphicNames = Maps
			.newIdentityHashMap();
	...
	private final Map<JsStatement, JDeclaredType> javaTypeByGlobalStatement = Maps
			.newHashMap();

	private final Map<JsStatement, JMethod> methodByGlobalStatement = Maps
			.newHashMap();
	...
	private final Map<JType, JDeclarationStatement> classLiteralDeclarationsByType = Maps
			.newLinkedHashMap();

	private GenerateJavaScriptAST(TreeLogger logger, JProgram program,
			JsProgram jsProgram, CompilerContext compilerContext,
		...
		this.stripStack = JsStackEmulator
				.getStackMode(properties) == JsStackEmulator.StackMode.STRIP;
		this.closureCompilerFormatEnabled = options
				.isClosureCompilerFormatEnabled();
		this.objectConstructorFunction = new JsFunction(SourceOrigin.UNKNOWN,
				topScope, topScope.findExistingName("Object"));
	}
	...
	private void contructTypeToClassLiteralDeclarationMap() {
		 ...
		 */
		JMethodBody clinitBody = (JMethodBody) program
				.getTypeClassLiteralHolder().getClinitMethod().getBody();
		for (JStatement stmt : clinitBody.getStatements()) {
			...
			JType type = program.getTypeByClassLiteralField(
					(JField) ((JDeclarationStatement) stmt).getVariableRef()
							.getTarget());
	...
	private Pair<JavaToJavaScriptMap, Set<JsNode>> execImpl() {
		...
		if (!incremental) {
			// TODO(rluble): pull out this analysis and make it a Java AST
			// optimization pass.
		...
		// SourceInfo
		JavaToJavaScriptMap jjsMap = new JavaToJavaScriptMapImpl(
				program.getDeclaredTypes(), names, javaTypeByGlobalStatement,
				methodByGlobalStatement);
		Set<JsNode> functionsForJsInlining = incremental
				? Collections.<JsNode> emptySet()
				: new CollectJsFunctionsForInlining()
	...
	private String mangleNameForGlobal(JMethod method) {
		String s = JjsUtils.mangleMemberName(
				method.getEnclosingType().getName(), method.getName()) + "__";
	...
	private String mangleNameForPackagePrivatePoly(JMethod method) {
		...
		/*
		 * Package private instance methods in different package should not
		 * override each other, so they must have distinct polymorphic names.
		 * Therefore, add the package to the mangled name.
		 */
		String mangledName = Joiner.on("$").join("package_private",
				JjsUtils.mangledNameString(
						method.getEnclosingType().getPackageName()),
				JjsUtils.mangledNameString(method));
		return StringInterner.get().intern(
				JjsUtils.constructManglingSignature(method, mangledName));
	}
	...
	private String mangleNameForPrivatePoly(JMethod method) {
		 ...
		 * each other, so they must have distinct polymorphic names. Therefore,
		 * add the class name to the mangled name.
		 */
		String mangledName = Joiner.on("$").join("private",
	...
	private class CollectJsFunctionsForInlining extends JVisitor {
		// JavaScript functions that arise from methods that were not inlined in
		// the Java AST
		// NOTE: We use a LinkedHashSet to preserve the order of insertion. So
		// that the following passes
		...
		public void endVisit(JMethod x, Context ctx) {
			if (x.isJsniMethod()) {
				// These are methods whose bodies where not traversed by the
				// Java method inliner.
				JsFunction function = jsFunctionsByJavaMethodBody
						.get(x.getBody());
		...
		public void endVisit(JMethodCall x, Context ctx) {
			...
			if (target.isInliningAllowed() && (target.isJsniMethod()
					|| program.getIndexedTypes()
							.contains(target.getEnclosingType())
					|| target.getInliningMode() == InliningMode.FORCE_INLINE)) {
				// These are either: 1) callsites to JSNI functions, in which
				// case MethodInliner did not
				// attempt to inline; 2) inserted by normalizations passes AFTER
				// all inlining or 3)
				// calls to methods annotated with @ForceInline that were not
				// inlined by the simple
				// MethodInliner.
				JsFunction function = jsFunctionsByJavaMethodBody
						.get(currentMethod.getBody());
				if (function != null && function.getBody() != null) {
					functionsForJsInlining.add(function);
				}
			}
	...
	private class CreateNamesAndScopesVisitor extends JVisitor {
		 ...
		 */
		private final Map<String, String> fileNameToUriString = Maps
				.newHashMap();
		...
		public void endVisit(JField x, Context ctx) {
			...
			if (x.isStatic()) {
				...
				jsName = JjsUtils.requiresJsName(x)
						? scopeStack.peek()
								.declareUnobfuscatableName(x.getJsName())
						: scopeStack.peek().declareName(mangleName(x),
								x.getName());
			}
			names.put(x, jsName);
		...
		public void endVisit(JParameter x, Context ctx) {
			if (x.isVarargs() && currentMethod.isJsMethodVarargs()) {
				names.put(x, scopeStack.peek()
						.declareUnobfuscatableName("arguments"));
				return;
			}
			names.put(x, scopeStack.peek().declareName(x.getName()));
		}
		...
		public boolean visit(JClassType x, Context ctx) {
			...
			JsName jsName = topScope.declareName(JjsUtils.mangledNameString(x),
					x.getShortName());
			names.put(x, jsName);
			recordSymbol(x, jsName);
			// My class scope
			if (x.getSuperClass() == null) {
				...
				/*
				 * WEIRD: we wedge the global interface scope in between object
				 * and all of its subclasses; this ensures that interface method
				 * names trump all (except Object method names)
				 */
				if (parentScope == objectScope) {
					parentScope = interfaceScope;
				}
				myScope = new JsNormalScope(parentScope,
						"class " + x.getShortName());
			}
		...
		public boolean visit(JMethod x, Context ctx) {
			...
			/*
			 * Only allocate a name for a function if it is native, not
			 * polymorphic, is a JNameOf target or stack-stripping is disabled.
			 */
			if (!stripStack || !polymorphicNames.containsKey(x)
					|| x.isJsniMethod() || nameOfTargets.contains(x)) {
			...
			if (x.isJsniMethod()) {
				...
				/*
				 * It would be more correct here to check for an inline
				 * assignment, such as var foo = function blah() {} and
				 * introduce a separate scope for the function's name according
				 * to EcmaScript-262, but this would mess up stack traces by
				 * allowing two inner scope function names to obfuscate to the
				 * same identifier, making function names no longer a 1:1
				 * mapping to obfuscated symbols. Leaving them in global scope
				 * causes no harm.
				 */
				function = new JsFunction(x.getSourceInfo(), topScope,
						globalName, !x.isJsNative());
			}
			jsFunctionsByJavaMethodBody.put(x.getBody(), function);
			scopeStack.push(function.getScope());
			// Don't traverse the method body of methods in referenceOnly types
			// since those method bodies
			// only exist in JS output of other modules it is their
			// responsibility to handle their naming.
			return !program.isReferenceOnly(x.getEnclosingType());
		}

		@Override
		public boolean visit(JProgram x, Context ctx) {
			// Scopes and name objects need to be calculated within all types,
			// even reference-only ones.
			// This information is used to be able to detect and avoid name
			// collisions during pretty or
		...
		public boolean visit(JTryStatement x, Context ctx) {
			...
			for (JTryStatement.CatchClause clause : x.getCatchClauses()) {
				...
				JsCatch jsCatch = new JsCatch(x.getSourceInfo(),
						scopeStack.peek(), arg.getTarget().getName());
	...
	/**
	 * Finds the nodes that are targets of JNameOf so that a name is assigned to
	 * them.
	 */
	...
	private class GenerateJavaScriptTransformer extends JTransformer<JsNode> {
		...
		private final Map<JDeclaredType, JsFunction> clinitFunctionForType = Maps
				.newHashMap();

		private JMethod currentMethod = null;

		private final JsName arrayLength = objectScope
				.declareUnobfuscatableName("length");

		private final JsName globalTemp = topScope
				.declareUnobfuscatableName("_");

		private final JsName prototype = objectScope
				.declareUnobfuscatableName("prototype");
		...
		private void addTypeDefinitionStatement(JDeclaredType x,
				JsStatement statement) {
		...
		private JsExpression buildClosureStyleCastMapFromArrayLiteral(
				...
				SourceInfo sourceInfo) {
			/*
			 * goog.object.createSet('foo', 'bar', 'baz') is optimized by
			 * closure compiler into {'foo': !0, 'bar': !0, baz: !0}
			 */
			JsNameRef createSet = new JsNameRef(sourceInfo,
					GOOG_OBJECT_CREATE_SET);
		...
		private JsExpression buildJsCastMapLiteral(
				List<JsExpression> runtimeTypeIdLiterals,
				SourceInfo sourceInfo) {
			if (JjsUtils.closureStyleLiteralsNeeded(incremental,
					closureCompilerFormatEnabled)) {
				return buildClosureStyleCastMapFromArrayLiteral(
						runtimeTypeIdLiterals, sourceInfo);
			}
			JsNumberLiteral one = new JsNumberLiteral(sourceInfo, 1);
			JsObjectLiteral.Builder objectLiteralBuilder = JsObjectLiteral
					.builder(sourceInfo).setInternable();
		 ...
		 * <p>
		 * In modular compiles the preamble has to include code for creating all
		 * 4 types of class literals.
		 */
		private boolean checkCoreModulePreambleComplete(JProgram program,
			...
			for (String createForMethodName : METHODS_PROVIDED_BY_PREAMBLE) {
				if (!calledMethods.contains(
						program.getIndexedMethod(createForMethodName))) {
					return false;
				}
			}
			return true;
			...
			for (JMethod method : methods) {
				...
				if (methodSignatures.contains(sig)) {
					throw new InternalCompilerException(
							"Signature collision in Type " + type.getName()
									+ " for method " + sig);
				}
		...
		private List<JDeclaredType> computeClassLiteralsSupportClasses(
				JProgram program, Set<JDeclaredType> alreadyProcessedTypes) {
			if (program.isReferenceOnly(program.getIndexedType("Class"))) {
				return Collections.emptyList();
			}
			// Include in the preamble all classes that are reachable for
			// Class.createForClass,
			// Class.createForInterface
			SortedSet<JDeclaredType> reachableClasses = computeReachableTypes(
					METHODS_PROVIDED_BY_PREAMBLE);
			...
			// TODO(rluble): The set of preamble types might be overly large, in
			// particular will include
			// JSOs that need clinit. This is due to {@link ControlFlowAnalyzer}
			// making all JSOs live if
			// there is a cast to that type anywhere in the program. See the use
			// of
			// {@link JTypeOracle.getInstantiatedJsoTypesViaCast} in the
			// constructor.
			return Lists.newArrayList(orderedPreambleClasses);
		}

		/**
		 * Computes the set of types whose methods or fields are reachable from
		 * {@code methods}.
		 */
		private SortedSet<JDeclaredType>
				computeReachableTypes(Iterable<String> methodNames) {
			...
			SortedSet<JDeclaredType> reachableTypes = ImmutableSortedSet
					.copyOf(HasName.BY_NAME_COMPARATOR,
							Iterables.filter(Iterables.transform(
									cfa.getLiveFieldsAndMethods(),
									new Function<JNode, JDeclaredType>() {
										...
										public JDeclaredType
											...
											if (member instanceof JMethod) {
												...
												assert member instanceof JParameter
														|| member instanceof JLocal;
												// Discard locals and
												// parameters, only need the
												// enclosing instances of
												// reachable
												// fields and methods.
												return null;
											}
		...
		/**
		 * Creates a (var) assignment a statement for a function call to an
		 * indexed function.
		 */
		private JsStatement constructFunctionCallStatement(
				JsName assignToVariableName, String indexedFunctionName,
				JsExpression... args) {
			return constructFunctionCallStatement(assignToVariableName,
					indexedFunctionName, Arrays.asList(args));
		}
		...
		private JsStatement constructFunctionCallStatement(
			...
			JsInvocation invocation = constructInvocation(indexedFunctionName,
					args);
		...
		private JsInvocation constructInvocation(SourceInfo sourceInfo,
			...
			return constructInvocation(sourceInfo, indexedFunctionName,
					Arrays.asList(args));
		}
		...
		private JsInvocation constructInvocation(SourceInfo sourceInfo,
			...
			JsName functionToInvoke = getIndexedMethodJsName(
					indexedFunctionName);
			return new JsInvocation(sourceInfo,
					functionToInvoke.makeRef(sourceInfo), args);
		}
		...
		private JsExpression constructJsFunctionObject(SourceInfo sourceInfo,
				JClassType type, JsName ctorName, JsNameRef ctorReference,
				JsExpression ctorArguments) {
			...
			JsNameRef funcNameRef = JsUtils.createQualifiedNameRef(sourceInfo,
					ctorName, prototype,
					polymorphicNames.get(jsFunctionMethod));
			// makeLambdaFunction(Foo.prototype.functionMethodName, new
			// Foo(...))
			return constructInvocation(sourceInfo,
					RuntimeConstants.RUNTIME_MAKE_LAMBDA_FUNCTION, funcNameRef,
					ctorReference, ctorArguments);
		}
		...
		private JsNameRef createStaticReference(JMember member,
				SourceInfo sourceInfo) {
			assert !member.needsDynamicDispatch();
			return member.isJsNative()
					? createGlobalQualifier(member.getQualifiedJsName(),
							sourceInfo)
					: names.get(member).makeRef(sourceInfo);
		}

		/*
		 * Declare an empty synthesized constructor that looks like: function
		 * ClassName(){}
		 *
		 * Closure Compiler's RewriteFunctionExpressions pass can be enabled to
		 * turn these back into a factory method after optimizations.
		 *
		 * TODO(goktug): throw Error in the body to prevent instantiation via
		 * this constructor.
		 */
		private JsName declareSynthesizedClosureConstructor(JDeclaredType x) {
			SourceInfo sourceInfo = x.getSourceInfo();
			JsName classVar = topScope
					.declareName(JjsUtils.mangledNameString(x));
			JsFunction closureCtor = JsUtils
					.createEmptyFunctionLiteral(sourceInfo, topScope, classVar);
			JsExprStmt statement = closureCtor.makeStmt();
			// This synthetic statement must be in the initial fragment, do not
			// add to typeDefinitions
			getGlobalStatements().add(statement);
		...
		private JsExpression dispatchToInstanceField(JFieldRef x,
			...
			return names.get(x.getField()).makeQualifiedRef(x.getSourceInfo(),
					instance);
		}

		private JsExpression dispatchToInstanceMethod(JsExpression instance,
				JMethod method, List<JsExpression> args,
				SourceInfo sourceInfo) {
			JsNameRef reference = polymorphicNames.get(method)
					.makeQualifiedRef(sourceInfo, instance);
			return createInvocationOrPropertyAccess(InvocationStyle.NORMAL,
					sourceInfo, method, instance, reference, args);
		}

		private JsExpression dispatchToJsFunction(JsExpression instance,
				JMethod method, List<JsExpression> args,
				SourceInfo sourceInfo) {
			return createInvocationOrPropertyAccess(InvocationStyle.FUNCTION,
					sourceInfo, method, instance, null, args);
		}

		private JsExpression dispatchToStatic(JsExpression unnecessaryQualifier,
				JMethod method, List<JsExpression> args,
				SourceInfo sourceInfo) {
			JsNameRef methodName = createStaticReference(method, sourceInfo);
			return JsUtils.createCommaExpression(unnecessaryQualifier,
					createInvocationOrPropertyAccess(InvocationStyle.NORMAL,
							sourceInfo, method, null, methodName, args));
		}

		private JsExpression dispatchToStaticField(JFieldRef fieldRef,
				JsExpression unnecessaryQualifier) {
			/*
			 * Note: the comma expressions here would cause an illegal tree
			 * state if the result expression ended up on the lhs of an
			 * assignment. {@link JsNormalizer} will fix this situation.
			 */
			JsExpression result = createStaticReference(fieldRef.getField(),
					fieldRef.getSourceInfo());
			return JsUtils.createCommaExpression(unnecessaryQualifier,
					maybeCreateClinitCall(fieldRef.getField()), result);
		}

		private JsExpression dispatchToSuper(JsExpression instance,
				JMethod method, List<JsExpression> args,
				SourceInfo sourceInfo) {
			JsNameRef methodNameRef;
			if (method.isJsNative()) {
				// Construct Constructor.prototype.jsname or Constructor.
				methodNameRef = createGlobalQualifier(
						method.getQualifiedJsName(), sourceInfo);
			} else if (method.isConstructor()) {
				/*
				 * Constructor calls through {@code this} and {@code super} are
				 * always dispatched statically using the constructor function
				 * name (constructors are always defined as top level
				 * functions).
				 *
				 * Because constructors are modeled like instance methods they
				 * have an implicit {@code this} parameter, hence they are
				 * invoked like: "constructor.call(this, ...)".
				 */
				methodNameRef = names.get(method).makeRef(sourceInfo);
			} else {
				// These are regular super method call. These calls are always
				// dispatched statically and
				// optimizations will devirtualize them (except in a few cases,
				// like being target of
				// {@link Impl.getNameOf} or calls to the native classes.
				JDeclaredType superClass = method.getEnclosingType();
				JsExpression protoRef = getPrototypeQualifierViaLookup(
						superClass, sourceInfo);
				methodNameRef = polymorphicNames.get(method)
						.makeQualifiedRef(sourceInfo, protoRef);
			}
			return JsUtils.createInvocationOrPropertyAccess(
					InvocationStyle.SUPER, sourceInfo, method, instance,
					methodNameRef, args);
		}
		...
		private void embedBindingProperties() {
			...
			// ...]
			// The outermost list is indexed by soft permutation id. Each item
			// represents
			// a map from binding properties to their values, but is stored as a
			// list of pairs
			// for easy iteration.
			JsArrayLiteral permutationProperties = new JsArrayLiteral(
					sourceInfo);
			for (Map<String, String> propertyValueByPropertyName : properties
					.findEmbeddedProperties(TreeLogger.NULL)) {
				JsArrayLiteral entryList = new JsArrayLiteral(sourceInfo);
				for (Entry<String, String> entry : propertyValueByPropertyName
						.entrySet()) {
			...
			getGlobalStatements().add(constructInvocation(sourceInfo,
					"ModuleUtils.setGwtProperty",
					new JsStringLiteral(sourceInfo, "permProps"),
					permutationProperties).makeStmt());
		}
			...
			for (JField field : type.getFields()) {
				...
				if (initializeAtTopScope(field)) {
					...
					initializer = transform(field.getLiteralInitializer());
						...
						.getDefaultValue() == JNullLiteral.INSTANCE) {
					// Fields whose default value is null are left uninitialized
					// and will
				...
				if (field.isStatic()) {
					...
					JsNameRef fieldRef = name.makeQualifiedRef(
							field.getSourceInfo(),
							getPrototypeQualifierOf(field));
					addTypeDefinitionStatement(type,
							createAssignment(fieldRef, initializer).makeStmt());
				}
			}
			addVarsIfNotEmpty(vars);
		}

		private void emitMethodImplementation(JMethod method,
				JsNameRef functionNameRef,
				JsExprStmt methodDefinitionStatement) {
			addMethodDefinitionStatement(method, methodDefinitionStatement);
			if (shouldEmitDisplayNames()) {
				JsExprStmt displayNameAssignment = outputDisplayName(
						functionNameRef, method);
				addMethodDefinitionStatement(method, displayNameAssignment);
			}
		}

		private void emitPropertyImplementation(JMethod method,
				JsNameRef prototype, JsNameRef name,
				JsExpression methodDefinitionStatement) {
			SourceInfo sourceInfo = method.getSourceInfo();
			// We use Object.defineProperties instead of Object.defineProperty
			// to make sure the
			// property name appears as an identifier and not as a string.
			// Some JS optimizers, e.g. the closure compiler, relies on this
			// subtle difference for
			// obfuscating property names.
			JsNameRef definePropertyMethod = getIndexedMethodJsName(
					RuntimeConstants.RUNTIME_DEFINE_PROPERTIES)
							.makeRef(sourceInfo);
			JsObjectLiteral definePropertyLiteral = JsObjectLiteral
					.builder(sourceInfo)
					// {name: {get: function() { ..... }} or {set : function (v)
					// {....}}}
					.add(name, JsObjectLiteral.builder(sourceInfo)
							// {get: function() { ..... }} or {set : function
							// (v) {....}}
							.add(method.getJsMemberType()
									.getPropertyAccessorKey(),
									methodDefinitionStatement)
							.build())
					.build();
			addMethodDefinitionStatement(method,
					new JsInvocation(sourceInfo, definePropertyMethod,
		...
		private void emitStaticMethods(JDeclaredType type) {
			...
			for (JMethod method : type.getMethods()) {
				...
				emitMethodImplementation(method,
						function.getName().makeRef(function.getSourceInfo()),
						function.makeStmt());
			}
		}
		...
		private JsExpression generateAbstractMethodDefinition(JMethod method) {
			return (closureCompilerFormatEnabled)
					? JsUtils.createQualifiedNameRef(GOOG_ABSTRACT_METHOD,
							method.getSourceInfo())
					: null;
		}

		private void generateCallToDefineClass(JClassType type,
				List<JsNameRef> constructorArgs) {
			JClassType superClass = type.getSuperClass();
			JExpression superTypeId = (superClass == null)
			...
			defineClassArguments.add(jsPrototype == null
					? transform(superTypeId)
			...
			// Runtime.defineClass(typeId, superTypeId, castableMap,
			// constructors)
			JsStatement defineClassStatement = constructInvocation(
					type.getSourceInfo(), RuntimeConstants.RUNTIME_DEFINE_CLASS,
					defineClassArguments).makeStmt();
			addTypeDefinitionStatement(type, defineClassStatement);
			maybeCopyJavaLangObjectProperties(type,
					getPrototypeQualifierViaLookup(
		...
		private JsExpression generateCastableTypeMap(JDeclaredType type) {
			...
			JsName castableTypeMapName = getIndexedFieldJsName(
		...
		private void
				generateClassLiterals(Iterable<? extends JType> orderedTypes) {
		 ...
		 *
		 * function ClassName() {} ClassName.prototype.method1 = function() {
		 * ... }; ClassName.prototype.method2 = function() { ... };
		 * ClassName.prototype.castableTypeMap = {...}
		 * ClassName.prototype.___clazz = classLit; function Ctor1() {} function
		 * Ctor2() {}
		 *
		 * goog$inherits(Ctor1, ClassName); goog$inherits(Ctor2, ClassName);
		 *
		 * The primary change is to make the prototype assignment look like
		 * regular closure code to help the compiler disambiguate which methods
		 * belong to which type. Elimination of defineClass() makes the setup
		 * more transparent and eliminates a global table holding a reference to
		 * every prototype.
		 */
		private void generateClosureTypeDefinition(JDeclaredType type) {
			...
			/*
			 * Closure style prefers 1 single ctor per type. To model this
			 * without radical changes, we simply model each concrete ctor as a
			 * subtype. This works because GWT doesn't use the native instanceof
			 * operator. So for example, class A() { A(int type){}, A(String
			 * s){} } becomes (pseudo code):
			 *
			 * function A() {} A.prototype.method = ...
			 *
			 * function A_int(x) {} function A_String(s) {} goog$inherits(A_int,
			 * A); goog$inherits(A_string, A);
			 *
			 */
			for (JMethod method : getPotentiallyAliveConstructors(type)) {
				SourceInfo typeSourceInfo = type.getSourceInfo();
				JsNameRef googInherits = JsUtils
						.createQualifiedNameRef(GOOG_INHERITS, typeSourceInfo);
				SourceInfo methodSourceInfo = method.getSourceInfo();
				JsExprStmt callGoogInherits = new JsInvocation(typeSourceInfo,
						googInherits,
						names.get(method).makeRef(methodSourceInfo),
						names.get(method.getEnclosingType())
								.makeRef(methodSourceInfo)).makeStmt();
		...
		private void generateEpilogue() {
			...
			generateExports();
			// Generate entry methods. Needs to be after class literal insertion
			// since class literal will
			// be referenced by runtime rebind and property provider
			// bootstrapping.
			setupGwtOnLoad();
		...
		private void generateExports() {
			...
			JsInteropExportsGenerator exportGenerator = closureCompilerFormatEnabled
					? new ClosureJsInteropExportsGenerator(
							getGlobalStatements(), names)
					: new DefaultJsInteropExportsGenerator(
							getGlobalStatements(), globalTemp,
							getIndexedMethodJsName(
									RuntimeConstants.RUNTIME_PROVIDE));
			// Gather exported things in JsNamespace order.
			for (JDeclaredType type : program.getDeclaredTypes()) {
				if (type.isJsNative()) {
					// JsNative types have no implementation and so shouldn't
					// export anything.
					continue;
				}
				if (type.isJsType()) {
					exportedMembersByExportName.put(type.getQualifiedJsName(),
							type);
				}
				for (JMember member : type.getMembers()) {
					if (member.isJsInteropEntryPoint()) {
						if (member.getJsMemberType() == JsMemberType.PROPERTY
								&& !member.isFinal()) {
							// TODO(goktug): Remove the warning when we export
						...
						exportedMembersByExportName
								.put(member.getQualifiedJsName(), member);
					}
				}
			}
			// Output the exports.
			for (Object exportedEntity : exportedMembersByExportName.values()) {
				if (exportedEntity instanceof JDeclaredType) {
					...
					exportGenerator.exportMember(member,
							names.get(member).makeRef(member.getSourceInfo()));
				}
			}
		}

		private void
				generateGoogGlobalInitialization(SourceInfo programSourceInfo) {
			// $wnd.goog = $wnd.goog || {}
			JsNameRef wndGoog = goog.makeQualifiedRef(programSourceInfo,
					wnd.makeRef(programSourceInfo));
			generatePropertyInitialization(programSourceInfo, wndGoog,
					JsObjectLiteral.builder(programSourceInfo).build());
			// $wnd.goog.global = $wnd.goog.global || $wnd
			JsNameRef wndGoogGlobal = global.makeQualifiedRef(programSourceInfo,
					wndGoog);
			generatePropertyInitialization(programSourceInfo, wndGoogGlobal,
					wnd.makeRef(programSourceInfo));
		}

		private void generateImmortalTypes(JsVars globals) {
			List<JClassType> immortalTypesReversed = Lists
					.reverse(program.immortalCodeGenTypes);
			...
			for (JClassType x : immortalTypesReversed) {
				...
				for (JMethod method : x.getMethods()) {
					/*
					 * Skip virtual methods and constructors. Even in cases
					 * where there is no constructor defined, the compiler will
					 * synthesize a default constructor which invokes a
					 * synthesized $init() method. We must skip both of these
					 * inserted methods.
					 */
					if (method.needsDynamicDispatch()
					...
					addMethodDefinitionStatement(1, method,
							function.makeStmt());
				}
				...
				for (JField field : x.getFields()) {
					...
					assert field.getInitializer() == field
							.getLiteralInitializer() : "'" + field.getName()
		...
		/**
		 * Does everything JCHSU.defineClass does, but inlined into global
		 * statements. Roughly parallels argument order of
		 * generateCallToDefineClass.
		 */
		private void generateInlinedDefineClass(JDeclaredType type,
				JsName classVar) {
			...
			JsNameRef parentCtor = jsPrototype != null
					...
					: superClass != null ? names.get(superClass).makeRef(info)
							: null;
			if (parentCtor != null) {
				JsNameRef googInherits = JsUtils
						.createQualifiedNameRef(GOOG_INHERITS, info);
				// Use goog$inherits(ChildCtor, ParentCtor) to setup inheritance
				JsExprStmt callGoogInherits = new JsInvocation(info,
						googInherits, classVar.makeRef(info), parentCtor)
								.makeStmt();
			...
			// inline assignment of castableTypeMap field instead of using
			// defineClass()
			setupCastMapOnPrototype(type);
			maybeCopyJavaLangObjectProperties(type,
					getPrototypeQualifierOf(program.getTypeJavaLangObject(),
							info),
		 ...
		 * _.method1 = function() { ... } _.method2 = function() { ... }
		 */
		private void generateJsClassDefinition(JClassType classType) {
			...
			for (JMethod method : getPotentiallyAliveConstructors(classType)) {
				constructorArgs.add(
						names.get(method).makeRef(classType.getSourceInfo()));
			}
		...
		private Set<JDeclaredType> generatePreamble(JProgram program) {
			...
			JsVars vars = new JsVars(programSourceInfo,
					new JsVar(programSourceInfo, globalTemp));
			addVarsIfNotEmpty(vars);
			...
			addTypeDefinitionStatement(
					...
					constructInvocation(program.getSourceInfo(),
							RuntimeConstants.RUNTIME_BOOTSTRAP).makeStmt());
			Set<JDeclaredType> alreadyProcessed = Sets
					.<JDeclaredType> newLinkedHashSet(
							program.immortalCodeGenTypes);
			alreadyProcessed.add(program.getTypeClassLiteralHolder());
			alreadyRan.addAll(alreadyProcessed);
			List<JDeclaredType> classLiteralSupportClasses = computeClassLiteralsSupportClasses(
					program, alreadyProcessed);
			...
			Set<JDeclaredType> preambleTypes = Sets
					.newLinkedHashSet(alreadyProcessed);
			preambleTypes.addAll(classLiteralSupportClasses);
			return preambleTypes;
		}

		private void generatePropertyInitialization(SourceInfo sourceInfo,
				JsNameRef qualifiedNameRef, JsExpression initializer) {
			getGlobalStatements().add(new JsBinaryOperation(sourceInfo,
					JsBinaryOperator.ASG, qualifiedNameRef,
					new JsBinaryOperation(sourceInfo, JsBinaryOperator.OR,
							qualifiedNameRef, initializer)).makeStmt());
		}

		private void generatePrototypeAssignment(JMethod method, JsName name,
				JsExpression rhs) {
			generatePrototypeAssignment(method, name, rhs,
					method.getJsMemberType());
		}

		/**
		 * Create a vtable assignment of the form _.polyname = rhs; and register
		 * the line as created for {@code method}.
		 */
		private void generatePrototypeAssignment(JMethod method, JsName name,
				JsExpression rhs, JsMemberType memberType) {
			SourceInfo sourceInfo = method.getSourceInfo();
			JsNameRef prototypeQualifierOf = getPrototypeQualifierOf(method);
			JsNameRef lhs = name.makeQualifiedRef(sourceInfo,
					prototypeQualifierOf);
			switch (memberType) {
			case GETTER:
			case SETTER:
				emitPropertyImplementation(method, prototypeQualifierOf,
						name.makeRef(sourceInfo), rhs);
				break;
			default:
				emitMethodImplementation(method, lhs,
						createAssignment(lhs, rhs).makeStmt());
				break;
			}
		}

		private void generatePrototypeAssignmentForJavaField(JDeclaredType type,
				String javaField, JsExpression rhs) {
			SourceInfo sourceInfo = type.getSourceInfo();
			JsNameRef protoRef = getPrototypeQualifierOf(type, sourceInfo);
			JsNameRef fieldRef = getIndexedFieldJsName(javaField)
					.makeQualifiedRef(sourceInfo, protoRef);
			addTypeDefinitionStatement(type,
					createAssignment(fieldRef, rhs).makeStmt());
		}

		private void generatePrototypeDefinition(JMethod method,
				JsExpression functionDefinition) {
			if (functionDefinition != null) {
				generatePrototypeAssignment(method,
						polymorphicNames.get(method), functionDefinition);
			}
			if (method.exposesNonJsMember()) {
				JsName internalMangledName = interfaceScope.declareName(
						mangleNameForPoly(method), method.getName());
			...
			if (method.exposesPackagePrivateMethod()) {
				...
				// }
				//
				// Up to this point it is clear that package private names need
				// to be different than
				// public names.
				//
				// Add class a.D extends a.A implements I { public m() }
				//
				// a.D collapses A::m and I::m into the same function and it was
				// clear that two
				// two different names were already needed, hence when creating
				// the vtable for a.D
				// both names have to point to the same function.
				generatePrototypeDefinitionAlias(method,
						getPackagePrivateName(method));
			}
		}

		private void generatePrototypeDefinitionAlias(JMethod method,
				JsName alias) {
			JsName polyName = polymorphicNames.get(method);
			JsExpression bridge = JsUtils.createBridge(method, polyName,
					topScope);
		...
		/**
		 * Creates the assignment for all polynames for a certain class, assumes
		 * that the global variable _ points the JavaScript prototype for
		 * {@code type}.
		 */
		private void generatePrototypeDefinitions(JDeclaredType type) {
			assert !program.isRepresentedAsNativeJsPrimitive(type);
			// Emit synthetic methods first. In JsInterop we allow a more user
			// written method to be named
			// with the same name as a synthetic bridge (required due to
			// generics) relying that the
			// synthetic method is output first into the prototype slot and
			// rewritten in this situation.
			// TODO(rluble): this is a band aid. The user written method (and
			// its overrides) should be
			// automatically JsIgnored. Otherwise some semantics become looser.
			// E.g. the synthetic bridge
			// method may be casting some of the parameters. Such casts are lost
			// in this scheme.
			Iterable<JMethod> orderedInstanceMethods = Iterables.concat(
					Iterables.filter(type.getMethods(),
							Predicates.and(JjsPredicates.IS_SYNTHETIC,
			...
			for (JMethod method : orderedInstanceMethods) {
				generatePrototypeDefinition(method,
						(JsExpression) transformMethod(method));
			}
		}

		private void generateRemainingClassLiterals() {
			if (!incremental) {
				// Emit classliterals that are references but whose classes are
				// not live.
				generateClassLiterals(Iterables.filter(
						classLiteralDeclarationsByType.keySet(),
						Predicates.not(Predicates.<JType> in(alreadyRan))));
				return;
			}
			// In incremental, class literal references to class literals that
			// were not generated
			// as part of the current compile have to be from reference only
			// classes.
			assert FluentIterable.from(classLiteralDeclarationsByType.keySet())
					...
					.filter(new Predicate<JType>() {
						@Override
						public boolean apply(JType type) {
							return !program
									.isReferenceOnly((JDeclaredType) type);
						}
					}).isEmpty();
			// In incremental only the class literals for the primitive types
		...
		private void generateTypeSetup(JDeclaredType type) {
			if (program.isRepresentedAsNativeJsPrimitive(type)
				...
				setupCastMapForUnboxedType(type,
						program.getRepresentedAsNativeTypesDispatchMap()
								.get(type).getCastMapField());
				return;
			}
		...
		private JField getClassLiteralField(JType type) {
			JDeclarationStatement decl = classLiteralDeclarationsByType
					.get(type);
			if (decl == null) {
				return null;
			}
		...
		private String getDisplayName(JMethod method) {
			...
			case ABBREVIATED:
				return method.getEnclosingType().getShortName() + "."
						+ method.getName();
			case FULL:
				return method.getEnclosingType().getName() + "."
						+ method.getName();
			default:
				assert false : "Invalid display mode option "
						+ methodNameMappingMode;
			}
			return null;
		}
		...
		private JsName getPackagePrivateName(JMethod method) {
			...
			throw new AssertionError(method.toString()
					+ " overrides a package private method but was not found.");
		}

		private Iterable<JMethod>
				getPotentiallyAliveConstructors(JDeclaredType x) {
		 ...
		 */
		private JsNameRef getPrototypeQualifierOf(JDeclaredType type,
				SourceInfo info) {
			return closureCompilerFormatEnabled
					? prototype.makeQualifiedRef(info,
							names.get(type).makeRef(info))
		...
		private JsExpression getPrototypeQualifierViaLookup(JDeclaredType type,
				SourceInfo sourceInfo) {
			if (closureCompilerFormatEnabled) {
				return getPrototypeQualifierOf(type, type.getSourceInfo());
			} else {
				// Construct JCHSU.getPrototypeFor(type).polyname
				// TODO(rluble): Ideally we would want to construct the
				// inheritance chain the JS way and
				// then we could do Type.prototype.polyname.call(this, ...).
				// Currently prototypes do not
				// have global names instead they are stuck into the
				// prototypesByTypeId array.
				return constructInvocation(sourceInfo,
						RuntimeConstants.RUNTIME_GET_CLASS_PROTOTYPE,
						(JsExpression) transform(
								getRuntimeTypeReference(type)));
			}
		}
		...
		private void handleClinit(JDeclaredType type,
				JsFunction clinitFunction) {
			clinitFunctionForType.put(type, clinitFunction);
			JDeclaredType superClass = type.getSuperClass();
			JsFunction superClinitFunction = superClass == null ? null
					: clinitFunctionForType.get(superClass.getClinitTarget());
			clinitFunction.setSuperClinit(superClinitFunction);
			List<JsStatement> statements = clinitFunction.getBody()
					.getStatements();
			SourceInfo sourceInfo = clinitFunction.getSourceInfo();
			// Self-assign to the global noop method immediately (to prevent
			// reentrancy). In incremental
			// mode the more costly Object constructor function is used as the
			// noop method since doing so
			// provides a better debug experience that does not step into
			// already used clinits.
			JsName emptyFunctionFnName = incremental
					? objectConstructorFunction.getName()
					: getIndexedMethodJsName(
							RuntimeConstants.RUNTIME_EMPTY_METHOD);
			JsExpression assignment = createAssignment(
					clinitFunction.getName().makeRef(sourceInfo),
					emptyFunctionFnName.makeRef(sourceInfo));
			statements.add(0, assignment.makeStmt());
		}

		/**
		 * If a field is a literal, we can potentially treat it as immutable and
		 * assign it once on the prototype, to be reused by all instances of the
		 * class, instead of re-assigning the same literal in each constructor.
		 */
		private boolean initializeAtTopScope(JField x) {
			if (x.getEnclosingType().isJsFunctionImplementation()) {
				// JsFunction implementation are plain JS functions with no
		...
		private void insertInTopologicalOrder(JDeclaredType type,
			...
			if (type == null || topologicallySortedSet.contains(type)
					|| program.isReferenceOnly(type)) {
				return;
			}
			insertInTopologicalOrder(type.getSuperClass(),
					topologicallySortedSet);
		...
		private void installClassLiterals(
			...
			for (JDeclaredType type : classLiteralTypesToInstall) {
				...
				SourceInfo sourceInfo = type.getSourceInfo();
				JsExpression protoRef = getPrototypeQualifierOf(type,
						sourceInfo);
				JsNameRef clazzField = getIndexedFieldJsName(
						RuntimeConstants.OBJECT_CLAZZ).makeRef(sourceInfo);
				clazzField.setQualifier(protoRef);
				JsExprStmt stmt = createAssignment(clazzField, classLiteralRef)
						.makeStmt();
		...
		/**
		 * Whether a method is a constructor that is actually newed. Note that
		 * in absence of whole world knowledge evey constructor is potentially
		 * live.
		 */
		...
		private boolean isMethodPotentiallyCalledAcrossClasses(JMethod method) {
			...
			return crossClassTargets == null
					|| crossClassTargets.contains(method)
		...
		private void maybeCopyJavaLangObjectProperties(JDeclaredType type,
				JsExpression javaLangObjectPrototype,
				JsExpression toPrototype) {
			if (getSuperPrototype(type) != null
					&& !type.isJsFunctionImplementation()) {
				JsStatement statement = constructInvocation(
						type.getSourceInfo(),
						RuntimeConstants.RUNTIME_COPY_OBJECT_PROPERTIES,
						javaLangObjectPrototype, toPrototype).makeStmt();
		...
		private JsInvocation maybeCreateClinitCall(JField x) {
			if (!x.isStatic() || x.isCompileTimeConstant()) {
				// Access to compile time constants do not trigger class
				// initialization (JLS 12.4.1).
				return null;
			}
			JDeclaredType targetType = x.getEnclosingType().getClinitTarget();
			if (targetType == null
					|| targetType == program.getTypeClassLiteralHolder()
					// When currentMethod == null, the clinit is being hoisted
					// to the global scope.
					|| (currentMethod != null && !currentMethod
							.getEnclosingType().checkClinitTo(targetType))) {
				return null;
			}
			JMethod clinitMethod = targetType.getClinitMethod();
			SourceInfo sourceInfo = x.getSourceInfo();
			return new JsInvocation(sourceInfo,
					names.get(clinitMethod).makeRef(sourceInfo));
		}
		...
		private JsInvocation maybeCreateClinitCall(JMethod method) {
			...
			JMethod clinitMethod = enclosingType.getClinitTarget()
					.getClinitMethod();
			SourceInfo sourceInfo = method.getSourceInfo();
			return new JsInvocation(sourceInfo,
					names.get(clinitMethod).makeRef(sourceInfo));
		...
		private void maybeGenerateClassLiteral(JType type) {
			...
			// TODO(rluble): refactor so that all output related to a class is
			// decided together.
			if (type != null && type instanceof JDeclaredType
					&& program.isReferenceOnly((JDeclaredType) type)) {
				// Only generate class literals for classes in the current
				// module.
				// TODO(rluble): In separate compilation some class literals
				// will be duplicated, which if
				// not done with care might violate java semantics of
				// getClass(). There are class literals
				// for primitives and arrays. Currently, because they will be
				// assigned to the same field
				// the one defined later will be the one used and Java semantics
				// are preserved.
				return;
			}
		...
		private void maybeGenerateObjectMethodsAliases(JDeclaredType type) {
			if (type == program.getTypeJavaLangObject()) {
				// special: setup a "toString" alias for
				// java.lang.Object.toString()
				Set<JMethod> overridableJavaLangObjectMethods = ImmutableSet.of(
						program.getIndexedMethodOrNull(
								RuntimeConstants.OBJECT_EQUALS),
						program.getIndexedMethodOrNull(
								RuntimeConstants.OBJECT_HASHCODE),
						program.getIndexedMethodOrNull(
								RuntimeConstants.OBJECT_TO_STRING));
				for (JMethod method : type.getMethods()) {
					if (overridableJavaLangObjectMethods.contains(method)) {
						JsName methodJsName = objectScope
								.declareUnobfuscatableName(method.getName());
		...
		private void maybeHoistClinit(Set<JDeclaredType> hoistedClinits,
				JMember member) {
			JDeclaredType enclosingType = member.getEnclosingType();
			if (hoistedClinits.contains(enclosingType)) {
				return;
			}
			JsInvocation clinitCall = member instanceof JMethod
					? maybeCreateClinitCall((JMethod) member)
		...
		private JsExprStmt outputDisplayName(JsNameRef function,
				JMethod method) {
			JsNameRef displayName = new JsNameRef(function.getSourceInfo(),
					"displayName");
			displayName.setQualifier(function);
			String displayStringName = getDisplayName(method);
			JsStringLiteral displayMethodName = new JsStringLiteral(
					function.getSourceInfo(), displayStringName);
		 ...
		 */
		private void setupCastMapForUnboxedType(JDeclaredType type,
				String castMapField) {
			// Cast.[castMapName] = /* cast map */ { ..:1, ..:1}
			JsName castableTypeMapName = getIndexedFieldJsName(castMapField);
			JsNameRef castMapVarRef = castableTypeMapName
					.makeRef(type.getSourceInfo());
			JsExpression castMapLiteral = generateCastableTypeMap(type);
			addTypeDefinitionStatement(type,
					createAssignment(castMapVarRef, castMapLiteral).makeStmt());
		}

		private void setupCastMapOnPrototype(JDeclaredType type) {
			JsExpression castMap = generateCastableTypeMap(type);
			generatePrototypeAssignmentForJavaField(type,
					"Object.castableTypeMap", castMap);
		}

		/**
		 * Sets up gwtOnLoad bootstrapping code. Unusually, the created code is
		 * executed as part of source loading and runs in the global scope (not
		 * inside of any function scope).
		 */
		private void setupGwtOnLoad() {
			...
			JsStatement entryVars = constructFunctionCallStatement(
					topScope.declareName("$entry"),
					"ModuleUtils.registerEntry");
			getGlobalStatements().add(entryVars);
			// var gwtOnLoad = ModuleUtils.gwtOnLoad;
			JsName gwtOnLoad = topScope
					.findExistingUnobfuscatableName("gwtOnLoad");
			JsVar varGwtOnLoad = new JsVar(sourceInfo, gwtOnLoad);
			varGwtOnLoad
					.setInitExpr(createAssignment(gwtOnLoad.makeRef(sourceInfo),
							getIndexedMethodJsName(
									RuntimeConstants.MODULE_UTILS_GWT_ON_LOAD)
											.makeRef(sourceInfo)));
		...
		private void
				setupTypeMarkerOnJavaLangObjectPrototype(JDeclaredType type) {
			JsName typeMarkerMethod = getIndexedMethodJsName(
					RuntimeConstants.RUNTIME_TYPE_MARKER_FN);
			generatePrototypeAssignmentForJavaField(type,
					RuntimeConstants.OBJECT_TYPEMARKER,
		...
		private boolean shouldNotEmitTypeDefinition(JDeclaredType type) {
			// Interfaces, Unboxed Types, JSOs, Native Types, JsFunction, and
			// uninstantiated types
			// Do not have vtables/prototype setup
			return type instanceof JInterfaceType
					&& !closureCompilerFormatEnabled
	...
	private static class JavaToJsOperatorMap {
		private static final Map<JBinaryOperator, JsBinaryOperator> bOpMap = Maps
				.newEnumMap(JBinaryOperator.class);

		private static final Map<JUnaryOperator, JsUnaryOperator> uOpMap = Maps
				.newEnumMap(JUnaryOperator.class);
		static {
	 ...
	 * <p>
	 * <ul>
	 * <li>1. whether a constructors are live directly (through being in a new
	 * operation) or indirectly (only called by other constructors). Only
	 * directly live constructors become JS constructor, otherwise they will
	 * behave like regular static functions.</li> 2. whether there exists cross
	 * class (static) calls or accesses that would need clinits to be triggered.
	 * If not clinits need only be called in constructors.
	 * <li></li>
	 * </ul>
	 */
	private class RecordCrossClassCallsAndConstructorLiveness extends JVisitor {
		// TODO(rluble): This analysis should be extracted from
		// GenerateJavaScriptAST into its own
		// JAVA optimization pass. Constructors that are not newed can be
		// transformed into statified
		// regular methods; and methods that are not called from outside the
		// class boundary can be
		// privatized. Currently we do not use the private modifier to avoid
		// emitting clinits, instead
		// we use the result of this analysis (private methods CAN be called
		// from JSNI in an unrelated
		// class, touche!).
		{
		...
		@Override
		public void endVisit(JMethod x, Context ctx) {
			// methods which are exported or static indexed methods may be
			// called externally
			if (x.isJsInteropEntryPoint() || (x.isStatic()
					&& program.getIndexedMethods().contains(x))) {
	...
	private static class SortVisitor extends JVisitor {
		...
		@Override
		public void endVisit(JProgram x, Context ctx) {
			Collections.sort(x.getEntryMethods(),
