...
/**
 * A base for classes that compile Java <code>JProgram</code> representations
 * into corresponding Js source.<br />
 *
 * Work is split between a precompile() stage which is only called once and
 * compilePerms() stage which is called once per permutation. This allow build
 * systems the option of distributing and parallelizing some of the work.
 */
public final class JavaToJavaScriptCompiler {
	/**
	 * Ending optimization passes when the rate of change has reached this value
	 * results in gaining nearly all of the impact while avoiding the long tail
	 * of costly but low-impact passes.
	 */
	private static final float EFFICIENT_CHANGE_RATE = 0.01f;

	/**
	 * Continuing to apply optimizations till the rate of change reaches this
	 * value causes the AST to reach a fixed point.
	 */
	private static final int FIXED_POINT_CHANGE_RATE = 0;

	/**
	 * Limits the number of optimization passes against the possible danger of
	 * an AST that does not converge.
	 */
	private static final int MAX_PASSES = 100;
	static {
		// Preload the internal compiler exception just in case we run out of
		// memory?.
	 ...
	 *            shared read only compiler state
	 * @param permutation
	 *            the permutation to compile
	 * @return the permutation result
	 * @throws UnableToCompleteException
	 *             if an error other than {@link OutOfMemoryError} occurs
	 */
	public static PermutationResult compilePermutation(UnifiedAst unifiedAst,
			TreeLogger logger, CompilerContext compilerContext,
			Permutation permutation) throws UnableToCompleteException {
		JavaToJavaScriptCompiler javaToJavaScriptCompiler = new JavaToJavaScriptCompiler(
				logger, compilerContext);
		return javaToJavaScriptCompiler.compilePermutation(permutation,
				unifiedAst);
	}
	...
	public static UnifiedAst precompile(TreeLogger logger,
			...
			throws UnableToCompleteException {
		return new JavaToJavaScriptCompiler(logger, compilerContext)
				.precompile(precompilationContext);
	}
	...
	private CompilationMetricsArtifact addCompilerMetricsArtifact(
			UnifiedAst unifiedAst, Permutation permutation, long startTimeMs,
		...
		if (options.isCompilerMetricsEnabled()) {
			compilationMetrics = new CompilationMetricsArtifact(
					permutation.getId());
			...
			compilationMetrics.setPermutationDescription(
					permutation.getProperties().prettyPrint());
	...
	private void addSourceMapArtifacts(int permutationId,
		...
		if (options.isJsonSoycEnabled()) {
			...
			permutationResult.addArtifacts(EntityRecorder.makeSoycArtifacts(
					...
					options.getSourceMapFilePrefix(), jjsmap, sizeBreakdowns,
					((DependencyGraphRecorder) dependenciesAndRecorder
							.getRight()),
					jprogram));
		} else if (isSourceMapsEnabled) {
			logger.log(TreeLogger.INFO, "Source Maps Enabled");
			permutationResult.addArtifacts(SourceMapRecorder.exec(permutationId,
	...
	private void addSoycArtifacts(UnifiedAst unifiedAst, int permutationId,
		...
		permutationResult.addArtifacts(makeSoycArtifacts(permutationId, js,
				sizeBreakdowns, options.isSoycExtra() ? sourceInfoMaps : null,
				dependenciesAndRecorder.getLeft(), jjsmap,
				internedLiteralByVariableName, unifiedAst.getModuleMetrics(),
				unifiedAst.getPrecompilationMetrics(), compilationMetrics,
				options.isSoycHtmlDisabled()));
	}

	private void addSyntheticArtifacts(UnifiedAst unifiedAst,
			Permutation permutation, long startTimeMs, int permutationId,
			JavaToJavaScriptMap jjsmap,
			Pair<SyntheticArtifact, MultipleDependencyGraphRecorder> dependenciesAndRecorder,
			Map<JsName, JsLiteral> internedLiteralByVariableName,
			boolean isSourceMapsEnabled, String[] jsFragments,
			SizeBreakdown[] sizeBreakdowns, List<JsSourceMap> sourceInfoMaps,
			PermutationResult permutationResult)
			throws IOException, UnableToCompleteException {
		assert internedLiteralByVariableName != null;
		Event event = SpeedTracerLogger
				.start(CompilerEventType.PERMUTATION_ARTIFACTS);
		CompilationMetricsArtifact compilationMetrics = addCompilerMetricsArtifact(
				unifiedAst, permutation, startTimeMs, sizeBreakdowns,
				permutationResult);
		addSoycArtifacts(unifiedAst, permutationId, jjsmap,
				dependenciesAndRecorder, internedLiteralByVariableName,
				jsFragments, sizeBreakdowns, sourceInfoMaps, permutationResult,
				compilationMetrics);
		addSourceMapArtifacts(permutationId, jjsmap, dependenciesAndRecorder,
				isSourceMapsEnabled, sizeBreakdowns, sourceInfoMaps,
				permutationResult);
	...
	/**
	 * Creates (and returns the name for) a new class to serve as the container
	 * for the invocation of registered entry point methods as part of module
	 * bootstrapping.<br />
	 *
	 * The resulting class will be invoked during bootstrapping like
	 * FooEntryMethodHolder.init(). By generating the class on the fly and
	 * naming it to match the current module, the resulting holder class can
	 * work in both monolithic and separate compilation schemes.
	 */
	private String buildEntryMethodHolder(StandardGeneratorContext context,
		...
		if (entryPointTypeNames.length == 0) {
			// Then there's no need to generate an EntryMethodHolder class to
			// launch them.
			return null;
		}
		EntryMethodHolderGenerator entryMethodHolderGenerator = new EntryMethodHolderGenerator();
		String entryMethodHolderTypeName = entryMethodHolderGenerator
				.generate(logger, context, module.getCanonicalName());
		context.finish(logger);
		// Ensures that unification traverses and keeps the class.
		allRootTypes.add(entryMethodHolderTypeName);
		// Ensures that JProgram knows to index this class's methods so that
		// later bootstrap
		// construction code is able to locate the FooEntryMethodHolder.init()
		// function.
		jprogram.addIndexedTypeName(entryMethodHolderTypeName);
		return entryMethodHolderTypeName;
	}

	private MultipleDependencyGraphRecorder
			chooseDependencyRecorder(OutputStream out) {
	 ...
	 * result classes are available and have not yet been pruned down to the set
	 * applicable for a particular permutation) that was previously constructed
	 * by the Precompiler and from that constructs output Js source code and
	 * related information. This Js source and related information is packaged
	 * into a Permutation instance and then returned.
	 *
	 ...
	 * </pre>
	 *
	 * There are some other types of work here (mostly metrics and data
	 * gathering) which do not serve the goal of output program construction.
	 * This work should really be moved into subclasses or some sort of callback
	 * or plugin system so as not to visually pollute the real compile
	 * logic.<br />
	 *
	 * Significant amounts of visitors implementing the intended above stages
	 * are triggered here but in the wrong order. They have been noted for
	 * future cleanup.
	 */
	private PermutationResult compilePermutation(Permutation permutation,
			UnifiedAst unifiedAst) throws UnableToCompleteException {
		Event jjsCompilePermutationEvent = SpeedTracerLogger.start(
				CompilerEventType.JJS_COMPILE_PERMUTATION, "name",
				permutation.getProperties().prettyPrint());
		/*
		 * Do not introduce any new pass here unless it is logically a part of
		 * one of the 9 defined stages and is physically located in that stage.
		 */
		long permStartMs = System.currentTimeMillis();
		try {
			Event javaEvent = SpeedTracerLogger
					.start(CompilerEventType.PERMUTATION_JAVA);
			// (1) Initialize local state.
			...
			logger.log(TreeLogger.INFO,
					"Compiling permutation " + permutationId + "...");
			// (2) Transform unresolved Java AST to resolved Java AST
			ResolvePermutationDependentValues.exec(jprogram, properties,
					permutation.getPropertyAndBindingInfos());
			// TODO(stalcup): hide metrics gathering in a callback or subclass
			// This has to happen before optimizations because functions might
			// be optimized out; we want those marked as "not executed", not
			// "not
			// instrumentable".
			...
			// type->type references need to be collected in two phases, 1)
			// before any process to the
			// AST has happened (to record for example reference to types
			// declaring compile-time
			// constants) and 2) after all normalizations to collect synthetic
			// references (e.g. to
			// record references to runtime classes like LongLib).
			maybeRecordReferencesAndControlFlow(false);
			// Rewrite calls to from boxed constructor types to specialized
			// unboxed methods
			...
			// Now that the AST has stopped mutating update with the final
			// references.
			maybeRecordReferencesAndControlFlow(true);
			javaEvent.end();
			Event javaScriptEvent = SpeedTracerLogger
					.start(CompilerEventType.PERMUTATION_JAVASCRIPT);
			// (5) Construct the Js AST
			Pair<? extends JavaToJavaScriptMap, Set<JsNode>> jjsMapAndInlineableFunctions = GenerateJavaScriptAST
					.exec(logger, jprogram, jsProgram, compilerContext,
							typeMapper, symbolTable, properties);
			JavaToJavaScriptMap jjsmap = jjsMapAndInlineableFunctions.getLeft();
			// TODO(stalcup): hide metrics gathering in a callback or subclass
			if (CoverageInstrumentor.isCoverageEnabled()) {
				CoverageInstrumentor.exec(jprogram, jsProgram, jjsmap,
						instrumentableLines);
			}
			// (6) Normalize the Js AST
			...
			Pair<SyntheticArtifact, MultipleDependencyGraphRecorder> dependenciesAndRecorder = splitJsIntoFragments(
					properties, permutationId, jjsmap);
			// TODO(stalcup): move to normalization
			EvalFunctionsAtTopScope.exec(jsProgram, jjsmap);
			// (7) Optimize the JS AST.
			final Set<JsNode> inlinableJsFunctions = jjsMapAndInlineableFunctions
					.getRight();
			...
			Map<JsName, JsLiteral> internedLiteralByVariableName = renameJsSymbols(
					properties, jjsmap);
			...
			// TODO(stalcup): move to normalization
			JsBreakUpLargeVarStatements.exec(jsProgram,
					properties.getConfigurationProperties());
			if (!options.isIncrementalCompileEnabled()) {
				// Verifies consistency between jsProgram and jjsmap if
			...
			boolean isSourceMapsEnabled = properties
					.isTrueInAnyPermutation("compiler.useSourceMaps");
			String[] jsFragments = new String[jsProgram.getFragmentCount()];
			StatementRanges[] ranges = new StatementRanges[jsFragments.length];
			SizeBreakdown[] sizeBreakdowns = options.isJsonSoycEnabled()
					|| options.isSoycEnabled()
					|| options.isCompilerMetricsEnabled()
							? new SizeBreakdown[jsFragments.length]
							: null;
			generateJavaScriptCode(jjsmap, jsFragments, ranges, sizeBreakdowns,
					sourceInfoMaps,
					isSourceMapsEnabled || options.isJsonSoycEnabled());
			javaScriptEvent.end();
			// (9) Construct and return a value
			PermutationResult permutationResult = new PermutationResultImpl(
					jsFragments, permutation, makeSymbolMap(symbolTable),
					ranges);
			// TODO(stalcup): hide metrics gathering in a callback or subclass
			addSyntheticArtifacts(unifiedAst, permutation, startTimeMs,
					permutationId, jjsmap, dependenciesAndRecorder,
					internedLiteralByVariableName, isSourceMapsEnabled,
			...
			if (logger.isLoggable(TreeLogger.TRACE)) {
				logger.log(TreeLogger.TRACE, "Permutation took "
						+ (System.currentTimeMillis() - permStartMs) + " ms");
			}
		}
	}
	...
	private Set<String> computeRootTypes(String[] entryPointTypeNames,
		...
		Iterables.addAll(allRootTypes,
				compilationState.getQualifiedJsInteropRootTypesNames());
		...
		for (com.google.gwt.core.ext.typeinfo.JClassType singleJsoIntf : typeOracle
			...
			allRootTypes.add(typeOracle.getSingleJsoImpl(singleJsoIntf)
	...
	private CompilationState
			constructJavaAst(PrecompilationContext precompilationContext)
					throws UnableToCompleteException {
		RebindPermutationOracle rpo = precompilationContext
				.getRebindPermutationOracle();
	...
	/**
	 * Generate Js code from the given Js ASTs. Also produces information about
	 * that transformation.
	 */
	...
	private void logAstTypeMetrics(
	...
	private void logTypeOracleMetrics(
		...
		if (precompilationMetrics != null) {
			...
			for (com.google.gwt.core.ext.typeinfo.JClassType type : compilationState
					.getTypeOracle().getTypes()) {
				finalTypeOracleTypes.add(
	...
	private Collection<? extends Artifact<?>> makeSoycArtifacts(
			int permutationId, String[] js, SizeBreakdown[] sizeBreakdowns,
			List<JsSourceMap> sourceInfoMaps, SyntheticArtifact dependencies,
			JavaToJavaScriptMap jjsmap,
			Map<JsName, JsLiteral> internedLiteralByVariableName,
			ModuleMetricsArtifact moduleMetricsArtifact,
			PrecompilationMetricsArtifact precompilationMetricsArtifact,
			CompilationMetricsArtifact compilationMetrics,
			boolean htmlReportsDisabled)
			throws IOException, UnableToCompleteException {
		...
		Event soycEvent = SpeedTracerLogger
				.start(CompilerEventType.MAKE_SOYC_ARTIFACTS);
		...
		SyntheticArtifact splitPoints = new SyntheticArtifact(
				SoycReportLinker.class,
				"splitPoints" + permutationId + ".xml.gz", baos.toByteArray());
		...
		if (sizeBreakdowns != null) {
			...
			sizeMaps = new SyntheticArtifact(SoycReportLinker.class,
					"stories" + permutationId + ".xml.gz", baos.toByteArray());
	...
	/**
	 * Adds generated artifacts from previous compiles when doing per-file
	 * compiles.
	 * <p>
	 * All generators are run on first compile but only some very small subset
	 * are rerun on recompiles. Care must be taken to ensure that all generated
	 * artifacts (such as png/html/css files) are still registered for output
	 * even when no generators are run in the current compile.
	 */
	private void
			maybeAddGeneratedArtifacts(PermutationResult permutationResult) {
		if (options.isIncrementalCompileEnabled()) {
			permutationResult.addArtifacts(compilerContext
					.getMinimalRebuildCache().getGeneratedArtifacts());
		}
	}
	...
	private Map<JsName, JsLiteral> maybeInternLiterals(int interningMask) {
		...
		// Only perform the interning optimization when optimizations are
		// enabled.
		if (options.isClosureCompilerFormatEnabled()) {
			// Do no intern strings in closure format as it breaks
			// goog.provides, etc.
	...
	private void maybeRecordReferencesAndControlFlow(boolean onlyUpdate) {
		if (options.isIncrementalCompileEnabled()) {
			// Per file compilation needs the type reference graph to construct
			// the set of reachable
			// types when linking.
			TypeReferencesRecorder.exec(jprogram, getMinimalRebuildCache(),
					onlyUpdate);
			ControlFlowRecorder.exec(jprogram,
					getMinimalRebuildCache().getTypeEnvironment(), onlyUpdate);
		}
	}
	...
	private OptimizerStats optimizeJavaOneTime(String passName, int numNodes,
		...
		Event optimizeEvent = SpeedTracerLogger
				.start(CompilerEventType.OPTIMIZE, "phase", "loop");
		// Clinits might have become empty become empty.
		jprogram.typeOracle
				.recomputeAfterOptimizations(jprogram.getDeclaredTypes());
		OptimizerStats stats = new OptimizerStats(passName);
		JavaAstVerifier.assertProgramIsConsistent(jprogram);
		stats.add(Pruner.exec(jprogram, true, optimizerCtx)
				.recordVisits(numNodes));
		stats.add(
				Finalizer.exec(jprogram, optimizerCtx).recordVisits(numNodes));
		stats.add(MakeCallsStatic
				.exec(jprogram, options.shouldAddRuntimeChecks(), optimizerCtx)
				.recordVisits(numNodes));
		stats.add(TypeTightener.exec(jprogram, optimizerCtx)
				.recordVisits(numNodes));
		stats.add(MethodCallTightener.exec(jprogram, optimizerCtx)
				.recordVisits(numNodes));
		// Note: Specialization should be done before inlining.
		stats.add(MethodCallSpecializer.exec(jprogram, optimizerCtx)
				.recordVisits(numNodes));
		stats.add(DeadCodeElimination.exec(jprogram, optimizerCtx)
				.recordVisits(numNodes));
		stats.add(MethodInliner.exec(jprogram, optimizerCtx)
				.recordVisits(numNodes));
		...
		if (options.shouldOrdinalizeEnums()) {
			stats.add(EnumOrdinalizer.exec(jprogram, optimizerCtx)
					.recordVisits(numNodes));
		}
		optimizeEvent.end();
		return stats;
	...
	private void optimizeJavaToFixedPoint() throws InterruptedException {
		Event optimizeEvent = SpeedTracerLogger
				.start(CompilerEventType.OPTIMIZE);
		List<OptimizerStats> allOptimizerStats = Lists.newArrayList();
		int passCount = 0;
		int nodeCount = jprogram.getNodeCount();
		int lastNodeCount;
		boolean atMaxLevel = options
				.getOptimizationLevel() == OptionOptimize.OPTIMIZE_LEVEL_MAX;
		int passLimit = atMaxLevel ? MAX_PASSES
				: options.getOptimizationLevel();
		float minChangeRate = atMaxLevel ? FIXED_POINT_CHANGE_RATE
				: EFFICIENT_CHANGE_RATE;
		...
		while (true) {
			...
			OptimizerStats stats = optimizeJavaOneTime("Pass " + passCount,
					nodeCount, optimizerCtx);
			...
			float sizeChangeRate = (lastNodeCount - nodeCount)
					/ (float) lastNodeCount;
			if (nodeChangeRate <= minChangeRate
					&& sizeChangeRate <= minChangeRate) {
				break;
			}
		}
	...
	private void optimizeJsLoop(Collection<JsNode> toInline)
		...
		while (true) {
			...
			Event optimizeJsEvent = SpeedTracerLogger
					.start(CompilerEventType.OPTIMIZE_JS);
			...
			if ((optimizationLevel < OptionOptimize.OPTIMIZE_LEVEL_MAX
					&& counter > optimizationLevel) || !stats.didChange()) {
				break;
			}
		}
	...
	private void postNormalizationOptimizeJava() {
		Event event = SpeedTracerLogger
				.start(CompilerEventType.JAVA_POST_NORMALIZER_OPTIMIZERS);
		try {
			if (shouldOptimize()) {
	...
	/**
	 * Takes as input a CompilationState and transforms that into a unified by
	 * not yet resolved Java AST (a Java AST wherein cross-class references have
	 * been connected and all rebind result classes are available and have not
	 * yet been pruned down to the set applicable for a particular permutation).
	 * This AST is packaged into a UnifiedAst instance and then returned.
	 *
	...
	private void recordJsoTypes(TypeOracle typeOracle) {
		...
		for (com.google.gwt.dev.javac.typemodel.JClassType subtype : typeOracle
				.getJavaScriptObject().getSubtypes()) {
			jsoTypeNames.add(subtype.getQualifiedBinaryName());
		}
		// Add names of interfaces that are always of a JSO (aka there are no
		// non-JSO implementors).
		Set<String> singleJsoImplInterfaceNames = Sets.newHashSet();
		for (com.google.gwt.core.ext.typeinfo.JClassType singleJsoImplInterface : typeOracle
				.getSingleJsoImplInterfaces()) {
			singleJsoImplInterfaceNames
					.add(singleJsoImplInterface.getQualifiedBinaryName());
		}
		// Add names of interfaces that are only sometimes a JSO (aka there are
		// both JSO and non-JSO
		// imlementors).
		Set<String> dualJsoImplInterfaceNames = Sets.newHashSet();
		for (com.google.gwt.core.ext.typeinfo.JClassType dualJsoImplInterface : typeOracle
				.getDualJsoImplInterfaces()) {
			dualJsoImplInterfaceNames
					.add(dualJsoImplInterface.getQualifiedBinaryName());
		}
	...
	private Map<JsName, JsLiteral> renameJsSymbols(
		...
		try {
			...
			case OBFUSCATED:
				internedLiteralByVariableName = runObfuscateNamer(options,
						properties, jjsmap);
				break;
			case PRETTY:
				internedLiteralByVariableName = runPrettyNamer(options,
						properties, jjsmap);
				break;
			case DETAILED:
				internedLiteralByVariableName = runDetailedNamer(
						properties.getConfigurationProperties());
				break;
			default:
		...
		return internedLiteralByVariableName == null
				? ImmutableMap.<JsName, JsLiteral> of()
				: internedLiteralByVariableName;
	}

	private Map<JsName, JsLiteral> runDetailedNamer(
			ConfigurationProperties config) throws IllegalNameException {
	...
	private void runIncrementalNamer(JJSOptions options,
		...
		JsIncrementalNamer.exec(jsProgram, configurationProperties,
				...
				jjsmap, options.getOutput() == JsOutputOption.OBFUSCATED);
	}
	...
	private Map<JsName, JsLiteral> runObfuscateNamer(JJSOptions options,
		...
		if (options.isIncrementalCompileEnabled()) {
			runIncrementalNamer(options,
					properties.getConfigurationProperties(), jjsmap);
			return null;
		}
		...
		FreshNameGenerator freshNameGenerator = JsObfuscateNamer.exec(jsProgram,
				properties.getConfigurationProperties());
		if (options.shouldRemoveDuplicateFunctions() && JsStackEmulator
	...
	private Map<JsName, JsLiteral> runPrettyNamer(JJSOptions options,
		...
		Map<JsName, JsLiteral> internedLiteralByVariableName = maybeInternLiterals(
				JsLiteralInterner.INTERN_ALL
						& ~JsLiteralInterner.INTERN_STRINGS);
	...
	private Pair<SyntheticArtifact, MultipleDependencyGraphRecorder>
			splitJsIntoFragments(PermutationProperties properties,
					int permutationId, JavaToJavaScriptMap jjsmap) {
		Pair<SyntheticArtifact, MultipleDependencyGraphRecorder> dependenciesAndRecorder;
		...
		if (options.isRunAsyncEnabled()) {
			...
			CodeSplitter.exec(logger, jprogram, jsProgram, jjsmap,
					expectedFragmentCount, minFragmentSize, dependencyRecorder);
			if (baos.size() == 0) {
			...
			dependencyRecorder = recordNonSplitDependencies(
	...
	private void synthesizeEntryMethodHolderInit(UnifyAst unifyAst,
			String[] entryPointTypeNames, String entryMethodHolderTypeName)
			throws UnableToCompleteException {
		// Get type references.
		JDeclaredType entryMethodHolderType = unifyAst.findType(
				entryMethodHolderTypeName,
				unifyAst.getSourceNameBasedTypeLocator());
		JDeclaredType gwtType = unifyAst.findType(
				"com.google.gwt.core.client.GWT",
				unifyAst.getSourceNameBasedTypeLocator());
		JDeclaredType entryPointType = unifyAst.findType(
				"com.google.gwt.core.client.EntryPoint",
				unifyAst.getSourceNameBasedTypeLocator());
		// Get method references.
		JMethod initMethod = entryMethodHolderType.findMethod("init()V", false);
		JMethod gwtCreateMethod = gwtType.findMethod(
				"create(Ljava/lang/Class;)Ljava/lang/Object;", false);
		// Synthesize all onModuleLoad() calls.
		JBlock initMethodBlock = ((JMethodBody) initMethod.getBody())
				.getBlock();
		SourceInfo origin = initMethodBlock.getSourceInfo().makeChild();
		for (String entryPointTypeName : entryPointTypeNames) {
			// Get type and onModuleLoad function for the current
			// entryPointTypeName.
			JDeclaredType specificEntryPointType = unifyAst.findType(
					entryPointTypeName,
					unifyAst.getSourceNameBasedTypeLocator());
			if (specificEntryPointType == null) {
				logger.log(TreeLogger.ERROR,
						"Could not find module entry point class '"
								+ entryPointTypeName + "'",
						null);
				throw new UnableToCompleteException();
			}
			JMethod onModuleLoadMethod = entryPointType
					.findMethod("onModuleLoad()V", true);
			JMethod specificOnModuleLoadMethod = specificEntryPointType
					.findMethod("onModuleLoad()V", true);
			if (specificOnModuleLoadMethod != null
					&& specificOnModuleLoadMethod.isStatic()) {
				// Synthesize a static invocation FooEntryPoint.onModuleLoad();
				// call.
				JMethodCall staticOnModuleLoadCall = new JMethodCall(origin,
						null, specificOnModuleLoadMethod);
				initMethodBlock.addStmt(staticOnModuleLoadCall.makeStatement());
			} else {
				// Synthesize
				// ((EntryPoint)GWT.create(FooEntryPoint.class)).onModuleLoad();
				JClassLiteral entryPointTypeClassLiteral = new JClassLiteral(
						origin, specificEntryPointType);
				JMethodCall createInstanceCall = new JMethodCall(origin, null,
						gwtCreateMethod, entryPointTypeClassLiteral);
				JCastOperation castToEntryPoint = new JCastOperation(origin,
						entryPointType, createInstanceCall);
				JMethodCall instanceOnModuleLoadCall = new JMethodCall(origin,
						castToEntryPoint, onModuleLoadMethod);
				initMethodBlock
						.addStmt(instanceOnModuleLoadCall.makeStatement());
			}
		}
	}
	...
	private void unifyJavaAst(PrecompilationContext precompilationContext)
		...
		String[] additionalRootTypes = precompilationContext
				.getAdditionalRootTypes();
		Set<String> allRootTypes = computeRootTypes(entryPointTypeNames,
				additionalRootTypes, rpo.getCompilationState());
		String entryMethodHolderTypeName = buildEntryMethodHolder(
				rpo.getGeneratorContext(), entryPointTypeNames, allRootTypes);
		UnifyAst unifyAst = new UnifyAst(logger, compilerContext, jprogram,
				jsProgram, precompilationContext);
		// Makes JProgram aware of these types so they can be accessed via
		// index.
		unifyAst.addRootTypes(allRootTypes);
		// Must synthesize entryPoint.onModuleLoad() calls because some
		// EntryPoint classes are
		// private.
		if (entryMethodHolderTypeName != null) {
			// Only synthesize the init method in the EntryMethodHolder class,
			// if there is an
			// EntryMethodHolder class.
			synthesizeEntryMethodHolderInit(unifyAst, entryPointTypeNames,
					entryMethodHolderTypeName);
		}
		if (entryMethodHolderTypeName != null) {
			// Only register the init method in the EntryMethodHolder class as
			// an entry method, if there
			// is an EntryMethodHolder class.
			jprogram.addEntryMethod(jprogram.getIndexedMethod(
					SourceName.getShortClassName(entryMethodHolderTypeName)
							+ ".init"));
		}
		unifyAst.exec();
		event.end();
	...
	private static class PermutationResultImpl implements PermutationResult {
		...
		public PermutationResultImpl(String[] jsFragments,
				Permutation permutation, SymbolData[] symbolMap,
