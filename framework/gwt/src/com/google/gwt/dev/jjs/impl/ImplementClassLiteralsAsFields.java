  ...
  public enum ClassLiteralFactoryMethod {
    ...
    CREATE_FOR_CLASS() {
      ...
      JMethodCall createCall(SourceInfo info, JProgram program, JType type,
        ...
        boolean generateLambdaCall=false;
        if(type instanceof JDeclaredType ){
        	 NestedClassDisposition disposition = ((JDeclaredType)type).getClassDisposition();
        	 switch(disposition){
        	 case LAMBDA:
        		 //not anonymous, since can inherit from non-java.lang.Object
//        	 case ANONYMOUS:
        		 generateLambdaCall=true;
        		 break;
        	 }
        }
        
        if(generateLambdaCall){
        	// Class.createForAnonymousClass( runtimeTypeReference)
	        JMethodCall call = new JMethodCall(info, null, program.getIndexedMethod(RuntimeConstants.CLASS_CREATE_FOR_LAMBDA));
	        call.addArg(new JRuntimeTypeReference(info, program.getTypeJavaLangObject(),
	            (JReferenceType) type));
	        return call;
        }else{
