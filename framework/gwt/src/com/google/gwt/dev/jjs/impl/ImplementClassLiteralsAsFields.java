 ...
 *
 * Class literal factory methods are responsible for installing references to
 * themselves on the Object.clazz field of their JS runtime prototype since
 * getClass() is no longer an overridden method. Prototypes can be looked up via
 * 'typeId' from the global prototypesByTypeId object, and so each class literal
 * factory method is passed the typeId of its type.
 * <p>
 */
public class ImplementClassLiteralsAsFields {
	private static Map<Class<? extends JType>, ClassLiteralFactoryMethod> literalFactoryMethodByTypeClass = new ImmutableMap.Builder()
			...
			.put(JInterfaceType.class,
					ClassLiteralFactoryMethod.CREATE_FOR_INTERFACE)
			.put(JPrimitiveType.class,
					ClassLiteralFactoryMethod.CREATE_FOR_PRIMITIVE)
			.build();

	public static void exec(JProgram program, boolean shouldOptimize) {
		Event normalizerEvent = SpeedTracerLogger
				.start(CompilerEventType.NORMALIZER);
	...
	private static String getClassLiteralFieldName(JType type) {
		return JjsUtils.classLiteralFieldNameFromJavahTypeSignatureName(
				type.getJavahSignatureName());
	}

	private final Map<JType, JField> classLiteralFields = Maps
			.newIdentityHashMap();
	...
	private ImplementClassLiteralsAsFields(JProgram program,
		...
		this.classLiteralHolderClinitBody = (JMethodBody) typeClassLiteralHolder
				.getClinitMethod().getBody();
		this.shouldOptimize = shouldOptimize;
		assert program.getDeclaredTypes().contains(typeClassLiteralHolder);
	}

	private JClassLiteral createDependentClassLiteral(SourceInfo info,
			JType type) {
		JClassLiteral classLiteral = new JClassLiteral(info.makeChild(), type);
		classLiteral
				.setField(resolveClassLiteralField(classLiteral.getRefType()));
		return classLiteral;
	}

	/**
	 * Create an expression that will evaluate, at run time, to the class
	 * literal. Causes recursive literal create (super type, array element
	 * type). Examples:
	 *
	 * Class:
	 *
	 ...
	 * </pre>
	 */
	private JMethodCall createLiteralCall(SourceInfo info, JProgram program,
			JType type) {
		...
		return literalFactoryMethodByTypeClass.get(typeClass).createCall(info,
				program, type, getSuperclassClassLiteral(info, type));
	}

	private void execImpl() {
		if (!shouldOptimize) {
			// Create all class literals regardless of whether they are
			// referenced or not.
	...
	private JLiteral getSuperclassClassLiteral(SourceInfo info, JType type) {
		if (!(type instanceof JClassType)
				|| ((JClassType) type).getSuperClass() == null) {
			return JNullLiteral.INSTANCE;
		}
		JClassType superClass = ((JClassType) type).getSuperClass();
		if (superClass.isJsNative()) {
			// Class object for subclasses of native JsType will return
			// Object.class as their super class
			// class literal; this is done so that in invariant that "super"
			// class literals are literals
	...
	private JField resolveClassLiteralField(JType type) {
		type = type.isJsNative() || type.isJsFunction()
				|| type.isJsFunctionImplementation()
						? program.getJavaScriptObject()
						: program.normalizeJsoType(type);
		JField field = classLiteralFields.get(type);
		if (field == null) {
			// Create the allocation expression FIRST since this may be
			// recursive on
			// super type (this forces the super type classLit to be created
			// first).
			SourceInfo info = type.getSourceInfo().makeChild();
			assert !(type instanceof JArrayType);
			JMethodCall classLiteralCreationExpression = createLiteralCall(info,
					program, type);
			// Create a field in the class literal holder to hold the object.
			field = new JField(info, getClassLiteralFieldName(type),
					typeClassLiteralHolder, program.getTypeJavaLangClass(),
			...
			JFieldRef fieldRef = new JFieldRef(info, null, field,
					typeClassLiteralHolder);
			JDeclarationStatement decl = new JDeclarationStatement(info,
					fieldRef, classLiteralCreationExpression);
	...
	public enum ClassLiteralFactoryMethod {
		CREATE_FOR_ENUM() {
			private JExpression getStandardMethodAsArg(SourceInfo info,
				...
				if (enumType != type) {
					// The type is an anonymous subclass that represents one of
					// the enum values
					return JNullLiteral.INSTANCE;
				}
				// This type is the base enum type not one of the anonymous
				// classes that represent
				// enum values.
				for (JMethod method : enumType.getMethods()) {
					if (method.isStatic() && method.getSignature()
							.startsWith(methodSignature)) {
						return new JsniMethodRef(info,
								method.getJsniSignature(true, false), method,
								program.getJavaScriptObject());
					}
				}
		...
		CREATE_FOR_CLASS() {
			...
			JMethodCall createCall(SourceInfo info, JProgram program,
					JType type, JLiteral superclassLiteral) {
				boolean generateAnonymousCall = false;
				if (type instanceof JDeclaredType) {
					NestedClassDisposition disposition = ((JDeclaredType) type)
							.getClassDisposition();
					switch (disposition) {
					case LAMBDA:
					case ANONYMOUS:
						generateAnonymousCall = true;
						break;
					}
				}
				if (generateAnonymousCall) {
					...
					// runtimeTypeReference, superclassliteral)
					JMethodCall call = createBaseCall(info, program, type,
							RuntimeConstants.CLASS_CREATE_FOR_CLASS);
					call.addArg(new JRuntimeTypeReference(info,
							program.getTypeJavaLangObject(),
							(JReferenceType) type));
					call.addArg(superclassLiteral);
		...
		CREATE_FOR_PRIMITIVE() {
			...
			JMethodCall createCall(SourceInfo info, JProgram program,
				...
				call.addArg(
						program.getStringLiteral(info, type.getShortName()));
				call.addArg(program.getStringLiteral(info,
						type.getJavahSignatureName()));
				return call;
			}
		...
		CREATE_FOR_INTERFACE() {
			...
			JMethodCall createCall(SourceInfo info, JProgram program,
				...
				return createBaseCall(info, program, type,
						RuntimeConstants.CLASS_CREATE_FOR_INTERFACE);
			}
		};

		private static JMethodCall createBaseCall(SourceInfo info,
				JProgram program, JType type, String indexedMethodName) {
			String[] compoundName = maybeMangleJSOTypeName(type);
			JMethodCall call = new JMethodCall(info, null,
					program.getIndexedMethod(indexedMethodName),
					program.getStringLiteral(info, type.getPackageName()),
					getCompoundNameLiteral(program, info, compoundName));
			return call;
		}

		private static JExpression getCompoundNameLiteral(
				final JProgram program, final SourceInfo info,
				String[] compoundName) {
			return program.getStringLiteral(info,
					Joiner.on('/').join(compoundName));
		}
		...
		private static String[] maybeMangleJSOTypeName(JType type) {
			...
			if (type.isJsoType()) {
				compoundName[compoundName.length
						- 1] = compoundName[compoundName.length - 1] + '$';
			}
			return compoundName;
		}

		abstract JMethodCall createCall(SourceInfo info, JProgram program,
				JType type, JLiteral superclassLiteral);
	}
	...
	private class NormalizeVisitor extends JModVisitor {
		...
		public void endVisit(JClassLiteral x, Context ctx) {
			...
			if (type instanceof JArrayType) {
				// Replace array class literals by an expression to obtain the
				// class literal from the
				// leaf type of the array.
				JArrayType arrayType = (JArrayType) type;
				JClassLiteral leafTypeClassLiteral = new JClassLiteral(
						x.getSourceInfo(), arrayType.getLeafType());
				resolveClassLiteral(leafTypeClassLiteral);
				int dims = type.isJsNative() ? 1 : arrayType.getDims();
				JExpression arrayClassLiteralExpression = program
						.createArrayClassLiteralExpression(x.getSourceInfo(),
								leafTypeClassLiteral, dims);
		...
		public void endVisit(JMethod x, Context ctx) {
			if (x.isJsMethodVarargs()) {
				// ImplementJsVarargs might insert an array creation for the
				// varargs parameter which is not
				// seen by this pass.
				JParameter varargsParameter = Iterables.getLast(x.getParams());
				assert varargsParameter.isVarargs();
				resolveClassLiteralField(
		...
		public void endVisit(JsniClassLiteral x, Context ctx) {
			// JsniClassLiterals will be traversed explicitly in JsniMethodBody.
			// For each JsniClassLiteral
			// in JsniMethodBody.classRefs there is at least a corresponding
			// JsNameRef (with a jsni
		...
		public void endVisit(final JsniMethodBody jsniMethodBody, Context ctx) {
			...
			final Multimap<String, JsniClassLiteral> jsniClassLiteralsByJsniReference = ArrayListMultimap
					.create();
			final JMethod getClassLiteralForArrayMethod = program
					.getIndexedMethod(
							RuntimeConstants.ARRAY_GET_CLASS_LITERAL_FOR_ARRAY);
			final String getClassLiteralForArrayMethodIdent = "@"
					+ getClassLiteralForArrayMethod.getJsniSignature(true,
							false);
			boolean areThereArrayClassLiterals = false;
			for (JsniClassLiteral jsniClassLiteral : jsniMethodBody
					.getClassRefs()) {
				...
				jsniClassLiteralsByJsniReference
						.put(jsniClassLiteral.getIdent(), jsniClassLiteral);
			}
			...
			// Replace all arrays JSNI class literals with the its construction
			// via the leaf type class
			// literal.
			JsModVisitor replaceJsniClassLiteralVisitor = new JsModVisitor() {
				...
				public void endVisit(JsNameRef x, JsContext ctx) {
					...
					if (jsniClassLiteralsByJsniReference.get(x.getIdent())
							.isEmpty()) {
						// The JsNameRef is not a class literal.
						return;
					}
					JsniClassLiteral jsniClassLiteral = jsniClassLiteralsByJsniReference
							.get(x.getIdent()).iterator().next();
					jsniClassLiteralsByJsniReference.remove(x.getIdent(),
							jsniClassLiteral);
					if (jsniClassLiteral.getRefType() instanceof JArrayType) {
						// Replace the array class literal by an expression that
						// retrieves it from
						// that of the leaf type.
						JArrayType arrayType = (JArrayType) jsniClassLiteral
								.getRefType();
						JType leafType = arrayType.getLeafType();
						jsniClassLiteral = new JsniClassLiteral(
								jsniClassLiteral.getSourceInfo(), leafType);
						// Array.getClassLiteralForArray(leafType.class,
						// dimensions)
						SourceInfo info = x.getSourceInfo();
						JsNameRef getArrayClassLiteralMethodNameRef = new JsNameRef(
								info, getClassLiteralForArrayMethodIdent);
						JsInvocation invocation = new JsInvocation(info,
								getArrayClassLiteralMethodNameRef,
								new JsNameRef(info,
										jsniClassLiteral.getIdent()),
			...
			JsniMethodBody newBody = new JsniMethodBody(
					jsniMethodBody.getSourceInfo(), jsniMethodBody.getFunc(),
					Lists.newArrayList(newClassRefs),
			...
			newBody.addJsniRef(new JsniMethodRef(jsniMethodBody.getSourceInfo(),
					getClassLiteralForArrayMethodIdent,
					getClassLiteralForArrayMethod,
					program.getJavaScriptObject()));
