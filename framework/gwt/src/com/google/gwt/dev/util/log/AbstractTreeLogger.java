...
public abstract class AbstractTreeLogger extends TreeLogger
	...
	public static final Comparator<String> LOG_LINE_COMPARATOR = new Comparator<String>() {
		...
		public int compare(String thisLine, String thatLine) {
			return ComparisonChain.start()
					...
					.compare(thisLine, thatLine).result();
		}
	};
	...
	/**
	 * Descendant tree loggers share the same MetricMap by default, but this can
	 * be overridden with {@link #resetMetricMap()}.
	 */
	 ...
	 */
	@Override
	public final synchronized TreeLogger branch(TreeLogger.Type type,
		...
		// also possible for someone to call branch() directly. In either case,
		// we
		...
		//
		if (isLoggable(type)) {
			// We can commit this branch entry eagerly since it is a-priori
			// loggable.
	...
	protected synchronized void commitMyBranchEntryInMyParentLogger() {
		...
		if (parent != null) {
			if (uncommitted != null) {
				...
				//
				parent.doCommitBranch(this, uncommitted.type,
						uncommitted.message, uncommitted.caught,
	...
	protected abstract void doCommitBranch(
			AbstractTreeLogger childBeingCommitted, TreeLogger.Type type,
			String msg, Throwable caught, HelpInfo helpInfo);
