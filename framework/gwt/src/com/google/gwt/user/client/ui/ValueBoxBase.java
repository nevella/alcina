 ...
 *            the value type
 */
public class ValueBoxBase<T> extends FocusWidget implements HasChangeHandlers,
		HasName, HasDirectionEstimator, HasValue<T>, HasText,
	 ...
	 */
	protected ValueBoxBase(Element elem, Renderer<T> renderer,
			Parser<T> parser) {
		super(elem);
	 ...
	 * implementation returns {@link ValueBoxEditor#of(ValueBoxBase)}.
	 * Subclasses may override this method to provide custom error-handling when
	 * using the Editor framework.
	 */
	@Override
	...
	public void onBrowserEvent(Event event) {
		...
		if ((type & Event.KEYEVENTS) != 0) {
			// Fire the keyboard event. Hang on to the current event object so
			// that
			// cancelKey() and setKey() can be implemented.
			currentEvent = event;
			// Call the superclass' onBrowserEvent as that includes the key
			// event
			// handlers.
	...
	/**
	 * If a keyboard event is currently being handled by the text box, this
	 * method replaces the unicode character or key code associated with it.
	 * This allows listeners to easily filter keyboard input.
	 * 
	 * @param key
	 *            the new key value
	...
	public void setSelectionRange(int pos, int length) {
		...
		if (pos < 0 || length + pos > getText().length()) {
			throw new IndexOutOfBoundsException(
					"From Index: " + pos + "  To Index: " + (pos + length)
