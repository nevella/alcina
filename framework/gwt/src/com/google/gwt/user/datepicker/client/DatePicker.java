 ...
 *
 * <p>
 * <h3>Example</h3> {@example com.google.gwt.examples.DatePickerExample}
 * </p>
 */
public class DatePicker extends Composite
		implements HasHighlightHandlers<Date>, HasShowRangeHandlers<Date>,
		HasValue<Date>, IsEditor<LeafValueEditor<Date>> {
	/**
	 * Convenience class to group css style names.
	 */
	static class StandardCss {
		static StandardCss DEFAULT = new StandardCss("gwt-DatePicker",
				"datePicker");
	...
	private static class DateStyler {
		...
		public void setStyleName(Date d, String styleName, boolean add) {
			// Code is easier to maintain if surrounded by " ", and on all
			// browsers
			// this is a no-op.
			styleName = " " + styleName + " ";
			String key = genKey(d);
	...
	public HandlerRegistration
			addHighlightHandler(HighlightHandler<Date> handler) {
		return addHandler(handler, HighlightEvent.getType());
	}

	@Override
	public HandlerRegistration
			addShowRangeHandler(ShowRangeHandler<Date> handler) {
	 ...
	 */
	public HandlerRegistration
			addShowRangeHandlerAndFire(ShowRangeHandler<Date> handler) {
	...
	/**
	 * Set the number of years to display in the years selection dropdown. The
	 * range of years will be centered on the selected date.
	 */
	public void setVisibleYearCount(int numberOfYears) {
		if (numberOfYears <= 0) {
			throw new IllegalArgumentException(
					"The number of years to display must be positive");
		}
	...
	/**
	 * Set if the user can navigate through the years via a set of backward and
	 * forward buttons.
	 */
	...
	/**
	 * If the <code>dropdownVisible</code> is equal to true, the user will be
	 * able to change the current month and the current year of the date picker
	 * via two dropdown lists.
	 */
	 ...
	 *
	 * @param newValue
	 *            the new value for this date picker
	 * @param fireEvents
	...
	protected void setup() {
		 ...
		 * expand to fill the available width, so we'd need to give it a size.
		 */
	...
	/**
	 * Gets the css associated with this date picker for use by extended month
	 * and cell grids.
	 *
	 * @return the css.
	 */
