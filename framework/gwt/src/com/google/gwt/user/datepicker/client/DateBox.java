 ...
 *
 * <p>
 * <h3>Example</h3> {@example com.google.gwt.examples.DateBoxExample}
 * </p>
 */
public class DateBox extends Composite
	...
	public static class DefaultFormat implements Format {
		 ...
		 *            the {@link DateTimeFormat} to use with this
		 *            {@link Format}.
		 */
		...
		@Override
		@SuppressWarnings("deprecation")
		public Date parse(DateBox dateBox, String dateText,
				boolean reportError) {
			Date date = null;
			try {
				...
				try {
					...
					// We have parsed the text at higher resolution so do a
					// round trip to get rid of extra:
	...
	private class DateBoxHandler
			implements ValueChangeHandler<Date>, FocusHandler, BlurHandler,
		...
		// Round trips on render & parse to convert the date to our
		// interpretation based on the format
		// See issue
		...
		private Date normalize(Date date) {
			...
			return getFormat().parse(dateBox, getFormat().format(dateBox, date),
					false);
		}
	}
	...
	private void updateDateFromTextBox() {
		...
		if (fireNullValues || (parsedDate != null)) {
			try {
				...
				// FIXME - refid - celltable issues, ignore (it's on blur
				// e.printStackTrace();
			}
		}
	}
