...
public abstract class AbstractCellTable<T> extends AbstractHasData<T> {
     ...
     */
    private static SafeHtml tableSectionToSafeHtml(TableSectionBuilder section,
            String tag) {
        if (!(section instanceof HtmlTableSectionBuilder)) {
            throw new IllegalArgumentException(
                    "Only HtmlTableSectionBuilder is supported at this time");
        }
     ...
     */
    public AbstractCellTable(Element elem, final int pageSize,
            Resources resources, ProvidesKey<T> keyProvider) {
     ...
     *            should act as its own key
     */
    public AbstractCellTable(Widget widget, final int pageSize,
            Resources resources, ProvidesKey<T> keyProvider) {
     ...
     * @return a {@link HandlerRegistration} to remove the handler
     */
    public HandlerRegistration addRowHoverHandler(
            RowHoverEvent.Handler handler) {
    ...
    public void insertColumn(int beforeIndex, Column<T, ?> col,
        ...
        if (beforeIndex <= keyboardSelectedColumn) {
            keyboardSelectedColumn = Math.min(keyboardSelectedColumn + 1,
                    columns.size() - 1);
        }
        ...
        if (isColumnInteractive(col)
                && ((keyboardSelectedColumn >= columns.size())
                        || !isColumnInteractive(
                                columns.get(keyboardSelectedColumn)))) {
    ...
    protected void onBlur() {
        ...
        if (td != null) {
            ...
            setRowStyleName(tr, style.keyboardSelectedRow(),
                    style.keyboardSelectedRowCell(), false);
        }
    }
    ...
    protected void onBrowserEvent2(Event event) {
         ...
         */
        TableRowElement targetTableRow = targetTableCell.getParentElement()
                .cast();
        ...
        if (targetTableSection == thead || targetTableSection == tfoot) {
            ...
            Element columnParent = isHeader ? headerColumnParent
                    : footerColumnParent;
            ...
            if (headerParent != null) {
                ...
                if (header != null) {
                    ...
                    Context context = new Context(headerIndex, col,
                            header.getKey());
                    ...
                    if (isSelect) {
                        ...
                        // event
                        shouldSortColumn = header.onPreviewColumnSortEvent(
            ...
            if (!skipRowHoverCheck) {
                ...
                if (BrowserEvents.MOUSEOVER.equals(eventType)) {
                    ...
                    if (hoveringRow != null && getTableBodyElement()
                            .isOrHasChild(hoveringRow)) {
             ...
             */
            Context context = new Context(absRow, col, getValueKey(value),
                    subrow);
            CellPreviewEvent<T> previewEvent = CellPreviewEvent.fire(this,
                    event, this, context, value, cellIsEditing,
                    isSelectionHandled);
            ...
            if (cellParent != null && !previewEvent.isCanceled()) {
                ...
                if (column != null) {
                    fireEventToCell(event, eventType, cellParent, value,
                            context, column);
                }
            }
        }
    ...
    protected void onFocus() {
        ...
        if (td != null) {
            ...
            setRowStyleName(tr, style.keyboardSelectedRow(),
                    style.keyboardSelectedRowCell(), true);
        }
    }
    ...
    protected final void renderRowValuesLegacy(SafeHtmlBuilder sb,
        ...
        for (int i = start; i < end; i++) {
            ...
            for (Column<T, ?> column : columns) {
                ...
                VerticalAlignmentConstant vAlign = column
    ...
    protected void replaceChildren(List<T> values, int start, SafeHtml html) {
        ...
        TABLE_IMPL.replaceChildren(this, getTableBodyElement(),
                CellBasedWidgetImpl.get().processHtml(html), start,
                values.size());
    }
    ...
    protected void setKeyboardSelected(int index, boolean selected,
        ...
        setRowStyleName(tr, style.keyboardSelectedRow(),
                style.keyboardSelectedRowCell(), selected);
        NodeList<TableCellElement> cells = tr.getCells();
        int keyboardColumn = Math.min(getKeyboardSelectedColumn(),
                cells.getLength() - 1);
        for (int i = 0; i < cells.getLength(); i++) {
            TableCellElement td = cells.getItem(i);
            boolean isKeyboardSelected = (i == keyboardColumn);
            // Update the selected style.
            setStyleName(td, cellStyle,
                    updatedSelection && selected && isKeyboardSelected);
            // Mark as focusable.
    ...
    public static class CellTableKeyboardSelectionHandler<T>
        ...
        public void onCellPreview(CellPreviewEvent<T> event) {
            ...
            if (BrowserEvents.KEYDOWN.equals(eventType)
                ...
                int keyCodeLineEnd = isRtl ? KeyCodes.KEY_LEFT
                        : KeyCodes.KEY_RIGHT;
                ...
                if ((table.getKeyboardSelectedColumn() != col)
                    ...
                    if (BrowserEvents.CLICK.equals(eventType)) {
                        ...
                        stealFocus = !CellBasedWidgetImpl.get()
                                .isFocusable(target);
                    }
    ...
    interface Template extends SafeHtmlTemplates {
        ...
        SafeHtml tdHorizontalAlign(String classes, String hAlign,
                SafeHtml contents);

        @SafeHtmlTemplates.Template("<td class=\"{0}\" valign=\"{1}\">{2}</td>")
        SafeHtml tdVerticalAlign(String classes, String vAlign,
                SafeHtml contents);
