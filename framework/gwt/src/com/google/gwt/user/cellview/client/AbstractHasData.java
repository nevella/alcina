 ...
 */
public abstract class AbstractHasData<T> extends Composite implements
		HasData<T>, HasKeyProvider<T>, Focusable, HasKeyboardPagingPolicy {
	/**
	...
	public static class DefaultKeyboardSelectionHandler<T>
		...
		/**
		 * The number of rows to jump when PAGE_UP or PAGE_DOWN is pressed and
		 * the {@link HasKeyboardPagingPolicy.KeyboardPagingPolicy} is
		...
		public void onCellPreview(CellPreviewEvent<T> event) {
			...
			if (BrowserEvents.KEYDOWN.equals(eventType)
				...
				/*
				 * Move keyboard focus to the clicked row, even if the Cell is
				 * being edited. Unlike key events, we aren't moving the
				...
				if (display.getKeyboardSelectedRow() != relRow) {
					...
					// Do not cancel the event as the click may have occurred on
					// a Cell.
					return;
				}
			}
		...
		// Visible for testing.
		void nextPage() {
			KeyboardPagingPolicy keyboardPagingPolicy = display
			...
			if (KeyboardPagingPolicy.CHANGE_PAGE == keyboardPagingPolicy) {
				...
				setKeyboardSelectedRow(
						display.getKeyboardSelectedRow() + PAGE_INCREMENT);
			}
		}
	...
	public static class RedrawEvent extends GwtEvent<RedrawEvent.Handler> {
		...
		public interface Handler extends EventHandler {
			/**
			 * Performs implementation-specific work when the cell list
			 * re-renders one or more existing rows.
			 */
			void onRedraw();
		}
	...
	private static class View<T> implements HasDataPresenter.View<T> {
		...
		@Override
		public void replaceAllChildren(List<T> values,
				SelectionModel<? super T> selectionModel, boolean stealFocus) {
			SafeHtml html = renderRowValues(values, hasData.getPageStart(),
					selectionModel);
		...
		public void replaceChildren(List<T> values, int start,
				SelectionModel<? super T> selectionModel, boolean stealFocus) {
			SafeHtml html = renderRowValues(values,
					hasData.getPageStart() + start, selectionModel);
			// Removing elements can fire a blur event, which we ignore.
			hasData.isFocused = hasData.isFocused || stealFocus;
			wasFocused = hasData.isFocused;
			hasData.isRefreshing = true;
		...
		private void fireValueChangeEvent() {
			...
			// constructor. We can't call ValueChangeEvent.fire() because this
			// class
			// doesn't implement HasValueChangeHandlers.
			hasData.fireEvent(
					new ValueChangeEvent<List<T>>(hasData.getVisibleItems()) {
					});
		}
		...
		private SafeHtml renderRowValues(List<T> values, int start,
			...
			try {
				...
				// If renderRowValues throws, the implementation will render
				// directly in
	 ...
	 */
	static Element convertToElements(Widget widget, Element tmpElem,
			SafeHtml html) {
	 ...
	 *            the container that holds the contents
	 * @param html
	 *            the html to set
	 */
	static void replaceAllChildren(Widget widget, Element childContainer,
		...
		// Render the HTML.
		childContainer
				.setInnerSafeHtml(CellBasedWidgetImpl.get().processHtml(html));
		// Detach the event listener.
		if (!widget.isAttached()) {
	 ...
	 *            the container that holds the contents
	 * @param newChildren
	 *            an element containing the new children
	 * @param start
	...
	static void replaceChildren(Widget widget, Element childContainer,
		...
		for (int i = 0; i < count; i++) {
			if (toReplace == null) {
				// The child will be removed from tmpElem, so always use index
				// 0.
	...
	/**
	 * A boolean indicating that the widget is refreshing, so all events should
	 * be ignored.
	 */
	...
	public AbstractHasData(Widget widget, final int pageSize,
			final ProvidesKey<T> keyProvider) {
		initWidget(widget);
		this.presenter = new HasDataPresenter<T>(this, new View<T>(this),
				pageSize, keyProvider);
		// Sink events.
		...
		eventTypes.add(BrowserEvents.KEYUP); // Used by subclasses for
												// selection.
		eventTypes.add(BrowserEvents.CLICK); // Used by subclasses for
												// selection.
		eventTypes.add(BrowserEvents.MOUSEDOWN); // No longer used, but here for
													// legacy support.
		...
		setKeyboardSelectionHandler(
				new DefaultKeyboardSelectionHandler<T>(this));
	}
	...
	/**
	 * Add a {@link LoadingStateChangeEvent.Handler} to be notified of changes
	 ...
	 * keyboard selected row refers to the row that the user navigated to via
	 ...
	 * This method delegates to {@link #getChildContainer()};
	 * 
	...
	/**
	 * Get the key for the specified value. If a keyProvider is not specified or
	 * the value is null, the value is returned. If the key provider is
	 * specified, it is used to get the key from the value.
	 * 
	 * @param value
	 *            the value
	 * @return the key
	...
	public final void onBrowserEvent(Event event) {
		...
		Element target = null;
		// Verify that the target is still a child of this widget. IE fires
		// focus
		...
		try {
			...
			/*
			 * LocalDom exception - but yes, it doesn't exist. Won't DEVEX this
			 * since celltable etc is going
			 */
			return;
		}
		...
		if (BrowserEvents.FOCUS.equals(eventType)) {
			...
			// If a natively focusable element was just clicked, then we must
			// have
			// focus.
			isFocused = true;
		}
	...
	public void redrawRow(int absRowIndex) {
		...
		setRowData(absRowIndex,
				Collections.singletonList(getVisibleItem(relRowIndex)));
	}
	 ...
	 * (that is, less than 0 or greater than or equal to the page size), the
	 * page or range will be adjusted depending on the keyboard paging policy.
	 * If the keyboard paging policy is limited to the current range, the row
	 * index will be clipped to the current page.
	 * </p>
	 * 
	 * @param row
	 ...
	 */
	public void setKeyboardSelectionHandler(
		...
		if (keyboardSelectionReg != null) {
			this.keyboardSelectionReg = addCellPreviewHandler(
					keyboardSelectionReg);
		}
	}
	 ...
	 * 
	 * @param pageStart
	 *            the index of the row that should appear at the start of the
	 *            page
	 * @see #setVisibleRange(Range)
	 * @see #getPageStart()
	 */
	 ...
	 * </p>
	 * <p>
	 * Equivalent to calling {@link #setRowCount(int)} with the length of the
	 * list of values, {@link #setVisibleRange(Range)} from 0 to the size of the
	 * list of values, and {@link #setRowData(int, List)} with a start of 0 and
	 ...
	 * 
	 * <p>
	 * By default, selection occurs when the user clicks on a Cell or presses
	 * the spacebar. If you need finer control over selection, you can specify a
	 ...
	 * implementations to handle checkbox based selection, as well as a
	 * blacklist or whitelist of columns to prevent or allow selection.
	 * </p>
	 * 
	 * @param selectionModel
	 *            the {@link SelectionModel}
	 ...
	 * 
	 * @param selectionModel
	 *            the {@link SelectionModel} that defines selection
	 * @param selectionEventManager
	 *            the handler that controls user selection
	 */
	public void setSelectionModel(SelectionModel<? super T> selectionModel,
		...
		if (selectionEventManager != null) {
			this.selectionManagerReg = addCellPreviewHandler(
	...
	protected void checkRowBounds(int row) {
		if (!isRowWithinBounds(row)) {
			throw new IndexOutOfBoundsException(
					"Row index: " + row + ", Row size: " + getRowCount());
		}
	}
	...
	/**
	 * Check if keyboard navigation is being suppressed, such as when the user
	 * is editing a cell.
	 * 
	 ...
	 * 
	 * <p>
	 * Subclasses can optionally throw an {@link UnsupportedOperationException}
	 * if they prefer to render the rows in
	 ...
	 * designed to supported legacy widgets that use {@link SafeHtmlBuilder},
	 * and newer widgets that use other builders, such as the ElementBuilder
	 * API.
	 * </p>
	 * 
	 * @param sb
	 ...
	 *            the element
	 * @param focusable
	 *            true to make focusable, false to make unfocusable
	 */
	protected void setFocusable(Element elem, boolean focusable) {
		if (focusable) {
			...
			// Chrome: Elements remain focusable after removing the tabIndex, so
			// set
			// it to -1 first.
			elem.setTabIndex(-1);
