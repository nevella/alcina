...
class CellTreeNodeView<T> extends UIObject {
	...
	/**
	 * A reference to the element that contains the children. Parent to the
	 * actual child nodes.
	 */
	 ...
	 * implement {@link TreeNode} directly because we want to transfer the
	 * user's handle to the {@link TreeNode} to the new {@link CellTreeNodeView}
	 * .
	 */
	 ...
	 * @param elem
	 *            the outer element of this {@link CellTreeNodeView}
	 * @param value
	...
	protected void cleanup(boolean destroy) {
		...
		if (destroy) {
			isDestroyed = true;
			// If this is the keyboard selected node, select the parent. The
			// children
			// have already been cleaned, so the selected node cannot be under
			// this
			// node.
	...
	Element ensureContentContainer() {
		if (contentContainer == null) {
			...
			// CellTree prevents strict-CSP violation by cancelling event
			// default action.
	...
	protected void fireEventToCell(NativeEvent event) {
		...
		boolean cellWasEditing = parentCell.isEditing(context, cellParent,
				value);
		// Update selection.
		boolean isSelectionHandled = parentCell.handlesSelection()
				|| KeyboardSelectionPolicy.BOUND_TO_SELECTION == tree
		...
		if (previewEvent.isCanceled() || !cellParent
				.isOrHasChild(Element.as(event.getEventTarget()))) {
			return;
		}
		...
		if (consumedEvents != null && consumedEvents.contains(eventType)) {
			...
			tree.cellIsEditing = parentCell.isEditing(context, cellParent,
					value);
	 ...
	 * includes the open/close image and the rendered value and spans the width
	 * of the tree.
	 *
		...
		if (!isRootNode()) {
			...
			if (selectionElem != null) {
				setStyleName(selectionElem,
						tree.getStyle().cellTreeKeyboardSelectedItem(),
						selected);
			}
		}
	}
	...
	public boolean setOpen(boolean open, boolean fireEvents) {
		...
		// synchronously, so we have to cancel the animation before attaching
		// the
		...
		if (open) {
			if (!nodeInfoLoaded) {
				...
				if (nodeInfo != null) {
					...
					Set<String> consumedEvents = nodeInfo.getCell()
							.getConsumedEvents();
			...
			if (nodeInfo != null) {
				...
				showOrHide(emptyMessageElem, false);
				if (!isRootNode()) {
					setStyleName(getCellParent(),
							tree.getStyle().cellTreeOpenItem(), true);
				}
			...
			if (!isRootNode()) {
				setStyleName(getCellParent(),
						tree.getStyle().cellTreeOpenItem(), false);
			}
			cleanup(false);
