...
import com.google.gwt.user.cellview.client.CellTreeNodeView.TreeNodeImpl;
...
public class CellTree extends AbstractCellTree
	...
	public <T> CellTree(TreeViewModel viewModel, T rootValue,
		...
		imageWidth = Math.max(
				Math.max(treeOpen.getWidth(), treeClosed.getWidth()),
	...
	public Element getTreeNodeElement(TreeNode node) {
		if (node instanceof TreeNodeImpl) {
			return ((TreeNodeImpl) node).nodeView.getElement();
		} else {
			return null;
		}
	}
	...
	public void onBrowserEvent(Event event) {
		...
		if (nodeView != null) {
			...
			} else if (isClick && showMoreElem != null
					&& showMoreElem.isOrHasChild(target)) {
				// Prevents strict-CSP violation due to javascript:'' link
				// target
				event.preventDefault();
			}
			...
			if (nodeView != rootNode
				...
				if (isClick) {
					 ...
					 */
					boolean isFocusable = CellBasedWidgetImpl.get()
							.isFocusable(target);
	...
	/**
	 * Set the animation used to open and close nodes in this tree. You must
	 * call {@link #setAnimationEnabled(boolean)} to enable or disable
	 * animation.
	 *
	 * @param animation
	...
	interface Template extends SafeHtmlTemplates {
		...
		SafeHtml imageWrapper(String classes, SafeStyles cssLayout,
				SafeHtml image);
	}
}
