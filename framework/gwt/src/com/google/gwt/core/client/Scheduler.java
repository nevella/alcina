...
public abstract class Scheduler {
	...
	/**
	 * An "entry" command will be executed before GWT-generated code is invoked
	 * by the browser's event loop. The {@link RepeatingCommand} will be called
	 * once per entry from the event loop until <code>false</code> is returned.
	 * This type of command is appropriate for instrumentation or code that
	...
	/**
	 * An "entry" command will be executed before GWT-generated code is invoked
	 * by the browser's event loop. This type of command is appropriate for code
	 * that needs to know when "something happens."
	 * <p>
	 * If an entry command schedules another entry command, the second command
	 * will be executed before control flow continues to the GWT-generated code.
	 */
	 ...
	 * control to the browser's event loop. The
	 * {@link RepeatingCommand#execute()} method will be called once per exit to
	 * the event loop until <code>false</code> is returned. This type of command
	 * is appropriate for instrumentation or cleanup code.
	 * <p>
	 * If a finally command schedules another finally command, the second
	 * command will be executed before control flow returns to the browser.
	 */
	 ...
	 * That is, the next invocation of the command will be scheduled for
	...
	/**
	 * Schedules a repeating command that performs incremental work. This type
	 * of command is encouraged for long-running processes that perform
	 * computation or that manipulate the DOM. The commands in this queue are
	 * invoked many times in rapid succession and are then deferred to allow the
	 * browser to process its event queue.
	 * 
	 * @param cmd
	 *            the command to execute
	 */
