...
public class StandardSymbolData implements SymbolData {
	public static StandardSymbolData forClass(String className,
			String uriString, int lineNumber, String typeId) {
		return new StandardSymbolData(className, null, null, uriString,
				lineNumber, typeId);
	}

	public static StandardSymbolData forMember(String className,
			String memberName, String methodSig, String uriString,
			int lineNumber) {
		return new StandardSymbolData(className, memberName, methodSig,
				uriString, lineNumber, null);
	}
	...
	private StandardSymbolData(String className, String memberName,
		...
		assert memberName != null
				|| methodSig == null : "methodSig without memberName";
	...
	/**
	 * Implemented by hand for speed (over using reflection) because there are
	 * so many of these. Note that {@link #className} and {@link #sourceUri} are
	 * done as writeObject because the actual String instances are very likely
	 * to be shared among many instances of this class, so the same objects can
	 * be reused in the stream. The other String fields are done as UTF because
	 * it's slightly faster and the String objects are unlikely to be shared
	 * among instances.
	 */
