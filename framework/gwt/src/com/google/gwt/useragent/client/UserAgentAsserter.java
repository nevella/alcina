...
public class UserAgentAsserter implements EntryPoint {
	...
	private static void assertCompileTimeUserAgent() {
		...
		if (!compileTimeValue.equals(runtimeValue)) {
			if (compileTimeValue.equals("safari")
					&& runtimeValue.equals("gecko1_8")) {
				// close enough
			} else {
				// Let it escape and get handled by UCEH:
				throw new UserAgentAssertionError(compileTimeValue,
						runtimeValue);
			}
		}
	}
	...
	/**
	 * An error object that indicates that the user agent detected at runtime
	 * did not match the user agent that the module expected to find.
	 */
