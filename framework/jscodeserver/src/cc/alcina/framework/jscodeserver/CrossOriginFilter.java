...
/**
 * <p>
 * Implementation of the <a href="http://www.w3.org/TR/cors/">cross-origin
 * resource sharing</a>.
 * </p>
 * <p>
 ...
 * <ul>
 * <li><b>allowedOrigins</b>, a comma separated list of origins that are allowed
 ...
 * characters are escaped to "\." and the resulting allowed origin interpreted
 ...
 * <li><b>allowedMethods</b>, a comma separated list of HTTP methods that are
 * allowed to be used when accessing the resources. Default value is
 * <b>GET,POST,HEAD</b></li>
 * <li><b>allowedHeaders</b>, a comma separated list of HTTP headers that are
 * allowed to be specified when accessing the resources. Default value is
 * <b>X-Requested-With,Content-Type,Accept,Origin</b></li>
 * <li><b>preflightMaxAge</b>, the number of seconds that preflight requests can
 * be cached by the client. Default value is <b>1800</b> seconds, or 30
 * minutes</li>
 ...
 * <li><b>exposeHeaders</b>, a comma separated list of HTTP headers that are
 * allowed to be exposed on the client. Default value is the <b>empty
 * list</b></li>
...
public class CrossOriginFilter implements Filter {
	...
	private static final List<String> SIMPLE_HTTP_METHODS = Arrays.asList("GET",
			"POST", "HEAD");
	...
	public void init(FilterConfig config) throws ServletException {
		String allowedOriginsConfig = config
				.getInitParameter(ALLOWED_ORIGINS_PARAM);
		...
		String allowedMethodsConfig = config
				.getInitParameter(ALLOWED_METHODS_PARAM);
		if (allowedMethodsConfig == null)
			allowedMethodsConfig = "GET,POST,HEAD";
		allowedMethods.addAll(Arrays.asList(allowedMethodsConfig.split(",")));
		String allowedHeadersConfig = config
				.getInitParameter(ALLOWED_HEADERS_PARAM);
		...
		String preflightMaxAgeConfig = config
				.getInitParameter(PREFLIGHT_MAX_AGE_PARAM);
		...
		String allowedCredentialsConfig = config
				.getInitParameter(ALLOW_CREDENTIALS_PARAM);
		...
		String exposedHeadersConfig = config
				.getInitParameter(EXPOSED_HEADERS_PARAM);
		if (exposedHeadersConfig == null)
			exposedHeadersConfig = "";
		exposedHeaders.addAll(Arrays.asList(exposedHeadersConfig.split(",")));
		String chainPreflightConfig = config
				.getInitParameter(OLD_CHAIN_PREFLIGHT_PARAM);
		if (chainPreflightConfig != null) // TODO remove this
			LOG.warn("DEPRECATED CONFIGURATION: Use " + CHAIN_PREFLIGHT_PARAM
					+ " instead of " + OLD_CHAIN_PREFLIGHT_PARAM);
		else
			chainPreflightConfig = config
					.getInitParameter(CHAIN_PREFLIGHT_PARAM);
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Cross-origin filter configuration: "
					...
					+ allowedMethodsConfig + ", " + ALLOWED_HEADERS_PARAM
					+ " = " + allowedHeadersConfig + ", "
					+ PREFLIGHT_MAX_AGE_PARAM + " = " + preflightMaxAgeConfig
					+ ", " + ALLOW_CREDENTIALS_PARAM + " = "
					+ allowedCredentialsConfig + "," + EXPOSED_HEADERS_PARAM
					+ " = " + exposedHeadersConfig + "," + CHAIN_PREFLIGHT_PARAM
	...
	private boolean areHeadersAllowed(HttpServletRequest request) {
		...
		LOG.debug("{} is {}", ACCESS_CONTROL_REQUEST_HEADERS_HEADER,
		...
		if (origin != null && isEnabled(request)) {
			if (originMatches(origin)) {
				if (isSimpleRequest(request)) {
					...
					LOG.debug(
							"Cross-origin request to {} is a preflight cross-origin request",
							request.getRequestURI());
					handlePreflightResponse(request, response, origin);
					if (chainPreflight)
						LOG.debug(
								"Preflight cross-origin request to {} forwarded to application",
								request.getRequestURI());
					else
						return;
				} else {
					LOG.debug(
							"Cross-origin request to {} is a non-simple cross-origin request",
							request.getRequestURI());
					handleSimpleResponse(request, response, origin);
				}
			} else {
				LOG.debug("Cross-origin request to " + request.getRequestURI()
						+ " with origin " + origin
	...
	private void handlePreflightResponse(HttpServletRequest request,
		...
		response.setHeader(ACCESS_CONTROL_ALLOW_METHODS_HEADER,
				commify(allowedMethods));
		response.setHeader(ACCESS_CONTROL_ALLOW_HEADERS_HEADER,
				commify(allowedHeaders));
	}

	private void handleSimpleResponse(HttpServletRequest request,
			HttpServletResponse response, String origin) {
		response.setHeader(ACCESS_CONTROL_ALLOW_ORIGIN_HEADER, origin);
		if (allowCredentials)
			response.setHeader(ACCESS_CONTROL_ALLOW_CREDENTIALS_HEADER, "true");
		if (!exposedHeaders.isEmpty())
			response.setHeader(ACCESS_CONTROL_EXPOSE_HEADERS_HEADER,
					commify(exposedHeaders));
	}

	private boolean isMethodAllowed(HttpServletRequest request) {
		String accessControlRequestMethod = request
				.getHeader(ACCESS_CONTROL_REQUEST_METHOD_HEADER);
		LOG.debug("{} is {}", ACCESS_CONTROL_REQUEST_METHOD_HEADER,
				accessControlRequestMethod);
		boolean result = false;
		if (accessControlRequestMethod != null)
			result = allowedMethods.contains(accessControlRequestMethod);
		LOG.debug(
				"Method {} is" + (result ? "" : " not")
						+ " among allowed methods {}",
	...
	protected boolean isEnabled(HttpServletRequest request) {
		...
		// response
		for (Enumeration connections = request
				.getHeaders("Connection"); connections.hasMoreElements();) {
			String connection = (String) connections.nextElement();
			if ("Upgrade".equalsIgnoreCase(connection)) {
				for (Enumeration upgrades = request
						.getHeaders("Upgrade"); upgrades.hasMoreElements();) {
