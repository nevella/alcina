...
bool ScriptableInstance::makeResult(bool isException,
...
bool ScriptableInstance::setProperty(NPIdentifier name,
...
bool ScriptableInstance::invoke(NPIdentifier name,
    const NPVariant * args, unsigned argCount,
        NPVariant * result) {
...
void ScriptableInstance::fatalError(HostChannel & channel,
...
void ScriptableInstance::freeValue(HostChannel & channel, int idCount,
    const int *
        const ids) {
...
void ScriptableInstance::loadJsni(HostChannel & channel,
    const string & js) {
    NPString npScript;
...
gwt::Value ScriptableInstance::clientMethod_getProperty(HostChannel & channel, int numArgs,
    const gwt::Value *
        const args) {
...
gwt::Value ScriptableInstance::clientMethod_setProperty(HostChannel & channel, int numArgs,
    const gwt::Value *
        const args) {
    if (numArgs != 2 || !args[0].isInt() || (!args[1].isString() && !args[1].isInt())) {
...
bool ScriptableInstance::invoke(HostChannel & channel,
    const gwt::Value & thisRef,
        const string & methodName, int numArgs,
            const gwt::Value *
                const args,
...
bool ScriptableInstance::JavaObject_getToStringTearOff(NPVariant * result) {
    ...
    NPVariantArray varArgs( * this, 3);
    temp.setNull();
    varArgs[0] = temp; // proxy: no proxy needed
    temp.setInt(0);
    varArgs[1] = temp; // dispId: always 0 for toString()
    temp.setInt(0);
    varArgs[2] = temp; // argCount: always 0 for toString()
...
void ScriptableInstance::sendFreeValues(HostChannel & channel) {
    ...
    if (n) {
        ...
        int i = 0;
        for (std::set < int > ::iterator it = javaObjectsToFree.begin(); it != javaObjectsToFree.end(); ++it) {
            ids[i++] = * it;
        }
