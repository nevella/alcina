...
public class CrossSiteIframeLinker extends SelectionScriptLinker {
    ...
    protected String fillSelectionScriptTemplate(StringBuffer ss,
        ...
        includeJs(ss, logger, getJsInstallScript(context),
                "__INSTALL_SCRIPT__");
        includeJs(ss, logger, getJsWaitForBodyLoaded(context),
        ...
        includeJs(ss, logger, getJsInstallLocation(context),
                "__INSTALL_LOCATION__");
        includeJs(ss, logger, getJsComputeScriptBase(context),
                "__COMPUTE_SCRIPT_BASE__");
        includeJs(ss, logger, getJsComputeUrlForResource(context),
                "__COMPUTE_URL_FOR_RESOURCE__");
        includeJs(ss, logger, getJsLoadExternalStylesheets(context),
                "__LOAD_STYLESHEETS__");
        includeJs(ss, logger, getJsRunAsync(context), "__RUN_ASYNC__");
        includeJs(ss, logger, getJsDevModeRedirectHook(context),
                "__DEV_MODE_REDIRECT_HOOK__");
        // Must go after __DEV_MODE_REDIRECT_HOOK__ because they're found in
        // DevModeRedirectHook.js.
        includeJs(ss, logger, getJsDevModeRedirectHookPermitted(context),
                "__DEV_MODE_REDIRECT_HOOK_PERMITTED__");
        includeJs(ss, logger, getJsDevModeUrlValidation(context),
        ...
        if (!scripts.isEmpty()) {
            ...
            for (ConfigurationProperty prop : context
                    .getConfigurationProperties()) {
                if (prop.getName()
                        .equalsIgnoreCase(FAIL_IF_SCRIPT_TAG_PROPERTY)) {
            ...
            if (failIfScriptTags) {
                ...
                if (logger.isLoggable(TreeLogger.INFO)) {
                    logger.log(TreeLogger.INFO,
                            "Ignoring the following script tags in the gwt.xml "
                                    + "file\n" + list);
                }
            }
        }
    ...
    @Override
    protected String getDeferredFragmentSuffix2(TreeLogger logger,
            LinkerContext context, int fragment, String strongName) {
     ...
     * returns
     * {@code "com/google/gwt/core/ext/linker/impl/computeUrlForResource.js"}.
     *
     * @param context
     *            a LinkerContext
     */
     ...
     * (May return either the JavaScript itself or the name of a Java resource
     * ending with ".js".)
     */
    protected String getJsDevModeRedirectHook(LinkerContext context) {
        ...
        // Enable Super Dev Mode for this app if the devModeRedirectEnabled
        // config property is true.
        if (getBooleanConfigurationProperty(context, "devModeRedirectEnabled",
                true)) {
    ...
    /**
     * Returns a JavaScript expression that determines whether Super Dev Mode
    ...
    protected String getJsDevModeUrlValidation(LinkerContext context) {
        ...
        // verify that it
        // matches devModeUrl.
        String regexp = getStringConfigurationProperty(context,
                "devModeUrlWhitelistRegexp",
                "http://(localhost|127\\.0\\.0\\.1)(:\\d+)?/.*");
        if (!regexp.isEmpty()) {
            return "" + "if (!/^" + regexp.replace("/", "\\/")
                    + "$/.test(devModeUrl)) {\n"
    ...
    /**
     * Returns the name of the {@code JsProperties} script. By default, returns
     * {@code "com/google/gwt/core/ext/linker/impl/properties.js"}.
     *
     * @param context
     *            a LinkerContext
     */
    ...
    /**
     * Returns the name of the {@code JsRunAsync} script. By default, returns
     * {@code "com/google/gwt/core/ext/linker/impl/runAsync.js"}.
     *
     * @param context
     *            a LinkerContext
     */
     ...
     * returns
     * {@code "com/google/gwt/core/ext/linker/impl/waitForBodyLoaded.js"}.
     *
     * @param context
     *            a LinkerContext
     */
    ...
    protected String getModulePrefix(TreeLogger logger, LinkerContext context,
        ...
        // Note: this code is included in both the primary fragment and
        // devmode.js
        // $wnd is the main window that the GWT code will affect and also the
        // location where the bootstrap function was defined. In iframe-based
        // linkers,
        // $wnd is set to window.parent. Usually, in others, $wnd = window.
        // By default, $wnd is not set when the module starts, but a replacement
        // for
        ...
        out.newlineOpt();
        // The functions for property access are set up in the bootstrap script
        // however
        // they will be called from, and expected to run in the scope of the GWT
        // code
        // (usually an iframe) so, here we set up those pointers.
        ...
        out.print("}");
        out.newlineOpt();
        // Even though we call the $sendStats function in the code written in
        // this
    ...
    protected String getModuleSuffix2(TreeLogger logger, LinkerContext context,
            String strongName) {
        // Note: this method won't be called if getModuleSuffix() is overridden
        ...
        out.newlineOpt();
        out.print("gwtOnLoad(" + "__gwtModuleFunction.__errFn, "
    ...
    protected String getSourceMapUrl(LinkerContext context, String strongName,
            int fragmentId) {
        String val = getStringConfigurationProperty(context,
                "includeSourceMapUrl", "false");
        if ("false".equalsIgnoreCase(val)) {
            return null;
        }
        if ("true".equalsIgnoreCase(val)) {
            return SymbolMapsLinker.SourceMapArtifact
                    .sourceMapFilenameForFragment(fragmentId);
        }
    ...
    protected void includeJs(StringBuffer selectionScript, TreeLogger logger,
            String jsSource, String templateVar)
        ...
        if (jsSource.endsWith(".js")) {
            try {
                ...
                logger.log(TreeLogger.ERROR, "Unable to read file: " + jsSource,
                        e);
                throw new UnableToCompleteException();
            }
        } else {
    ...
    protected void maybeAddHostedModeFile(TreeLogger logger,
        ...
        if (isSinglePermutation != shouldIncludeBootstrapInPrimaryFragment(
                context)) {
            return;
        }
        ...
        String script = generatePrimaryFragmentString(logger, context, result,
                buffer.toString(), 1, artifacts);
        EmittedArtifact devArtifact = emitString(logger, script, outputFilename,
                lastModified);
    ...
    protected boolean shouldIncludeBootstrapInPrimaryFragment(
            LinkerContext context) {
        return false;
    }
    ...
    protected boolean shouldInstallCode(LinkerContext context) {
        return getBooleanConfigurationProperty(context, "installCode",
                context.isOutputCompact());
    }
    ...
    protected String wrapPrimaryFragment(TreeLogger logger,
        ...
        if (shouldIncludeBootstrapInPrimaryFragment(context)) {
            ...
            out.append("if (" + context.getModuleFunctionName()
                    + ".succeeded) {\n");
        }
        if (shouldInstallCode(context)) {
            ...
            Iterable<String> chunks = Splitter
                    .on(getScriptChunkSeparator(logger, context)).split(script);
