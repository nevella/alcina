...
class MethodDispatch implements DispatchMethod {
	...
	public MethodDispatch(CompilingClassLoader classLoader,
			MethodAdaptor method) {
	 ...
	 *
	 * @param jsthis
	 *            JavaScript reference to Java object
	 * @param jsargs
	 *            array of JavaScript values for parameters
	 * @param returnValue
	 *            JavaScript value to return result in
	 * @return <code>true</code> if an exception was thrown
	 * @throws RuntimeException
	 *             if improper arguments are supplied
	 */
	@Override
	public boolean invoke(JsValue jsthis, JsValue[] jsargs,
			JsValue returnValue) {
		...
		try {
			...
			System.err.println("MethodDispatch.invoke, method="
					+ method.toString() + ": argument mismatch");
			for (int i = 0; i < argc; ++i) {
				System.err.println(
						" param " + i + " type is " + paramTypes[i].toString()
								+ " value is type " + jsargs[i].getTypeString()
								+ " = " + args[i].toString());
			}
			throw e;
		}
	 ...
	 * Throwable as a Java Object to be sent over the wire, or if the exception
	 * is a JavaScriptException unwinding through the stack, send the original
