...
public class BrowserChannelServer extends BrowserChannel implements Runnable {
	...
	public BrowserChannelServer(TreeLogger initialLogger, Socket socket,
			SessionHandlerServer handler, boolean ignoreRemoteDeath)
			throws IOException {
	 ...
	 *
	 * @param ids
	 *            array of JS object IDs that have been freeded
	 */
	...
	public void invokeJavascript(CompilingClassLoader ccl, JsValueOOPHM jsthis,
		...
		try {
			InvokeOnClientMessage invokeMessage = new InvokeOnClientMessage(
					this, methodName, vthis, vargs);
			invokeMessage.send();
			final ReturnMessage msg = reactToMessagesWhileWaitingForReturn(
					handler);
			...
			if (msg.isException()) {
				...
				if (returnValue.isNull() || returnValue.isUndefined()) {
					...
					exceptionValue = JsValueGlue
							.createJavaScriptObject(returnJsValue, ccl);
				} else if (returnValue.isJavaObject()) {
					Object object = remoteObjects
							.get(returnValue.getJavaObject().getRefid());
					Object target = ((JsValueOOPHM.DispatchObjectOOPHM) object)
							.getTarget();
					if (target instanceof Throwable) {
						throw (Throwable) (target);
					} else {
						// JS throwing random Java Objects, which we'll wrap in
						// JSException
						exceptionValue = target;
					}
				} else {
					// JS throwing random primitives, which we'll wrap as a
					// string in
					// JSException
					exceptionValue = returnValue.getValue().toString();
				}
				RuntimeException exception = ModuleSpace
						.createJavaScriptException(ccl, exceptionValue,
								methodName + "(" + Arrays.toString(args) + ")");
				// reset the stack trace to here to minimize GWT infrastructure
				// in
				// the stack trace
	 ...
	 *
	 * @param jsni
	 *            JSNI source to load into the browser
	 */
	...
	protected void processConnection()
		...
		case OLD_LOAD_MODULE:
			// v1 client
			OldLoadModuleMessage oldLoadModule = OldLoadModuleMessage
					.receive(this);
		...
		case CHECK_VERSIONS:
			...
			if (minVersion > PROTOCOL_VERSION_CURRENT
				...
				if (!HostedHtmlVersion.validHostedHtmlVersion(logger,
					...
					new FatalErrorMessage(this,
							"Invalid hosted.html version - check log window")
									.send();
					return;
				}
			}
			if (connectError != null) {
				logger.log(TreeLogger.ERROR,
						"Connection error: " + connectError, null);
			...
			// Optionally allow client to request switch of transports. Inband
			// is
			...
			if (type == MessageType.CHOOSE_TRANSPORT) {
				ChooseTransportMessage chooseTransport = ChooseTransportMessage
						.receive(this);
				String transport = selectTransport(
						chooseTransport.getTransports());
				String transportArgs = null;
				if (transport != null) {
					transportArgs = createTransport(transport);
				}
				new SwitchTransportMessage(this, transport, transportArgs)
						.send();
				type = Message.readMessageType(getStreamFromOtherSide());
			}
				...
				if (iconCache.containsKey(userAgent)) {
					...
					if (type != MessageType.USER_AGENT_ICON) {
						logger.log(TreeLogger.ERROR, "Unexpected message type "
								+ type + "; expecting UserAgentIcon");
						return;
					}
					UserAgentIconMessage uaIconMessage = UserAgentIconMessage
							.receive(this);
		...
		Thread.currentThread().setName("Code server for " + moduleName
				+ " from " + userAgent + " on " + url + " @ " + sessionKey);
		createDevModeSession();
		logger = handler.loadModule(this, moduleName, userAgent, url, tabKey,
				sessionKey, iconBytes);
		if (logger == null) {
			// got an error
			try {
	 ...
	 */
	public abstract static class SessionHandlerServer
			extends SessionHandler<BrowserChannelServer> {
		public abstract ExceptionOrReturnValue getProperty(
				BrowserChannelServer channel, int refId, int dispId);

		public abstract ExceptionOrReturnValue invoke(
				BrowserChannelServer channel, Value thisObj, int dispId,
				Value[] args);
		 ...
		 * @param userAgent
		 * @param url
		 *            top-level URL of the main page, null if using an old
		 *            plugin
		 * @param tabKey
		 *            opaque key of the tab, may be empty if the plugin can't
		 *            distinguish tabs or null if using an old plugin
		 * @param sessionKey
		 *            opaque key for this session, null if using an old plugin
		 * @param userAgentIcon
		 *            byte array containing an icon (which fits within 24x24)
		 *            representing the user agent or null if unavailable
		 * @return a TreeLogger to use for the module's logs, or null if the
		 *         module load failed
		 */
		...
		public abstract ExceptionOrReturnValue setProperty(
				BrowserChannelServer channel, int refId, int dispId,
				Value newValue);
