...
public abstract class BrowserChannel {
	...
	public static class JavaObjectRef implements RemoteObjectRef {
		...
		public JavaObjectRef(int refId, Object wrapped) {
			...
			if (special != null) {
				setArrayValues(CompilingClassLoader.getArrayValues(wrapped));
			}
		}
	 ...
	 * versions and the version from hosted.html (so stale copies on an external
	...
	protected static class CheckVersionsMessage extends Message {
		...
		public void send() throws IOException {
			DataOutputStream stream = getBrowserChannel()
					.getStreamToOtherSide();
	 ...
	 * and requesting the server choose one of them to switch protocol traffic
	 * to.
	 */
	 ...
	 * there is no response to this message, and this must only be sent
	 * immediately before an Invoke or Return message.
	 */
	 ...
	 *
	 * Note that MessageType.INVOKE can refer to either this class or
	 * {@link InvokeOnServerMessage} depending on the direction, as the protocol
	 * is asymmetric (Java needs a dispatch ID, Javascript needs a name).
	 */
	protected static class InvokeOnClientMessage extends Message {
		public static InvokeOnClientMessage receive(BrowserChannel channel)
			...
			return new InvokeOnClientMessage(channel, methodName, thisRef,
					args);
		}
		...
		@Override
		public void send() throws IOException {
			final DataOutputStream stream = getBrowserChannel()
					.getStreamToOtherSide();
	 ...
	 *
	 * Note that MessageType.INVOKE can refer to either this class or
	 * {@link InvokeOnClientMessage} depending on the direction, as the protocol
	 * is asymmetric (Java needs a dispatch ID, Javascript needs a name).
	 */
	protected static class InvokeOnServerMessage extends Message {
		...
		public InvokeOnServerMessage(BrowserChannel channel,
				int methodDispatchId, Value thisRef, Value[] args) {
			super(channel);
			this.thisRef = thisRef;
		...
		public static InvokeSpecialMessage receive(BrowserChannel channel)
			...
			if (specialMethodInt < 0 || specialMethodInt >= ids.length) {
				throw new BrowserChannelException(
						"Invalid dispatch id " + specialMethodInt);
			}
	...
	protected static class LoadModuleMessage extends Message {
		 ...
		 * @param url
		 *            URL of main top-level window - may not be null
		 * @param tabKey
		 *            opaque key identifying the tab in the browser, or an empty
		 *            string if it cannot be determined - may not be null
		 * @param sessionKey
		 *            opaque key identifying a particular session (ie, group of
		 *            modules) - may not be null
		 * @param moduleName
		 *            name of GWT module to load - may not be null
		 * @param userAgent
		 *            user agent identifier of the browser - may not be null
		 */
		public LoadModuleMessage(BrowserChannel channel, String url,
				String tabKey, String sessionKey, String moduleName,
				String userAgent) {
			super(channel);
			assert url != null;
			assert tabKey != null;
	...
	protected abstract static class Message {
		...
		/**
		 * @return true if this message type is asynchronous and does not expect
		 *         a return message.
		 */
		public boolean isAsynchronous() {
			return false;
		...
		/**
		 * @throws IOException
		 *             if a subclass encounters an I/O error
		 */
		public void send() throws IOException {
	...
	protected static class ProtocolVersionMessage extends Message {
		...
		public ProtocolVersionMessage(BrowserChannel channel,
				int protocolVersion) {
			super(channel);
	...
	protected static class RequestIconMessage extends Message {
		/**
		 * Receive a RequestIconMessage, assuming the message tag has already
		 * been read.
		 *
		 * @throws IOException
		 */
	...
	protected static class SwitchTransportMessage extends Message {
		public static SwitchTransportMessage receive(BrowserChannel channel)
			...
			return new SwitchTransportMessage(channel, transport,
					transportArgs);
		}
		...
		public SwitchTransportMessage(BrowserChannel channel, String transport,
	...
	protected static void writeJavaObject(DataOutputStream stream,
		...
		if (value.getSpecial() != null) {
			...
			case JS_INT_LIST:
				...
				stream.writeInt(arrayValues.length);
	...
	protected static void writeNull(DataOutputStream stream)
			throws IOException {
	...
	protected static void writeTaggedDouble(DataOutputStream stream,
	...
	protected static void writeTaggedString(DataOutputStream stream,
	...
	protected Value readValue(DataInputStream stream) throws IOException {
		...
		case JS_OBJECT:
			value.setJsObject(
					objectRefFactory.getJsObjectRef(stream.readInt()));
			break;
		case JAVA_OBJECT:
			value.setJavaObject(
