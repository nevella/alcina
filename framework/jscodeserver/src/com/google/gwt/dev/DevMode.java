...
public class DevMode extends DevModeBase implements RestartServerCallback {
	...
	private JsCodeServerHttp codeServerHttp;
	...
	protected HostedModeOptions createOptions() {
		...
		compilerContext = compilerContextBuilder.options(hostedModeOptions)
				.build();
	...
	protected boolean doSlowStartup() {
		...
		if (tempWorkDir) {
			try {
				...
				System.err
						.println("Unable to create hosted mode work directory");
				e.printStackTrace();
				return false;
			}
		}
		TreeLogger branch = getTopLogger().branch(TreeLogger.TRACE,
				"Linking modules");
		Event slowStartupEvent = SpeedTracerLogger
				.start(DevModeEventType.SLOW_STARTUP);
		try {
	...
	protected boolean doStartup() {
		...
		if (!options.isNoServer()) {
			if (webXml.exists()) {
				servletValidator = ServletValidator.create(getTopLogger(),
						webXml);
			} else {
		...
		try {
			for (String moduleName : options.getModuleNames()) {
				...
				// Create a hard reference to the module to avoid gc-ing it
				// until we
				// actually load the module from the browser.
				startupModules.put(module.getName(), module);
				if (!options.isNoServer()) {
					validateServletTags(moduleBranch, servletValidator,
							servletWriter, module);
				}
			}
	...
	protected int doStartUpServer() {
		...
		Event jettyStartupEvent = SpeedTracerLogger
				.start(DevModeEventType.JETTY_STARTUP);
		boolean clearCallback = true;
		try {
			ui.setCallback(RestartServerEvent.getType(), this);
			ServletContainerLauncher scl = options
			 ...
			 * have to figure out a better way to do this for SCLs in general.
			 */
			...
			if (serverLogger.isLoggable(TreeLogger.TRACE)) {
				serverLogger.log(TreeLogger.TRACE,
			...
			System.err.println("Port " + options.getBindAddress() + ':'
					+ getPort()
	...
	protected void ensureCodeServerListener() {
		if (listener == null) {
			if (options.isSuperDevMode()) {
				...
				listener = new BrowserListener(getTopLogger(), options,
						new OophmSessionHandler(getTopLogger(), browserHost));
				codeServerHttp = new JsCodeServerHttp();
				new Thread() {
					@Override
	...
	protected void inferStartupUrls() {
		...
		File warDir = options.getWarDir();
		if (!warDir.exists()) {
			// if the war directory doesn't exist, there are no startup files
			// there
			return;
		}
	...
	protected static class ArgHandlerServer extends ArgHandlerString {
		...
		public boolean setString(String arg) {
			...
			System.err.println(
					"Unable to load server class '" + sclClassName + "'");
			t.printStackTrace();
			return false;
		}
	}
